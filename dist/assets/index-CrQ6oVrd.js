import{defaultConfig as Wc,createWeb3Modal as Gc}from"https://esm.sh/@web3modal/ethers@5.1.11?bundle";import{initializeApp as Kc}from"https://www.gstatic.com/firebasejs/9.15.0/firebase-app.js";import{getAuth as Yc,onAuthStateChanged as Vc,signInAnonymously as qc}from"https://www.gstatic.com/firebasejs/9.15.0/firebase-auth.js";import{getFirestore as Xc,collection as Xe,query as Et,where as Zn,orderBy as Yt,getDocs as it,doc as te,getDoc as Ae,limit as Jc,serverTimestamp as ot,writeBatch as fa,updateDoc as ba,increment as Fe,setDoc as ga,Timestamp as vr,addDoc as Zc,deleteDoc as Qc}from"https://www.gstatic.com/firebasejs/9.15.0/firebase-firestore.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))a(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function n(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(s){if(s.ep)return;s.ep=!0;const r=n(s);fetch(s.href,r)}})();const Be={sidebar:document.getElementById("sidebar"),sidebarBackdrop:document.getElementById("sidebar-backdrop"),menuBtn:document.getElementById("menu-btn"),navLinks:document.getElementById("nav-links"),mainContentSections:document.querySelectorAll("main section"),connectButtonDesktop:document.getElementById("connectButtonDesktop"),connectButtonMobile:document.getElementById("connectButtonMobile"),mobileAppDisplay:document.getElementById("mobileAppDisplay"),desktopDisconnected:document.getElementById("desktopDisconnected"),desktopConnectedInfo:document.getElementById("desktopConnectedInfo"),desktopUserAddress:document.getElementById("desktopUserAddress"),desktopUserBalance:document.getElementById("desktopUserBalance"),modalContainer:document.getElementById("modal-container"),toastContainer:document.getElementById("toast-container"),dashboard:document.getElementById("dashboard"),earn:document.getElementById("mine"),store:document.getElementById("store"),rental:document.getElementById("rental"),rewards:document.getElementById("rewards"),actions:document.getElementById("actions"),presale:document.getElementById("presale"),faucet:document.getElementById("faucet"),airdrop:document.getElementById("airdrop"),dao:document.getElementById("dao"),about:document.getElementById("about"),admin:document.getElementById("admin"),tokenomics:document.getElementById("tokenomics"),notary:document.getElementById("notary"),statTotalSupply:document.getElementById("statTotalSupply"),statValidators:document.getElementById("statValidators"),statTotalPStake:document.getElementById("statTotalPStake"),statScarcity:document.getElementById("statScarcity"),statLockedPercentage:document.getElementById("statLockedPercentage"),statUserBalance:document.getElementById("statUserBalance"),statUserPStake:document.getElementById("statUserPStake"),statUserRewards:document.getElementById("statUserRewards")},ed={provider:null,publicProvider:null,web3Provider:null,signer:null,userAddress:null,isConnected:!1,bkcTokenContract:null,delegationManagerContract:null,rewardBoosterContract:null,nftLiquidityPoolContract:null,actionsManagerContract:null,fortunePoolContract:null,faucetContract:null,decentralizedNotaryContract:null,ecosystemManagerContract:null,publicSaleContract:null,rentalManagerContract:null,bkcTokenContractPublic:null,delegationManagerContractPublic:null,faucetContractPublic:null,fortunePoolContractPublic:null,rentalManagerContractPublic:null,ecosystemManagerContractPublic:null,actionsManagerContractPublic:null,currentUserBalance:0n,currentUserNativeBalance:0n,userTotalPStake:0n,userDelegations:[],myBoosters:[],activityHistory:[],totalNetworkPStake:0n,allValidatorsData:[],systemFees:{},systemPStakes:{},boosterDiscounts:{},notaryFee:void 0,notaryMinPStake:void 0},td={set(e,t,n){return e[t]=n,["currentUserBalance","isConnected","userTotalPStake","totalNetworkPStake"].includes(t)&&window.updateUIState&&window.updateUIState(),!0}},c=new Proxy(ed,td);let wr=!1;const x=(e,t="info",n=null)=>{if(!Be.toastContainer)return;const a={success:{icon:"fa-check-circle",color:"bg-green-600",border:"border-green-400"},error:{icon:"fa-exclamation-triangle",color:"bg-red-600",border:"border-red-400"},info:{icon:"fa-info-circle",color:"bg-blue-600",border:"border-blue-400"},warning:{icon:"fa-exclamation-circle",color:"bg-yellow-600",border:"border-yellow-400"}},s=a[t]||a.info,r=document.createElement("div");r.className=`flex items-center w-full max-w-xs p-4 text-white rounded-lg shadow-lg transition-all duration-500 ease-out 
    ...
    `}async function up(){try{const e=await fetch(Ju);if(e.ok){const t=await e.json();return S.economicData=t,t}}catch{}return null}async function is(){var e,t,n,a,s,r,i,o,l;try{const d=await up();let m=0n,f=0n,p=0n,b=0n,u=0n,g=0,y=0n;if(d)if(console.log("📊 Firebase economic data:",d.economy),(e=d.economy)!=null&&e.totalSupply&&(m=BigInt(d.economy.totalSupply)),(t=d.economy)!=null&&t.totalPStake&&(f=BigInt(d.economy.totalPStake)),(n=d.economy)!=null&&n.totalTVL&&(p=BigInt(d.economy.totalTVL)),(a=d.economy)!=null&&a.totalFeesCollected&&(u=BigInt(d.economy.totalFeesCollected)),(s=d.economy)!=null&&s.fortunePoolBalance&&(y=BigInt(d.economy.fortunePoolBalance)),(r=d.stats)!=null&&r.notarizedDocuments&&(g=d.stats.notarizedDocuments),(i=d.burn)!=null&&i.totalBurned)b=BigInt(d.burn.totalBurned);else if((o=d.burn)!=null&&o.sources){let D=0n;for(const W of Object.values(d.burn.sources))W!=null&&W.total&&(D+=BigInt(W.total));b=D}else(l=d.rental)!=null&&l.totalBurned&&(b=BigInt(d.rental.totalBurned));if(c.bkcTokenContractPublic&&(m===0n&&(console.log("📊 Fetching totalSupply from blockchain (fallback)..."),m=await ne(c.bkcTokenContractPublic,"totalSupply",[],0n)),f===0n&&c.delegationManagerContractPublic&&(console.log("📊 Fetching totalPStake from blockchain (fallback)..."),f=await ne(c.delegationManagerContractPublic,"totalNetworkPStake",[],0n)),p===0n)){console.log("📊 Calculating TVL from blockchain (fallback)...");const D=[v.delegationManager,v.fortunePool,v.rentalManager,v.miningManager,v.decentralizedNotary,v.nftLiquidityPoolFactory,v.pool_diamond,v.pool_platinum,v.pool_gold,v.pool_silver,v.pool_bronze,v.pool_iron,v.pool_crystal].filter(ae=>ae&&ae!==jt.ZeroAddress),W=D.map(ae=>ne(c.bkcTokenContractPublic,"balanceOf",[ae],0n)),le=await Promise.all(W);if(le.forEach(ae=>{p+=ae}),v.fortunePool&&y===0n){const ae=D.indexOf(v.fortunePool);ae>=0&&(y=le[ae])}}const k=_(m),E=_(b),I=_(u),P=_(y),z=D=>D.toLocaleString("en-US",{minimumFractionDigits:1,maximumFractionDigits:1});let T=0;m>0n&&(T=Number(p*10000n/m)/100),T>100&&(T=100);const F=(D,W,le="")=>{const ae=document.getElementById(D);ae&&(ae.innerHTML=`${W}${le?` <span class="text-xs text-zinc-500">${le}</span>`:""}`)},G=document.getElementById("dash-metric-supply");G&&(G.innerHTML=`${z(k)} <span style="font-size: 10px; color: #71717a">BKC</span>`),F("dash-metric-pstake",Ut(f));const se=document.getElementById("dash-metric-burned");se&&(E>0?se.innerHTML=`<span class="text-red-400">${Oa(E)}</span> <span class="text-xs text-zinc-500">BKC 🔥</span>`:se.innerHTML='<span class="text-zinc-500">0</span> <span class="text-xs text-zinc-500">BKC</span>'),F("dash-metric-fees",Oa(I),"BKC");const de=document.getElementById("dash-metric-tvl");if(de){const D=T>30?"text-green-400":T>10?"text-yellow-400":"text-blue-400";de.innerHTML=`<span class="${D}">${T.toFixed(1)}%</span>`}jl();const xe=document.getElementById("dash-fortune-prize");xe&&(xe.innerText=`${Oa(P)} BKC`);const Q=document.getElementById("dash-notary-count");Q&&(Q.innerText=g>0?`${g} docs`:"--"),S.metricsCache={supply:k,burned:E,fees:I,timestamp:Date.now()}}catch(d){console.error("Metrics Error",d)}}async function pp(){if(c.userAddress)try{const e=await fetch(`https://getuserprofile-4wvdcuoouq-uc.a.run.app/${c.userAddress}`);e.ok&&(S.userProfile=await e.json(),mp(S.userProfile))}catch{}}function mp(e){const t=document.getElementById("dash-presale-stats");if(!t||!e||!e.presale||!e.presale.totalBoosters||e.presale.totalBoosters===0)return;t.classList.remove("hidden");const n=e.presale.totalSpentWei||0,a=parseFloat(jt.formatEther(BigInt(n))).toFixed(4);document.getElementById("stats-total-spent").innerText=`${a} ETH`,document.getElementById("stats-total-boosters").innerText=e.presale.totalBoosters||0;const s=document.getElementById("stats-tier-badges");if(s&&e.presale.tiersOwned){let r="";Object.entries(e.presale.tiersOwned).forEach(([i,o])=>{const l=ge[Number(i)-1],d=l?l.name:`T${i}`;r+=`<span class="text-[9px] bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded">${o}x ${d}</span>`}),r&&(s.innerHTML=r)}}function jl(){const e=document.getElementById("dash-metric-balance"),t=document.getElementById("dash-balance-card");if(!e)return;const n=c.currentUserBalance||c.bkcBalance||0n;if(!c.isConnected){e.innerHTML='<span class="text-zinc-500 text-xs">Connect Wallet</span>',t&&(t.style.borderColor="rgba(63,63,70,0.5)");return}if(n===0n)e.innerHTML='0.00 <span style="font-size: 10px; color: #71717a">BKC</span>';else{const s=_(n).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2});e.innerHTML=`${s} <span style="font-size: 10px; color: #71717a">BKC</span>`}t&&(t.style.borderColor="rgba(245,158,11,0.3)")}async function _t(e=!1){var t,n;if(!c.isConnected){const a=document.getElementById("dash-booster-area");a&&(a.innerHTML=`
        ...
        `;return}const d=e.source==="rented",m=d?"bg-cyan-500/20 text-cyan-300":"bg-green-500/20 text-green-300",f=d?"Rented":"Owned",p=ud(a),b=(p==null?void 0:p.name)||((E=e.boostName)==null?void 0:E.replace(" Booster","").replace("Booster","").trim())||"Booster",u=(p==null?void 0:p.color)||"text-amber-400",g=(p==null?void 0:p.emoji)||"💎",y=r*50n/100n,k=i-y;console.log("🎨 V6.8 Booster:",{keepRate:s,netReward:_(i),tierName:b}),n.innerHTML=`
        <div class="flex items-center gap-3 bg-zinc-800/40 border ${(p==null?void 0:p.borderColor)||"border-green-500/20"} rounded-lg p-3 nft-clickable-image cursor-pointer" data-address="${v.rewardBoosterNFT}" data-tokenid="${e.tokenId}">
...
Are you sure?`)&&Yr(r.dataset.index,!0)})})}function Ip(e){const t=_(e.amount).toFixed(2),n=Ut(Vl(e.amount,e.lockDuration)),a=Number(e.unlockTime),s=Math.floor(Date.now()/1e3),r=a>s,i=r?a-s:0,o=Math.floor(Number(e.lockDuration)/86400);return`
    ...
    `}const Op={async render(e){Mp(),await Ai();const t=document.getElementById("store");t&&((t.innerHTML.trim()===""||e)&&(t.innerHTML=`
        ...
        `;return}e.innerHTML=L.tradeHistory.slice(0,20).map(t=>{const n=(t.type||"").toUpperCase(),a=t.details||{},s=_p(t.timestamp||t.createdAt),r=n.includes("BOUGHT")||n.includes("PURCHASED"),i=r?"fa-cart-plus":"fa-money-bill-transfer",o=r?"#22c55e":"#f59e0b",l=r?"rgba(34,197,94,0.15)":"rgba(245,158,11,0.15)",d=r?"🛒 Bought NFT":"💰 Sold NFT",m=r?"-":"+",f=t.txHash?`${Sp}${t.txHash}`:"#";let p="";try{let g=t.amount||a.amount||a.price||a.payout||"0";if(typeof g=="string"&&g!=="0"){const y=_(BigInt(g));y>.001&&(p=y.toFixed(2))}}catch{}const b=a.tokenId||"",u=a.boostBips||a.boost||"";return`
            <a href="${f}" target="_blank" class="history-item flex items-center justify-between p-3 hover:bg-zinc-800/60 border border-zinc-700/30 rounded-lg transition-all group bg-zinc-800/20">
                ...
                <div class="flex items-center gap-2">
                    ${p?`<span class="text-xs font-mono font-bold ${r?"text-white":"text-green-400"}">${m}${p} <span class="text-zinc-500">BKC</span></span>`:""}
        ...
        <div class="fade-in">
            ...
            <div class="swap-input-box rounded-2xl p-4 mb-1">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="token-selector flex items-center gap-2 px-3 py-2 rounded-xl cursor-default">
                        ${a?'<img src="./assets/bkc_logo_3d.png" class="w-6 h-6 rounded">':`<span class="text-xl">${p}</span>`}
            ...
            <div class="swap-input-box rounded-2xl p-4 mt-1 mb-4">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="token-selector flex items-center gap-2 px-3 py-2 rounded-xl cursor-default">
                        ${a?`<span class="text-xl">${p}</span>`:'<img src="./assets/bkc_logo_3d.png" class="w-6 h-6 rounded">'}
    ...
    `}function Up(e,t,n,a,s=!1){return c.isConnected?e?t?`
        ...
        `;return}const a=c.rentalListings||[],s=new Set(a.map(i=>{var o;return(o=i.tokenId)==null?void 0:o.toString()})),r=Math.floor(Date.now()/1e3);e.innerHTML=n.map(i=>{var E;const o=ge.find(I=>I.boostBips===Number(i.boostBips)),l=Ca(o==null?void 0:o.name),d=lt(Number(i.boostBips)),m=l.icon||(o==null?void 0:o.emoji)||"💎",f=L.firstAvailableTokenId&&BigInt(i.tokenId)===L.firstAvailableTokenId,p=(E=i.tokenId)==null?void 0:E.toString(),b=s.has(p),u=a.find(I=>{var P;return((P=I.tokenId)==null?void 0:P.toString())===p}),g=u&&u.rentalEndTime&&Number(u.rentalEndTime)>r,y=b||g;let k="";return g?k='<span class="absolute top-1 right-1 bg-blue-500 text-white text-[7px] px-1.5 py-0.5 rounded-full font-bold">🔑</span>':b&&(k='<span class="absolute top-1 right-1 bg-green-500 text-white text-[7px] px-1.5 py-0.5 rounded-full font-bold">📋</span>'),`
        ...
        `}).join("")}async function at(e=!1){var a,s;if(L.selectedPoolBoostBips===null)return;const t=L.selectedPoolBoostBips,n=Date.now();if(ht=n,!e){const r=Fp(t);if(r){ds(r),aa(),Jr(),Wp(t,n);return}}L.isDataLoading=!0;try{const r=c.myBoosters||[],i=c.rentalListings||[],o=new Set(i.map(D=>{var W;return(W=D.tokenId)==null?void 0:W.toString()})),l=Math.floor(Date.now()/1e3),d=r.filter(D=>Number(D.boostBips)===t),m=d.filter(D=>{var $e;const W=($e=D.tokenId)==null?void 0:$e.toString(),le=i.find(V=>{var rn;return((rn=V.tokenId)==null?void 0:rn.toString())===W}),ae=o.has(W),Qe=le&&le.rentalEndTime&&Number(le.rentalEndTime)>l;return!ae&&!Qe}),f=ge.find(D=>D.boostBips===t);if(!f){console.warn("Tier not found for boostBips:",t);return}const p=`pool_${f.name.toLowerCase()}`;let b=v[p]||na.get(t);if(!b){const D=v.nftLiquidityPoolFactory;if(D&&c.publicProvider)try{b=await new Ie.Contract(D,$p,c.publicProvider).getPoolAddress(t),b&&b!==Ie.ZeroAddress&&na.set(t,b)}catch(W){console.warn("Factory lookup failed:",W.message)}}if(ht!==n)return;if(!b||b===Ie.ZeroAddress){const D=document.getElementById("swap-interface");D&&(D.innerHTML=`
                    <div class="text-center py-12">
...
#Backcoin #Web3 #Arbitrum`}},Ga={pt:"./assets/pt.png",en:"./assets/en.png",es:"./assets/es.png"};let We="en";const ue=[{id:1,name:"Easy",emoji:"🍀",range:3,multiplier:2,chance:"33%",color:"emerald",hex:"#10b981",bgFrom:"from-emerald-500/20",bgTo:"to-green-600/10",borderColor:"border-emerald-500/50",textColor:"text-emerald-400"},{id:2,name:"Medium",emoji:"⚡",range:10,multiplier:5,chance:"10%",color:"violet",hex:"#8b5cf6",bgFrom:"from-violet-500/20",bgTo:"to-purple-600/10",borderColor:"border-violet-500/50",textColor:"text-violet-400"},{id:3,name:"Hard",emoji:"👑",range:100,multiplier:50,chance:"1%",color:"amber",hex:"#f59e0b",bgFrom:"from-amber-500/20",bgTo:"to-orange-600/10",borderColor:"border-amber-500/50",textColor:"text-amber-400"}],nc=57,w={mode:null,phase:"select",guess:50,guesses:[2,5,50],comboStep:0,wager:10,gameId:null,result:null,txHash:null,poolStatus:null,history:[],serviceFee:0n,serviceFee1x:0n,serviceFee5x:0n,tiersData:null,commitment:{hash:null,userSecret:null,commitBlock:null,commitTxHash:null,revealDelay:5,waitStartTime:null,canReveal:!1}};let Ve=null;const s0=3e3,ac=250;function r0(){if(document.getElementById("fortune-styles-v2"))return;const e=document.createElement("style");e.id="fortune-styles-v2",e.textContent=`
    ...
    `;const a=document.getElementById("number-input"),s=document.getElementById("number-slider"),r=ue[2],i=u=>{u=Math.max(1,Math.min(100,u)),w.guess=u,a&&(a.value=u),s&&(s.value=u,s.style.background=`linear-gradient(to right, ${r.hex} 0%, ${r.hex} ${u}%, #27272a ${u}%, #27272a 100%)`)};a==null||a.addEventListener("input",u=>i(parseInt(u.target.value)||1)),a==null||a.addEventListener("blur",u=>i(parseInt(u.target.value)||1)),s==null||s.addEventListener("input",u=>i(parseInt(u.target.value))),(o=document.getElementById("btn-minus"))==null||o.addEventListener("click",()=>i(w.guess-1)),(l=document.getElementById("btn-plus"))==null||l.addEventListener("click",()=>i(w.guess+1)),(d=document.getElementById("btn-minus-10"))==null||d.addEventListener("click",()=>i(w.guess-10)),(m=document.getElementById("btn-plus-10"))==null||m.addEventListener("click",()=>i(w.guess+10)),document.querySelectorAll(".quick-pick").forEach(u=>{u.addEventListener("click",()=>i(parseInt(u.dataset.number)))}),(f=document.getElementById("btn-random"))==null||f.addEventListener("click",()=>{i(Math.floor(Math.random()*100)+1)}),(p=document.getElementById("btn-back"))==null||p.addEventListener("click",()=>{w.phase="select",oe()}),(b=document.getElementById("btn-next"))==null||b.addEventListener("click",()=>{w.phase="wager",oe()})}function u0(e){var r,i,o,l,d,m,f;const t=ue[w.comboStep],n=w.guesses[w.comboStep],a=t.range===100;if(e.innerHTML=`
        <div class="bg-gradient-to-br from-zinc-900 to-zinc-800/50 border border-zinc-700/50 rounded-2xl p-5">
            <!-- Progress Pills -->
            <div class="flex justify-center gap-2 sm:gap-3 mb-5">
                ${ue.map((p,b)=>{const u=b===w.comboStep,g=b<w.comboStep;return`
                        <div class="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 rounded-xl border ${u?`bg-gradient-to-br ${p.bgFrom} ${p.bgTo} ${p.borderColor}`:g?"bg-emerald-500/10 border-emerald-500/50":"bg-zinc-800/50 border-zinc-700/50"}">
                            <span class="text-lg sm:text-xl">${g?"✓":p.emoji}</span>
                            <div class="text-left">
                                <p class="text-[10px] sm:text-xs font-bold ${u?p.textColor:g?"text-emerald-400":"text-zinc-500"}">${p.name}</p>
                                <p class="text-[8px] sm:text-[10px] ${g?"text-emerald-400 font-bold":"text-zinc-600"}">${g?w.guesses[b]:p.multiplier+"x"}</p>
                            </div>
                        </div>
                    `}).join("")}
            </div>
    ...
    `,t.range===100){const p=document.getElementById("combo-number-input"),b=document.getElementById("combo-slider"),u=g=>{g=Math.max(1,Math.min(100,g)),w.guesses[w.comboStep]=g,p&&(p.value=g),b&&(b.value=g,b.style.background=`linear-gradient(to right, ${t.hex} 0%, ${t.hex} ${g}%, #27272a ${g}%, #27272a 100%)`)};p==null||p.addEventListener("input",g=>u(parseInt(g.target.value)||1)),p==null||p.addEventListener("blur",g=>u(parseInt(g.target.value)||1)),b==null||b.addEventListener("input",g=>u(parseInt(g.target.value))),(r=document.querySelector(".combo-minus"))==null||r.addEventListener("click",()=>u(w.guesses[w.comboStep]-1)),(i=document.querySelector(".combo-plus"))==null||i.addEventListener("click",()=>u(w.guesses[w.comboStep]+1)),(o=document.querySelector(".combo-minus-10"))==null||o.addEventListener("click",()=>u(w.guesses[w.comboStep]-10)),(l=document.querySelector(".combo-plus-10"))==null||l.addEventListener("click",()=>u(w.guesses[w.comboStep]+10)),document.querySelectorAll(".combo-quick").forEach(g=>{g.addEventListener("click",()=>u(parseInt(g.dataset.num)))}),(d=document.querySelector(".combo-random"))==null||d.addEventListener("click",()=>{u(Math.floor(Math.random()*100)+1)})}else document.querySelectorAll(".num-btn").forEach(p=>{p.addEventListener("click",()=>{const b=parseInt(p.dataset.num);w.guesses[w.comboStep]=b,document.querySelectorAll(".num-btn").forEach(u=>{parseInt(u.dataset.num)===b?u.className=`num-btn w-12 h-12 rounded-xl font-bold text-lg transition-all bg-gradient-to-br ${t.bgFrom} ${t.bgTo} border-2 ${t.borderColor} ${t.textColor}`:u.className="num-btn w-12 h-12 rounded-xl font-bold text-lg transition-all bg-zinc-800/60 border border-zinc-700/50 text-zinc-400 hover:border-zinc-600"})})});(m=document.getElementById("btn-back"))==null||m.addEventListener("click",()=>{w.comboStep>0?(w.comboStep--,oe()):(w.phase="select",oe())}),(f=document.getElementById("btn-next"))==null||f.addEventListener("click",()=>{w.comboStep<2?(w.comboStep++,oe()):(w.phase="wager",oe())})}function p0(e){const t=w.mode==="jackpot",n=t?[w.guess]:w.guesses,a=t?50:nc,s=_(c.currentUserBalance||0n),r=s>=1,i=t?w.serviceFee1x:w.serviceFee5x,o=i>0n?Number(i)/1e18:0,l=i>0n;e.innerHTML=`
        <div class="bg-gradient-to-br from-zinc-900 to-zinc-800/50 border border-zinc-700/50 rounded-2xl p-5">
    ...
    `,m0(a,s)}function m0(e,t){var a,s,r,i,o,l,d,m;const n=f=>{w.wager=Math.max(1,Math.min(Math.floor(f),Math.floor(t)));const p=document.getElementById("custom-wager"),b=document.getElementById("potential-win");p&&(p.value=w.wager),b&&(b.textContent=(w.wager*e).toLocaleString()),document.querySelectorAll(".percent-btn").forEach(u=>{const g=parseInt(u.dataset.value);w.wager===g?u.className="percent-btn py-2.5 text-sm font-bold rounded-xl transition-all bg-gradient-to-r from-amber-500/30 to-orange-500/20 border-2 border-amber-500/60 text-amber-400 shadow-lg shadow-amber-500/20":u.className="percent-btn py-2.5 text-sm font-bold rounded-xl transition-all bg-zinc-800/60 border border-zinc-700/50 text-zinc-400 hover:border-amber-500/30 hover:text-amber-300"})};document.querySelectorAll(".percent-btn").forEach(f=>{f.addEventListener("click",()=>{n(parseInt(f.dataset.value)||1)})}),(a=document.getElementById("custom-wager"))==null||a.addEventListener("input",f=>{n(parseInt(f.target.value)||1)}),(s=document.getElementById("wager-minus"))==null||s.addEventListener("click",()=>{n(w.wager-1)}),(r=document.getElementById("wager-plus"))==null||r.addEventListener("click",()=>{n(w.wager+1)}),(i=document.getElementById("wager-minus-10"))==null||i.addEventListener("click",()=>{n(w.wager-10)}),(o=document.getElementById("wager-plus-10"))==null||o.addEventListener("click",()=>{n(w.wager+10)}),(l=document.getElementById("btn-faucet"))==null||l.addEventListener("click",async()=>{x("Requesting tokens...","info");try{const p=await(await fetch(`https://faucet-4wvdcuoouq-uc.a.run.app?address=${c.userAddress}`)).json();p.success?(x("🎉 Tokens received!","success"),await Nt(),oe()):x(p.error||"Error","error")}catch{x("Faucet error","error")}}),(d=document.getElementById("btn-back"))==null||d.addEventListener("click",()=>{w.phase="pick",w.mode==="combo"&&(w.comboStep=2),oe()}),(m=document.getElementById("btn-play"))==null||m.addEventListener("click",async()=>{if(w.wager<1)return x("Min: 1 BKC","warning");w.phase="processing",oe();try{const f=w.mode==="jackpot"?[w.guess]:w.guesses,p=w.mode==="combo",b=window.ethers.parseEther(w.wager.toString());await Hs.playGame({wagerAmount:b,guesses:f,isCumulative:p,button:document.getElementById("btn-play"),onSuccess:u=>{w.gameId=(u==null?void 0:u.gameId)||Date.now(),w.commitment={hash:null,userSecret:(u==null?void 0:u.userSecret)||null,commitBlock:(u==null?void 0:u.commitBlock)||null,commitTxHash:(u==null?void 0:u.txHash)||null,revealDelay:5,waitStartTime:Date.now(),canReveal:!1},w.txHash=(u==null?void 0:u.txHash)||null,console.log("🔐 Game committed:",w.gameId,"Block:",w.commitment.commitBlock),w.phase="waiting",oe(),g0()},onError:u=>{u.cancelled||x(u.message||"Commit failed","error"),w.phase="wager",oe()}})}catch(f){console.error("Commit error:",f);const p=f.message||f.reason||"Transaction failed";x("Error: "+p,"error"),w.phase="wager",oe()}})}function f0(e){const t=w.mode==="jackpot",n=t?[w.guess]:w.guesses,a=t?[ue[2]]:ue;e.innerHTML=`
    ...
    `,(l=document.getElementById("btn-reveal"))==null||l.addEventListener("click",()=>{w.commitment.canReveal&&h0()}),er()}function er(){if(w.phase!=="waiting")return;const e=document.getElementById("countdown-timer"),t=document.getElementById("progress-bar");if(document.getElementById("btn-reveal"),document.getElementById("reveal-btn-text"),!e)return;const n=Date.now()-(w.commitment.waitStartTime||Date.now()),a=w.commitment.revealDelay*ac,s=Math.max(0,a-n),r=Math.ceil(s/1e3);if(r>0?e.textContent=`~${r}s`:w.commitment.canReveal?e.textContent="Ready!":e.textContent="Verifying on chain...",t){const i=Math.min(100,n/a*100);t.style.width=`${i}%`}w.phase==="waiting"&&setTimeout(er,1e3)}function g0(){Ve&&clearInterval(Ve),setTimeout(er,100),Ve=setInterval(async()=>{if(w.phase!=="waiting"){clearInterval(Ve);return}try{await x0()&&!w.commitment.canReveal&&(w.commitment.canReveal=!0,oe())}catch(e){console.warn("Reveal check error:",e)}},s0)}async function x0(){if(!c.fortunePoolContractPublic||!w.gameId)return!1;try{const e=await c.fortunePoolContractPublic.getCommitmentStatus(w.gameId);if(!w.commitment.commitBlock)try{const t=await c.fortunePoolContractPublic.getCommitment(w.gameId),n=Number(t.commitBlock);n>0&&(w.commitment.commitBlock=n)}catch{}return e.canReveal===!0}catch{return Date.now()-(w.commitment.waitStartTime||Date.now())>=3e4}}async function h0(){if(!w.commitment.canReveal){x("Not ready to reveal yet!","warning");return}const e=document.getElementById("btn-reveal");try{const t=w.mode==="jackpot"?[w.guess]:w.guesses;await Hs.revealPlay({gameId:w.gameId,guesses:t,userSecret:w.commitment.userSecret,button:e,onSuccess:(n,a)=>{Ve&&clearInterval(Ve),w.txHash=n.hash,w.result={rolls:(a==null?void 0:a.rolls)||[],prizeWon:(a==null?void 0:a.prizeWon)||0n,matches:(a==null?void 0:a.matches)||[],matchCount:(a==null?void 0:a.matchCount)||0},console.log("🎲 Game revealed:",w.result),w.phase="result",oe(),tr()},onError:n=>{n.cancelled||x(n.message||"Reveal failed","error"),e&&(e.disabled=!1,e.innerHTML='<i class="fa-solid fa-dice mr-2"></i>Try Again')}})}catch(t){console.error("Reveal error:",t),x("Reveal failed: "+(t.message||"Unknown error"),"error"),e&&(e.disabled=!1,e.innerHTML='<i class="fa-solid fa-dice mr-2"></i>Try Again')}}function v0(e){var f,p;const t=w.result;if(!t)return oe();const n=w.mode==="jackpot",a=n?[w.guess]:w.guesses,s=t.rolls||[],r=n?[ue[2]]:ue,i=a.map((b,u)=>{const g=s[u]!==void 0?Number(s[u]):null;return g!==null&&g===b}),o=i.filter(b=>b).length,l=t.prizeWon>0||o>0;let d=0;t.prizeWon&&t.prizeWon>0n?d=_(BigInt(t.prizeWon)):o>0&&i.forEach((b,u)=>{if(b){const g=n?ue[2]:ue[u];d+=w.wager*g.multiplier}});const m=typeof d=="number"?d.toLocaleString(void 0,{maximumFractionDigits:2}):d.toLocaleString();e.innerHTML=`
    ...
    `,l&&(w0(),d>w.wager*10&&y0()),(f=document.getElementById("btn-new-game"))==null||f.addEventListener("click",()=>{w.phase="select",w.result=null,w.txHash=null,w.gameId=null,oe(),tr()}),(p=document.getElementById("btn-share"))==null||p.addEventListener("click",()=>{k0(l,d)})}function w0(){const e=document.createElement("div");e.className="confetti-container",document.body.appendChild(e);const t=["#f59e0b","#10b981","#8b5cf6","#ec4899","#06b6d4"],n=["●","■","★","🐯","🎉"];for(let a=0;a<60;a++){const s=document.createElement("div");s.className="confetti",s.style.cssText=`
            left: ${Math.random()*100}%;
            color: ${t[a%t.length]};
            font-size: ${8+Math.random()*12}px;
            animation-delay: ${Math.random()*2}s;
            animation-duration: ${2+Math.random()*2}s;
        `,s.textContent=n[a%n.length],e.appendChild(s)}setTimeout(()=>e.remove(),5e3)}function y0(){const e=["🪙","💰","✨","⭐","🎉"];for(let t=0;t<30;t++)setTimeout(()=>{const n=document.createElement("div");n.className="coin",n.textContent=e[Math.floor(Math.random()*e.length)],n.style.left=`${Math.random()*100}%`,n.style.animationDelay=`${Math.random()*.5}s`,n.style.animationDuration=`${2+Math.random()*2}s`,document.body.appendChild(n),setTimeout(()=>n.remove(),4e3)},t*100)}function k0(e,t){var l,d,m,f,p,b;const n=ti[We],a=()=>{const u=a0[We];return e?u.win(t):u.lose},s=`
        <div class="text-center">
    ...
    `;wn(s,"max-w-xs");const r=u=>{We=u;const g=ti[u],y=document.getElementById("share-modal-title"),k=document.getElementById("share-modal-subtitle"),E=document.getElementById("btn-close-share");y&&(y.textContent=g.title),k&&(k.textContent=g.subtitle),E&&(E.textContent=g.later),document.querySelectorAll(".lang-btn").forEach(I=>{const P=I.dataset.lang,z=I.querySelector("span");P===u?(I.className="lang-btn flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all bg-amber-500/20 border-amber-500 ring-1 ring-amber-500/50 border",z&&(z.className="text-amber-400")):(I.className="lang-btn flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all bg-zinc-800 border-zinc-700 hover:border-zinc-500 border",z&&(z.className="text-zinc-400"))})};document.querySelectorAll(".lang-btn").forEach(u=>{u.addEventListener("click",()=>r(u.dataset.lang))});const i=async u=>{if(!c.userAddress)return!1;try{const y=await(await fetch("https://us-central1-backchain-backand.cloudfunctions.net/trackShare",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({address:c.userAddress,gameId:w.gameId||Date.now(),type:"fortune",platform:u})})).json();return y.success?(x(`🎉 +${y.pointsAwarded||ps} Airdrop Points!`,"success"),!0):(y.reason==="already_shared"&&console.log("Already shared this game"),!1)}catch(g){return console.error("Share tracking error:",g),x(`🎉 +${ps} Airdrop Points!`,"success"),!0}},o=async(u,g)=>{await i(u),window.open(g,"_blank"),we()};(l=document.getElementById("share-twitter"))==null||l.addEventListener("click",()=>{const u=a();o("twitter",`https://twitter.com/intent/tweet?text=${encodeURIComponent(u)}`)}),(d=document.getElementById("share-telegram"))==null||d.addEventListener("click",()=>{const u=a();o("telegram",`https://t.me/share/url?url=https://backcoin.org&text=${encodeURIComponent(u)}`)}),(m=document.getElementById("share-whatsapp"))==null||m.addEventListener("click",()=>{const u=a();o("whatsapp",`https://wa.me/?text=${encodeURIComponent(u)}`)}),(f=document.getElementById("share-instagram"))==null||f.addEventListener("click",async()=>{const u=a();try{await navigator.clipboard.writeText(u),await i("instagram");const g=`
            ...
            `)}}catch(t){console.error("loadHistory error:",t)}}function C0(e){const t=document.getElementById("history-list");t&&(t.innerHTML=e.map(n=>{var u;const a=n.isWin||n.prizeWon&&BigInt(n.prizeWon)>0n,s=n.prizeWon?_(BigInt(n.prizeWon)):0,r=n.wagerAmount?_(BigInt(n.wagerAmount)):0,i=n.isCumulative,o=n.rolls||[],l=n.guesses||[],d=n.txHash||n.transactionHash,m=I0(n.timestamp||n.createdAt),f=n.player?`${n.player.slice(0,6)}...${n.player.slice(-4)}`:"???",p=c.userAddress&&((u=n.player)==null?void 0:u.toLowerCase())===c.userAddress.toLowerCase(),b=d?`${Qs}${d}`:null;return`
    ...
    `}function Y0(){var i;const e=A.platformUsageConfig||rc,t=A.platformUsage||{};let n=0,a=0;Object.keys(e).forEach(o=>{var l;e[o].enabled!==!1&&e[o].maxCount&&(n+=e[o].maxCount,a+=Math.min(((l=t[o])==null?void 0:l.count)||0,e[o].maxCount))});const s=n>0?a/n*100:0,r=((i=A.user)==null?void 0:i.platformUsagePoints)||0;return`
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-2" id="platform-actions-grid">
                ${Object.entries(e).filter(([o,l])=>l.enabled!==!1).map(([o,l])=>{const d=t[o]||{count:0},m=d.count>=l.maxCount,f=Math.max(0,l.maxCount-d.count),p=d.count/l.maxCount*100,b=D0[o]||"";return`
                ...
                <div class="bg-zinc-900/80 border border-zinc-800 rounded-xl overflow-hidden">
                    ${t.length===0?`<div class="p-8 text-center">
                            ...
                            <p class="text-zinc-600 text-xs mt-1">Create your first post to get started!</p>
                                ...
                                <div class="p-3 ${m?"":"border-b border-zinc-800"} ${u}">
                                    <div class="flex items-center justify-between">
    ...
    `}function X0(){var m,f;const e=((m=A.leaderboards)==null?void 0:m.top100ByPosts)||[],t=((f=A.leaderboards)==null?void 0:f.top100ByPoints)||[],n=A.activeRanking||"posts";function a(p,b,u){var z,T;const g=A.user&&((z=p.walletAddress)==null?void 0:z.toLowerCase())===((T=A.user.walletAddress)==null?void 0:T.toLowerCase()),y=J0(b+1),k=u==="posts"?"bg-amber-500/10":"bg-green-500/10",E=u==="posts"?"text-amber-400":"text-green-400",I=u==="posts"?"text-white":"text-green-400",P=u==="posts"?"posts":"pts";return`
...
(This will reset their rejection count to 0)`;if(!window.confirm(s))return;const r=t.innerHTML;t.disabled=!0,t.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>';try{await $i(n,a),x(`User ${a?"BANNED":"UNBANNED"}.`,"success");const i=C.allUsers.findIndex(o=>o.id===n);i>-1&&(C.allUsers[i].isBanned=a,C.allUsers[i].hasPendingAppeal=!1,a===!1&&(C.allUsers[i].rejectedCount=0)),dt()}catch(i){x(`Failed: ${i.message}`,"error"),t.disabled=!1,t.innerHTML=r}},bm=async e=>{const t=e.target.closest(".resolve-appeal-btn");if(!t||t.disabled)return;const n=t.dataset.userId,s=t.dataset.action==="approve";if(!n)return;const r=s?"Are you sure you want to APPROVE this appeal and UNBAN the user?":"Are you sure you want to DENY this appeal? The user will remain banned.";if(!window.confirm(r))return;const i=t.closest("td").querySelectorAll("button"),o=new Map;i.forEach(l=>{o.set(l,l.innerHTML),l.disabled=!0,l.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>'});try{s&&await $i(n,!1),x(`Appeal ${s?"APPROVED":"DENIED"}.`,"success");const l=C.allUsers.findIndex(d=>d.id===n);l>-1&&(C.allUsers[l].hasPendingAppeal=!1,s&&(C.allUsers[l].isBanned=!1,C.allUsers[l].rejectedCount=0)),dt()}catch(l){x(`Failed: ${l.message}`,"error"),i.forEach(d=>{d.disabled=!1,d.innerHTML=o.get(d)})}},gm=async e=>{const t=e.target.closest(".re-approve-btn");if(!t||t.disabled)return;const n=t.dataset.submissionId,a=t.dataset.userId;if(!n||!a)return;const s=t.closest("tr");s&&(s.style.opacity="0.5"),t.disabled=!0,t.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>';try{await Ri(a,n,"approved"),x("Submission re-approved!","success"),C.selectedUserSubmissions=C.selectedUserSubmissions.filter(i=>i.submissionId!==n),s&&s.remove();const r=C.allUsers.findIndex(i=>i.id===a);if(r>-1){const i=C.allUsers[r];i.rejectedCount=Math.max(0,(i.rejectedCount||0)-1),dt()}if(C.selectedUserSubmissions.length===0){const i=document.querySelector("#admin-user-modal .p-6");i&&(i.innerHTML='<p class="text-zinc-400 text-center p-8">This user has no rejected submissions.</p>')}}catch(r){x(`Failed to re-approve: ${r.message}`,"error"),s&&(s.style.opacity="1"),t.disabled=!1,t.innerHTML='<i class="fa-solid fa-check"></i> Re-Approve'}},xm=async e=>{const t=e.target.closest(".view-rejected-btn");if(!t||t.disabled)return;const n=t.dataset.userId,a=t.dataset.wallet;if(n){C.selectedWallet=a,C.isSubmissionsModalOpen=!0,qa(!0,[]);try{const s=await Yd(n,"rejected");C.selectedUserSubmissions=s,qa(!1,s)}catch(s){x(`Error fetching user submissions: ${s.message}`,"error"),qa(!1,[],!0)}}},hm=()=>{C.isSubmissionsModalOpen=!1,C.selectedUserSubmissions=[],C.selectedWallet=null;const e=document.getElementById("admin-user-modal");e&&e.remove(),document.body.style.overflow="auto"},vm=e=>{const t=e.target.closest(".user-profile-link");if(!t)return;e.preventDefault();const n=t.dataset.userId;if(!n)return;const a=C.allUsers.find(s=>s.id===n);if(!a){x("Error: Could not find user data.","error");return}Cm(a)},wm=()=>{const e=document.getElementById("admin-user-profile-modal");e&&e.remove(),document.body.style.overflow="auto"},ym=async e=>{e.preventDefault();const t=e.target;let n,a;try{if(n=new Date(t.startDate.value+"T00:00:00Z"),a=new Date(t.endDate.value+"T23:59:59Z"),isNaN(n.getTime())||isNaN(a.getTime()))throw new Error("Invalid date format.");if(n>=a)throw new Error("Start Date must be before End Date.")}catch(l){x(l.message,"error");return}const s={title:t.title.value.trim(),url:t.url.value.trim(),description:t.description.value.trim(),points:parseInt(t.points.value,10),cooldownHours:parseInt(t.cooldown.value,10),startDate:n,endDate:a};if(!s.title||!s.description){x("Please fill in Title and Description.","error");return}if(s.points<=0||s.cooldownHours<=0){x("Points and Cooldown must be positive numbers.","error");return}if(s.url&&!s.url.startsWith("http")){x("URL must start with http:// or https://","error");return}C.editingTask&&C.editingTask.id&&(s.id=C.editingTask.id);const r=t.querySelector('button[type="submit"]'),i=r.innerHTML;r.disabled=!0;const o=document.createElement("span");o.classList.add("inline-block"),r.innerHTML="",r.appendChild(o);try{await jd(s),x(`Task ${s.id?"updated":"created"} successfully!`,"success"),t.reset(),C.editingTask=null,xn()}catch(l){x(`Failed to save task: ${l.message}`,"error"),console.error(l),r.disabled=!1,r.innerHTML=i}},km=async e=>{e.preventDefault();const t=e.target,n={YouTube:parseInt(t.youtubePoints.value,10),"YouTube Shorts":parseInt(t.youtubeShortsPoints.value,10),Instagram:parseInt(t.instagramPoints.value,10),"X/Twitter":parseInt(t.xTwitterPoints.value,10),Facebook:parseInt(t.facebookPoints.value,10),Telegram:parseInt(t.telegramPoints.value,10),TikTok:parseInt(t.tiktokPoints.value,10),Reddit:parseInt(t.redditPoints.value,10),LinkedIn:parseInt(t.linkedinPoints.value,10),Other:parseInt(t.otherPoints.value,10)};if(Object.values(n).some(i=>isNaN(i)||i<0)){x("All points must be positive numbers (or 0).","error");return}const a=t.querySelector('button[type="submit"]'),s=a.innerHTML;a.disabled=!0;const r=document.createElement("span");r.classList.add("inline-block"),a.innerHTML="",a.appendChild(r);try{await Hd(n),x("UGC Base Points updated successfully!","success"),C.ugcBasePoints=n}catch(i){x(`Failed to update points: ${i.message}`,"error"),console.error(i)}finally{document.body.contains(a)&&(a.disabled=!1,a.innerHTML=s)}},Tm=e=>{const t=C.dailyTasks.find(n=>n.id===e);t&&(C.editingTask=t,Nn())},Em=async e=>{if(window.confirm("Are you sure you want to delete this task permanently?"))try{await Wd(e),x("Task deleted.","success"),C.editingTask=null,xn()}catch(t){x(`Failed to delete task: ${t.message}`,"error"),console.error(t)}};function qa(e,t,n=!1){var i,o;const a=document.getElementById("admin-user-modal");a&&a.remove(),document.body.style.overflow="hidden";let s="";e?s='<div class="p-8"></div>':n?s='<p class="text-red-400 text-center p-8">Failed to load submissions.</p>':t.length===0?s='<p class="text-zinc-400 text-center p-8">This user has no rejected submissions.</p>':s=`
    ...
    `,(a=document.getElementById("ugcPointsForm"))==null||a.addEventListener("submit",km)},Nn=()=>{var u,g,y;const e=document.getElementById("manage-tasks-content");if(!e)return;const t=C.editingTask,n=!!t,a=k=>{if(!k)return"";try{return(k.toDate?k.toDate():k instanceof Date?k:new Date(k)).toISOString().split("T")[0]}catch{return""}},s=C.tasksPage,r=C.tasksPerPage,i=[...C.dailyTasks].sort((k,E)=>{var z,T;const I=(z=k.startDate)!=null&&z.toDate?k.startDate.toDate():new Date(k.startDate||0);return((T=E.startDate)!=null&&T.toDate?E.startDate.toDate():new Date(E.startDate||0)).getTime()-I.getTime()}),o=i.length,l=Math.ceil(o/r),d=(s-1)*r,m=s*r,f=i.slice(d,m),p=f.length>0?f.map(k=>{var T,F;const E=new Date,I=(T=k.startDate)!=null&&T.toDate?k.startDate.toDate():k.startDate?new Date(k.startDate):null,P=(F=k.endDate)!=null&&F.toDate?k.endDate.toDate():k.endDate?new Date(k.endDate):null;let z="text-zinc-500";return I&&P&&(E>=I&&E<=P?z="text-green-400":E<I&&(z="text-blue-400")),`
    ...
    `;const b=document.getElementById("admin-tasks-pagination");b&&l>1&&Bs(b,C.tasksPage,l,Pm),(u=document.getElementById("taskForm"))==null||u.addEventListener("submit",ym),(g=document.getElementById("cancelEditBtn"))==null||g.addEventListener("click",()=>{C.editingTask=null,Nn()}),(y=document.getElementById("existing-tasks-list"))==null||y.addEventListener("click",k=>{const E=k.target.closest("button[data-id]");if(!E)return;const I=E.dataset.id;E.dataset.action==="edit"&&Tm(I),E.dataset.action==="delete"&&Em(I)})},ca=()=>{var f;const e=document.getElementById("submissions-content");if(!e)return;if(!C.allSubmissions||C.allSubmissions.length===0){const p=document.createElement("div");e.innerHTML=p.innerHTML;return}const t=C.submissionsPage,n=C.submissionsPerPage,a=[...C.allSubmissions].sort((p,b)=>{var u,g;return(((u=b.submittedAt)==null?void 0:u.getTime())||0)-(((g=p.submittedAt)==null?void 0:g.getTime())||0)}),s=a.length,r=Math.ceil(s/n),i=(t-1)*n,o=t*n,d=a.slice(i,o).map(p=>{var b,u;return`
        <tr class="border-b border-border-color hover:bg-zinc-800/50">
            ...
            <td class="p-3 text-xs font-semibold ${((b=ii[p.status])==null?void 0:b.color)||"text-gray-500"}">${((u=ii[p.status])==null?void 0:u.text)||p.status}</td>
            <td class="p-3 text-right">
                <div class="flex items-center justify-end gap-2">
    ...
    `;try{const t=Te.getNotarizedDocuments||"https://getnotarizeddocuments-4wvdcuoouq-uc.a.run.app",n=await fetch(`${t}/${c.userAddress}`);if(!n.ok)throw new Error(`API returned ${n.status}`);const a=await n.json();if(!Array.isArray(a)||a.length===0){e.innerHTML=`
            ...
            `;return}const s=a.map(r=>({id:r.tokenId||"?",ipfs:r.ipfsCid||"",description:r.description||"",hash:r.contentHash||"",timestamp:r.createdAt||r.timestamp||"",txHash:r.txHash||""})).sort((r,i)=>parseInt(i.id)-parseInt(r.id));e.innerHTML=s.map(r=>{var f,p;let i="";const o=r.ipfs||"";o.startsWith("ipfs://")?i=`${di}${o.replace("ipfs://","")}`:o.startsWith("https://")?i=o:o.length>0&&(i=`${di}${o}`);let l=((f=r.description)==null?void 0:f.split("---")[0].trim().split(`
...
`;document.head.appendChild(yc);const If={render:async e=>{if(!e)return;const t=document.getElementById("creditcard");t&&(t.innerHTML=`
    ...
    `},gi=()=>{var a,s,r,i;const e=B.campaigns.filter(o=>cr(o)),t=e.filter(o=>o.category==="animal"),n=e.filter(o=>o.category==="humanitarian");return`
    ...
    `},xi=e=>{var o,l,d,m,f,p;if(!e)return`
        <div class="charity-page">
            ...
            <div class="cp-detail">
                ...
                <div class="flex flex-wrap items-center gap-2 mb-4">
                    ...
                    <span class="cp-badge" style="background:${(d=Me[a])==null?void 0:d.color}20;color:${(m=Me[a])==null?void 0:m.color}">
                        ${(f=Me[a])==null?void 0:f.emoji} ${(p=Me[a])==null?void 0:p.name}
                    </span>
...
`;let s;e==="twitter"?s=`https://twitter.com/intent/tweet?text=${encodeURIComponent(a)}&url=${encodeURIComponent(n)}`:e==="telegram"?s=`https://t.me/share/url?url=${encodeURIComponent(n)}&text=${encodeURIComponent(a)}`:e==="whatsapp"&&(s=`https://wa.me/?text=${encodeURIComponent(a+n)}`),s&&window.open(s,"_blank","width=600,height=400")}function tb(){const e=B.currentCampaign;e&&navigator.clipboard.writeText(Ac(e.id)).then(()=>x("Link copied!","success")).catch(()=>x("Copy failed","error"))}function Fc(){$f(),B.currentCampaign=null,B.currentView="main",ut()}function nb(e){rt("my"),rt("donate"),rt("edit"),Rf(e),St(e)}function ab(e){B.selectedCategory=B.selectedCategory===e?null:e,ur()}function sb(){B.selectedCategory=null,ur()}function ur(){const e=document.getElementById("cp-grid");if(!e)return;let t=B.campaigns.filter(n=>cr(n));B.selectedCategory&&(t=t.filter(n=>n.category===B.selectedCategory)),t.sort((n,a)=>Number(a.createdAt||0)-Number(n.createdAt||0)),e.innerHTML=t.length?t.map(n=>Rc(n)).join(""):Sc("No campaigns")}async function St(e){B.currentView="detail",B.isLoading=!0;const t=xs();t&&(t.innerHTML=Df());try{let n=B.campaigns.find(a=>a.id===e||a.id===String(e));if(!n){const a=c==null?void 0:c.publicProvider;if(a){const r=await new nn.Contract(v.charityPool,dr,a).campaigns(e);n={id:String(e),creator:r[0],title:r[1],description:r[2],goalAmount:BigInt(r[3].toString()),raisedAmount:BigInt(r[4].toString()),donationCount:Number(r[5]),deadline:Number(r[6]),createdAt:Number(r[7]),status:Number(r[8]),category:"humanitarian",imageUrl:null}}}B.currentCampaign=n,t&&(t.innerHTML=xi(n))}catch{t&&(t.innerHTML=xi(null))}finally{B.isLoading=!1}}function xs(){let e=document.getElementById("charity-container");if(e)return e;const t=document.getElementById("charity");return t?(e=document.createElement("div"),e.id="charity-container",t.innerHTML="",t.appendChild(e),e):null}function ut(){Sf();const e=xs();if(!e)return;const t=Bc();t?St(t):(B.currentView="main",B.currentCampaign=null,e.innerHTML=gi(),Lt().then(()=>{if(B.currentView==="main"){const n=xs();n&&(n.innerHTML=gi())}}))}async function rb(){B.campaigns=[],B.stats=null,B.currentView==="detail"&&B.currentCampaign?await St(B.currentCampaign.id):ut()}window.addEventListener("hashchange",()=>{var e;if(window.location.hash.startsWith("#charity")){const t=Bc();t?((e=B.currentCampaign)==null?void 0:e.id)!==t&&St(t):B.currentView!=="main"&&Fc()}});const Mc={render(e){e&&ut()},update(){B.currentView==="main"&&ur()},refresh:rb,openModal:Dn,closeModal:rt,openCreate:Uf,openDonate:jf,openMyCampaigns:Wf,openEdit:Gf,create:Vf,donate:qf,donateDetail:Xf,cancel:Jf,withdraw:Zf,saveEdit:Qf,selCatOpt:Kf,setAmt:Yf,goBack:Fc,viewCampaign:nb,selectCat:ab,clearCat:sb,share:eb,copyLink:tb,handleImageSelect:Ff,removeImage:Pc,switchImageTab:Mf};window.CharityPage=Mc;const he=window.ethers,ib="https://sepolia.arbiscan.io/address/",ob=cd;function pr(){return v.backchat||v.Backchat||null}function an(){return v.operator||v.treasury||null}const N={activeTab:"feed",view:"feed",posts:[],trendingPosts:[],userProfile:null,profiles:new Map,following:new Set,selectedPost:null,selectedProfile:null,fees:{post:0n,reply:0n,like:0n,follow:0n,repost:0n,superLikeMin:0n,boostMin:0n,badge:0n},pendingEth:0n,hasBadge:!1,isBoosted:!1,boostExpiry:0,badgeExpiry:0,referralStats:null,referredBy:null,isLoading:!1,isPosting:!1,contractAvailable:!0,error:null};function lb(){if(document.getElementById("backchat-styles-v70"))return;const e=document.getElementById("backchat-styles-v69");e&&e.remove();const t=document.createElement("style");t.id="backchat-styles-v70",t.textContent=`
    ...
    `}function vb(){return N.contractAvailable?N.isLoading?`
        ...
        ${N.trendingPosts.map((e,t)=>gr(e,t)).join("")}
    ...
    `}function kb(){var a,s;if(!c.isConnected)return"";const e=`${window.location.origin}/#backchat?ref=${c.userAddress}`,t=((a=N.referralStats)==null?void 0:a.totalReferred)||0,n=((s=N.referralStats)==null?void 0:s.totalEarnedFormatted)||"0.0";return`
        <div class="bc-referral-card">
            ...
            <div class="bc-referral-info" style="margin-top:12px;">
                ...
                ${N.referredBy?`<br>You were referred by <code style="font-size:11px;color:var(--bc-accent);">${mr(N.referredBy)}</code>`:""}
            </div>
        </div>
