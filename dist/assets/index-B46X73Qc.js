import{defaultConfig as ei,createWeb3Modal as ti}from"https://esm.sh/@web3modal/ethers@5.1.11?bundle";import{initializeApp as si}from"https://www.gstatic.com/firebasejs/9.15.0/firebase-app.js";import{getAuth as ni,onAuthStateChanged as ai,signInAnonymously as ii}from"https://www.gstatic.com/firebasejs/9.15.0/firebase-auth.js";import{getFirestore as oi,doc as _,getDoc as te,writeBatch as gt,increment as oe,collection as ye,query as Me,where as Mt,orderBy as Ke,getDocs as ze,limit as ri,serverTimestamp as Ce,updateDoc as Yt,setDoc as Kt,Timestamp as un,addDoc as li,deleteDoc as ci}from"https://www.gstatic.com/firebasejs/9.15.0/firebase-firestore.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function s(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(a){if(a.ep)return;a.ep=!0;const i=s(a);fetch(a.href,i)}})();const ne={sidebar:document.getElementById("sidebar"),sidebarBackdrop:document.getElementById("sidebar-backdrop"),menuBtn:document.getElementById("menu-btn"),navLinks:document.getElementById("nav-links"),mainContentSections:document.querySelectorAll("main section"),connectButtonDesktop:document.getElementById("connectButtonDesktop"),connectButtonMobile:document.getElementById("connectButtonMobile"),mobileAppDisplay:document.getElementById("mobileAppDisplay"),desktopDisconnected:document.getElementById("desktopDisconnected"),desktopConnectedInfo:document.getElementById("desktopConnectedInfo"),desktopUserAddress:document.getElementById("desktopUserAddress"),desktopUserBalance:document.getElementById("desktopUserBalance"),modalContainer:document.getElementById("modal-container"),toastContainer:document.getElementById("toast-container"),dashboard:document.getElementById("dashboard"),earn:document.getElementById("mine"),store:document.getElementById("store"),rental:document.getElementById("rental"),rewards:document.getElementById("rewards"),actions:document.getElementById("actions"),presale:document.getElementById("presale"),faucet:document.getElementById("faucet"),airdrop:document.getElementById("airdrop"),dao:document.getElementById("dao"),about:document.getElementById("about"),admin:document.getElementById("admin"),tokenomics:document.getElementById("tokenomics"),notary:document.getElementById("notary"),statTotalSupply:document.getElementById("statTotalSupply"),statValidators:document.getElementById("statValidators"),statTotalPStake:document.getElementById("statTotalPStake"),statScarcity:document.getElementById("statScarcity"),statLockedPercentage:document.getElementById("statLockedPercentage"),statUserBalance:document.getElementById("statUserBalance"),statUserPStake:document.getElementById("statUserPStake"),statUserRewards:document.getElementById("statUserRewards")},di={provider:null,publicProvider:null,web3Provider:null,signer:null,userAddress:null,isConnected:!1,bkcTokenContract:null,delegationManagerContract:null,rewardBoosterContract:null,nftLiquidityPoolContract:null,actionsManagerContract:null,fortunePoolContract:null,faucetContract:null,decentralizedNotaryContract:null,ecosystemManagerContract:null,publicSaleContract:null,rentalManagerContract:null,bkcTokenContractPublic:null,delegationManagerContractPublic:null,faucetContractPublic:null,fortunePoolContractPublic:null,rentalManagerContractPublic:null,ecosystemManagerContractPublic:null,actionsManagerContractPublic:null,currentUserBalance:0n,currentUserNativeBalance:0n,userTotalPStake:0n,userDelegations:[],myBoosters:[],activityHistory:[],totalNetworkPStake:0n,allValidatorsData:[],systemFees:{},systemPStakes:{},boosterDiscounts:{},notaryFee:void 0,notaryMinPStake:void 0},ui={set(e,t,s){return e[t]=s,["currentUserBalance","isConnected","userTotalPStake","totalNetworkPStake"].includes(t)&&window.updateUIState&&window.updateUIState(),!0}},c=new Proxy(di,ui);let pn=!1;const b=(e,t="info",s=null)=>{if(!ne.toastContainer)return;const n={success:{icon:"fa-check-circle",color:"bg-green-600",border:"border-green-400"},error:{icon:"fa-exclamation-triangle",color:"bg-red-600",border:"border-red-400"},info:{icon:"fa-info-circle",color:"bg-blue-600",border:"border-blue-400"},warning:{icon:"fa-exclamation-circle",color:"bg-yellow-600",border:"border-yellow-400"}},a=n[t]||n.info,i=document.createElement("div");i.className=`flex items-center w-full max-w-xs p-4 text-white rounded-lg shadow-lg transition-all duration-500 ease-out 
    ...
    `}async function Dt(){const e=document.getElementById("dash-activity-list"),t=document.getElementById("activity-title");try{if(c.isConnected){if(E.activities.length===0){e&&(e.innerHTML=`
                    ...
                    `);const s=await fetch(`${re.getHistory}/${c.userAddress}`);s.ok&&(E.activities=await s.json())}if(E.activities.length>0){t&&(t.textContent="Your Activity"),$s();return}}t&&(t.textContent="Network Activity"),await $n()}catch(s){console.error("Activity fetch error:",s),t&&(t.textContent="Network Activity"),await $n()}}async function $n(){const e=document.getElementById("dash-activity-list");if(!e||E.isLoadingNetworkActivity)return;const t=Date.now()-E.networkActivitiesTimestamp;if(E.networkActivities.length>0&&t<3e5){An();return}E.isLoadingNetworkActivity=!0,e.innerHTML=`
        <div class="flex flex-col items-center justify-center py-8">
            <div class="relative">
                <div class="absolute inset-0 bg-amber-500/30 blur-xl rounded-full animate-pulse"></div>
                <img src="./assets/bkc_logo_3d.png" class="relative w-12 h-12 object-contain animate-bounce" alt="Loading">
    ...
    `,fr(),c.isConnected?Xe(!0):en())}async function Xe(e=!1){var s;if(!c.isConnected){en();return}const t=Date.now();if(!(!e&&We)&&!(!e&&t-ft<1e4)){We=!0,ft=t;try{const n=await Ge();es=n!=null&&n.tokenId?BigInt(n.tokenId):0n;try{const a=await fetch("https://getsystemdata-4wvdcuoouq-uc.a.run.app");if(a.ok){const i=await a.json();(s=i==null?void 0:i.economy)!=null&&s.totalPStake&&(_e=BigInt(i.economy.totalPStake),console.log("📊 Network pStake from Firebase:",Fe(_e)))}}catch{console.log("Firebase system data not available, using blockchain fallback")}if(_e===0n&&(c.delegationManagerContractPublic||c.delegationManagerContract)){const a=c.delegationManagerContractPublic||c.delegationManagerContract;_e=await O(a,"totalNetworkPStake",[],0n),console.log("📊 Network pStake from blockchain (fallback)")}await Promise.all([V(!0),Gs(!0),Ks()]),cr(),Ca(),at(),lr()}catch(n){console.error("Staking load error:",n)}finally{We=!1}}}let st="ALL";async function lr(){if(c.userAddress)try{const e=re.getHistory||"https://gethistory-4wvdcuoouq-uc.a.run.app",t=await fetch(`${e}/${c.userAddress}`);t.ok&&(Ls=(await t.json()||[]).filter(n=>{const a=(n.type||"").toUpperCase();return a.includes("DELEGAT")||a.includes("STAKE")||a.includes("UNDELEGAT")||a.includes("CLAIM")||a.includes("REWARD")||a.includes("FORCE")}),za())}catch(e){console.error("History load error:",e)}}function za(){const e=document.getElementById("staking-history-list");if(!e)return;let t=Ls;if(st!=="ALL"&&(t=Ls.filter(s=>{const n=(s.type||"").toUpperCase();switch(st){case"STAKE":return(n.includes("DELEGAT")||n.includes("STAKE"))&&!n.includes("UNSTAKE")&&!n.includes("UNDELEGAT")&&!n.includes("FORCE");case"UNSTAKE":return n.includes("UNSTAKE")||n.includes("UNDELEGAT")||n.includes("FORCE");case"CLAIM":return n.includes("CLAIM")||n.includes("REWARD");default:return!0}})),t.length===0){e.innerHTML=`
        ...
        `}).join("")}function cr(){var l;const e=(d,u)=>{const m=document.getElementById(d);m&&(m.textContent=u)};e("stat-network",Fe(_e||c.totalNetworkPStake||0n)),e("stat-pstake",Fe(c.userTotalPStake||0n)),e("balance-display",B(c.currentUserBalance||0n).toFixed(2));const t=c.userTotalPStake||0n,s=_e||c.totalNetworkPStake||0n;let n=0;s>0n&&t>0n&&(n=Number(t*10000n/s)/100);const a=document.getElementById("stat-pstake-percent");a&&(a.textContent=n>0?`${n.toFixed(2)}% of network`:"0% of network");const i=((l=c.systemFees)==null?void 0:l.DELEGATION_FEE_BIPS)||50n,o=Number(i)/100,r=document.getElementById("fee-info");r&&(r.textContent=`${o}% fee`),Jt().then(({stakingRewards:d,minerRewards:u})=>{const m=d+u;e("stat-rewards",B(m).toFixed(4));const p=document.getElementById("claim-btn");p&&(p.disabled=m<=0n,m>0n&&(p.onclick=()=>pr(d,u,p)))})}function en(){const e=(a,i)=>{const o=document.getElementById(a);o&&(o.textContent=i)};e("stat-network","--"),e("stat-pstake","--"),e("stat-rewards","--"),e("balance-display","0.00"),e("delegation-count","0");const t=document.getElementById("stat-pstake-percent");t&&(t.textContent="--% of network");const s=document.getElementById("delegations-list");s&&(s.innerHTML=`
...
Are you sure you want to continue?`)&&Sn(a.dataset.index,!0)})})}function Ln(){const e=document.querySelectorAll(".countdown-timer"),t=Math.floor(Date.now()/1e3);e.forEach(s=>{const a=parseInt(s.dataset.unlockTime)-t;s.textContent=ka(a)})}function dr(e){const t=B(e.amount).toFixed(2),s=Fe(Ta(e.amount,e.lockDuration)),n=Number(e.unlockTime),a=Math.floor(Date.now()/1e3),i=n>a,o=i?n-a:0;return`
    ...
    `}const yr={async render(e){hr(),await ea();const t=document.getElementById("store");t&&((t.innerHTML.trim()===""||e)&&(t.innerHTML=`
        ...
        <div class="fade-in">
            ...
            <div class="flex justify-between items-center text-[10px] text-zinc-600 mb-4 px-1">
                ...
                <span>Fee Discount: ${C.bestBoosterBips>0?`${((m=c.boosterDiscounts)==null?void 0:m[C.bestBoosterBips])||0}%`:"None"}</span>
            </div>
    ...
    `}function Tr(e,t,s,n,a=!1){return c.isConnected?e?t?`
        ...
        `;return}const n=c.rentalListings||[],a=new Set(n.map(o=>{var r;return(r=o.tokenId)==null?void 0:r.toString()})),i=Math.floor(Date.now()/1e3);e.innerHTML=s.map(o=>{var k;const r=ee.find(I=>I.boostBips===Number(o.boostBips)),l=ts(r==null?void 0:r.name),d=jt(o.image||(r==null?void 0:r.img)),u=C.firstAvailableTokenId&&BigInt(o.tokenId)===C.firstAvailableTokenId,m=(k=o.tokenId)==null?void 0:k.toString(),p=a.has(m),g=n.find(I=>{var L;return((L=I.tokenId)==null?void 0:L.toString())===m}),f=g&&g.rentalEndTime&&Number(g.rentalEndTime)>i,x=p||f;let y="";return f?y='<span class="absolute top-1 right-1 bg-blue-500 text-white text-[7px] px-1.5 py-0.5 rounded-full font-bold">🔑 RENTED</span>':p&&(y='<span class="absolute top-1 right-1 bg-green-500 text-white text-[7px] px-1.5 py-0.5 rounded-full font-bold">📋 LISTED</span>'),`
        ...
        `}).join("")}async function Pe(){var s,n;if(C.isDataLoading||C.selectedPoolBoostBips===null)return;const e=Date.now();if(e-C.lastFetchTimestamp<2e3)return;C.isDataLoading=!0,C.lastFetchTimestamp=e;const t=C.selectedPoolBoostBips;try{const a=await Ge();C.bestBoosterTokenId=a!=null&&a.tokenId?BigInt(a.tokenId):0n,C.bestBoosterBips=(a==null?void 0:a.boostBips)||0,await Promise.all([Qt(!0),xt()]);const i=c.rentalListings||[],o=new Set(i.map(S=>{var X;return(X=S.tokenId)==null?void 0:X.toString()})),r=Math.floor(Date.now()/1e3),d=(c.myBoosters||[]).filter(S=>Number(S.boostBips)===t),u=d.filter(S=>{var Ct;const X=(Ct=S.tokenId)==null?void 0:Ct.toString(),ce=i.find(as=>{var dn;return((dn=as.tokenId)==null?void 0:dn.toString())===X}),K=o.has(X),zt=ce&&ce.rentalEndTime&&Number(ce.rentalEndTime)>r;return!K&&!zt});C.userBalanceOfSelectedNFT=d.length,C.availableToSellCount=u.length;const m=C.firstAvailableTokenId;m&&u.some(S=>BigInt(S.tokenId)===m)||(C.firstAvailableTokenId=u.length>0?BigInt(u[0].tokenId):null);const g=ee.find(S=>S.boostBips===t);if(!g){console.warn("Tier not found for boostBips:",t);return}const f=`pool_${g.name.toLowerCase()}`;let x=v[f];if(!x){const S=v.nftLiquidityPoolFactory;if(S&&c.publicProvider)try{x=await new $e.Contract(S,xr,c.publicProvider).getPoolAddress(t),x&&x!==$e.ZeroAddress&&Ea.set(t,x)}catch(X){console.warn("Factory lookup failed:",X.message)}}if(!x||x===$e.ZeroAddress){const S=document.getElementById("swap-interface");S&&(S.innerHTML=`
                ...
                `);return}const y=new $e.Contract(x,Us,c.publicProvider);let k=$e.MaxUint256,I=0n,L=[],A=((s=c.systemFees)==null?void 0:s.NFT_POOL_SELL_TAX_BIPS)||1000n,$=BigInt(((n=c.boosterDiscounts)==null?void 0:n[C.bestBoosterBips])||0);try{k=await O(y,"getBuyPrice",[],$e.MaxUint256)}catch(S){console.warn("getBuyPrice failed:",S.message)}try{I=await O(y,"getSellPrice",[],0n)}catch(S){console.warn("getSellPrice failed:",S.message)}try{const S=await y.getAvailableNFTs();L=Array.isArray(S)?[...S]:[]}catch(S){console.warn("getAvailableNFTs failed:",S.message),L=[]}C.poolNFTCount=L.length,C.firstAvailableTokenIdForBuy=L.length>0?BigInt(L[L.length-1]):null,C.buyPrice=k===$e.MaxUint256?0n:k,C.sellPrice=I;const T=typeof A=="bigint"?A:BigInt(A),F=typeof $=="bigint"?$:BigInt($),q=T>F?T-F:0n,le=I*q/10000n;C.netSellPrice=I-le}catch(a){console.warn("Store Data Warning:",a.message);const i=document.getElementById("swap-interface");i&&(i.innerHTML=`
                <div class="text-center py-12">
                    <div class="w-12 h-12 bg-zinc-800 rounded-full flex items-center justify-center mx-auto mb-3">
    ...
    `,document.head.appendChild(e)}const tn={async render(e){Ir();const t=document.getElementById("rewards");t&&((t.innerHTML.trim()===""||e)&&(t.innerHTML=Sr()),c.isConnected?(Pr(),this.update(e)):Rn())},async update(e=!1){if(!c.isConnected){Rn();return}const t=Date.now();if(!(!e&&fs)&&!(!e&&t-Ss<6e4)){fs=!0,it("loading");try{let s={highestBoost:0,boostName:"None",tokenId:null,source:"none"},n={netClaimAmount:0n,feeAmount:0n,totalRewards:0n},a={stakingRewards:0n,minerRewards:0n};try{await V()}catch{}try{s=await Ge()||s}catch{}try{n=await sa()||n}catch{}try{a=await Jt()||a}catch{}try{await $r()}catch{}Aa(n,a,s),Ss=t,it("idle")}catch(s){console.error("Rewards Error:",s)}finally{fs=!1}}}};function it(e){const t=document.getElementById("reward-mascot");if(t)switch(t.className="w-12 h-12 object-contain",e){case"loading":t.classList.add("reward-spin");break;case"claiming":t.classList.add("reward-bounce");break;case"success":t.classList.add("reward-success");break;default:t.classList.add("reward-float","reward-pulse")}}async function $r(){if(c.userAddress)try{const e=await fetch(`${re.getHistory}/${c.userAddress}`);e.ok&&(Ye=(await e.json()).filter(s=>{const n=(s.type||"").toUpperCase();return n.includes("REWARD")||n.includes("CLAIM")}).slice(0,15))}catch{Ye=[]}}function Ar(){return Ye.length===0?`
        ...
        `:Ye.map(e=>{const t=Lr(e.timestamp||e.createdAt),s=e.details||{},n=e.txHash||"",a=n?`${Er}${n}`:"#";let i="0",o="0";s.amountReceived?i=s.amountReceived:e.amount?i=e.amount:s.amount&&(i=s.amount),s.feePaid?o=s.feePaid:s.feeAmount&&(o=s.feeAmount);const r=Fn(i),l=Fn(o);let d,u,m,p;const g=(e.type||"").toUpperCase();return g.includes("STAKING")||g.includes("DELEGAT")?(d="fa-lock",u="#a855f7",m="rgba(168,85,247,0.15)",p="🔒 Staking Reward"):g.includes("MINING")||g.includes("MINER")?(d="fa-hammer",u="#f97316",m="rgba(249,115,22,0.15)",p="⛏️ Mining Reward"):g.includes("CLAIM")||g.includes("REWARD")?(d="fa-gift",u="#22c55e",m="rgba(34,197,94,0.15)",p="🎁 Claimed"):(d="fa-coins",u="#eab308",m="rgba(234,179,8,0.15)",p="💰 Reward"),`
            <a href="${a}" target="_blank" rel="noopener" 
               class="history-item flex items-center justify-between p-3 bg-zinc-800/30 hover:bg-zinc-800/50 rounded-xl transition-all group border border-zinc-700/30">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center" style="background: ${m}">
    ...
    `)}function Pr(){Aa({netClaimAmount:0n,feeAmount:0n,totalRewards:0n},{stakingRewards:0n,minerRewards:0n},{highestBoost:0,boostName:"None",tokenId:null,source:"none"})}function Aa(e,t,s){const n=document.getElementById("rewards-content");if(!n)return;const a=e||{},i=t||{},o=s||{},r=a.netClaimAmount||0n,l=a.totalRewards||0n,d=a.feeAmount||0n,u=i.stakingRewards||0n,m=i.minerRewards||0n,p=o.highestBoost||0,g=a.baseFeeBips||5e3,f=p/100,y=100-(a.finalFeeBips||g-g*p/1e4)/100,k=r>0n,I=p>0,L=BigInt(o.tokenId||0);Nt={stakingRewards:u,minerRewards:m,boosterTokenId:L};let A=0,$=0,T=0,F=0,q=0;try{A=B?B(r):Number(r)/1e18,$=B?B(l):Number(l)/1e18,T=B?B(d):Number(d)/1e18,F=B?B(u):Number(u)/1e18,q=B?B(m):Number(m)/1e18}catch{}const le=2*Math.PI*45,S=le-y/100*le;n.innerHTML=`
        <div class="space-y-4">
            <!-- MAIN CLAIM CARD -->
                ...
                <div class="flex flex-col items-center mb-5">
                    <div class="relative w-32 h-32 mb-3">
                        <svg class="w-full h-full" style="transform: rotate(-90deg)" viewBox="0 0 100 100">
                            ...
                            <circle cx="50" cy="50" r="45" fill="none" stroke="${I?"#4ade80":"#f59e0b"}" stroke-width="6" stroke-linecap="round" stroke-dasharray="${le}" stroke-dashoffset="${S}"/>
                        </svg>
                    ...
                    <p class="text-xs text-zinc-500">You keep <span class="${I?"text-green-400":"text-amber-400"} font-bold">${y.toFixed(1)}%</span> of earnings</p>
                </div>
            ...
            <div class="bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden">
                <div class="p-3 border-b border-zinc-800/50 flex items-center justify-between">
                    ...
                    ${I?'<span class="px-2 py-0.5 bg-green-500/20 text-green-400 text-[10px] font-bold rounded-full">ACTIVE</span>':'<span class="px-2 py-0.5 bg-zinc-700 text-zinc-400 text-[10px] font-bold rounded-full">NONE</span>'}
                </div>
                <div class="p-4">
                    ${I?`
                        <div class="flex items-center gap-3">
...
#Backcoin #Web3 #Arbitrum`}},ms={pt:"./assets/pt.png",en:"./assets/en.png",es:"./assets/es.png"};let he="en";const Q=[{id:1,name:"Easy",emoji:"🍀",range:3,multiplier:200,chance:"33%",color:"emerald",hex:"#10b981",bgFrom:"from-emerald-500/20",bgTo:"to-green-600/10",borderColor:"border-emerald-500/50",textColor:"text-emerald-400"},{id:2,name:"Medium",emoji:"⚡",range:10,multiplier:500,chance:"10%",color:"violet",hex:"#8b5cf6",bgFrom:"from-violet-500/20",bgTo:"to-purple-600/10",borderColor:"border-violet-500/50",textColor:"text-violet-400"},{id:3,name:"Hard",emoji:"👑",range:100,multiplier:5e3,chance:"1%",color:"amber",hex:"#f59e0b",bgFrom:"from-amber-500/20",bgTo:"to-orange-600/10",borderColor:"border-amber-500/50",textColor:"text-amber-400"}],Pa=5700,h={mode:null,phase:"select",guess:50,guesses:[2,5,50],comboStep:0,wager:10,gameId:null,result:null,txHash:null,poolStatus:null,history:[],serviceFee:0n,serviceFee1x:0n,serviceFee5x:0n,tiersData:null};function Rr(){if(document.getElementById("fortune-styles-v2"))return;const e=document.createElement("style");e.id="fortune-styles-v2",e.textContent=`
    ...
    `;const n=document.getElementById("number-input"),a=document.getElementById("number-slider"),i=Q[2],o=f=>{f=Math.max(1,Math.min(100,f)),h.guess=f,n&&(n.value=f),a&&(a.value=f,a.style.background=`linear-gradient(to right, ${i.hex} 0%, ${i.hex} ${f}%, #27272a ${f}%, #27272a 100%)`)};n==null||n.addEventListener("input",f=>o(parseInt(f.target.value)||1)),n==null||n.addEventListener("blur",f=>o(parseInt(f.target.value)||1)),a==null||a.addEventListener("input",f=>o(parseInt(f.target.value))),(r=document.getElementById("btn-minus"))==null||r.addEventListener("click",()=>o(h.guess-1)),(l=document.getElementById("btn-plus"))==null||l.addEventListener("click",()=>o(h.guess+1)),(d=document.getElementById("btn-minus-10"))==null||d.addEventListener("click",()=>o(h.guess-10)),(u=document.getElementById("btn-plus-10"))==null||u.addEventListener("click",()=>o(h.guess+10)),document.querySelectorAll(".quick-pick").forEach(f=>{f.addEventListener("click",()=>o(parseInt(f.dataset.number)))}),(m=document.getElementById("btn-random"))==null||m.addEventListener("click",()=>{o(Math.floor(Math.random()*100)+1)}),(p=document.getElementById("btn-back"))==null||p.addEventListener("click",()=>{h.phase="select",G()}),(g=document.getElementById("btn-next"))==null||g.addEventListener("click",()=>{h.phase="wager",G()})}function Or(e){var i,o,r,l,d,u,m;const t=Q[h.comboStep],s=h.guesses[h.comboStep],n=t.range===100;if(e.innerHTML=`
        <div class="bg-gradient-to-br from-zinc-900 to-zinc-800/50 border border-zinc-700/50 rounded-2xl p-5">
            <!-- Progress Pills -->
            <div class="flex justify-center gap-2 sm:gap-3 mb-5">
                ${Q.map((p,g)=>{const f=g===h.comboStep,x=g<h.comboStep;return`
                        <div class="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 rounded-xl border ${f?`bg-gradient-to-br ${p.bgFrom} ${p.bgTo} ${p.borderColor}`:x?"bg-emerald-500/10 border-emerald-500/50":"bg-zinc-800/50 border-zinc-700/50"}">
                            <span class="text-lg sm:text-xl">${x?"✓":p.emoji}</span>
                            <div class="text-left">
                                <p class="text-[10px] sm:text-xs font-bold ${f?p.textColor:x?"text-emerald-400":"text-zinc-500"}">${p.name}</p>
    ...
    `,t.range===100){const p=document.getElementById("combo-number-input"),g=document.getElementById("combo-slider"),f=x=>{x=Math.max(1,Math.min(100,x)),h.guesses[h.comboStep]=x,p&&(p.value=x),g&&(g.value=x,g.style.background=`linear-gradient(to right, ${t.hex} 0%, ${t.hex} ${x}%, #27272a ${x}%, #27272a 100%)`)};p==null||p.addEventListener("input",x=>f(parseInt(x.target.value)||1)),p==null||p.addEventListener("blur",x=>f(parseInt(x.target.value)||1)),g==null||g.addEventListener("input",x=>f(parseInt(x.target.value))),(i=document.querySelector(".combo-minus"))==null||i.addEventListener("click",()=>f(h.guesses[h.comboStep]-1)),(o=document.querySelector(".combo-plus"))==null||o.addEventListener("click",()=>f(h.guesses[h.comboStep]+1)),(r=document.querySelector(".combo-minus-10"))==null||r.addEventListener("click",()=>f(h.guesses[h.comboStep]-10)),(l=document.querySelector(".combo-plus-10"))==null||l.addEventListener("click",()=>f(h.guesses[h.comboStep]+10)),document.querySelectorAll(".combo-quick").forEach(x=>{x.addEventListener("click",()=>f(parseInt(x.dataset.num)))}),(d=document.querySelector(".combo-random"))==null||d.addEventListener("click",()=>{f(Math.floor(Math.random()*100)+1)})}else document.querySelectorAll(".num-btn").forEach(p=>{p.addEventListener("click",()=>{const g=parseInt(p.dataset.num);h.guesses[h.comboStep]=g,document.querySelectorAll(".num-btn").forEach(f=>{parseInt(f.dataset.num)===g?f.className=`num-btn w-12 h-12 rounded-xl font-bold text-lg transition-all bg-gradient-to-br ${t.bgFrom} ${t.bgTo} border-2 ${t.borderColor} ${t.textColor}`:f.className="num-btn w-12 h-12 rounded-xl font-bold text-lg transition-all bg-zinc-800/60 border border-zinc-700/50 text-zinc-400 hover:border-zinc-600"})})});(u=document.getElementById("btn-back"))==null||u.addEventListener("click",()=>{h.comboStep>0?(h.comboStep--,G()):(h.phase="select",G())}),(m=document.getElementById("btn-next"))==null||m.addEventListener("click",()=>{h.comboStep<2?(h.comboStep++,G()):(h.phase="wager",G())})}function Wr(e){const t=h.mode==="jackpot",s=t?[h.guess]:h.guesses,n=t?50:Pa,a=B(c.currentUserBalance||0n),i=a>=1,o=t?h.serviceFee1x:h.serviceFee5x,r=o>0n?Number(o)/1e18:0,l=o>0n;e.innerHTML=`
        <div class="bg-gradient-to-br from-zinc-900 to-zinc-800/50 border border-zinc-700/50 rounded-2xl p-5">
    ...
    `,n.forEach((a,i)=>{const o=document.getElementById(`spin-${i}`);if(!o)return;setInterval(()=>{o.textContent=Math.floor(Math.random()*a.range)+1},80)})}function Kr(e){var u,m;const t=h.result;if(!t)return G();const s=h.mode==="jackpot",n=s?[h.guess]:h.guesses,a=t.rolls||[],i=s?[Q[2]]:Q,o=n.map((p,g)=>{const f=a[g]!==void 0?Number(a[g]):null;return f!==null&&f===p}),r=o.filter(p=>p).length,l=t.prizeWon>0||r>0;let d=t.prizeWon;!d&&r>0&&(d=0,o.forEach((p,g)=>{if(p){const f=s?Q[2]:Q[g];d+=h.wager*f.multiplier}})),e.innerHTML=`
        <div class="bg-gradient-to-br ${l?"from-emerald-900/30 to-green-900/10 border-emerald-500/30":"from-zinc-900 to-zinc-800/50 border-zinc-700/50"} border rounded-2xl p-4 sm:p-6 relative overflow-hidden" id="result-container">
            ...
            <div class="grid ${s?"grid-cols-1 max-w-[200px] mx-auto":"grid-cols-3"} gap-2 sm:gap-3 mb-4">
                ${i.map((p,g)=>{const f=s?n[0]:n[g],x=a[g],y=o[g];return`
                            ...
                            <div class="flex items-center justify-center gap-2">
                                <div class="text-center">
                                    ...
                                    <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-gradient-to-br ${p.bgFrom} ${p.bgTo} border ${p.borderColor} flex items-center justify-center">
                                        <span class="text-lg sm:text-xl font-black ${p.textColor}">${f}</span>
                                    </div>
                                </div>
    ...
    `,l&&(Gr(),d>h.wager*10&&Vr()),(u=document.getElementById("btn-new-game"))==null||u.addEventListener("click",()=>{h.phase="select",h.result=null,h.txHash=null,h.gameId=null,G(),sn()}),(m=document.getElementById("btn-share"))==null||m.addEventListener("click",()=>{Xr(l,d)})}function Gr(){const e=document.createElement("div");e.className="confetti-container",document.body.appendChild(e);const t=["#f59e0b","#10b981","#8b5cf6","#ec4899","#06b6d4"],s=["●","■","★","🐯","🎉"];for(let n=0;n<60;n++){const a=document.createElement("div");a.className="confetti",a.style.cssText=`
            left: ${Math.random()*100}%;
            color: ${t[n%t.length]};
            font-size: ${8+Math.random()*12}px;
            animation-delay: ${Math.random()*2}s;
            animation-duration: ${2+Math.random()*2}s;
        `,a.textContent=s[n%s.length],e.appendChild(a)}setTimeout(()=>e.remove(),5e3)}function Vr(){const e=["🪙","💰","✨","⭐","🎉"];for(let t=0;t<30;t++)setTimeout(()=>{const s=document.createElement("div");s.className="coin",s.textContent=e[Math.floor(Math.random()*e.length)],s.style.left=`${Math.random()*100}%`,s.style.animationDelay=`${Math.random()*.5}s`,s.style.animationDuration=`${2+Math.random()*2}s`,document.body.appendChild(s),setTimeout(()=>s.remove(),4e3)},t*100)}function Xr(e,t){var l,d,u,m,p,g;const s=Dn[he],n=()=>{const f=Fr[he];return e?f.win(t):f.lose},a=`
        <div class="text-center">
    ...
    `;ct(a,"max-w-xs");const i=f=>{he=f;const x=Dn[f],y=document.getElementById("share-modal-title"),k=document.getElementById("share-modal-subtitle"),I=document.getElementById("btn-close-share");y&&(y.textContent=x.title),k&&(k.textContent=x.subtitle),I&&(I.textContent=x.later),document.querySelectorAll(".lang-btn").forEach(L=>{const A=L.dataset.lang,$=L.querySelector("span");A===f?(L.className="lang-btn flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all bg-amber-500/20 border-amber-500 ring-1 ring-amber-500/50 border",$&&($.className="text-amber-400")):(L.className="lang-btn flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all bg-zinc-800 border-zinc-700 hover:border-zinc-500 border",$&&($.className="text-zinc-400"))})};document.querySelectorAll(".lang-btn").forEach(f=>{f.addEventListener("click",()=>i(f.dataset.lang))});const o=async f=>{if(!c.userAddress)return!1;try{const y=await(await fetch("https://us-central1-backchain-backand.cloudfunctions.net/trackShare",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({address:c.userAddress,gameId:h.gameId||Date.now(),type:"fortune",platform:f})})).json();return y.success?(b(`🎉 +${y.pointsAwarded||Ns} Airdrop Points!`,"success"),!0):(y.reason==="already_shared"&&console.log("Already shared this game"),!1)}catch(x){return console.error("Share tracking error:",x),b(`🎉 +${Ns} Airdrop Points!`,"success"),!0}},r=async(f,x)=>{await o(f),window.open(x,"_blank"),se()};(l=document.getElementById("share-twitter"))==null||l.addEventListener("click",()=>{const f=n();r("twitter",`https://twitter.com/intent/tweet?text=${encodeURIComponent(f)}`)}),(d=document.getElementById("share-telegram"))==null||d.addEventListener("click",()=>{const f=n();r("telegram",`https://t.me/share/url?url=https://backcoin.org&text=${encodeURIComponent(f)}`)}),(u=document.getElementById("share-whatsapp"))==null||u.addEventListener("click",()=>{const f=n();r("whatsapp",`https://wa.me/?text=${encodeURIComponent(f)}`)}),(m=document.getElementById("share-instagram"))==null||m.addEventListener("click",async()=>{const f=n();try{await navigator.clipboard.writeText(f),await o("instagram");const x=`
            ...
            `)}}catch(t){console.error("loadHistory error:",t)}}function Zr(e){const t=document.getElementById("history-list");t&&(t.innerHTML=e.map(s=>{var f;const n=s.isWin||s.prizeWon&&BigInt(s.prizeWon)>0n,a=s.prizeWon?B(BigInt(s.prizeWon)):0,i=s.wagerAmount?B(BigInt(s.wagerAmount)):0,o=s.isCumulative,r=s.rolls||[],l=s.guesses||[],d=s.txHash||s.transactionHash,u=el(s.timestamp||s.createdAt),m=s.player?`${s.player.slice(0,6)}...${s.player.slice(-4)}`:"???",p=c.userAddress&&((f=s.player)==null?void 0:f.toLowerCase())===c.userAddress.toLowerCase(),g=d?`${La}${d}`:null;return`
            <a href="${g||"#"}" target="${g?"_blank":"_self"}" rel="noopener" 
               ...
               ${g?"":'onclick="return false;"'}>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex gap-1">
                        ${(o?Q:[Q[2]]).map((x,y)=>{const k=l[y],I=r[y];return`
                                <div class="w-6 h-6 rounded text-[10px] font-bold flex items-center justify-center ${k!==void 0&&I!==void 0&&Number(k)===Number(I)?"bg-emerald-500/30 text-emerald-400":"bg-zinc-700/50 text-zinc-500"}">
                                    ${I??"?"}
                                </div>
                            `}).join("")}
                    </div>
    ...
    `}function hl(){var o;const e=z.platformUsageConfig||Ma,t=z.platformUsage||{};let s=0,n=0;Object.keys(e).forEach(r=>{var l;e[r].enabled!==!1&&e[r].maxCount&&(s+=e[r].maxCount,n+=Math.min(((l=t[r])==null?void 0:l.count)||0,e[r].maxCount))});const a=s>0?n/s*100:0,i=((o=z.user)==null?void 0:o.platformUsagePoints)||0;return`
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-2" id="platform-actions-grid">
                ${Object.entries(e).filter(([r,l])=>l.enabled!==!1).map(([r,l])=>{const d=t[r]||{count:0},u=d.count>=l.maxCount,m=Math.max(0,l.maxCount-d.count),p=d.count/l.maxCount*100,g=fl[r]||"";return`
    ...
    `}function kl(){var u,m;const e=((u=z.leaderboards)==null?void 0:u.top100ByPosts)||[],t=((m=z.leaderboards)==null?void 0:m.top100ByPoints)||[],s=z.activeRanking||"posts";function n(p,g,f){var $,T;const x=z.user&&(($=p.walletAddress)==null?void 0:$.toLowerCase())===((T=z.user.walletAddress)==null?void 0:T.toLowerCase()),y=Tl(g+1),k=f==="posts"?"bg-amber-500/10":"bg-green-500/10",I=f==="posts"?"text-amber-400":"text-green-400",L=f==="posts"?"text-white":"text-green-400",A=f==="posts"?"posts":"pts";return`
...
(This will reset their rejection count to 0)`;if(!window.confirm(a))return;const i=t.innerHTML;t.disabled=!0,t.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>';try{await la(s,n),b(`User ${n?"BANNED":"UNBANNED"}.`,"success");const o=w.allUsers.findIndex(r=>r.id===s);o>-1&&(w.allUsers[o].isBanned=n,w.allUsers[o].hasPendingAppeal=!1,n===!1&&(w.allUsers[o].rejectedCount=0)),Be()}catch(o){b(`Failed: ${o.message}`,"error"),t.disabled=!1,t.innerHTML=i}},_l=async e=>{const t=e.target.closest(".resolve-appeal-btn");if(!t||t.disabled)return;const s=t.dataset.userId,a=t.dataset.action==="approve";if(!s)return;const i=a?"Are you sure you want to APPROVE this appeal and UNBAN the user?":"Are you sure you want to DENY this appeal? The user will remain banned.";if(!window.confirm(i))return;const o=t.closest("td").querySelectorAll("button"),r=new Map;o.forEach(l=>{r.set(l,l.innerHTML),l.disabled=!0,l.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>'});try{a&&await la(s,!1),b(`Appeal ${a?"APPROVED":"DENIED"}.`,"success");const l=w.allUsers.findIndex(d=>d.id===s);l>-1&&(w.allUsers[l].hasPendingAppeal=!1,a&&(w.allUsers[l].isBanned=!1,w.allUsers[l].rejectedCount=0)),Be()}catch(l){b(`Failed: ${l.message}`,"error"),o.forEach(d=>{d.disabled=!1,d.innerHTML=r.get(d)})}},Ul=async e=>{const t=e.target.closest(".re-approve-btn");if(!t||t.disabled)return;const s=t.dataset.submissionId,n=t.dataset.userId;if(!s||!n)return;const a=t.closest("tr");a&&(a.style.opacity="0.5"),t.disabled=!0,t.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>';try{await ra(n,s,"approved"),b("Submission re-approved!","success"),w.selectedUserSubmissions=w.selectedUserSubmissions.filter(o=>o.submissionId!==s),a&&a.remove();const i=w.allUsers.findIndex(o=>o.id===n);if(i>-1){const o=w.allUsers[i];o.rejectedCount=Math.max(0,(o.rejectedCount||0)-1),Be()}if(w.selectedUserSubmissions.length===0){const o=document.querySelector("#admin-user-modal .p-6");o&&(o.innerHTML='<p class="text-zinc-400 text-center p-8">This user has no rejected submissions.</p>')}}catch(i){b(`Failed to re-approve: ${i.message}`,"error"),a&&(a.style.opacity="1"),t.disabled=!1,t.innerHTML='<i class="fa-solid fa-check"></i> Re-Approve'}},Ol=async e=>{const t=e.target.closest(".view-rejected-btn");if(!t||t.disabled)return;const s=t.dataset.userId,n=t.dataset.wallet;if(s){w.selectedWallet=n,w.isSubmissionsModalOpen=!0,vs(!0,[]);try{const a=await Yi(s,"rejected");w.selectedUserSubmissions=a,vs(!1,a)}catch(a){b(`Error fetching user submissions: ${a.message}`,"error"),vs(!1,[],!0)}}},Wl=()=>{w.isSubmissionsModalOpen=!1,w.selectedUserSubmissions=[],w.selectedWallet=null;const e=document.getElementById("admin-user-modal");e&&e.remove(),document.body.style.overflow="auto"},ql=e=>{const t=e.target.closest(".user-profile-link");if(!t)return;e.preventDefault();const s=t.dataset.userId;if(!s)return;const n=w.allUsers.find(a=>a.id===s);if(!n){b("Error: Could not find user data.","error");return}Jl(n)},Yl=()=>{const e=document.getElementById("admin-user-profile-modal");e&&e.remove(),document.body.style.overflow="auto"},Kl=async e=>{e.preventDefault();const t=e.target;let s,n;try{if(s=new Date(t.startDate.value+"T00:00:00Z"),n=new Date(t.endDate.value+"T23:59:59Z"),isNaN(s.getTime())||isNaN(n.getTime()))throw new Error("Invalid date format.");if(s>=n)throw new Error("Start Date must be before End Date.")}catch(l){b(l.message,"error");return}const a={title:t.title.value.trim(),url:t.url.value.trim(),description:t.description.value.trim(),points:parseInt(t.points.value,10),cooldownHours:parseInt(t.cooldown.value,10),startDate:s,endDate:n};if(!a.title||!a.description){b("Please fill in Title and Description.","error");return}if(a.points<=0||a.cooldownHours<=0){b("Points and Cooldown must be positive numbers.","error");return}if(a.url&&!a.url.startsWith("http")){b("URL must start with http:// or https://","error");return}w.editingTask&&w.editingTask.id&&(a.id=w.editingTask.id);const i=t.querySelector('button[type="submit"]'),o=i.innerHTML;i.disabled=!0;const r=document.createElement("span");r.classList.add("inline-block"),i.innerHTML="",i.appendChild(r);try{await Ui(a),b(`Task ${a.id?"updated":"created"} successfully!`,"success"),t.reset(),w.editingTask=null,ot()}catch(l){b(`Failed to save task: ${l.message}`,"error"),console.error(l),i.disabled=!1,i.innerHTML=o}},Gl=async e=>{e.preventDefault();const t=e.target,s={YouTube:parseInt(t.youtubePoints.value,10),"YouTube Shorts":parseInt(t.youtubeShortsPoints.value,10),Instagram:parseInt(t.instagramPoints.value,10),"X/Twitter":parseInt(t.xTwitterPoints.value,10),Facebook:parseInt(t.facebookPoints.value,10),Telegram:parseInt(t.telegramPoints.value,10),TikTok:parseInt(t.tiktokPoints.value,10),Reddit:parseInt(t.redditPoints.value,10),LinkedIn:parseInt(t.linkedinPoints.value,10),Other:parseInt(t.otherPoints.value,10)};if(Object.values(s).some(o=>isNaN(o)||o<0)){b("All points must be positive numbers (or 0).","error");return}const n=t.querySelector('button[type="submit"]'),a=n.innerHTML;n.disabled=!0;const i=document.createElement("span");i.classList.add("inline-block"),n.innerHTML="",n.appendChild(i);try{await Hi(s),b("UGC Base Points updated successfully!","success"),w.ugcBasePoints=s}catch(o){b(`Failed to update points: ${o.message}`,"error"),console.error(o)}finally{document.body.contains(n)&&(n.disabled=!1,n.innerHTML=a)}},Vl=e=>{const t=w.dailyTasks.find(s=>s.id===e);t&&(w.editingTask=t,mt())},Xl=async e=>{if(window.confirm("Are you sure you want to delete this task permanently?"))try{await Oi(e),b("Task deleted.","success"),w.editingTask=null,ot()}catch(t){b(`Failed to delete task: ${t.message}`,"error"),console.error(t)}};function vs(e,t,s=!1){var o,r;const n=document.getElementById("admin-user-modal");n&&n.remove(),document.body.style.overflow="hidden";let a="";e?a='<div class="p-8"></div>':s?a='<p class="text-red-400 text-center p-8">Failed to load submissions.</p>':t.length===0?a='<p class="text-zinc-400 text-center p-8">This user has no rejected submissions.</p>':a=`
         ...
         <div id="admin-user-modal" class="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" style="backdrop-filter: blur(5px);">
             <div class="bg-sidebar border border-border-color rounded-xl shadow-lg w-full max-w-3xl max-h-[90vh] flex flex-col">
                 <div class="p-5 border-b border-border-color flex justify-between items-center">
                     <h2 class="text-xl font-bold text-white">Rejected Posts for ${Ve(w.selectedWallet)}</h2>
    ...
    `;const I=document.getElementById("admin-users-pagination");I&&g>1&&Js(I,w.usersPage,g,ec),(A=document.getElementById("admin-users-tbody"))==null||A.addEventListener("click",T=>{T.target.closest(".user-profile-link")&&ql(T),T.target.closest(".ban-user-btn")&&Hl(T),T.target.closest(".view-rejected-btn")&&Ol(T),T.target.closest(".resolve-appeal-btn")&&_l(T)}),($=document.getElementById("user-filters-nav"))==null||$.addEventListener("click",Ql);const L=document.getElementById("user-search-input");if(L){let T;L.addEventListener("keyup",F=>{clearTimeout(T),T=setTimeout(()=>Zl(F),300)})}},Wn=()=>{var n;const e=document.getElementById("manage-ugc-points-content");if(!e)return;const t=w.ugcBasePoints;if(!t)return;const s={YouTube:5e3,"YouTube Shorts":2500,Instagram:3e3,"X/Twitter":1500,Facebook:2e3,Telegram:1e3,TikTok:3500,Reddit:1800,LinkedIn:2200,Other:1e3};e.innerHTML=`
    ...
    `,(n=document.getElementById("ugcPointsForm"))==null||n.addEventListener("submit",Gl)},mt=()=>{var f,x,y;const e=document.getElementById("manage-tasks-content");if(!e)return;const t=w.editingTask,s=!!t,n=k=>{if(!k)return"";try{return(k.toDate?k.toDate():k instanceof Date?k:new Date(k)).toISOString().split("T")[0]}catch{return""}},a=w.tasksPage,i=w.tasksPerPage,o=[...w.dailyTasks].sort((k,I)=>{var $,T;const L=($=k.startDate)!=null&&$.toDate?k.startDate.toDate():new Date(k.startDate||0);return((T=I.startDate)!=null&&T.toDate?I.startDate.toDate():new Date(I.startDate||0)).getTime()-L.getTime()}),r=o.length,l=Math.ceil(r/i),d=(a-1)*i,u=a*i,m=o.slice(d,u),p=m.length>0?m.map(k=>{var T,F;const I=new Date,L=(T=k.startDate)!=null&&T.toDate?k.startDate.toDate():k.startDate?new Date(k.startDate):null,A=(F=k.endDate)!=null&&F.toDate?k.endDate.toDate():k.endDate?new Date(k.endDate):null;let $="text-zinc-500";return L&&A&&(I>=L&&I<=A?$="text-green-400":I<L&&($="text-blue-400")),`
    ...
    `;const g=document.getElementById("admin-tasks-pagination");g&&l>1&&Js(g,w.tasksPage,l,sc),(f=document.getElementById("taskForm"))==null||f.addEventListener("submit",Kl),(x=document.getElementById("cancelEditBtn"))==null||x.addEventListener("click",()=>{w.editingTask=null,mt()}),(y=document.getElementById("existing-tasks-list"))==null||y.addEventListener("click",k=>{const I=k.target.closest("button[data-id]");if(!I)return;const L=I.dataset.id;I.dataset.action==="edit"&&Vl(L),I.dataset.action==="delete"&&Xl(L)})},Ut=()=>{var m;const e=document.getElementById("submissions-content");if(!e)return;if(!w.allSubmissions||w.allSubmissions.length===0){const p=document.createElement("div");e.innerHTML=p.innerHTML;return}const t=w.submissionsPage,s=w.submissionsPerPage,n=[...w.allSubmissions].sort((p,g)=>{var f,x;return(((f=g.submittedAt)==null?void 0:f.getTime())||0)-(((x=p.submittedAt)==null?void 0:x.getTime())||0)}),a=n.length,i=Math.ceil(a/s),o=(t-1)*s,r=t*s,d=n.slice(o,r).map(p=>{var g,f;return`
        <tr class="border-b border-border-color hover:bg-zinc-800/50">
            ...
            <td class="p-3 text-xs font-semibold ${((g=On[p.status])==null?void 0:g.color)||"text-gray-500"}">${((f=On[p.status])==null?void 0:f.text)||p.status}</td>
            <td class="p-3 text-right">
                <div class="flex items-center justify-end gap-2">
    ...
    `;const u=document.getElementById("admin-submissions-pagination");u&&i>1&&Js(u,w.submissionsPage,i,tc),(m=document.getElementById("admin-submissions-tbody"))==null||m.addEventListener("click",jl)},Ot=()=>{var i,o;const e=document.getElementById("platform-usage-content");if(!e)return;const t=w.platformUsageConfig||ss;let s=0;Object.values(t).forEach(r=>{r.enabled!==!1&&(s+=(r.points||0)*(r.maxCount||1))});const n=Object.entries(t).map(([r,l])=>`
...
`)[0].trim()||"Notarized Document";const m=Wa("",u),p=l&&l.length>10,g=Bc(r.timestamp);return`
                    ...
                    <div class="h-28 bg-gradient-to-br from-zinc-800/50 to-zinc-900/50 flex items-center justify-center relative overflow-hidden">
                        ${p?`
                            ...
                            <div class="hidden flex-col items-center justify-center h-full absolute inset-0 bg-zinc-900">
                                <div class="w-12 h-12 rounded-xl ${m.bg} flex items-center justify-center mb-1">
                                    <i class="${m.icon} text-2xl ${m.color}"></i>
                                </div>
                            </div>
                        `:`
                            <div class="flex flex-col items-center justify-center">
                                <div class="w-14 h-14 rounded-xl ${m.bg} flex items-center justify-center mb-2">
                                    <i class="${m.icon} text-2xl ${m.color}"></i>
                                </div>
        ...
        `}}}function Rc(){var e;(e=document.getElementById("btn-refresh"))==null||e.addEventListener("click",async()=>{const t=document.getElementById("btn-refresh");t&&(t.innerHTML='<i class="fa-solid fa-rotate fa-spin"></i> Loading...'),await ns(),t&&(t.innerHTML='<i class="fa-solid fa-rotate"></i> Refresh')})}async function Dc(){const e=Date.now();if(!(e-P.lastFetch<3e4&&c.notaryFee>0n))try{!c.ecosystemManagerContractPublic&&!c.ecosystemManagerContract&&await Ks();const t=c.ecosystemManagerContractPublic||c.ecosystemManagerContract;if(t){let s=0n;try{const n=Ue.id("NOTARY_SERVICE");s=await O(t,"getFee",[n],0n)}catch(n){console.warn("getFee with key failed:",n.message);try{s=await O(t,"notaryFee",[],0n)}catch(a){console.warn("notaryFee failed:",a.message);try{const i=Ue.id("NOTARY_SERVICE");s=await O(t,"fees",[i],0n)}catch(i){console.warn("fees mapping failed:",i.message)}}}s>0n?(c.notaryFee=s,console.log("📜 Notary fee loaded:",B(s),"BKC")):(c.notaryFee=Ue.parseEther("1"),console.log("📜 Using default notary fee: 1 BKC")),P.lastFetch=e}if(c.isConnected&&c.userAddress)try{if(c.bkcTokenContract||c.bkcTokenContractPublic){const s=c.bkcTokenContract||c.bkcTokenContractPublic,n=await O(s,"balanceOf",[c.userAddress],0n);n>0n&&(c.currentUserBalance=n,console.log("📜 User balance loaded:",B(n),"BKC"))}}catch(s){console.warn("Balance load error:",s.message)}Wt()}catch(t){console.warn("Notary data error:",t),(!c.notaryFee||c.notaryFee===0n)&&(c.notaryFee=Ue.parseEther("1"))}}const Ya={async render(e){e&&(Ic(),await Dc(),c.isConnected&&await V(),Wt(),xe())},reset(){P.file=null,P.description="",P.step=1,xe()},update(){Wt(),P.isProcessing||xe()},refreshHistory(){ns()},async addToWallet(e,t){var s,n,a,i;try{const o=["https://dweb.link/ipfs/","https://w3s.link/ipfs/","https://nftstorage.link/ipfs/","https://cloudflare-ipfs.com/ipfs/","https://ipfs.io/ipfs/"],r=m=>m?m.startsWith("ipfs://")?m.replace("ipfs://","").split("?")[0]:m.includes("/ipfs/")?m.split("/ipfs/")[1].split("?")[0]:m.match(/^Qm[a-zA-Z0-9]{44}/)||m.match(/^bafy[a-zA-Z0-9]+/)?m:"":"",l=(m,p=0)=>{if(!m)return"";if(m.startsWith("https://")&&!m.includes("/ipfs/")&&!m.includes("ipfs.io"))return m;const g=r(m);return g?`${o[p]}${g}`:m};let d=l(t||"");if(console.log("📜 Input imageUrl:",t),console.log("📜 Converted to:",d),c.decentralizedNotaryContract)try{const m=await c.decentralizedNotaryContract.tokenURI(e);if(console.log("📜 TokenURI response:",m==null?void 0:m.slice(0,200)),m&&m.startsWith("data:application/json;base64,")){const p=m.replace("data:application/json;base64,",""),g=JSON.parse(atob(p));console.log("📜 Parsed metadata:",JSON.stringify(g).slice(0,300)),g.image&&(d=l(g.image),console.log("📜 Image from metadata:",d))}}catch(m){console.warn("Could not fetch tokenURI:",m.message)}console.log("📜 Final image URL for wallet:",d);const u=((s=c.decentralizedNotaryContract)==null?void 0:s.target)||((n=c.decentralizedNotaryContract)!=null&&n.getAddress?await c.decentralizedNotaryContract.getAddress():null);if(!u){b("Contract address not found","error");return}console.log("📜 Contract address:",u),console.log("📜 Token ID:",e),b("Adding NFT #"+e+" to wallet...","info");try{await window.ethereum.request({method:"wallet_watchAsset",params:{type:"ERC721",options:{address:u,tokenId:String(e),image:d}}})?b("📜 NFT #"+e+" added to wallet!","success"):b("NFT not added (cancelled)","warning");return}catch(m){if(console.warn("First attempt with image failed:",(a=m.message)==null?void 0:a.slice(0,100)),m.code!==4001)try{await window.ethereum.request({method:"wallet_watchAsset",params:{type:"ERC721",options:{address:u,tokenId:String(e)}}})?b("📜 NFT #"+e+" added to wallet!","success"):b("NFT not added (cancelled)","warning");return}catch(p){throw p}throw m}}catch(o){if(console.error("Add to wallet error:",o),o.code===4001)return;o.code===4100||(i=o.message)!=null&&i.includes("spam")?b("MetaMask blocked request. Wait a few seconds and try again.","warning"):b("Could not add NFT: "+(o.message||"Unknown error"),"error")}}};window.NotaryPage=Ya;const bt=window.ethers,je="./assets/airbnft.png",jc="https://sepolia.arbiscan.io/tx/",j={activeTab:"marketplace",filterTier:"ALL",selectedRentalId:null,isLoading:!1,isTransactionPending:!1,rentalHistory:[]};function yt(e){return e?e.startsWith("https://")||e.startsWith("http://")?e:e.includes("ipfs.io/ipfs/")?`${Y}${e.split("ipfs.io/ipfs/")[1]}`:e.startsWith("ipfs://")?`${Y}${e.substring(7)}`:e:"./assets/nft.png"}function Ka(e){const t=Math.floor(Date.now()/1e3),s=e-t;if(s<=0)return{text:"Expired",expired:!0};const n=Math.floor(s/3600),a=Math.floor(s%3600/60);return n>0?{text:`${n}h ${a}m`,expired:!1}:{text:`${a}m`,expired:!1}}function He(e){return ee.find(s=>s.boostBips===Number(e))||{name:"Unknown",img:"./assets/bkc_logo_3d.png",boostBips:0}}function Ga(e){return{Diamond:"from-cyan-500 to-blue-600",Platinum:"from-slate-300 to-gray-500",Gold:"from-yellow-400 to-amber-600",Silver:"from-gray-300 to-zinc-500",Bronze:"from-orange-500 to-amber-700"}[e]||"from-zinc-500 to-zinc-700"}function Hc(e){if(!e)return"";try{const t=e.seconds||e._seconds||new Date(e).getTime()/1e3;return new Date(t*1e3).toLocaleString("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return""}}function _c(e="Loading..."){return`
