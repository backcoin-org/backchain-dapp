...
`)[0].trim()||"Notarized Document";const p=Hr("",u),m=l&&l.length>10,b=Xp(r.timestamp);return`
    ...
    <div class="min-h-screen pb-12">
        ...
        ${k0()}
        ${T0()}
    </div>`}function Kr(){const e=c.rentalListings||[],n=e.filter(i=>c.isConnected&&Lt(i.owner,c.userAddress)).reduce((i,o)=>i+Number(ta.formatEther(BigInt(o.totalEarnings||0))),0),a=Math.floor(Date.now()/1e3),s=e.filter(i=>!i.isRented&&!(i.rentalEndTime&&Number(i.rentalEndTime)>a)).length;return`
        ...
        </div>`}function Jt(){const e=document.getElementById("content");if(e){switch(j.countdownIntervals.forEach(clearInterval),j.countdownIntervals=[],e.classList.remove("r-fadeUp"),e.offsetWidth,e.classList.add("r-fadeUp"),j.activeTab){case"marketplace":e.innerHTML=f0();break;case"my-listings":e.innerHTML=b0();break;case"my-rentals":e.innerHTML=h0(),y0();break}document.getElementById("header-stats").innerHTML=Kr(),m0()}}function m0(){const t=(c.rentalListings||[]).filter(o=>c.isConnected&&Lt(o.owner,c.userAddress)),n=Math.floor(Date.now()/1e3),a=(c.myRentals||[]).filter(o=>Lt(o.tenant,c.userAddress)&&Number(o.endTime)>n),s=document.getElementById("cnt-listings"),i=document.getElementById("cnt-rentals");s&&(s.textContent=t.length),i&&(i.textContent=a.length)}function f0(){const e=c.rentalListings||[],t=Math.floor(Date.now()/1e3);let n=e.filter(a=>!(a.isRented||a.rentalEndTime&&Number(a.rentalEndTime)>t||j.filterTier!=="ALL"&&ht(a.boostBips).name!==j.filterTier));return n.sort((a,s)=>{const i=BigInt(a.pricePerHour||0),o=BigInt(s.pricePerHour||0);return j.sortBy==="price-low"?i<o?-1:1:j.sortBy==="price-high"?i>o?-1:1:(s.boostBips||0)-(a.boostBips||0)}),`
        <div>
            ...
            `}
        </div>`}function g0(e,t){const n=ht(e.boostBips),a=aa(n.name),s=$(BigInt(e.pricePerHour||0)).toFixed(2),i=xe(e.tokenId),o=c.isConnected&&Lt(e.owner,c.userAddress);return`
        ...
        </div>`}function b0(){if(!c.isConnected)return Yr("View your listings");const e=c.rentalListings||[],t=e.filter(i=>Lt(i.owner,c.userAddress)),n=new Set(e.map(i=>xe(i.tokenId))),a=(c.myBoosters||[]).filter(i=>!n.has(xe(i.tokenId)));return`
        <div>
        ...
        </div>`}function h0(){if(!c.isConnected)return Yr("View your rentals");const e=Math.floor(Date.now()/1e3),t=(c.myRentals||[]).filter(s=>Lt(s.tenant,c.userAddress)),n=t.filter(s=>Number(s.endTime)>e),a=t.filter(s=>Number(s.endTime)<=e).slice(0,5);return`
        <div>
...
`;document.head.appendChild(qr);const L0={render:async e=>{if(!e)return;const t=document.getElementById("creditcard");t&&(t.innerHTML=`
    ...
    `}function Na(e,t,n,a,s,i,o){const r=Ps[bt];let l=`
        <div class="mb-10">
