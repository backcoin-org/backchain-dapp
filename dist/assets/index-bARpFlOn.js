import{defaultConfig as Qa,createWeb3Modal as Za}from"https://esm.sh/@web3modal/ethers@5.1.11?bundle";import{initializeApp as ei}from"https://www.gstatic.com/firebasejs/9.15.0/firebase-app.js";import{getAuth as ti,onAuthStateChanged as si,signInAnonymously as ni}from"https://www.gstatic.com/firebasejs/9.15.0/firebase-auth.js";import{getFirestore as ai,doc as _,getDoc as te,writeBatch as bt,increment as ie,collection as we,query as Ne,where as Nt,orderBy as Ke,getDocs as Te,limit as ii,serverTimestamp as ze,updateDoc as qt,setDoc as Yt,Timestamp as cn,addDoc as oi,deleteDoc as ri}from"https://www.gstatic.com/firebasejs/9.15.0/firebase-firestore.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function s(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(a){if(a.ep)return;a.ep=!0;const i=s(a);fetch(a.href,i)}})();const se={sidebar:document.getElementById("sidebar"),sidebarBackdrop:document.getElementById("sidebar-backdrop"),menuBtn:document.getElementById("menu-btn"),navLinks:document.getElementById("nav-links"),mainContentSections:document.querySelectorAll("main section"),connectButtonDesktop:document.getElementById("connectButtonDesktop"),connectButtonMobile:document.getElementById("connectButtonMobile"),mobileAppDisplay:document.getElementById("mobileAppDisplay"),desktopDisconnected:document.getElementById("desktopDisconnected"),desktopConnectedInfo:document.getElementById("desktopConnectedInfo"),desktopUserAddress:document.getElementById("desktopUserAddress"),desktopUserBalance:document.getElementById("desktopUserBalance"),modalContainer:document.getElementById("modal-container"),toastContainer:document.getElementById("toast-container"),dashboard:document.getElementById("dashboard"),earn:document.getElementById("mine"),store:document.getElementById("store"),rental:document.getElementById("rental"),rewards:document.getElementById("rewards"),actions:document.getElementById("actions"),presale:document.getElementById("presale"),faucet:document.getElementById("faucet"),airdrop:document.getElementById("airdrop"),dao:document.getElementById("dao"),about:document.getElementById("about"),admin:document.getElementById("admin"),tokenomics:document.getElementById("tokenomics"),notary:document.getElementById("notary"),statTotalSupply:document.getElementById("statTotalSupply"),statValidators:document.getElementById("statValidators"),statTotalPStake:document.getElementById("statTotalPStake"),statScarcity:document.getElementById("statScarcity"),statLockedPercentage:document.getElementById("statLockedPercentage"),statUserBalance:document.getElementById("statUserBalance"),statUserPStake:document.getElementById("statUserPStake"),statUserRewards:document.getElementById("statUserRewards")},li={provider:null,publicProvider:null,web3Provider:null,signer:null,userAddress:null,isConnected:!1,bkcTokenContract:null,delegationManagerContract:null,rewardBoosterContract:null,nftLiquidityPoolContract:null,actionsManagerContract:null,fortunePoolContract:null,faucetContract:null,decentralizedNotaryContract:null,ecosystemManagerContract:null,publicSaleContract:null,rentalManagerContract:null,bkcTokenContractPublic:null,delegationManagerContractPublic:null,faucetContractPublic:null,fortunePoolContractPublic:null,rentalManagerContractPublic:null,ecosystemManagerContractPublic:null,actionsManagerContractPublic:null,currentUserBalance:0n,currentUserNativeBalance:0n,userTotalPStake:0n,userDelegations:[],myBoosters:[],activityHistory:[],totalNetworkPStake:0n,allValidatorsData:[],systemFees:{},systemPStakes:{},boosterDiscounts:{},notaryFee:void 0,notaryMinPStake:void 0},ci={set(e,t,s){return e[t]=s,["currentUserBalance","isConnected","userTotalPStake","totalNetworkPStake"].includes(t)&&window.updateUIState&&window.updateUIState(),!0}},c=new Proxy(li,ci);let dn=!1;const m=(e,t="info",s=null)=>{if(!se.toastContainer)return;const n={success:{icon:"fa-check-circle",color:"bg-green-600",border:"border-green-400"},error:{icon:"fa-exclamation-triangle",color:"bg-red-600",border:"border-red-400"},info:{icon:"fa-info-circle",color:"bg-blue-600",border:"border-blue-400"},warning:{icon:"fa-exclamation-circle",color:"bg-yellow-600",border:"border-yellow-400"}},a=n[t]||n.info,i=document.createElement("div");i.className=`flex items-center w-full max-w-xs p-4 text-white rounded-lg shadow-lg transition-all duration-500 ease-out 
    ...
    `;if(s){const r=`https://sepolia.arbiscan.io/tx/${s}`;o+=`<a href="${r}" target="_blank" title="View on Arbiscan" class="ml-3 flex-shrink-0 text-white/80 hover:text-white transition-colors">
                ...
                </button>`,i.innerHTML=o,se.toastContainer.appendChild(i),requestAnimationFrame(()=>{i.classList.remove("translate-x-full","opacity-0"),i.classList.add("translate-x-0","opacity-100")}),setTimeout(()=>{i.classList.remove("translate-x-0","opacity-100"),i.classList.add("translate-x-full","opacity-0"),setTimeout(()=>i.remove(),500)},5e3)},he=()=>{if(!se.modalContainer)return;const e=document.getElementById("modal-backdrop");if(e){const t=document.getElementById("modal-content");t&&(t.classList.remove("animate-fade-in-up"),t.classList.add("animate-fade-out-down")),e.classList.remove("opacity-100"),e.classList.add("opacity-0"),setTimeout(()=>{se.modalContainer.innerHTML=""},300)}},Kt=(e,t="max-w-md",s=!0)=>{var i,o;if(!se.modalContainer)return;const a=`
    ...
    `;se.modalContainer.innerHTML=a,requestAnimationFrame(()=>{const r=document.getElementById("modal-backdrop");r&&r.classList.remove("opacity-0"),r&&r.classList.add("opacity-100")}),(i=document.getElementById("modal-backdrop"))==null||i.addEventListener("click",r=>{s&&r.target.id==="modal-backdrop"&&he()}),(o=document.getElementById("modal-content"))==null||o.querySelectorAll(".closeModalBtn").forEach(r=>{r.addEventListener("click",he)})};async function di(e,t){if(!t||!window.ethereum){m("No wallet detected.","error");return}try{m(`Requesting wallet to track NFT #${t}...`,"info"),await window.ethereum.request({method:"wallet_watchAsset",params:{type:"ERC721",options:{address:e,tokenId:t.toString()}}})?m(`NFT #${t} added successfully!`,"success"):m("Action cancelled by user.","warning")}catch(s){console.error("Add NFT Error:",s),s.code===-32002||s.message&&s.message.includes("not owned")?m("MetaMask cannot sync this NFT on Testnet yet. Please add manually.","warning"):m(`Error: ${s.message}`,"error")}}function ui(e){const t=window.location.origin,n=encodeURIComponent("🚀 Discover Backcoin - The next generation of crypto mining! Proof-of-Purchase, NFT Boosters, Fortune Pool & more. Join the revolution!"),a=encodeURIComponent(t),i={twitter:`https://twitter.com/intent/tweet?text=${n}&url=${a}`,telegram:`https://t.me/share/url?url=${a}&text=${n}`,whatsapp:`https://wa.me/?text=${n}%20${a}`,facebook:`https://www.facebook.com/sharer/sharer.php?u=${a}`,linkedin:`https://www.linkedin.com/sharing/share-offsite/?url=${a}`,instagram:"https://www.instagram.com/",tiktok:"https://www.tiktok.com/"},o=`
    ...
    `;Kt(o,"max-w-md")}const un=e=>{window.navigateTo?window.navigateTo(e):console.error("navigateTo function not found."),he()};function pi(){var a,i,o,r;if(dn)return;dn=!0;const e="https://backcoin.org/presale",t="https://t.me/BackCoinorg";Kt(`
    ...
    `,"max-w-sm",!1);const n=document.getElementById("modal-content");n&&((a=n.querySelector("#btnPresale"))==null||a.addEventListener("click",()=>{window.open(e,"_blank"),he()}),(i=n.querySelector("#btnAirdrop"))==null||i.addEventListener("click",()=>{un("airdrop")}),(o=n.querySelector("#btnSocials"))==null||o.addEventListener("click",()=>{un("socials")}),(r=n.querySelector("#btnTelegram"))==null||r.addEventListener("click",()=>{window.open(t,"_blank")}))}const fi=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1";console.log(`Environment: ${fi?"DEVELOPMENT":"PRODUCTION"}`);const Gn="ZWla0YY4A0Hw7e_rwyOXB",Ae=[{name:"Alchemy",url:`https://arb-sepolia.g.alchemy.com/v2/${Gn}`,priority:1,isPublic:!1,corsCompatible:!0},{name:"BlockPI",url:"https://arbitrum-sepolia.blockpi.network/v1/rpc/public",priority:2,isPublic:!0,corsCompatible:!0},{name:"PublicNode",url:"https://arbitrum-sepolia-rpc.publicnode.com",priority:3,isPublic:!0,corsCompatible:!0},{name:"Arbitrum Official",url:"https://sepolia-rollup.arbitrum.io/rpc",priority:4,isPublic:!0,corsCompatible:!1}].filter(e=>e.url!==null),Ds=`https://arb-sepolia.g.alchemy.com/v2/${Gn}`;let me=0,ct=new Map;function dt(){var e;return((e=Ae[me])==null?void 0:e.url)||Ds}function Vn(){const e=me;do{me=(me+1)%Ae.length;const s=Ae[me];if(!s.corsCompatible){console.warn(`⏭️ Skipping ${s.name} (CORS incompatible)`);continue}if(me===e)return console.warn("⚠️ All RPCs have been tried. Resetting to primary."),me=0,Ae[0].url}while(ct.get(Ae[me].url)==="unhealthy");const t=Ae[me];return console.log(`🔄 Switched to RPC: ${t.name}`),t.url}function mi(e){ct.set(e,"unhealthy"),console.warn(`❌ RPC marked unhealthy: ${e}`),setTimeout(()=>{ct.delete(e),console.log(`♻️ RPC health reset: ${e}`)},6e4)}function bi(e){ct.set(e,"healthy")}function gi(){me=0,ct.clear(),console.log(`✅ Reset to primary RPC: ${Ae[0].name}`)}const Y="https://white-defensive-eel-240.mypinata.cloud/ipfs/",x={},xi={bkcToken:null,ecosystemManager:null,delegationManager:null,rewardBoosterNFT:null,rentalManager:null,nftLiquidityPoolFactory:null,fortunePool:null,fortunePoolV2:null,backchainRandomness:null,publicSale:null,decentralizedNotary:null,faucet:null,miningManager:null};async function vi(){try{const e=await fetch(`./deployment-addresses.json?t=${Date.now()}`);if(!e.ok)throw new Error(`Failed to fetch deployment-addresses.json: ${e.status}`);const t=await e.json(),n=["bkcToken","delegationManager","ecosystemManager","miningManager"].filter(a=>!t[a]);if(n.length>0)throw new Error(`Missing required addresses: ${n.join(", ")}`);return Object.assign(x,t),x.fortunePoolV2=t.fortunePoolV2||t.fortunePool,x.fortunePool=t.fortunePool,x.actionsManager=t.fortunePool,x.rentalManager=t.rentalManager||t.RentalManager||t.rental_manager||null,x.decentralizedNotary=t.decentralizedNotary||t.notary||t.Notary||null,x.bkcDexPoolAddress=t.bkcDexPoolAddress||"#",x.backchainRandomness=t.backchainRandomness||null,Object.assign(xi,t),console.log("✅ Contract addresses loaded"),console.log("   FortunePool V2:",x.fortunePoolV2),!0}catch(e){return console.error("❌ Failed to load contract addresses:",e),!1}}const Z=[{name:"Diamond",boostBips:7e3,color:"text-cyan-400",img:`${Y}bafybeicgip72jcqgsirlrhn3tq5cc226vmko6etnndzl6nlhqrktfikafq/diamond_booster.json`,realImg:`${Y}bafybeicgip72jcqgsirlrhn3tq5cc226vmko6etnndzl6nlhqrktfikafq`,borderColor:"border-cyan-400/50",glowColor:"bg-cyan-500/10"},{name:"Platinum",boostBips:6e3,color:"text-gray-300",img:`${Y}bafybeigc2wgkccckhnjotejve7qyxa2o2z4fsgswfmsxyrbp5ncpc7plei/platinum_booster.json`,realImg:`${Y}bafybeigc2wgkccckhnjotejve7qyxa2o2z4fsgswfmsxyrbp5ncpc7plei`,borderColor:"border-gray-300/50",glowColor:"bg-gray-400/10"},{name:"Gold",boostBips:5e3,color:"text-amber-400",img:`${Y}bafybeifponccrbicg2pcjrn2hrfoqgc77xhm2r4ld7hdpw6cxxkbsckf44/gold_booster.json`,realImg:`${Y}bafybeifponccrbicg2pcjrn2hrfoqgc77xhm2r4ld7hdpw6cxxkbsckf44`,borderColor:"border-amber-400/50",glowColor:"bg-amber-500/10"},{name:"Silver",boostBips:4e3,color:"text-gray-400",img:`${Y}bafybeihvi2inujm5zpi7tl667g4srq273536pjkglwyrtbwmgnskmu7jg4/silver_booster.json`,realImg:`${Y}bafybeihvi2inujm5zpi7tl667g4srq273536pjkglwyrtbwmgnskmu7jg4`,borderColor:"border-gray-400/50",glowColor:"bg-gray-500/10"},{name:"Bronze",boostBips:3e3,color:"text-yellow-600",img:`${Y}bafybeiclqidb67rt3tchhjpsib62s624li7j2bpxnr6b5w5mfp4tomhu7m/bronze_booster.json`,realImg:`${Y}bafybeiclqidb67rt3tchhjpsib62s624li7j2bpxnr6b5w5mfp4tomhu7m`,borderColor:"border-yellow-600/50",glowColor:"bg-yellow-600/10"},{name:"Iron",boostBips:2e3,color:"text-slate-500",img:`${Y}bafybeiaxhv3ere2hyto4dlb5xqn46ehfglxqf3yzehpy4tvdnifyzpp4wu/iron_booster.json`,realImg:`${Y}bafybeiaxhv3ere2hyto4dlb5xqn46ehfglxqf3yzehpy4tvdnifyzpp4wu`,borderColor:"border-slate-500/50",glowColor:"bg-slate-600/10"},{name:"Crystal",boostBips:1e3,color:"text-indigo-300",img:`${Y}bafybeib6nacggrhgcp72xksbhsqcofg3lzhfb576kuebj5ioxpk2id5m7u/crystal_booster.json`,realImg:`${Y}bafybeib6nacggrhgcp72xksbhsqcofg3lzhfb576kuebj5ioxpk2id5m7u`,borderColor:"border-indigo-300/50",glowColor:"bg-indigo-300/10"}],js=["function name() view returns (string)","function symbol() view returns (string)","function decimals() view returns (uint8)","function totalSupply() view returns (uint256)","function balanceOf(address owner) view returns (uint256)","function transfer(address to, uint256 amount) returns (bool)","function approve(address spender, uint256 amount) returns (bool)","function allowance(address owner, address spender) view returns (uint256)","function transferFrom(address from, address to, uint256 amount) returns (bool)","function MAX_SUPPLY() view returns (uint256)","function TGE_SUPPLY() view returns (uint256)","function remainingMintableSupply() view returns (uint256)","event Transfer(address indexed from, address indexed to, uint256 value)","event Approval(address indexed owner, address indexed spender, uint256 value)"],Re=["function totalNetworkPStake() view returns (uint256)","function userTotalPStake(address _user) view returns (uint256)","function pendingRewards(address _user) view returns (uint256)","function MIN_LOCK_DURATION() view returns (uint256)","function MAX_LOCK_DURATION() view returns (uint256)","function getDelegationsOf(address _user) view returns (tuple(uint256 amount, uint64 unlockTime, uint64 lockDuration)[])","function delegate(uint256 _amount, uint256 _lockDuration, uint256 _boosterTokenId) external","function unstake(uint256 _delegationIndex, uint256 _boosterTokenId) external","function forceUnstake(uint256 _delegationIndex, uint256 _boosterTokenId) external","function claimReward(uint256 _boosterTokenId) external","function getUnstakePenaltyBips() view returns (uint256)","event Delegated(address indexed user, uint256 amount, uint256 lockDuration, uint256 pStake)","event Unstaked(address indexed user, uint256 amount, uint256 pStakeReduced)","event RewardClaimed(address indexed user, uint256 amount)"],hi=["function balanceOf(address owner) view returns (uint256)","function tokenOfOwnerByIndex(address owner, uint256 index) view returns (uint256)","function ownerOf(uint256 tokenId) view returns (address)","function approve(address to, uint256 tokenId)","function setApprovalForAll(address operator, bool approved)","function safeTransferFrom(address from, address to, uint256 tokenId)","function boostBips(uint256 _tokenId) view returns (uint256)","function tokenURI(uint256 tokenId) view returns (string)","function totalSupply() view returns (uint256)","event Transfer(address indexed from, address indexed to, uint256 indexed tokenId)","event Approval(address indexed owner, address indexed approved, uint256 indexed tokenId)"],Be=["function listNFT(uint256 _tokenId, uint256 _pricePerHour, uint256 _minHours, uint256 _maxHours) external","function updateListing(uint256 _tokenId, uint256 _newPricePerHour, uint256 _newMinHours, uint256 _newMaxHours) external","function withdrawNFT(uint256 _tokenId) external","function rentNFT(uint256 _tokenId, uint256 _hours) external","function getListing(uint256 _tokenId) view returns (tuple(address owner, uint256 pricePerHour, uint256 minHours, uint256 maxHours, bool isActive, uint256 totalEarnings, uint256 rentalCount))","function getRental(uint256 _tokenId) view returns (tuple(address tenant, uint256 startTime, uint256 endTime, uint256 paidAmount))","function isRented(uint256 _tokenId) view returns (bool)","function hasRentalRights(uint256 _tokenId, address _user) view returns (bool)","function getRemainingRentalTime(uint256 _tokenId) view returns (uint256)","function getAllListedTokenIds() view returns (uint256[])","function getListingCount() view returns (uint256)","function getRentalCost(uint256 _tokenId, uint256 _hours) view returns (uint256 totalCost, uint256 protocolFee, uint256 ownerPayout)","function getMarketplaceStats() view returns (uint256 activeListings, uint256 totalVol, uint256 totalFees, uint256 rentals)","event NFTListed(uint256 indexed tokenId, address indexed owner, uint256 pricePerHour, uint256 minHours, uint256 maxHours)","event NFTRented(uint256 indexed tokenId, address indexed tenant, address indexed owner, uint256 hours_, uint256 totalCost, uint256 protocolFee, uint256 ownerPayout, uint256 endTime)","event NFTWithdrawn(uint256 indexed tokenId, address indexed owner)","event RentalExpired(uint256 indexed tokenId, address indexed tenant)"],Hs=["function getBuyPrice() view returns (uint256)","function getSellPrice() view returns (uint256)","function getBuyPriceWithTax() view returns (uint256)","function getSellPriceAfterTax() view returns (uint256)","function buyNFT() external payable returns (uint256)","function buySpecificNFT(uint256 _tokenId) external payable","function buyNFTWithSlippage(uint256 _maxPrice) external payable returns (uint256)","function sellNFT(uint256 _tokenId, uint256 _minPayout) external","function getPoolInfo() view returns (uint256 tokenBalance, uint256 nftCount, uint256 k, bool isInitialized)","function getAvailableNFTs() view returns (uint256[])","function boostBips() view returns (uint256)","event NFTPurchased(address indexed buyer, uint256 indexed tokenId, uint256 price, uint256 tax, uint256 newBkcBalance, uint256 newNftCount)","event NFTSold(address indexed seller, uint256 indexed tokenId, uint256 payout, uint256 tax, uint256 newBkcBalance, uint256 newNftCount)"],_s=["function participate(uint256 _wagerAmount, uint256[] calldata _guesses, bool _isCumulative) external payable","function oracleFee() view returns (uint256)","function gameFeeBips() view returns (uint256)","function getRequiredOracleFee(bool _isCumulative) view returns (uint256)","function activeTierCount() view returns (uint256)","function gameCounter() view returns (uint256)","function prizePoolBalance() view returns (uint256)","function getExpectedGuessCount(bool _isCumulative) view returns (uint256)","function isGameFulfilled(uint256 _gameId) view returns (bool)","function getGameResults(uint256 _gameId) view returns (uint256[])","function getJackpotTierId() view returns (uint256)","function getJackpotTier() view returns (uint256 tierId, uint128 maxRange, uint64 multiplierBips, bool active)","function getAllTiers() view returns (uint128[] ranges, uint64[] multipliers)","function prizeTiers(uint256 tierId) view returns (uint128 maxRange, uint64 multiplierBips, bool active)","function calculatePotentialWinnings(uint256 _wagerAmount, bool _isCumulative) view returns (uint256 maxPrize, uint256 netWager)","event GameRequested(uint256 indexed gameId, address indexed player, uint256 wagerAmount, uint256[] guesses, bool isCumulative, uint256 targetTier)","event GameFulfilled(uint256 indexed gameId, address indexed player, uint256 prizeWon, uint256[] rolls, uint256[] guesses, bool isCumulative)"],Us=["function play(uint256 _wagerAmount, uint256[] calldata _guesses, bool _isCumulative) external payable returns (uint256 gameId, uint256[] memory rolls, uint256 prizeWon)","function serviceFee() view returns (uint256)","function getRequiredServiceFee(bool _isCumulative) view returns (uint256)","function prizePoolBalance() view returns (uint256)","function gameCounter() view returns (uint256)","function activeTierCount() view returns (uint256)","function gameFeeBips() view returns (uint256)","function getExpectedGuessCount(bool _isCumulative) view returns (uint256)","function getTier(uint256 _tierId) view returns (uint128 maxRange, uint64 multiplierBips, bool active)","function getAllTiers() view returns (uint128[] ranges, uint64[] multipliers)","function calculatePotentialWinnings(uint256 _wagerAmount, bool _isCumulative) view returns (uint256 maxPrize, uint256 netWager, uint256 fee)","function getGameResult(uint256 _gameId) view returns (address player, uint256 wagerAmount, uint256 prizeWon, uint256 timestamp, bool isCumulative, uint256 matchCount)","function getGameDetails(uint256 _gameId) view returns (address player, uint256 wagerAmount, uint256 prizeWon, uint256[] guesses, uint256[] rolls, bool[] matches, bool isCumulative)","function getPlayerStats(address _player) view returns (uint256 gamesPlayed, uint256 totalWageredAmount, uint256 totalWonAmount, int256 netProfit)","function getPoolStats() view returns (uint256 poolBalance, uint256 gamesPlayed, uint256 wageredAllTime, uint256 paidOutAllTime, uint256 winsAllTime, uint256 currentFee)","event GamePlayed(uint256 indexed gameId, address indexed player, uint256 wagerAmount, uint256 prizeWon, bool isCumulative, uint8 matchCount)","event GameDetails(uint256 indexed gameId, uint256[] guesses, uint256[] rolls, bool[] matches)","event JackpotWon(uint256 indexed gameId, address indexed player, uint256 prizeAmount, uint256 tier)"],Gt=["function tiers(uint256 tierId) view returns (uint256 priceInWei, uint64 maxSupply, uint64 mintedCount, uint16 boostBips, bool isConfigured, bool isActive, string metadataFile, string name)","function buyNFT(uint256 _tierId) external payable","function buyMultipleNFTs(uint256 _tierId, uint256 _quantity) external payable","function getTierPrice(uint256 _tierId) view returns (uint256)","function getTierSupply(uint256 _tierId) view returns (uint64 maxSupply, uint64 mintedCount)","function isTierActive(uint256 _tierId) view returns (bool)","event NFTSold(address indexed buyer, uint256 indexed tierId, uint256 indexed tokenId, uint256 price)"],ks=["function balanceOf(address owner) view returns (uint256)","function tokenURI(uint256 tokenId) view returns (string)","function ownerOf(uint256 tokenId) view returns (address)","function getDocument(uint256 tokenId) view returns (tuple(string ipfsCid, string description, bytes32 contentHash, uint256 timestamp))","function documents(uint256 tokenId) view returns (string ipfsCid, string description, bytes32 contentHash, uint256 timestamp)","function getBaseFee() view returns (uint256)","function calculateFee(uint256 _boosterTokenId) view returns (uint256)","function notarize(string _ipfsCid, string _description, bytes32 _contentHash, uint256 _boosterTokenId) external returns (uint256)","event DocumentNotarized(uint256 indexed tokenId, address indexed owner, string ipfsCid, bytes32 indexed contentHash, uint256 feePaid)"],Os=["function canClaim(address _user) view returns (bool)","function getCooldownRemaining(address _user) view returns (uint256)","function getUserInfo(address _user) view returns (uint256 lastClaimTime, uint256 totalClaimed)","function getFaucetStatus() view returns (uint256 bkcBalance, uint256 ethBalance, bool isActive)","function COOLDOWN_PERIOD() view returns (uint256)","function TOKEN_AMOUNT() view returns (uint256)","function ETH_AMOUNT() view returns (uint256)","event TokensDistributed(address indexed recipient, uint256 tokenAmount, uint256 ethAmount, address indexed relayer)"],Ws=["function getServiceRequirements(bytes32 _serviceKey) view returns (uint256 fee, uint256 pStake)","function getFee(bytes32 _serviceKey) view returns (uint256)","function getBoosterDiscount(uint256 _boostBips) view returns (uint256)","function getMiningDistributionBips() view returns (uint256 stakingBips, uint256 minerBips, uint256 treasuryBips)","function getFeeDistributionBips() view returns (uint256 burnBips, uint256 treasuryBips, uint256 poolBips)","function getTreasuryAddress() view returns (address)","function getDelegationManagerAddress() view returns (address)","function getBKCTokenAddress() view returns (address)","function getBoosterAddress() view returns (address)","function getNFTLiquidityPoolFactoryAddress() view returns (address)","function getMiningManagerAddress() view returns (address)","function getFortunePoolAddress() view returns (address)","function getNotaryAddress() view returns (address)","function getRentalManagerAddress() view returns (address)","function getPublicSaleAddress() view returns (address)","function isInitialized() view returns (bool)","function owner() view returns (address)"],wi=window.ethers,yi=5e3,ki=6e4,Ti=15e3,zi=3e4,Ci=1e4;let is=null,pn=0;const fn=new Map,os=new Map,mn=new Map,bn=e=>new Promise(t=>setTimeout(t,e));async function Vt(e,t){const s=new AbortController,n=setTimeout(()=>s.abort(),t);try{const a=await fetch(e,{signal:s.signal});return clearTimeout(n),a}catch(a){throw clearTimeout(n),a.name==="AbortError"?new Error("API request timed out."):a}}const oe={getHistory:"https://gethistory-4wvdcuoouq-uc.a.run.app",getBoosters:"https://getboosters-4wvdcuoouq-uc.a.run.app",getSystemData:"https://getsystemdata-4wvdcuoouq-uc.a.run.app",getNotaryHistory:"https://getnotaryhistory-4wvdcuoouq-uc.a.run.app",getRentalListings:"https://getrentallistings-4wvdcuoouq-uc.a.run.app",getUserRentals:"https://getuserrentals-4wvdcuoouq-uc.a.run.app",fortuneGames:"https://getfortunegames-4wvdcuoouq-uc.a.run.app",uploadFileToIPFS:"/api/upload",claimAirdrop:"https://us-central1-airdropbackchainnew.cloudfunctions.net/claimAirdrop"};function Xn(e){var t;return((t=e==null?void 0:e.error)==null?void 0:t.code)===429||(e==null?void 0:e.code)===429||e.message&&(e.message.includes("429")||e.message.includes("Too Many Requests")||e.message.includes("rate limit"))}function Jn(e){var s,n;const t=((s=e==null?void 0:e.error)==null?void 0:s.code)||(e==null?void 0:e.code);return t===-32603||t===-32e3||((n=e.message)==null?void 0:n.includes("Internal JSON-RPC"))}function Xt(e,t,s){if(s)return s;if(!e||!c.publicProvider)return null;try{return new wi.Contract(e,t,c.publicProvider)}catch{return null}}const O=async(e,t,s=[],n=0n,a=2,i=!1)=>{if(!e)return n;const o=e.target||e.address,r=JSON.stringify(s,(p,f)=>typeof f=="bigint"?f.toString():f),l=`${o}-${t}-${r}`,d=Date.now(),u=["getPoolInfo","getBuyPrice","getSellPrice","getAvailableTokenIds","getAllListedTokenIds","tokenURI","boostBips","getListing","balanceOf","totalSupply","totalNetworkPStake","MAX_SUPPLY","TGE_SUPPLY","userTotalPStake","pendingRewards","isRented","getRental","ownerOf","getDelegationsOf","allowance","prizeTiers","activeTierCount","prizePoolBalance"];if(!i&&u.includes(t)){const p=fn.get(l);if(p&&d-p.timestamp<Ti)return p.value}for(let p=0;p<=a;p++)try{const f=await e[t](...s);return u.includes(t)&&fn.set(l,{value:f,timestamp:d}),f}catch(f){if(Xn(f)&&p<a){const g=Math.floor(Math.random()*1e3),b=1e3*Math.pow(2,p)+g;await bn(b);continue}if(Jn(f)&&p<a){await bn(500);continue}break}return n},Bi=async(e,t,s=!1)=>{const n=`balance-${(e==null?void 0:e.target)||(e==null?void 0:e.address)}-${t}`,a=Date.now();if(!s){const o=mn.get(n);if(o&&a-o.timestamp<Ci)return o.value}const i=await O(e,"balanceOf",[t],0n,2,s);return mn.set(n,{value:i,timestamp:a}),i};async function Qn(){c.systemFees||(c.systemFees={}),c.systemPStakes||(c.systemPStakes={}),c.boosterDiscounts||(c.boosterDiscounts={});const e=Date.now();if(is&&e-pn<ki)return gn(is),!0;try{const t=await Vt(oe.getSystemData,yi);if(!t.ok)throw new Error(`API Status: ${t.status}`);const s=await t.json();return gn(s),is=s,pn=e,!0}catch{return c.systemFees.NOTARY_SERVICE||(c.systemFees.NOTARY_SERVICE=100n),c.systemFees.CLAIM_REWARD_FEE_BIPS||(c.systemFees.CLAIM_REWARD_FEE_BIPS=500n),!1}}function gn(e){if(e.fees)for(const t in e.fees)try{c.systemFees[t]=BigInt(e.fees[t])}catch{c.systemFees[t]=0n}if(e.pStakeRequirements)for(const t in e.pStakeRequirements)try{c.systemPStakes[t]=BigInt(e.pStakeRequirements[t])}catch{c.systemPStakes[t]=0n}if(e.discounts)for(const t in e.discounts)try{c.boosterDiscounts[t]=BigInt(e.discounts[t])}catch{c.boosterDiscounts[t]=0n}if(e.oracleFeeInWei){c.systemData=c.systemData||{};try{c.systemData.oracleFeeInWei=BigInt(e.oracleFeeInWei)}catch{c.systemData.oracleFeeInWei=0n}}}async function qs(){!c.publicProvider||!c.bkcTokenContractPublic||await Promise.allSettled([O(c.bkcTokenContractPublic,"totalSupply",[],0n),Qn()])}async function V(e=!1){var t;if(!(!c.isConnected||!c.userAddress))try{const[s,n]=await Promise.allSettled([Bi(c.bkcTokenContract,c.userAddress,e),(t=c.provider)==null?void 0:t.getBalance(c.userAddress)]);if(s.status==="fulfilled"&&(c.currentUserBalance=s.value),n.status==="fulfilled"&&(c.currentUserNativeBalance=n.value),await Qt(e),c.delegationManagerContract){const a=await O(c.delegationManagerContract,"userTotalPStake",[c.userAddress],0n,2,e);c.userTotalPStake=a}}catch(s){console.error("Error loading user data:",s)}}async function Ys(e=!1){if(!c.isConnected||!c.delegationManagerContract)return[];try{const t=await O(c.delegationManagerContract,"getDelegationsOf",[c.userAddress],[],2,e);return c.userDelegations=t.map((s,n)=>({amount:s[0]||s.amount||0n,unlockTime:BigInt(s[1]||s.unlockTime||0),lockDuration:BigInt(s[2]||s.lockDuration||0),index:n})),c.userDelegations}catch(t){return console.error("Error loading delegations:",t),[]}}async function gt(e=!1){let t=[];try{const n=await Vt(oe.getRentalListings,4e3);n.ok&&(t=await n.json())}catch{}if(t&&t.length>0){const n=t.map(a=>{var o,r,l,d,u;const i=Z.find(p=>p.boostBips===Number(a.boostBips||0));return{...a,tokenId:((o=a.tokenId)==null?void 0:o.toString())||((r=a.id)==null?void 0:r.toString()),pricePerHour:((l=a.pricePerHour)==null?void 0:l.toString())||((d=a.price)==null?void 0:d.toString())||"0",totalEarnings:((u=a.totalEarnings)==null?void 0:u.toString())||"0",rentalCount:Number(a.rentalCount||0),img:(i==null?void 0:i.img)||"./assets/nft.png",name:(i==null?void 0:i.name)||"Booster NFT"}});return c.rentalListings=n,n}const s=Xt(x.rentalManager,Be,c.rentalManagerContractPublic);if(!s)return c.rentalListings=[],[];try{const n=await O(s,"getAllListedTokenIds",[],[],2,!0);if(!n||n.length===0)return c.rentalListings=[],[];const i=n.slice(0,30).map(async l=>{var d,u,p,f,g,b;try{const v=await O(s,"getListing",[l],null,1,!0);if(v&&v.isActive){const y=await O(s,"getRental",[l],null,1,!0),k=await Zn(l),E=Math.floor(Date.now()/1e3),L=y&&BigInt(y.endTime||0)>BigInt(E);return{tokenId:l.toString(),owner:v.owner,pricePerHour:((d=v.pricePerHour)==null?void 0:d.toString())||((u=v.price)==null?void 0:u.toString())||"0",minHours:((p=v.minHours)==null?void 0:p.toString())||"1",maxHours:((f=v.maxHours)==null?void 0:f.toString())||"1",totalEarnings:((g=v.totalEarnings)==null?void 0:g.toString())||"0",rentalCount:Number(v.rentalCount||0),boostBips:k.boostBips,img:k.img||"./assets/nft.png",name:k.name,isRented:L,currentTenant:L?y.tenant:null,rentalEndTime:L?(b=y.endTime)==null?void 0:b.toString():null}}}catch{}return null}),r=(await Promise.all(i)).filter(l=>l!==null);return c.rentalListings=r,r}catch{return c.rentalListings=[],[]}}async function Ii(e=!1){var s,n,a,i;if(!c.userAddress)return c.myRentals=[],[];try{const o=await Vt(`${oe.getUserRentals}/${c.userAddress}`,4e3);if(o.ok){const l=(await o.json()).map(d=>{const u=Z.find(p=>p.boostBips===Number(d.boostBips||0));return{...d,img:(u==null?void 0:u.img)||"./assets/nft.png",name:(u==null?void 0:u.name)||"Booster NFT"}});return c.myRentals=l,l}}catch{}const t=Xt(x.rentalManager,Be,c.rentalManagerContractPublic);if(!t)return c.myRentals=[],[];try{const o=await O(t,"getAllListedTokenIds",[],[],2,e),r=[],l=Math.floor(Date.now()/1e3);for(const d of o.slice(0,30))try{const u=await O(t,"getRental",[d],null,1,e);if(u&&((s=u.tenant)==null?void 0:s.toLowerCase())===c.userAddress.toLowerCase()&&BigInt(u.endTime||0)>BigInt(l)){const p=await Zn(d);r.push({tokenId:d.toString(),tenant:u.tenant,startTime:((n=u.startTime)==null?void 0:n.toString())||"0",endTime:((a=u.endTime)==null?void 0:a.toString())||"0",paidAmount:((i=u.paidAmount)==null?void 0:i.toString())||"0",boostBips:p.boostBips,img:p.img,name:p.name})}}catch{}return c.myRentals=r,r}catch{return c.myRentals=[],[]}}let Ct=null,xn=0;const Ei=3e4;async function Ge(e=!1){const t=Date.now();if(!e&&Ct&&t-xn<Ei)return Ct;await Qt(e);let s=0,n=null,a="none";if(c.myBoosters&&c.myBoosters.length>0){const l=c.myBoosters.reduce((d,u)=>u.boostBips>d.boostBips?u:d,c.myBoosters[0]);l.boostBips>s&&(s=l.boostBips,n=l.tokenId,a="owned")}if(c.myRentals&&c.myRentals.length>0){const l=c.myRentals.reduce((d,u)=>u.boostBips>d.boostBips?u:d,c.myRentals[0]);l.boostBips>s&&(s=l.boostBips,n=l.tokenId,a="rented")}const i=Z.find(l=>l.boostBips===s),o=(i==null?void 0:i.realImg)||(i==null?void 0:i.img)||"assets/bkc_logo_3d.png",r=i!=null&&i.name?`${i.name} Booster`:a!=="none"?"Booster NFT":"None";return Ct={highestBoost:s,boostName:r,imageUrl:o,tokenId:n?n.toString():null,source:a},xn=Date.now(),Ct}async function Zn(e){const t=["function boostBips(uint256) view returns (uint256)"],s=Xt(x.rewardBoosterNFT,t,c.rewardBoosterContractPublic);if(!s)return{boostBips:0,img:"assets/bkc_logo_3d.png",name:"Unknown"};try{const n=await O(s,"boostBips",[e],0n),a=Number(n),i=Z.find(o=>o.boostBips===a);return{boostBips:a,img:(i==null?void 0:i.img)||"./assets/nft.png",name:(i==null?void 0:i.name)||`Booster #${e}`}}catch{return{boostBips:0,img:"assets/bkc_logo_3d.png",name:"Unknown"}}}async function Jt(){if(!c.isConnected||!c.delegationManagerContract)return{stakingRewards:0n,minerRewards:0n,totalRewards:0n};try{const e=await O(c.delegationManagerContract,"pendingRewards",[c.userAddress],0n);return{stakingRewards:e,minerRewards:0n,totalRewards:e}}catch{return{stakingRewards:0n,minerRewards:0n,totalRewards:0n}}}async function ea(){var o,r;if(!c.delegationManagerContract||!c.userAddress)return{netClaimAmount:0n,feeAmount:0n,discountPercent:0,totalRewards:0n};const{totalRewards:e}=await Jt();if(e===0n)return{netClaimAmount:0n,feeAmount:0n,discountPercent:0,totalRewards:0n};let t=((o=c.systemFees)==null?void 0:o.CLAIM_REWARD_FEE_BIPS)||500n;const s=await Ge();let n=((r=c.boosterDiscounts)==null?void 0:r[s.highestBoost])||0n;const a=t>n?t-n:0n,i=e*a/10000n;return{netClaimAmount:e-i,feeAmount:i,discountPercent:Number(n)/100,totalRewards:e}}let rs=!1,ls=0,Bt=0;const $i=3e4,Ai=3,Li=12e4;async function Qt(e=!1){if(!c.userAddress)return[];const t=Date.now();if(rs)return c.myBoosters||[];if(!e&&t-ls<$i)return c.myBoosters||[];if(Bt>=Ai){if(t-ls<Li)return c.myBoosters||[];Bt=0}rs=!0,ls=t;try{const s=await Vt(`${oe.getBoosters}/${c.userAddress}`,5e3);if(!s.ok)throw new Error(`API Error: ${s.status}`);let n=await s.json();const a=["function ownerOf(uint256) view returns (address)","function boostBips(uint256) view returns (uint256)"],i=Xt(x.rewardBoosterNFT,a,c.rewardBoosterContractPublic);if(i&&n.length>0){const o=await Promise.all(n.slice(0,50).map(async r=>{const l=BigInt(r.tokenId),d=`ownerOf-${l}`,u=Date.now();let p=Number(r.boostBips||r.boost||0);if(p===0)try{const f=await i.boostBips(l);p=Number(f)}catch{}if(!e&&os.has(d)){const f=os.get(d);if(u-f.timestamp<zi)return f.owner.toLowerCase()===c.userAddress.toLowerCase()?{tokenId:l,boostBips:p,imageUrl:r.imageUrl||r.image||null}:null}try{const f=await i.ownerOf(l);return os.set(d,{owner:f,timestamp:u}),f.toLowerCase()===c.userAddress.toLowerCase()?{tokenId:l,boostBips:p,imageUrl:r.imageUrl||r.image||null}:null}catch(f){return Xn(f)||Jn(f)?{tokenId:l,boostBips:p,imageUrl:r.imageUrl||r.image||null}:null}}));c.myBoosters=o.filter(r=>r!==null)}else c.myBoosters=n.map(o=>({tokenId:BigInt(o.tokenId),boostBips:Number(o.boostBips||o.boost||0),imageUrl:o.imageUrl||o.image||null}));return Bt=0,c.myBoosters}catch{return Bt++,c.myBoosters||(c.myBoosters=[]),c.myBoosters}finally{rs=!1}}const Si={apiKey:"AIzaSyDKhF2_--fKtot96YPS8twuD0UoCpS-3T4",authDomain:"airdropbackchainnew.firebaseapp.com",projectId:"airdropbackchainnew",storageBucket:"airdropbackchainnew.appspot.com",messagingSenderId:"108371799661",appId:"1:108371799661:web:d126fcbd0ba56263561964",measurementId:"G-QD9EBZ0Y09"},ta=ei(Si),It=ti(ta),N=ai(ta);let ye=null,Q=null,Et=null;async function sa(e){if(!e)throw new Error("Wallet address is required for Firebase sign-in.");const t=e.toLowerCase();return Q=t,ye?(Et=await tt(t),ye):It.currentUser?(ye=It.currentUser,Et=await tt(t),ye):new Promise((s,n)=>{const a=si(It,async i=>{if(a(),i){ye=i;try{Et=await tt(t),s(i)}catch(o){console.error("Error linking airdrop user profile:",o),n(o)}}else ni(It).then(async o=>{ye=o.user,Et=await tt(t),s(ye)}).catch(o=>{console.error("Firebase Anonymous sign-in failed:",o),n(o)})},i=>{console.error("Firebase Auth state change error:",i),a(),n(i)})})}function xe(){if(!ye)throw new Error("User not authenticated with Firebase. Please sign-in first.");if(!Q)throw new Error("Wallet address not set. Please connect wallet first.")}async function Ks(){const e=_(N,"airdrop_public_data","data_v1"),t=await te(e);if(t.exists()){const s=t.data(),n=(s.dailyTasks||[]).map(o=>{var d,u;const r=(d=o.startDate)!=null&&d.toDate?o.startDate.toDate():o.startDate?new Date(o.startDate):null,l=(u=o.endDate)!=null&&u.toDate?o.endDate.toDate():o.endDate?new Date(o.endDate):null;return{...o,id:o.id||null,startDate:r instanceof Date&&!isNaN(r)?r:null,endDate:l instanceof Date&&!isNaN(l)?l:null}}).filter(o=>o.id),a=Date.now(),i=n.filter(o=>{const r=o.startDate?o.startDate.getTime():0,l=o.endDate?o.endDate.getTime():1/0;return r<=a&&a<l});return{config:s.config||{ugcBasePoints:{}},leaderboards:s.leaderboards||{top100ByPoints:[],top100ByPosts:[],lastUpdated:null},dailyTasks:i,platformUsageConfig:s.platformUsageConfig||null}}else return console.warn("Public airdrop data document 'airdrop_public_data/data_v1' not found. Returning defaults."),{config:{isActive:!1,roundName:"Loading...",ugcBasePoints:{}},leaderboards:{top100ByPoints:[],top100ByPosts:[],lastUpdated:null},dailyTasks:[],platformUsageConfig:null}}function vn(){const e="ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";let t="";for(let s=0;s<6;s++)t+=e.charAt(Math.floor(Math.random()*e.length));return t}function Mt(e){return e>=100?10:e>=90?9:e>=80?8:e>=70?7:e>=60?6:e>=50?5:e>=40?4:e>=30?3:e>=20?2:1}async function tt(e){xe(),e||(e=Q);const t=e.toLowerCase(),s=_(N,"airdrop_users",t),n=await te(s);if(n.exists()){const a=n.data(),i={};if(a.referralCode||(i.referralCode=vn()),typeof a.approvedSubmissionsCount!="number"&&(i.approvedSubmissionsCount=0),typeof a.rejectedCount!="number"&&(i.rejectedCount=0),typeof a.isBanned!="boolean"&&(i.isBanned=!1),typeof a.totalPoints!="number"&&(i.totalPoints=0),typeof a.pointsMultiplier!="number"&&(i.pointsMultiplier=1),a.walletAddress!==t&&(i.walletAddress=t),Object.keys(i).length>0)try{return await qt(s,i),{id:n.id,...a,...i}}catch(o){return console.error("Error updating user default fields:",o),{id:n.id,...a}}return{id:n.id,...a}}else{const a=vn(),i={walletAddress:t,referralCode:a,totalPoints:0,pointsMultiplier:1,approvedSubmissionsCount:0,rejectedCount:0,isBanned:!1,createdAt:ze()};return await Yt(s,i),{id:s.id,...i,createdAt:new Date}}}async function na(e,t){if(xe(),!e||typeof e!="string"||e.trim()==="")return console.warn(`isTaskEligible called with invalid taskId: ${e}`),{eligible:!1,timeLeft:0};const s=_(N,"airdrop_users",Q,"task_claims",e),n=await te(s),a=t*60*60*1e3;if(!n.exists())return{eligible:!0,timeLeft:0};const i=n.data(),o=i==null?void 0:i.timestamp;if(typeof o!="string"||o.trim()==="")return console.warn(`Missing/invalid timestamp for task ${e}. Allowing claim.`),{eligible:!0,timeLeft:0};try{const r=new Date(o);if(isNaN(r.getTime()))return console.warn(`Invalid timestamp format for task ${e}:`,o,". Allowing claim."),{eligible:!0,timeLeft:0};const l=r.getTime(),u=Date.now()-l;return u>=a?{eligible:!0,timeLeft:0}:{eligible:!1,timeLeft:a-u}}catch(r){return console.error(`Error parsing timestamp string for task ${e}:`,o,r),{eligible:!0,timeLeft:0}}}async function Pi(e,t){if(xe(),!e||!e.id)throw new Error("Invalid task data provided.");if(!(await na(e.id,e.cooldownHours)).eligible)throw new Error("Cooldown period is still active for this task.");const n=_(N,"airdrop_users",Q),a=Math.round(e.points);if(isNaN(a)||a<0)throw new Error("Invalid points value for the task.");await qt(n,{totalPoints:ie(a)});const i=_(N,"airdrop_users",Q,"task_claims",e.id);return await Yt(i,{timestamp:new Date().toISOString(),points:a}),a}async function Ni(e){var r;const t=e.trim().toLowerCase();let s="Other",n=!0;if(t.includes("youtube.com/shorts/")){s="YouTube Shorts";const l=t.match(/\/shorts\/([a-zA-Z0-9_-]+)/);if(!l||!l[1])throw n=!1,new Error("Invalid YouTube Shorts URL: Video ID not found or incorrect format.")}else if(t.includes("youtube.com/watch?v=")||t.includes("youtu.be/")){s="YouTube";const l=t.match(/(?:v=|\/)([a-zA-Z0-9_-]{11})(?:[?&]|$)/);if(!l||l[1].length!==11)throw n=!1,new Error("Invalid YouTube URL: Video ID not found or incorrect format.")}else{if(t.includes("youtube.com/"))throw s="YouTube",n=!1,new Error("Invalid YouTube URL: Only video links (youtube.com/watch?v=... or youtu.be/...) or Shorts links are accepted.");if(t.includes("instagram.com/p/")||t.includes("instagram.com/reel/")){s="Instagram";const l=t.match(/\/(?:p|reel)\/([a-zA-Z0-9_.-]+)/);(!l||!l[1])&&(n=!1)}else t.includes("twitter.com/")||t.includes("x.com/")?t.match(/(\w+)\/(?:status|statuses)\/(\d+)/)&&(s="X/Twitter"):t.includes("facebook.com/")&&t.includes("/posts/")?s="Facebook":t.includes("t.me/")||t.includes("telegram.org/")?s="Telegram":t.includes("tiktok.com/")?s="TikTok":t.includes("reddit.com/r/")?s="Reddit":t.includes("linkedin.com/posts/")&&(s="LinkedIn")}const i=((r=(await Ks()).config)==null?void 0:r.ugcBasePoints)||{},o=i[s]||i.Other||1e3;if(isNaN(o)||o<0)throw new Error(`Invalid base points configured for platform: ${s}. Please contact admin.`);return{platform:s,basePoints:o,isValid:n,normalizedUrl:t}}async function Mi(e){var re;xe();const t=_(N,"airdrop_users",Q),s=we(N,"airdrop_users",Q,"submissions"),n=we(N,"all_submissions_log"),a=e.trim();if(!a||!a.toLowerCase().startsWith("http://")&&!a.toLowerCase().startsWith("https://"))throw new Error("The provided URL must start with http:// or https://.");let i;try{i=await Ni(a)}catch(S){throw S}const{platform:o,basePoints:r,isValid:l,normalizedUrl:d}=i;if(!l)throw new Error(`The provided URL for ${o} does not appear valid for submission.`);const u=Ne(s,Ke("submittedAt","desc"),ii(1)),p=await Te(u);if(!p.empty){const X=(re=p.docs[0].data().submittedAt)==null?void 0:re.toDate();if(X){const le=new Date,K=5*60*1e3,Tt=le.getTime()-X.getTime();if(Tt<K){const zt=K-Tt,as=Math.ceil(zt/6e4);throw new Error(`We're building a strong community, and we value quality over quantity. To prevent spam, please wait ${as} more minute(s) before submitting another post. We appreciate your thoughtful contribution!`)}}}const f=Ne(n,Nt("normalizedUrl","==",d),Nt("status","in",["pending","approved","auditing","flagged_suspicious"]));if(!(await Te(f)).empty)throw new Error("This content link has already been submitted. Repeatedly submitting duplicate or fraudulent content may lead to account suspension.");const b=await te(t);if(!b.exists())throw new Error("User profile not found.");const v=b.data(),y=v.approvedSubmissionsCount||0,k=Mt(y),E=Math.round(r*k),L=ze(),A={url:a,platform:o,status:"pending",basePoints:r,_pointsCalculated:E,_multiplierApplied:k,pointsAwarded:0,submittedAt:L,resolvedAt:null},$={userId:Q,walletAddress:v.walletAddress,normalizedUrl:d,platform:o,status:"pending",basePoints:r,submittedAt:L,resolvedAt:null},T=bt(N),F=_(s);T.set(F,A);const q=_(n,F.id);T.set(q,$),await T.commit()}async function Fi(){xe();const e=we(N,"airdrop_users",Q,"submissions"),t=Ne(e,Ke("submittedAt","desc"));return(await Te(t)).docs.map(n=>{var i,o;const a=n.data();return{submissionId:n.id,...a,submittedAt:(i=a.submittedAt)!=null&&i.toDate?a.submittedAt.toDate():null,resolvedAt:(o=a.resolvedAt)!=null&&o.toDate?a.resolvedAt.toDate():null}})}async function Ri(e){xe();const t=Q,s=_(N,"airdrop_users",t),n=_(N,"airdrop_users",t,"submissions",e),a=_(N,"all_submissions_log",e),i=await te(n);if(!i.exists())throw new Error("Cannot confirm submission: Document not found or already processed.");const o=i.data(),r=o.status;if(r==="approved"||r==="rejected")throw new Error(`Submission is already in status: ${r}.`);let l=o._pointsCalculated,d=o._multiplierApplied;if(typeof l!="number"||isNaN(l)||l<=0){console.warn(`[ConfirmSubmission] Legacy/Invalid submission ${e} missing/invalid _pointsCalculated. Recalculating...`);const p=o.basePoints||0,f=await te(s);if(!f.exists())throw new Error("User profile not found for recalculation.");const b=f.data().approvedSubmissionsCount||0;d=Mt(b),l=Math.round(p*d),(isNaN(l)||l<=0)&&(console.warn(`[ConfirmSubmission] Recalculation failed (basePoints: ${p}). Using fallback 1000.`),l=Math.round(1e3*d))}const u=bt(N);u.update(s,{totalPoints:ie(l),approvedSubmissionsCount:ie(1)}),u.update(n,{status:"approved",pointsAwarded:l,_pointsCalculated:l,_multiplierApplied:d,resolvedAt:ze()}),await te(a).then(p=>p.exists())&&u.update(a,{status:"approved",resolvedAt:ze()}),await u.commit()}async function aa(e){xe();const s=_(N,"airdrop_users",Q,"submissions",e),n=_(N,"all_submissions_log",e),a=await te(s);if(!a.exists())return console.warn(`Delete submission skipped: Document ${e} not found.`);const i=a.data().status;if(i==="approved"||i==="rejected")throw new Error(`This submission was already ${i} and cannot be deleted.`);if(i==="flagged_suspicious")throw new Error("Flagged submissions must be resolved, not deleted.");const o=bt(N);o.update(s,{status:"deleted_by_user",resolvedAt:ze()}),await te(n).then(r=>r.exists())&&o.update(n,{status:"deleted_by_user",resolvedAt:ze(),pointsAwarded:0}),await o.commit()}async function Di(e){const t=_(N,"airdrop_public_data","data_v1");await Yt(t,{config:{ugcBasePoints:e}},{merge:!0})}async function ji(){const e=we(N,"daily_tasks"),t=Ne(e,Ke("endDate","asc"));return(await Te(t)).docs.map(n=>{var a,i;return{id:n.id,...n.data(),startDate:(a=n.data().startDate)!=null&&a.toDate?n.data().startDate.toDate():null,endDate:(i=n.data().endDate)!=null&&i.toDate?n.data().endDate.toDate():null}})}async function Hi(e){const t={...e};t.startDate instanceof Date&&(t.startDate=cn.fromDate(t.startDate)),t.endDate instanceof Date&&(t.endDate=cn.fromDate(t.endDate));const s=e.id;if(!s)delete t.id,await oi(we(N,"daily_tasks"),t);else{const n=_(N,"daily_tasks",s);delete t.id,await Yt(n,t,{merge:!0})}}async function _i(e){if(!e)throw new Error("Task ID is required for deletion.");await ri(_(N,"daily_tasks",e))}async function Ui(){const e=we(N,"all_submissions_log"),t=Ne(e,Nt("status","in",["pending","auditing","flagged_suspicious"]),Ke("submittedAt","desc"));return(await Te(t)).docs.map(n=>{var i,o;const a=n.data();return{userId:a.userId,walletAddress:a.walletAddress,submissionId:n.id,...a,submittedAt:(i=a.submittedAt)!=null&&i.toDate?a.submittedAt.toDate():null,resolvedAt:(o=a.resolvedAt)!=null&&o.toDate?a.resolvedAt.toDate():null}})}async function ia(e,t,s){var k,E,L;if(!e)throw new Error("User ID (walletAddress) is required.");const n=e.toLowerCase(),a=_(N,"airdrop_users",n),i=_(N,"airdrop_users",n,"submissions",t),o=_(N,"all_submissions_log",t),[r,l,d]=await Promise.all([te(a),te(i),te(o)]);if(!l.exists())throw new Error("Submission not found in user collection.");if(!r.exists())throw new Error("User profile not found.");d.exists()||console.warn(`Log entry ${t} not found. Log will not be updated.`);const u=l.data(),p=r.data(),f=u.status;if(f===s){console.warn(`Admin action ignored: Submission ${t} already has status ${s}.`);return}const g=bt(N),b={};let v=0,y=u._multiplierApplied||0;if(s==="approved"){let A=u._pointsCalculated;if(typeof A!="number"||isNaN(A)||A<=0){console.warn(`[Admin] Legacy/Invalid submission ${t} missing/invalid _pointsCalculated. Recalculating...`);const $=u.basePoints||0,T=p.approvedSubmissionsCount||0,F=Mt(T);if(A=Math.round($*F),isNaN(A)||A<=0){console.warn(`[Admin] Recalculation failed (basePoints: ${$}). Using fallback 1000.`);const q=Mt(T);A=Math.round(1e3*q)}y=F}v=A,b.totalPoints=ie(A),b.approvedSubmissionsCount=ie(1),f==="rejected"&&(b.rejectedCount=ie(-1))}else if(s==="rejected"){if(f!=="rejected"){const A=p.rejectedCount||0;b.rejectedCount=ie(1),A+1>=3&&(b.isBanned=!0)}else if(f==="approved"){const A=u.pointsAwarded||0;b.totalPoints=ie(-A),b.approvedSubmissionsCount=ie(-1);const $=p.rejectedCount||0;b.rejectedCount=ie(1),$+1>=3&&(b.isBanned=!0)}v=0}if(((k=b.approvedSubmissionsCount)==null?void 0:k.operand)<0&&(p.approvedSubmissionsCount||0)<=0&&(b.approvedSubmissionsCount=0),((E=b.rejectedCount)==null?void 0:E.operand)<0&&(p.rejectedCount||0)<=0&&(b.rejectedCount=0),((L=b.totalPoints)==null?void 0:L.operand)<0){const A=p.totalPoints||0,$=Math.abs(b.totalPoints.operand);A<$&&(b.totalPoints=0)}Object.keys(b).length>0&&g.update(a,b),g.update(i,{status:s,pointsAwarded:v,_pointsCalculated:s==="approved"?v:u._pointsCalculated||0,_multiplierApplied:y,resolvedAt:ze()}),d.exists()&&g.update(o,{status:s,resolvedAt:ze()}),await g.commit()}async function Oi(){const e=we(N,"airdrop_users"),t=Ne(e,Ke("totalPoints","desc"));return(await Te(t)).docs.map(n=>({id:n.id,...n.data()}))}async function Wi(e,t){if(!e)throw new Error("User ID is required.");const s=e.toLowerCase(),n=we(N,"airdrop_users",s,"submissions"),a=Ne(n,Nt("status","==",t),Ke("resolvedAt","desc"));return(await Te(a)).docs.map(o=>{var r,l;return{submissionId:o.id,userId:s,...o.data(),submittedAt:(r=o.data().submittedAt)!=null&&r.toDate?o.data().submittedAt.toDate():null,resolvedAt:(l=o.data().resolvedAt)!=null&&l.toDate?o.data().resolvedAt.toDate():null}})}async function oa(e,t){if(!e)throw new Error("User ID is required.");const s=e.toLowerCase(),n=_(N,"airdrop_users",s),a={isBanned:t};t===!1&&(a.rejectedCount=0),await qt(n,a)}const hn={faucet:{points:1e3,maxCount:1,cooldownHours:0,enabled:!0},delegation:{points:2e3,maxCount:10,cooldownHours:24,enabled:!0},fortune:{points:1500,maxCount:10,cooldownHours:1,enabled:!0},buyNFT:{points:2500,maxCount:10,cooldownHours:0,enabled:!0},sellNFT:{points:1500,maxCount:10,cooldownHours:0,enabled:!0},listRental:{points:1e3,maxCount:10,cooldownHours:0,enabled:!0},rentNFT:{points:2e3,maxCount:10,cooldownHours:0,enabled:!0},notarize:{points:2e3,maxCount:10,cooldownHours:0,enabled:!0},claimReward:{points:1e3,maxCount:10,cooldownHours:24,enabled:!0},unstake:{points:500,maxCount:10,cooldownHours:0,enabled:!0}};async function qi(){try{const e=_(N,"airdrop_public_data","data_v1"),t=await te(e);return t.exists()&&t.data().platformUsageConfig?t.data().platformUsageConfig:hn}catch(e){return console.error("Error fetching platform usage config:",e),hn}}async function wn(){xe();try{const e=we(N,"airdrop_users",Q,"platform_usage"),t=await Te(e),s={};return t.forEach(n=>{s[n.id]=n.data()}),s}catch(e){return console.error("Error fetching platform usage:",e),{}}}async function Yi(e,t){var a;xe();const n=(await qi())[e];if(!n)return console.error(`Invalid action type: ${e}`),{success:!1,reason:"invalid_action"};if(n.enabled===!1)return{success:!1,reason:"action_disabled"};try{const i=_(N,"airdrop_users",Q,"platform_usage",e),o=await te(i),r=o.exists()?o.data():{count:0,totalPoints:0,txHashes:[],lastClaimed:null};if(r.count>=n.maxCount)return console.log(`Max usage reached for ${e}: ${r.count}/${n.maxCount}`),{success:!1,reason:"max_reached"};if(n.cooldownHours>0&&r.lastClaimed){const b=new Date(r.lastClaimed),v=(Date.now()-b.getTime())/(1e3*60*60);if(v<n.cooldownHours){const y=n.cooldownHours-v;return console.log(`Cooldown active for ${e}: ${y.toFixed(1)}h remaining`),{success:!1,reason:"cooldown",hoursLeft:Math.ceil(y)}}}if(t&&((a=r.txHashes)!=null&&a.includes(t)))return console.log(`Duplicate txHash for ${e}: ${t}`),{success:!1,reason:"duplicate_tx"};const l=n.points,d=r.count+1,u=r.totalPoints+l,p=t?[...r.txHashes||[],t].slice(-20):r.txHashes||[],f=bt(N);f.set(i,{count:d,totalPoints:u,lastClaimed:new Date().toISOString(),txHashes:p});const g=_(N,"airdrop_users",Q);return f.update(g,{platformUsagePoints:ie(l),totalPoints:ie(l)}),await f.commit(),console.log(`✅ Platform usage recorded: ${e} +${l} pts (${d}/${n.maxCount})`),{success:!0,pointsAwarded:l,newCount:d,maxCount:n.maxCount}}catch(i){return console.error(`Error recording platform usage for ${e}:`,i),{success:!1,reason:"error",message:i.message}}}async function ra(e){xe();const t=_(N,"airdrop_public_data","data_v1");await qt(t,{platformUsageConfig:e}),console.log("✅ Platform usage config saved:",e)}const R=window.ethers,Gs=421614,Ki="0x66eee";let ce=null,yn=0,$e=0;const Gi=5e3,kn=3,Vi=1e4;let la=0;const Xi=3;let ca=null;const Ji="cd4bdedee7a7e909ebd3df8bbc502aed",Qi={chainId:Gs,name:"Arbitrum Sepolia",currency:"ETH",explorerUrl:"https://sepolia.arbiscan.io",rpcUrl:Ds},Zi={name:"Backcoin Protocol",description:"DeFi Ecosystem",url:window.location.origin,icons:[window.location.origin+"/assets/bkc_logo_3d.png"]},eo=Qa({metadata:Zi,enableEIP6963:!0,enableInjected:!0,enableCoinbase:!1,rpcUrl:Ds,defaultChainId:Gs,enableEmail:!0,enableEns:!1,auth:{email:!0,showWallets:!0,walletFeatures:!0}}),Le=Za({ethersConfig:eo,chains:[Qi],projectId:Ji,enableAnalytics:!0,themeMode:"dark",themeVariables:{"--w3m-accent":"#f59e0b","--w3m-border-radius-master":"1px","--w3m-z-index":100}});function to(e){var n,a;const t=((n=e==null?void 0:e.message)==null?void 0:n.toLowerCase())||"",s=(e==null?void 0:e.code)||((a=e==null?void 0:e.error)==null?void 0:a.code);return s===-32603||s===-32e3||s===429||t.includes("failed to fetch")||t.includes("network error")||t.includes("timeout")||t.includes("rate limit")||t.includes("too many requests")||t.includes("internal json-rpc")||t.includes("unexpected token")||t.includes("<html")}function Ts(e){return new R.JsonRpcProvider(e||dt())}async function da(e,t=Xi){var n;let s=null;for(let a=0;a<t;a++)try{const i=await e();return bi(dt()),la=0,i}catch(i){if(s=i,to(i)){console.warn(`⚠️ RPC error (attempt ${a+1}/${t}):`,(n=i.message)==null?void 0:n.slice(0,80)),mi(dt());const o=Vn();console.log(`🔄 Switching to: ${o}`),await ua(),await new Promise(r=>setTimeout(r,500*(a+1)))}else throw i}throw console.error("❌ All RPC attempts failed"),s}async function ua(){const e=dt();try{c.publicProvider=Ts(e),ca=c.publicProvider,H(x.bkcToken)&&(c.bkcTokenContractPublic=new R.Contract(x.bkcToken,js,c.publicProvider)),H(x.delegationManager)&&(c.delegationManagerContractPublic=new R.Contract(x.delegationManager,Re,c.publicProvider)),H(x.faucet)&&(c.faucetContractPublic=new R.Contract(x.faucet,Os,c.publicProvider)),H(x.rentalManager)&&(c.rentalManagerContractPublic=new R.Contract(x.rentalManager,Be,c.publicProvider)),H(x.ecosystemManager)&&(c.ecosystemManagerContractPublic=new R.Contract(x.ecosystemManager,Ws,c.publicProvider)),H(x.actionsManager)&&(c.actionsManagerContractPublic=new R.Contract(x.actionsManager,_s,c.publicProvider));const t=x.fortunePoolV2||x.fortunePool;H(t)&&(c.fortunePoolContractPublic=new R.Contract(t,Us,c.publicProvider)),console.log(`✅ Public provider recreated with: ${e.slice(0,50)}...`)}catch(t){console.error("Failed to recreate public provider:",t)}}function so(e){if(!e)return!1;try{return R.isAddress(e)}catch{return!1}}function H(e){return e&&e!==R.ZeroAddress&&!e.startsWith("0x...")}function no(e){if(!e)return;const t=localStorage.getItem(`balance_${e.toLowerCase()}`);if(t)try{c.currentUserBalance=BigInt(t),window.updateUIState&&window.updateUIState()}catch{}}function ao(e){try{H(x.bkcToken)&&(c.bkcTokenContract=new R.Contract(x.bkcToken,js,e)),H(x.delegationManager)&&(c.delegationManagerContract=new R.Contract(x.delegationManager,Re,e)),H(x.rewardBoosterNFT)&&(c.rewardBoosterContract=new R.Contract(x.rewardBoosterNFT,hi,e)),H(x.publicSale)&&(c.publicSaleContract=new R.Contract(x.publicSale,Gt,e)),H(x.faucet)&&(c.faucetContract=new R.Contract(x.faucet,Os,e)),H(x.rentalManager)&&(c.rentalManagerContract=new R.Contract(x.rentalManager,Be,e)),H(x.actionsManager)&&(c.actionsManagerContract=new R.Contract(x.actionsManager,_s,e)),H(x.decentralizedNotary)&&(c.decentralizedNotaryContract=new R.Contract(x.decentralizedNotary,ks,e)),H(x.ecosystemManager)&&(c.ecosystemManagerContract=new R.Contract(x.ecosystemManager,Ws,e));const t=x.fortunePoolV2||x.fortunePool;H(t)&&(c.fortunePoolContract=new R.Contract(t,Us,e))}catch{console.warn("Contract init partial failure")}}function pa(){if(ce&&(clearInterval(ce),ce=null),!c.bkcTokenContractPublic||!c.userAddress){console.warn("Cannot start balance polling: missing contract or address");return}$e=0,la=0,setTimeout(()=>{Tn()},1e3),ce=setInterval(Tn,Vi),console.log("✅ Balance polling started (10s interval)")}async function Tn(){var t;if(document.hidden||!c.isConnected||!c.userAddress||!c.bkcTokenContractPublic)return;const e=Date.now();try{const s=await da(async()=>await c.bkcTokenContractPublic.balanceOf(c.userAddress),2);$e=0;const n=c.currentUserBalance||0n;s.toString()!==n.toString()&&(c.currentUserBalance=s,localStorage.setItem(`balance_${c.userAddress.toLowerCase()}`,s.toString()),e-yn>Gi&&(yn=e,window.updateUIState&&window.updateUIState(!1)))}catch(s){$e++,$e<=3&&console.warn(`⚠️ Balance check failed (${$e}/${kn}):`,(t=s.message)==null?void 0:t.slice(0,50)),$e>=kn&&(console.warn("❌ Too many balance check errors. Stopping polling temporarily."),ce&&(clearInterval(ce),ce=null),setTimeout(()=>{console.log("🔄 Attempting to restart balance polling with primary RPC..."),gi(),ua().then(()=>{$e=0,pa()})},6e4))}}async function io(e){try{const t=await e.getNetwork();if(Number(t.chainId)===Gs)return!0;try{return await e.send("wallet_switchEthereumChain",[{chainId:Ki}]),!0}catch{return!0}}catch{return!0}}async function zn(e,t){try{if(!so(t))return!1;await io(e),c.provider=e;try{c.signer=await e.getSigner()}catch(s){c.signer=e,console.warn(`Could not get standard Signer. Using Provider as read-only. Warning: ${s.message}`)}c.userAddress=t,c.isConnected=!0,no(t),ao(c.signer);try{sa(c.userAddress)}catch{}return V().then(()=>{window.updateUIState&&window.updateUIState(!1)}).catch(()=>{}),pa(),!0}catch(s){return console.error("Setup warning:",s),!!t}}async function oo(){try{const e=dt();console.log(`🌐 Initializing public provider with: ${e.slice(0,50)}...`),c.publicProvider=Ts(e),ca=c.publicProvider,H(x.bkcToken)&&(c.bkcTokenContractPublic=new R.Contract(x.bkcToken,js,c.publicProvider)),H(x.delegationManager)&&(c.delegationManagerContractPublic=new R.Contract(x.delegationManager,Re,c.publicProvider)),H(x.faucet)&&(c.faucetContractPublic=new R.Contract(x.faucet,Os,c.publicProvider)),H(x.rentalManager)&&(c.rentalManagerContractPublic=new R.Contract(x.rentalManager,Be,c.publicProvider)),H(x.ecosystemManager)&&(c.ecosystemManagerContractPublic=new R.Contract(x.ecosystemManager,Ws,c.publicProvider)),H(x.actionsManager)&&(c.actionsManagerContractPublic=new R.Contract(x.actionsManager,_s,c.publicProvider));const t=x.fortunePoolV2||x.fortunePool;H(t)&&(c.fortunePoolContractPublic=new R.Contract(t,Us,c.publicProvider),console.log("✅ FortunePool V2 contract initialized:",t));try{await da(async()=>{await qs()})}catch{console.warn("Initial public data load failed, will retry on user interaction")}window.updateUIState&&window.updateUIState(),console.log("✅ Public provider initialized")}catch(e){console.error("Public provider error:",e);const t=Vn();console.log(`🔄 Retrying with: ${t}`);try{c.publicProvider=Ts(t),console.log("✅ Public provider initialized with fallback RPC")}catch{console.error("❌ All RPC endpoints failed")}}}function ro(e){let t=Le.getAddress();if(Le.getIsConnected()&&t){const n=Le.getWalletProvider();if(n){const a=new R.BrowserProvider(n);c.web3Provider=n,e({isConnected:!0,address:t,isNewConnection:!1}),zn(a,t)}}const s=async({provider:n,address:a,chainId:i,isConnected:o})=>{try{if(o){let r=a||Le.getAddress();if(!r&&n)try{r=await(await new R.BrowserProvider(n).getSigner()).getAddress()}catch{}if(r){const l=new R.BrowserProvider(n);c.web3Provider=n,e({isConnected:!0,address:r,chainId:i,isNewConnection:!0}),await zn(l,r)}else ce&&clearInterval(ce),c.isConnected=!1,c.userAddress=null,c.signer=null,e({isConnected:!1})}else ce&&clearInterval(ce),c.isConnected=!1,c.userAddress=null,c.signer=null,e({isConnected:!1})}catch{}};Le.subscribeProvider(s)}function fa(){Le.open()}async function lo(){await Le.disconnect()}const co=window.ethers,B=(e,t=18)=>{if(e===null||typeof e>"u")return 0;if(typeof e=="number")return e;if(typeof e=="string")return parseFloat(e);try{const s=BigInt(e);return parseFloat(co.formatUnits(s,t))}catch{return 0}},Ve=e=>!e||typeof e!="string"||!e.startsWith("0x")?"...":`${e.substring(0,6)}...${e.substring(e.length-4)}`,Me=e=>{try{if(e==null)return"0";const t=typeof e=="bigint"?e:BigInt(e);if(t<1000n)return t.toString();const s=Number(t);if(!isFinite(s))return t.toLocaleString("en-US");const n=["","k","M","B","T"],a=Math.floor((""+Math.floor(s)).length/3);let i=parseFloat((a!==0?s/Math.pow(1e3,a):s).toPrecision(3));return i%1!==0&&(i=i.toFixed(2)),i+n[a]}catch{return"0"}},uo=(e="Loading...")=>`<div class="flex items-center justify-center p-4 text-zinc-400"><div class="loader inline-block mr-2"></div> ${e}</div>`,po=(e="An error occurred.")=>`<div class="bg-red-900/20 border border-red-500/30 rounded-lg p-4 text-center text-red-400 text-sm">${e}</div>`,fo=(e="No data available.")=>`<div class="text-center p-8 bg-zinc-900/30 border border-dashed border-zinc-800 rounded-lg col-span-full">
                <i class="fa-regular fa-folder-open text-2xl text-zinc-600 mb-2"></i>
                <p class="text-zinc-500 italic text-sm">${e}</p>
            </div>`;function Vs(e,t,s,n){if(!e)return;if(s<=1){e.innerHTML="";return}const a=`
    ...
    `;e.innerHTML=a,e.querySelectorAll(".pagination-btn").forEach(i=>{i.addEventListener("click",()=>{i.hasAttribute("disabled")||n(parseInt(i.dataset.page))})})}const U=window.ethers,Cn={arbitrumSepolia:"https://www.alchemy.com/faucets/arbitrum-sepolia",alternativeFaucet:"https://faucet.quicknode.com/arbitrum/sepolia"},ke={MAX_RETRIES:3,RETRY_DELAY_MS:2e3,APPROVAL_WAIT_MS:1500,SIMULATION_TIMEOUT_MS:1e4},qe=e=>new Promise(t=>setTimeout(t,e)),Qe={MAX_RETRIES:3,RETRY_DELAY_MS:1500};async function de(e,t){if(!t){console.warn("trackPlatformUsage: No txHash provided, skipping");return}(async()=>{for(let s=1;s<=Qe.MAX_RETRIES;s++)try{const n=await Yi(e,t);if(n.success){mo(n.pointsAwarded,n.newCount,n.maxCount),console.log(`✅ Airdrop tracking: ${e} +${n.pointsAwarded} pts (${n.newCount}/${n.maxCount})`);return}else if(n.reason==="max_reached"){console.log(`Airdrop tracking: ${e} max reached (${n.reason})`);return}else if(n.reason==="cooldown"){console.log(`Airdrop tracking: ${e} on cooldown`);return}else if(n.reason==="duplicate_tx"){console.log(`Airdrop tracking: ${e} duplicate tx`);return}else throw new Error(n.reason||"Unknown error")}catch(n){if(console.warn(`Airdrop tracking attempt ${s}/${Qe.MAX_RETRIES} failed:`,n.message),s<Qe.MAX_RETRIES){await qe(Qe.RETRY_DELAY_MS*s);continue}console.error(`Airdrop tracking failed for ${e} after ${Qe.MAX_RETRIES} attempts`)}})()}function mo(e,t,s){const n=`🎯 +${e.toLocaleString()} Airdrop Points! (${t}/${s})`;m(n,"success"),console.log(`🎯 AIRDROP POINTS AWARDED: +${e} (${t}/${s})`)}async function ue(){if(!c.isConnected||!c.userAddress)return m("Please connect wallet first","error"),null;try{let e=c.web3Provider||c.provider||window.ethereum;return e?await new U.BrowserProvider(e).getSigner():(m("No wallet provider found","error"),null)}catch(e){return console.error("Signer error:",e),m("Wallet connection error","error"),null}}function bo(e){const t=(e==null?void 0:e.reason)||(e==null?void 0:e.shortMessage)||(e==null?void 0:e.message)||"Unknown error";return t.includes("user rejected")||t.includes("User denied")?"Transaction cancelled by user":t.includes("insufficient funds")||t.includes("exceeds the balance")?"INSUFFICIENT_GAS":t.includes("exceeds balance")&&!t.includes("gas")?"Insufficient token balance":t.includes("0xfb550858")||t.includes("InsufficientOracleFee")?"Insufficient oracle fee (ETH)":t.includes("0xbcfa8e99")||t.includes("InvalidGuessCount")?"Wrong number of guesses":t.includes("0x5c844fb4")||t.includes("InvalidGuessRange")?"Guess out of range":t.includes("ZeroAmount")?"Amount cannot be zero":t.includes("NoActiveTiers")?"No active game tiers":t.includes("InsufficientAllowance")?"Please approve tokens first":t.includes("InsufficientBalance")?"Insufficient BKC balance":t.includes("DelegationNotFound")||t.includes("not found")?"Delegation not found":t.includes("DelegationLocked")||t.includes("still locked")?"Delegation is still locked":t.includes("InvalidIndex")||t.includes("invalid index")?"Invalid delegation index":t.includes("NotOwner")||t.includes("not owner")?"Not the delegation owner":t.includes("require(false)")?"Transaction rejected by contract":t.includes("Internal JSON-RPC")?"Network error - please try again":t.includes("network")||t.includes("timeout")?"Network timeout - please try again":t.slice(0,100)}function go(){m("⛽ You're out of ETH for gas fees!","error"),setTimeout(()=>{xo()},500)}function xo(){const e=document.getElementById("gas-faucet-modal");e&&e.remove();const t=document.createElement("div");t.id="gas-faucet-modal",t.className="fixed inset-0 z-[9999] flex items-center justify-center p-4",t.innerHTML=`
        ...
        <div class="relative bg-zinc-900 border border-red-500/50 rounded-2xl p-6 max-w-md w-full shadow-2xl animate-fade-in">
            ...
            <div class="space-y-3 mb-6">
                <a href="${Cn.arbitrumSepolia}" target="_blank" rel="noopener noreferrer"
                ...
                <a href="${Cn.alternativeFaucet}" target="_blank" rel="noopener noreferrer"
    ...
    `,document.body.appendChild(t)}function ne(e){const t=bo(e);return t==="INSUFFICIENT_GAS"?(go(),"Insufficient ETH for gas"):(m(t,"error"),t)}function ma(e){const t=(e==null?void 0:e.message)||(e==null?void 0:e.reason)||"";return["Internal JSON-RPC","network","timeout","ETIMEDOUT","ECONNRESET","rate limit","Too Many Requests","nonce","replacement transaction","already known"].some(n=>t.toLowerCase().includes(n.toLowerCase()))}function ba(e){const t=(e==null?void 0:e.message)||(e==null?void 0:e.reason)||"";return t.includes("user rejected")||t.includes("User denied")||t.includes("cancelled")||t.includes("canceled")}async function xt(e,t,s,n,a=ke.MAX_RETRIES){const i=["function approve(address,uint256) returns (bool)","function allowance(address,address) view returns (uint256)"],o=new U.Contract(e,i,n);let r;try{r=await o.allowance(c.userAddress,t),console.log("Current allowance:",U.formatEther(r),"BKC")}catch(d){console.warn("Could not check allowance:",d.message),r=0n}if(r>=s)return console.log("✅ Already approved"),!0;const l=s*10n;for(let d=1;d<=a;d++)try{m(`Approve tokens... ${d>1?`(attempt ${d})`:""}`,"info");const u=await o.approve(t,l);if(m("Waiting for approval confirmation...","info"),(await u.wait()).status===0)throw new Error("Approval transaction reverted");if(await qe(ke.APPROVAL_WAIT_MS),await o.allowance(c.userAddress,t)>=s)return m("✅ Approved!","success"),!0;throw new Error("Approval not reflected yet")}catch(u){if(console.error(`Approval attempt ${d} failed:`,u),ba(u))return m("Approval cancelled","error"),!1;if(d<a&&ma(u)){m(`Retrying approval... (${d}/${a})`,"warning"),await qe(ke.RETRY_DELAY_MS*d);continue}return ne(u),!1}return!1}async function pe(e,t={}){const{retries:s=ke.MAX_RETRIES,onAttempt:n=()=>{},onSuccess:a=()=>{},description:i="Transaction"}=t;for(let o=1;o<=s;o++)try{n(o);const r=await e();m("Waiting for confirmation...","info");const l=await r.wait();if(l.status===0)throw new Error("Transaction reverted");return a(l),{success:!0,receipt:l,txHash:l.hash}}catch(r){if(console.error(`${i} attempt ${o} failed:`,r),ba(r))return m("Transaction cancelled","error"),{success:!1,cancelled:!0};if(o<s&&ma(r)){m(`Network issue, retrying... (${o}/${s})`,"warning"),await qe(ke.RETRY_DELAY_MS*o);continue}return ne(r),{success:!1,error:r}}return{success:!1}}async function vo(e,t,s,n){const a=await ue();if(!a)return!1;try{const i=BigInt(e),o=BigInt(t),r=BigInt(s||0);if(await c.bkcTokenContract.balanceOf(c.userAddress)<i)return m("Insufficient BKC balance","error"),!1;if(!await xt(x.bkcToken,x.delegationManager,i,a))return!1;const u=new U.Contract(x.delegationManager,Re,a),p=await pe(()=>u.delegate(i,o,r),{description:"Delegation",onAttempt:f=>{f===1&&m("Confirm delegation in wallet...","info")},onSuccess:()=>{m("✅ Delegation successful!","success"),V()}});return p.success&&p.txHash&&de("delegation",p.txHash),p.success}catch(i){return console.error("Delegation error:",i),ne(i),!1}finally{}}async function ho(e,t,s){const n=await ue();if(!n)return!1;try{const a=Number(e);if(isNaN(a)||a<0)return m("Invalid delegation index","error"),!1;const i=BigInt(a),o=BigInt(t||0);console.log("Unstake params:",{index:i.toString(),boosterId:o.toString()});const r=new U.Contract(x.delegationManager,Re,n),l=await pe(()=>r.unstake(i,o),{description:"Unstake",onAttempt:d=>{d===1&&m("Confirm unstake in wallet...","info")},onSuccess:async()=>{m("✅ Unstaked!","success"),await V(!0),await Ys(!0)}});return l.success&&l.txHash&&de("unstake",l.txHash),l.success}catch(a){return console.error("Unstake error:",a),ne(a),!1}finally{}}async function wo(e,t,s){const n=await ue();if(!n)return!1;try{const a=Number(e);if(isNaN(a)||a<0)return m("Invalid delegation index","error"),!1;const i=BigInt(a),o=BigInt(t||0),r=new U.Contract(x.delegationManager,Re,n),l=await pe(()=>r.forceUnstake(i,o),{description:"Force Unstake",onAttempt:d=>{d===1&&m("Confirm force unstake in wallet...","info")},onSuccess:async()=>{m("✅ Force unstaked (penalty applied)","success"),await V(!0),await Ys(!0)}});return l.success&&l.txHash&&de("unstake",l.txHash),l.success}catch(a){return console.error("Force unstake error:",a),ne(a),!1}finally{}}async function Xs(e,t,s,n){const a=await ue();if(!a)return!1;const i=(n==null?void 0:n.innerHTML)||"Claim";n&&(n.innerHTML='<div class="loader inline-block"></div> Processing...',n.disabled=!0);try{let o=0n;try{const d=Number(s||0);d>0&&c.myBoosters&&c.myBoosters.length>0&&c.myBoosters.some(p=>Number(p.tokenId)===d)&&(o=BigInt(d))}catch(d){console.warn("Invalid booster ID for claim, using 0:",d)}const r=new U.Contract(x.delegationManager,Re,a),l=await pe(()=>r.claimReward(o),{description:"Claim Rewards",onAttempt:d=>{d===1&&m("Confirm claim in wallet...","info")},onSuccess:async()=>{m("✅ Rewards claimed!","success"),await V(!0)}});return l.success&&l.txHash&&de("claimReward",l.txHash),l.success}catch(o){return console.error("Claim error:",o),ne(o),!1}finally{n&&(n.innerHTML=i,n.disabled=!1)}}async function yo(e,t,s){var i;const n=await ue();if(!n)return{success:!1};const a=(s==null?void 0:s.innerHTML)||"Buy";s&&(s.innerHTML='<div class="loader inline-block"></div> Processing...',s.disabled=!0);try{const o=BigInt(t);if(!await xt(x.bkcToken,e,o,n))return{success:!1};s&&(s.innerHTML='<div class="loader inline-block"></div> Buying...');const l=new U.Contract(e,Hs,n),d=await pe(()=>l.buyNFT(),{description:"Buy NFT",onAttempt:u=>{u===1&&m("Confirm purchase in wallet...","info")}});if(d.success){let u=null;for(const p of d.receipt.logs)try{const f=l.interface.parseLog(p);if((f==null?void 0:f.name)==="NFTPurchased"){u=(i=f.args.tokenId)==null?void 0:i.toString();break}}catch{}return m(`✅ NFT purchased! ${u?`#${u}`:""}`,"success"),V(),de("buyNFT",d.txHash),{success:!0,tokenId:u,txHash:d.txHash}}return{success:!1}}catch(o){return console.error("Buy NFT error:",o),ne(o),{success:!1}}finally{s&&(s.innerHTML=a,s.disabled=!1)}}async function ko(e,t,s,n){const a=await ue();if(!a)return{success:!1};const i=(n==null?void 0:n.innerHTML)||"Sell";n&&(n.innerHTML='<div class="loader inline-block"></div> Processing...',n.disabled=!0);try{let o=t;if(typeof t=="object"&&t!==null&&(o=t.tokenId||t.id||t.token_id,console.warn("executeSellNFT: Object passed instead of tokenId, extracted:",o)),o==null||o==="")return console.error("executeSellNFT: Invalid tokenId:",t),m("Invalid NFT token ID","error"),{success:!1};try{o=BigInt(o)}catch{return console.error("executeSellNFT: Cannot convert tokenId to BigInt:",o),m("Invalid NFT token ID format","error"),{success:!1}}let r=0n;if(s!=null&&s!=="")try{r=BigInt(s)}catch{console.warn("executeSellNFT: Invalid minPayout, using 0"),r=0n}const l=["function approve(address,uint256)","function getApproved(uint256) view returns (address)"],d=new U.Contract(x.rewardBoosterNFT,l,a);(await d.getApproved(o)).toLowerCase()!==e.toLowerCase()&&(m("Approving NFT...","info"),await(await d.approve(e,o)).wait(),await qe(ke.APPROVAL_WAIT_MS)),n&&(n.innerHTML='<div class="loader inline-block"></div> Selling...');const p=new U.Contract(e,Hs,a),f=await pe(()=>p.sellNFT(o,r),{description:"Sell NFT",onAttempt:g=>{g===1&&m("Confirm sale in wallet...","info")}});return f.success?(m("✅ NFT sold!","success"),V(),de("sellNFT",f.txHash),{success:!0,txHash:f.txHash}):{success:!1}}catch(o){return console.error("Sell NFT error:",o),ne(o),{success:!1}}finally{n&&(n.innerHTML=i,n.disabled=!1)}}async function To(e,t){const s=await ue();if(!s)return{success:!1};const n=(t==null?void 0:t.innerHTML)||"List";t&&(t.innerHTML='<div class="loader inline-block"></div> Processing...',t.disabled=!0);try{let a,i,o,r;typeof e=="object"&&e!==null?(a=e.tokenId,i=e.pricePerHour,o=e.minHours,r=e.maxHours):(a=e,i=arguments[1],o=arguments[2],r=arguments[3],t=arguments[4]);const l=x.rentalManager;if(!l)return m("Rental Manager not configured","error"),{success:!1};const d=["function setApprovalForAll(address,bool)","function isApprovedForAll(address,address) view returns (bool)"],u=new U.Contract(x.rewardBoosterNFT,d,s);await u.isApprovedForAll(c.userAddress,l)||(m("Approving NFTs for rental...","info"),await(await u.setApprovalForAll(l,!0)).wait(),await qe(ke.APPROVAL_WAIT_MS)),t&&(t.innerHTML='<div class="loader inline-block"></div> Listing...');const f=new U.Contract(l,Be,s),g=await pe(()=>f.listNFT(BigInt(a),BigInt(i),BigInt(o),BigInt(r)),{description:"List NFT",onAttempt:b=>{b===1&&m("Confirm listing in wallet...","info")}});return g.success?(m(`✅ NFT #${a} listed for rent!`,"success"),gt(!0),de("listRental",g.txHash),{success:!0,txHash:g.txHash}):{success:!1}}catch(a){return console.error("List NFT error:",a),ne(a),{success:!1}}finally{t&&(t.innerHTML=n,t.disabled=!1)}}async function zo(e,t){const s=await ue();if(!s)return{success:!1};const n=(t==null?void 0:t.innerHTML)||"Rent";t&&(t.innerHTML='<div class="loader inline-block"></div> Processing...',t.disabled=!0);try{let a,i,o;typeof e=="object"&&e!==null?(a=e.tokenId,i=e.hours,o=e.totalCost):(a=e,i=arguments[1],o=arguments[2],t=arguments[3]);const r=x.rentalManager;if(!r)return m("Rental Manager not configured","error"),{success:!1};const l=BigInt(o);if(!await xt(x.bkcToken,r,l,s))return{success:!1};t&&(t.innerHTML='<div class="loader inline-block"></div> Renting...');const u=new U.Contract(r,Be,s),p=await pe(()=>u.rentNFT(BigInt(a),BigInt(i)),{description:"Rent NFT",onAttempt:f=>{f===1&&m("Confirm rental in wallet...","info")}});return p.success?(m(`✅ NFT #${a} rented for ${i} hours!`,"success"),V(),gt(!0),de("rentNFT",p.txHash),{success:!0,txHash:p.txHash}):{success:!1}}catch(a){return console.error("Rent NFT error:",a),ne(a),{success:!1}}finally{t&&(t.innerHTML=n,t.disabled=!1)}}async function Co(e,t){const s=await ue();if(!s)return{success:!1};const n=(t==null?void 0:t.innerHTML)||"Withdraw";t&&(t.innerHTML='<div class="loader inline-block"></div> Processing...',t.disabled=!0);try{const a=x.rentalManager,i=new U.Contract(a,Be,s),o=await pe(()=>i.withdrawNFT(e),{description:"Withdraw NFT",onAttempt:r=>{r===1&&m("Confirm withdrawal in wallet...","info")}});return o.success?(m("✅ NFT withdrawn!","success"),V(),gt(),{success:!0,txHash:o.txHash}):{success:!1}}catch(a){return console.error("Withdraw NFT error:",a),ne(a),{success:!1}}finally{t&&(t.innerHTML=n,t.disabled=!1)}}async function Bo(e,t){var a;const s=await ue();if(!s)return!1;const n=(t==null?void 0:t.innerHTML)||"Notarize";t&&(t.innerHTML='<div class="loader inline-block"></div> Processing...',t.disabled=!0);try{const{ipfsUri:i,contentHash:o,description:r}=e,l=x.decentralizedNotary||x.notary;if(!l)return m("Notary contract not configured","error"),!1;let d=U.parseEther("1");try{d=await new U.Contract(l,ks,s.provider).calculateFee(0)}catch{}if(!await xt(x.bkcToken,l,d,s))return!1;t&&(t.innerHTML='<div class="loader inline-block"></div> Notarizing...');const p=new U.Contract(l,ks,s);let f=0n;try{const b=await Ge();b!=null&&b.tokenId&&(f=BigInt(b.tokenId))}catch{}const g=await pe(()=>p.notarize(i,r||"",o,f),{description:"Notarize",onAttempt:b=>{b===1&&m("Confirm notarization in wallet...","info")}});if(g.success){let b=null;for(const v of g.receipt.logs)try{const y=p.interface.parseLog(v);if((y==null?void 0:y.name)==="DocumentNotarized"){b=(a=y.args.tokenId)==null?void 0:a.toString();break}}catch{}return m(`✅ Document notarized! ${b?`#${b}`:""}`,"success"),de("notarize",g.txHash),{success:!0,tokenId:b,txHash:g.txHash}}return!1}catch(i){return console.error("Notarize error:",i),ne(i),!1}finally{t&&(t.innerHTML=n,t.disabled=!1)}}const Io=Bo,Eo="https://faucet-4wvdcuoouq-uc.a.run.app";async function $o(e){if(!c.isConnected||!c.userAddress)return m("Please connect wallet first","error"),{success:!1};const t=(e==null?void 0:e.innerHTML)||"Claim";e.innerHTML='<div class="loader inline-block"></div> Claiming...',e.disabled=!0;try{const s=await fetch(`${Eo}?address=${c.userAddress}`,{method:"GET",headers:{Accept:"application/json"}}),n=await s.json();if(s.ok&&n.success)return m("✅ Tokens received!","success"),V(),n.txHash&&de("faucet",n.txHash),{success:!0,txHash:n.txHash,bkcAmount:n.bkcAmount,ethAmount:n.ethAmount};{const a=n.error||n.message||"Faucet unavailable";return m(a,"error"),{success:!1,error:a}}}catch(s){return console.error("Faucet error:",s),m("Faucet unavailable","error"),{success:!1}}finally{e.innerHTML=t,e.disabled=!1}}const Ao=["function play(uint256 _wagerAmount, uint256[] calldata _guesses, bool _isCumulative) external payable","function prizePoolBalance() external view returns (uint256)","function gameCounter() external view returns (uint256)","function activeTierCount() external view returns (uint256)","function serviceFee() external view returns (uint256)","function getRequiredServiceFee(bool _isCumulative) external view returns (uint256)","event GamePlayed(uint256 indexed gameId, address indexed player, uint256 wagerAmount, uint256 prizeWon, bool isCumulative, uint8 matchCount)","event GameDetails(uint256 indexed gameId, uint256[] guesses, uint256[] rolls, bool[] matches)","event JackpotWon(uint256 indexed gameId, address indexed player, uint256 prizeAmount, uint256 tier)"];async function Lo(e,t,s,n){const a=await ue();if(!a)return{success:!1};try{const i=U.parseEther(e.toString()),o=Array.isArray(t)?t.map(b=>BigInt(b)):[BigInt(t)];console.log("Fortune V2 Play params:",{wager:e.toString(),wagerWei:i.toString(),guesses:o.map(b=>b.toString()),isCumulative:s});const r=x.fortunePoolV2||x.fortunePool||x.actionsManager;if(!r)return m("Fortune Pool not configured","error"),{success:!1};const l=new U.Contract(r,Ao,a);let d=0n;try{if(d=await l.activeTierCount(),d===0n)return m("No active game tiers","error"),{success:!1};const b=s?Number(d):1;if(o.length!==b)return m(`Need ${b} guess(es), got ${o.length}`,"error"),{success:!1}}catch(b){console.warn("Could not verify tier count:",b.message)}let u=0n;try{u=await l.getRequiredServiceFee(s),console.log("Required service fee:",U.formatEther(u),"ETH")}catch(b){console.warn("Could not get service fee:",b.message)}if(u>0n&&await a.provider.getBalance(c.userAddress)<u)return m(`Need ${U.formatEther(u)} ETH for game fee`,"error"),{success:!1};if(await c.bkcTokenContract.balanceOf(c.userAddress)<i)return m("Insufficient BKC balance","error"),{success:!1};if(!await xt(x.bkcToken,r,i,a))return{success:!1};const g=await pe(()=>l.play(i,o,s,{value:u}),{description:"Fortune Game V2",retries:ke.MAX_RETRIES,onAttempt:b=>{b===1&&m("Confirm game in wallet...","info")}});if(g.success){let b=null,v=[],y=0,k=[],E=0,L=!1;for(const A of g.receipt.logs)try{const $=l.interface.parseLog({topics:A.topics,data:A.data});($==null?void 0:$.name)==="GamePlayed"&&(b=Number($.args.gameId),y=Number(U.formatEther($.args.prizeWon)),E=Number($.args.matchCount)),($==null?void 0:$.name)==="GameDetails"&&(v=$.args.rolls.map(T=>Number(T)),k=Array.from($.args.matches)),($==null?void 0:$.name)==="JackpotWon"&&(L=!0)}catch{}return y>0?m(L?`🎰🎰🎰 JACKPOT! +${y.toLocaleString()} BKC!`:`🎉 YOU WON ${y.toLocaleString()} BKC!`,"success"):m("No match this time. Try again!","warning"),V(),de("fortune",g.txHash),{success:!0,gameId:b,rolls:v,guesses:o.map(A=>Number(A)),prizeWon:y,matches:k,matchCount:E,isJackpot:L,isCumulative:s,txHash:g.txHash}}return{success:!1}}catch(i){return console.error("Fortune V2 error:",i),ne(i),{success:!1}}finally{}}const Zt=window.ethers,I={hasRenderedOnce:!1,lastUpdate:0,activities:[],networkActivities:[],filteredActivities:[],userProfile:null,pagination:{currentPage:1,itemsPerPage:8},filters:{type:"ALL",sort:"NEWEST"},metricsCache:{},economicData:null,isLoadingNetworkActivity:!1,networkActivitiesTimestamp:0,faucet:{canClaim:!0,cooldownEnd:null,isLoading:!1,lastCheck:0}},So="https://sepolia.arbiscan.io/tx/",Po="https://sepolia.arbiscan.io/address/",No="https://faucet-4wvdcuoouq-uc.a.run.app",Mo="https://getrecentactivity-4wvdcuoouq-uc.a.run.app",Fo="https://getsystemdata-4wvdcuoouq-uc.a.run.app",zs="1,000",Cs="0.01",Ro=Zt.parseUnits("100",18),W={STAKING:{icon:"fa-lock",color:"#4ade80",bg:"rgba(34,197,94,0.15)",label:"🔒 Staked",emoji:"🔒"},UNSTAKING:{icon:"fa-unlock",color:"#fb923c",bg:"rgba(249,115,22,0.15)",label:"🔓 Unstaked",emoji:"🔓"},FORCE_UNSTAKE:{icon:"fa-bolt",color:"#ef4444",bg:"rgba(239,68,68,0.15)",label:"⚡ Force Unstaked",emoji:"⚡"},CLAIM:{icon:"fa-coins",color:"#fbbf24",bg:"rgba(245,158,11,0.15)",label:"🪙 Rewards Claimed",emoji:"🪙"},NFT_BUY:{icon:"fa-bag-shopping",color:"#4ade80",bg:"rgba(34,197,94,0.15)",label:"🛍️ Bought NFT",emoji:"🛍️"},NFT_SELL:{icon:"fa-hand-holding-dollar",color:"#fb923c",bg:"rgba(249,115,22,0.15)",label:"💰 Sold NFT",emoji:"💰"},NFT_MINT:{icon:"fa-gem",color:"#fde047",bg:"rgba(234,179,8,0.15)",label:"💎 Minted Booster",emoji:"💎"},NFT_TRANSFER:{icon:"fa-arrow-right-arrow-left",color:"#60a5fa",bg:"rgba(59,130,246,0.15)",label:"↔️ Transfer",emoji:"↔️"},RENTAL_LIST:{icon:"fa-tag",color:"#4ade80",bg:"rgba(34,197,94,0.15)",label:"🏷️ Listed NFT",emoji:"🏷️"},RENTAL_RENT:{icon:"fa-clock",color:"#22d3ee",bg:"rgba(6,182,212,0.15)",label:"⏰ Rented NFT",emoji:"⏰"},RENTAL_WITHDRAW:{icon:"fa-rotate-left",color:"#fb923c",bg:"rgba(249,115,22,0.15)",label:"↩️ Withdrawn",emoji:"↩️"},FORTUNE_BET:{icon:"fa-paw",color:"#f97316",bg:"rgba(249,115,22,0.2)",label:"🐯 Fortune Bet",emoji:"🐯"},FORTUNE_ORACLE:{icon:"fa-eye",color:"#e879f9",bg:"rgba(232,121,249,0.25)",label:"🔮 Oracle Response",emoji:"🔮"},FORTUNE_WIN:{icon:"fa-crown",color:"#facc15",bg:"rgba(234,179,8,0.25)",label:"🏆 Fortune Winner!",emoji:"🏆"},FORTUNE_LOSE:{icon:"fa-paw",color:"#71717a",bg:"rgba(39,39,42,0.5)",label:"🐯 No Luck",emoji:"😿"},NOTARY:{icon:"fa-stamp",color:"#818cf8",bg:"rgba(99,102,241,0.15)",label:"📜 Notarized",emoji:"📜"},FAUCET:{icon:"fa-droplet",color:"#22d3ee",bg:"rgba(6,182,212,0.15)",label:"💧 Faucet Claim",emoji:"💧"},DEFAULT:{icon:"fa-circle",color:"#71717a",bg:"rgba(39,39,42,0.5)",label:"Activity",emoji:"📋"}};function Do(e){if(!e)return"Just now";try{const t=e.seconds||e._seconds||new Date(e).getTime()/1e3,s=new Date(t*1e3),a=new Date-s,i=Math.floor(a/6e4),o=Math.floor(a/36e5),r=Math.floor(a/864e5);return i<1?"Just now":i<60?`${i}m ago`:o<24?`${o}h ago`:r<7?`${r}d ago`:s.toLocaleDateString()}catch{return"Recent"}}function jo(e){if(!e)return"";try{const t=e.seconds||e._seconds||new Date(e).getTime()/1e3;return new Date(t*1e3).toLocaleString("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return""}}function cs(e){return e>=1e6?(e/1e6).toFixed(2)+"M":e>=1e3?(e/1e3).toFixed(1)+"K":e.toFixed(0)}function Ho(e){return e?`${e.slice(0,6)}...${e.slice(-4)}`:""}function _o(e){if(!e)return"";const t=Date.now(),n=new Date(e).getTime()-t;if(n<=0)return"";const a=Math.floor(n/36e5),i=Math.floor(n%36e5/6e4);return a>0?`${a}h ${i}m`:`${i}m`}function Uo(e,t={}){const s=(e||"").toUpperCase().trim();return s==="STAKING"||s==="STAKED"||s==="STAKE"||s==="DELEGATED"||s==="DELEGATION"||s.includes("DELEGAT")?W.STAKING:s==="UNSTAKE"||s==="UNSTAKED"||s==="UNSTAKING"||s.includes("UNDELEGAT")?s.includes("FORCE")?W.FORCE_UNSTAKE:W.UNSTAKING:s==="CLAIMREWARD"||s==="CLAIM"||s==="CLAIMED"||s.includes("REWARD")||s.includes("CLAIM")?W.CLAIM:s==="NFTBOUGHT"||s.includes("NFTBOUGHT")||s.includes("NFT_BOUGHT")?W.NFT_BUY:s==="NFTSOLD"||s.includes("NFTSOLD")||s.includes("NFT_SOLD")?W.NFT_SELL:s==="BOOSTERBUY"||s.includes("BOOSTER")||s.includes("PRESALE")||s.includes("MINTED")?W.NFT_MINT:s==="TRANSFER"||s.includes("TRANSFER")?W.NFT_TRANSFER:s==="RENTALLISTED"||s.includes("LISTED")||s.includes("LIST")?W.RENTAL_LIST:s==="RENTALRENTED"||s==="RENTED"||s.includes("RENTAL")&&s.includes("RENT")?W.RENTAL_RENT:s==="RENTALWITHDRAWN"||s.includes("WITHDRAW")?W.RENTAL_WITHDRAW:s==="GAMEREQUESTED"||s.includes("GAMEREQUESTED")||s.includes("GAME_REQUEST")||s.includes("REQUEST")?W.FORTUNE_BET:s==="GAMEFULFILLED"||s.includes("FULFILLED")||s.includes("ORACLE")?W.FORTUNE_ORACLE:s==="GAMERESULT"||s.includes("RESULT")?(t==null?void 0:t.isWin)||(t==null?void 0:t.prizeWon)>0?W.FORTUNE_WIN:W.FORTUNE_LOSE:s.includes("FORTUNE")||s.includes("GAME")?W.FORTUNE_BET:s==="NOTARYREGISTER"||s==="NOTARIZED"||s.includes("NOTARY")||s.includes("DOCUMENT")?W.NOTARY:s==="FAUCETCLAIM"||s.includes("FAUCET")||s.includes("DISTRIBUTED")?W.FAUCET:W.DEFAULT}let ds=null,Ie=0n;function ga(e){const t=document.getElementById("dash-user-rewards");if(!t||!c.isConnected){ds&&cancelAnimationFrame(ds);return}const s=e-Ie;s>-1000000000n&&s<1000000000n?Ie=e:Ie+=s/8n,Ie<0n&&(Ie=0n),t.innerHTML=`${B(Ie).toFixed(4)} <span class="text-sm text-amber-500/80">BKC</span>`,Ie!==e&&(ds=requestAnimationFrame(()=>ga(e)))}async function Bn(e){if(!c.isConnected||!c.userAddress)return m("Connect wallet first","error");const t=e.innerHTML;e.disabled=!0,e.innerHTML='<i class="fa-solid fa-circle-notch fa-spin mr-2"></i> Sending...',I.faucet.isLoading=!0;try{const s=await fetch(`${No}?address=${c.userAddress}`,{method:"GET",headers:{Accept:"application/json"}}),n=await s.json();if(s.ok&&n.success)m(`✅ Faucet Sent! ${zs} BKC + ${Cs} ETH`,"success"),I.faucet.canClaim=!1,I.faucet.cooldownEnd=new Date(Date.now()+24*60*60*1e3).toISOString(),Bs(),setTimeout(()=>{ha.update(!0)},4e3);else{const a=n.error||n.message||"Faucet unavailable";if(a.toLowerCase().includes("cooldown")||a.toLowerCase().includes("wait")||a.toLowerCase().includes("hour")){m(`⏳ ${a}`,"warning");const i=a.match(/(\d+)\s*hour/i);if(i){const o=parseInt(i[1]);I.faucet.canClaim=!1,I.faucet.cooldownEnd=new Date(Date.now()+o*60*60*1e3).toISOString(),Bs()}}else m(`❌ ${a}`,"error")}}catch(s){console.error("Faucet error:",s),m("Faucet Offline - Try again later","error")}finally{I.faucet.isLoading=!1,e.disabled=!1,e.innerHTML=t}}function Oo(){return c.isConnected?(c.currentUserBalance||c.bkcBalance||0n)<Ro:!1}function Bs(){const e=document.getElementById("dashboard-faucet-widget");if(!e)return;if(!Oo()){e.classList.add("hidden");return}e.classList.remove("hidden");const t=c.currentUserBalance||c.bkcBalance||0n,s=t===0n,n=_o(I.faucet.cooldownEnd),a=I.faucet.canClaim&&!n,i=document.getElementById("faucet-title"),o=document.getElementById("faucet-desc"),r=document.getElementById("faucet-status"),l=document.getElementById("faucet-action-btn");if(e.className="glass-panel border-l-4 p-4",l&&(l.className="w-full sm:w-auto font-bold py-2.5 px-5 rounded-lg text-sm transition-all"),!a&&n)e.classList.add("border-zinc-500"),i&&(i.innerText="⏳ Faucet Cooldown"),o&&(o.innerText="Come back when the timer ends"),r&&(r.classList.remove("hidden"),r.innerHTML=`<i class="fa-solid fa-clock mr-1"></i> ${n} remaining`),l&&(l.classList.add("bg-zinc-700","text-zinc-400","cursor-not-allowed"),l.innerHTML='<i class="fa-solid fa-hourglass-half mr-2"></i> On Cooldown',l.disabled=!0);else if(s)e.classList.add("border-green-500"),i&&(i.innerText="🎉 Welcome to BackCoin!"),o&&(o.innerText=`Claim your free starter pack: ${zs} BKC + ${Cs} ETH for gas`),r&&r.classList.add("hidden"),l&&(l.classList.add("bg-green-600","hover:bg-green-500","text-white","hover:scale-105"),l.innerHTML='<i class="fa-solid fa-gift mr-2"></i> Claim Starter Pack',l.disabled=!1);else{const d=B(t).toFixed(2);e.classList.add("border-cyan-500"),i&&(i.innerText="💧 Need More BKC?"),o&&(o.innerText=`Balance: ${d} BKC • Get ${zs} BKC + ${Cs} ETH`),r&&r.classList.add("hidden"),l&&(l.classList.add("bg-cyan-600","hover:bg-cyan-500","text-white","hover:scale-105"),l.innerHTML='<i class="fa-solid fa-faucet mr-2"></i> Request Tokens',l.disabled=!1)}}async function Wo(){try{if(await c.provider.getBalance(c.userAddress)<Zt.parseEther("0.002")){const t=document.getElementById("no-gas-modal-dash");return t&&(t.classList.remove("hidden"),t.classList.add("flex")),!1}return!0}catch{return!0}}function qo(){if(!se.dashboard)return;const e=x.ecosystemManager||"",t=e?`${Po}${e}`:"#";se.dashboard.innerHTML=`
        <div class="flex flex-col gap-6 pb-10 max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                ${Yo("Total Supply","fa-coins","dash-metric-supply","Total BKC tokens in circulation")}
                ${$t("Net pStake","fa-layer-group","dash-metric-pstake","Total staking power on network","purple")}
                ${$t("Economic Output","fa-chart-line","dash-metric-economic","Total value generated (Mined + Fees)","green")}
                ${$t("Fees Collected","fa-receipt","dash-metric-fees","Total fees generated by the ecosystem","orange")}
                ${$t("TVL %","fa-lock","dash-metric-tvl","Percentage of supply locked in contracts","blue")}
                ${Ko()}
            </div>
        ...
        ${Go()}
        ${Vo()}
    `,er()}function $t(e,t,s,n,a="zinc"){const i={zinc:"text-zinc-400",purple:"text-purple-400",blue:"text-blue-400",orange:"text-orange-400",green:"text-green-400"},o=i[a]||i.zinc;return`
    ...
    `}function Yo(e,t,s,n,a="zinc"){const i={zinc:"text-zinc-400",purple:"text-purple-400",blue:"text-blue-400",orange:"text-orange-400",green:"text-green-400"},o=i[a]||i.zinc;return`
        <div class="glass-panel p-3 sm:p-4 group hover:border-zinc-600 transition-all cursor-default" title="${n}">
            <div class="flex items-center gap-1.5 mb-1">
                <i class="fa-solid ${t} ${o} text-xs"></i>
                <span class="text-[10px] text-zinc-500 uppercase font-bold tracking-wider truncate">${e}</span>
    ...
    `}async function Xo(){try{const e=await fetch(Fo);if(e.ok){const t=await e.json();return I.economicData=t,t}}catch{}return null}async function Is(){var e,t,s,n,a,i,o;try{const r=await Xo();let l=0n,d=0n,u=0n,p=0n,f=0n,g=0,b=0n;if(r&&(console.log("📊 Firebase economic data:",r.economy),(e=r.economy)!=null&&e.totalSupply&&(l=BigInt(r.economy.totalSupply)),(t=r.economy)!=null&&t.totalPStake&&(d=BigInt(r.economy.totalPStake)),(s=r.economy)!=null&&s.totalTVL&&(u=BigInt(r.economy.totalTVL)),(n=r.economy)!=null&&n.economicOutput&&(p=BigInt(r.economy.economicOutput)),(a=r.economy)!=null&&a.totalFeesCollected&&(f=BigInt(r.economy.totalFeesCollected)),(i=r.economy)!=null&&i.fortunePoolBalance&&(b=BigInt(r.economy.fortunePoolBalance)),(o=r.stats)!=null&&o.notarizedDocuments&&(g=r.stats.notarizedDocuments)),c.bkcTokenContractPublic&&(l===0n&&(console.log("📊 Fetching totalSupply from blockchain (fallback)..."),l=await O(c.bkcTokenContractPublic,"totalSupply",[],0n)),d===0n&&c.delegationManagerContractPublic&&(console.log("📊 Fetching totalPStake from blockchain (fallback)..."),d=await O(c.delegationManagerContractPublic,"totalNetworkPStake",[],0n)),u===0n)){console.log("📊 Calculating TVL from blockchain (fallback)...");const S=[x.delegationManager,x.fortunePool,x.rentalManager,x.miningManager,x.decentralizedNotary,x.nftLiquidityPoolFactory,x.pool_diamond,x.pool_platinum,x.pool_gold,x.pool_silver,x.pool_bronze,x.pool_iron,x.pool_crystal].filter(K=>K&&K!==Zt.ZeroAddress),X=S.map(K=>O(c.bkcTokenContractPublic,"balanceOf",[K],0n)),le=await Promise.all(X);if(le.forEach(K=>{u+=K}),x.fortunePool&&b===0n){const K=S.indexOf(x.fortunePool);K>=0&&(b=le[K])}}const v=B(l),y=B(p),k=B(f),E=B(b),L=S=>S.toLocaleString("en-US",{minimumFractionDigits:1,maximumFractionDigits:1});let A=0;l>0n&&(A=Number(u*10000n/l)/100),A>100&&(A=100);const $=(S,X,le="")=>{const K=document.getElementById(S);K&&(K.innerHTML=`${X}${le?` <span class="text-xs text-zinc-500">${le}</span>`:""}`)},T=document.getElementById("dash-metric-supply");T&&(T.innerHTML=`${L(v)} <span style="font-size: 10px; color: #71717a">BKC</span>`),$("dash-metric-pstake",Me(d)),$("dash-metric-economic",cs(y),"BKC"),$("dash-metric-fees",cs(k),"BKC");const F=document.getElementById("dash-metric-tvl");if(F){const S=A>30?"text-green-400":A>10?"text-yellow-400":"text-blue-400";F.innerHTML=`<span class="${S}">${A.toFixed(1)}%</span>`}xa();const q=document.getElementById("dash-fortune-prize");q&&(q.innerText=`${cs(E)} BKC`);const re=document.getElementById("dash-notary-count");re&&(re.innerText=g>0?`${g} docs`:"--"),I.metricsCache={supply:v,economic:y,fees:k,timestamp:Date.now()}}catch(r){console.error("Metrics Error",r)}}async function Jo(){if(c.userAddress)try{const e=await fetch(`https://getuserprofile-4wvdcuoouq-uc.a.run.app/${c.userAddress}`);e.ok&&(I.userProfile=await e.json(),Qo(I.userProfile))}catch{}}function Qo(e){const t=document.getElementById("dash-presale-stats");if(!t||!e||!e.presale||!e.presale.totalBoosters||e.presale.totalBoosters===0)return;t.classList.remove("hidden");const s=e.presale.totalSpentWei||0,n=parseFloat(Zt.formatEther(BigInt(s))).toFixed(4);document.getElementById("stats-total-spent").innerText=`${n} ETH`,document.getElementById("stats-total-boosters").innerText=e.presale.totalBoosters||0;const a=document.getElementById("stats-tier-badges");if(a&&e.presale.tiersOwned){let i="";Object.entries(e.presale.tiersOwned).forEach(([o,r])=>{const l=Z[Number(o)-1],d=l?l.name:`T${o}`;i+=`<span class="text-[9px] bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded">${r}x ${d}</span>`}),i&&(a.innerHTML=i)}}function xa(){const e=document.getElementById("dash-metric-balance"),t=document.getElementById("dash-balance-card");if(!e)return;const s=c.currentUserBalance||c.bkcBalance||0n;if(!c.isConnected){e.innerHTML='<span class="text-zinc-500 text-xs">Connect Wallet</span>',t&&(t.style.borderColor="rgba(63,63,70,0.5)");return}if(s===0n)e.innerHTML='0.00 <span style="font-size: 10px; color: #71717a">BKC</span>';else{const a=B(s).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2});e.innerHTML=`${a} <span style="font-size: 10px; color: #71717a">BKC</span>`}t&&(t.style.borderColor="rgba(245,158,11,0.3)")}async function Ft(e=!1){var t,s;if(!c.isConnected){const n=document.getElementById("dash-booster-area");n&&(n.innerHTML=`
                <div class="text-center">
                ...
                </div>`);return}try{const n=document.getElementById("dash-user-rewards");e&&n&&n.classList.add("animate-pulse","opacity-70");const[,a,i]=await Promise.all([V(),ea(),Ge()]),o=(a==null?void 0:a.netClaimAmount)||0n;ga(o),n&&n.classList.remove("animate-pulse","opacity-70");const r=document.getElementById("dashboardClaimBtn");r&&(r.disabled=o<=0n);const l=document.getElementById("dash-user-pstake");if(l){let d=((t=c.userData)==null?void 0:t.pStake)||((s=c.userData)==null?void 0:s.userTotalPStake)||c.userTotalPStake||0n;if(d===0n&&c.delegationManagerContractPublic&&c.userAddress)try{d=await O(c.delegationManagerContractPublic,"userTotalPStake",[c.userAddress],0n)}catch{}l.innerText=Me(d)}xa(),Zo(i,a),Jo(),Bs()}catch(n){console.error("User Hub Error:",n)}}function Zo(e,t){const s=document.getElementById("dash-booster-area");if(!s)return;const n=(e==null?void 0:e.highestBoost)||0;if(n===0){const d=((t==null?void 0:t.totalRewards)||0n)*5000n/10000n;if(d>0n){const u=document.getElementById("dash-user-gain-area");u&&(u.classList.remove("hidden"),document.getElementById("dash-user-potential-gain").innerText=B(d).toFixed(2))}s.innerHTML=`
        ...
        `;return}const a=e.source==="rented",i=a?"bg-cyan-500/20 text-cyan-300":"bg-green-500/20 text-green-300",o=a?"Rented":"Owned";let r=e.imageUrl;if(!r||r.includes("placeholder")){const l=Z.find(d=>d.boostBips===n);l&&l.realImg&&(r=l.realImg)}s.innerHTML=`
        <div class="flex items-center gap-3 bg-zinc-800/40 border border-green-500/20 rounded-lg p-3 nft-clickable-image cursor-pointer" data-address="${x.rewardBoosterNFT}" data-tokenid="${e.tokenId}">
    ...
    `}async function Rt(){const e=document.getElementById("dash-activity-list"),t=document.getElementById("activity-title");try{if(c.isConnected){if(I.activities.length===0){e&&(e.innerHTML=`
                        <div class="flex flex-col items-center justify-center py-8">
                            <div class="relative">
                                <div class="absolute inset-0 bg-amber-500/30 blur-xl rounded-full animate-pulse"></div>
                                <img src="./assets/bkc_logo_3d.png" class="relative w-12 h-12 object-contain animate-bounce" alt="Loading">
                    ...
                    `);const s=await fetch(`${oe.getHistory}/${c.userAddress}`);s.ok&&(I.activities=await s.json())}if(I.activities.length>0){t&&(t.textContent="Your Activity"),Es();return}}t&&(t.textContent="Network Activity"),await In()}catch(s){console.error("Activity fetch error:",s),t&&(t.textContent="Network Activity"),await In()}}async function In(){const e=document.getElementById("dash-activity-list");if(!e||I.isLoadingNetworkActivity)return;const t=Date.now()-I.networkActivitiesTimestamp;if(I.networkActivities.length>0&&t<3e5){En();return}I.isLoadingNetworkActivity=!0,e.innerHTML=`
        <div class="flex flex-col items-center justify-center py-8">
            <div class="relative">
                <div class="absolute inset-0 bg-amber-500/30 blur-xl rounded-full animate-pulse"></div>
                <img src="./assets/bkc_logo_3d.png" class="relative w-12 h-12 object-contain animate-bounce" alt="Loading">
    ...
    `;try{const s=await fetch(`${Mo}?limit=30`);if(s.ok){const n=await s.json();I.networkActivities=Array.isArray(n)?n:n.activities||[],I.networkActivitiesTimestamp=Date.now()}else I.networkActivities=[]}catch(s){console.error("Network activity fetch error:",s),I.networkActivities=[]}finally{I.isLoadingNetworkActivity=!1}En()}function En(){const e=document.getElementById("dash-activity-list"),t=document.getElementById("dash-pagination-controls");if(e){if(I.networkActivities.length===0){e.innerHTML=`
        ...
        `,t&&t.classList.add("hidden");return}e.innerHTML=I.networkActivities.slice(0,15).map(s=>va(s,!0)).join(""),t&&t.classList.add("hidden")}}function Es(){let e=[...I.activities];const t=I.filters.type,s=n=>(n||"").toUpperCase();t!=="ALL"&&(e=e.filter(n=>{const a=s(n.type);return t==="STAKE"?a.includes("DELEGATION")||a.includes("DELEGAT")||a.includes("STAKE")||a.includes("UNSTAKE"):t==="CLAIM"?a.includes("REWARD")||a.includes("CLAIM"):t==="NFT"?a.includes("BOOSTER")||a.includes("RENT")||a.includes("NFT")||a.includes("TRANSFER"):t==="GAME"?a.includes("FORTUNE")||a.includes("GAME")||a.includes("REQUEST")||a.includes("RESULT")||a.includes("FULFILLED"):t==="NOTARY"?a.includes("NOTARY")||a.includes("NOTARIZED")||a.includes("DOCUMENT"):t==="FAUCET"?a.includes("FAUCET"):!0})),e.sort((n,a)=>{const i=o=>o.timestamp&&o.timestamp._seconds?o.timestamp._seconds:o.createdAt&&o.createdAt._seconds?o.createdAt._seconds:o.timestamp?new Date(o.timestamp).getTime()/1e3:0;return I.filters.sort==="NEWEST"?i(a)-i(n):i(n)-i(a)}),I.filteredActivities=e,I.pagination.currentPage=1,$s()}function $s(){const e=document.getElementById("dash-activity-list"),t=document.getElementById("dash-pagination-controls");if(!e)return;if(I.filteredActivities.length===0){e.innerHTML=`
            <div class="flex flex-col items-center justify-center py-8 px-4">
                <div class="relative mb-4">
                    <div class="absolute inset-0 bg-amber-500/20 blur-xl rounded-full animate-pulse"></div>
        ...
        `,t&&t.classList.add("hidden");return}const s=(I.pagination.currentPage-1)*I.pagination.itemsPerPage,n=s+I.pagination.itemsPerPage,a=I.filteredActivities.slice(s,n);if(e.innerHTML=a.map(i=>va(i,!1)).join(""),t){const i=Math.ceil(I.filteredActivities.length/I.pagination.itemsPerPage);i>1?(t.classList.remove("hidden"),document.getElementById("page-indicator").innerText=`${I.pagination.currentPage}/${i}`,document.getElementById("page-prev").disabled=I.pagination.currentPage===1,document.getElementById("page-next").disabled=I.pagination.currentPage>=i):t.classList.add("hidden")}}function va(e,t=!1){const s=Do(e.timestamp||e.createdAt),n=jo(e.timestamp||e.createdAt),a=e.user||e.userAddress||e.from||"",i=Ho(a),o=Uo(e.type,e.details);let r="";const l=(e.type||"").toUpperCase().trim(),d=e.details||{};if(l.includes("GAMEREQUESTED")||l.includes("REQUEST")){const b=d.guesses||e.guesses;b&&Array.isArray(b)&&b.length>0&&(r=`<span class="ml-2 px-2 py-0.5 rounded text-[10px] font-bold" style="background: rgba(249,115,22,0.2); color: #f97316">🎯 ${b.join(" • ")}</span>`);const v=d.amount||d.wagerAmount;if(v){const y=B(BigInt(v)).toFixed(2);r+=`<span class="ml-1 text-[10px] text-zinc-500">(${y} BKC)</span>`}}if(l.includes("FULFILLED")||l.includes("ORACLE")){const b=d.rolls||e.rolls||d.oracleNumbers;b&&Array.isArray(b)&&b.length>0&&(r=`<span class="ml-2 px-2 py-0.5 rounded text-[11px] font-bold" style="background: linear-gradient(135deg, rgba(168,85,247,0.3), rgba(232,121,249,0.3)); color: #e879f9; border: 1px solid rgba(232,121,249,0.4)">🔮 ${b.join(" • ")}</span>`)}if(l.includes("RESULT")||l.includes("GAMERESULT")){const b=d.isWin||d.prizeWon>0,v=d.rolls||e.rolls;v&&Array.isArray(v)&&v.length>0&&(b?r=`<span class="ml-2 px-2 py-0.5 rounded text-[10px] font-bold" style="background: rgba(234,179,8,0.2); color: #facc15">🏆 ${v.join(" • ")}</span>`:r=`<span class="ml-2 text-[9px] text-zinc-500">[${v.join(", ")}]</span>`)}if(l.includes("NOTARY")){const b=d.ipfsCid;b&&(r=`<span class="ml-2 text-[9px] text-indigo-400 font-mono">${b.replace("ipfs://","").slice(0,12)+"..."}</span>`)}if(l.includes("STAKING")||l.includes("DELEGAT")){const b=d.pStakeGenerated;b&&(r=`<span class="ml-2 text-[10px] text-purple-400">+${B(BigInt(b)).toFixed(0)} pStake</span>`)}if(l.includes("CLAIM")||l.includes("REWARD")){const b=d.feePaid;b&&BigInt(b)>0n&&(r=`<span class="ml-2 text-[9px] text-zinc-500">(fee: ${B(BigInt(b)).toFixed(2)})</span>`)}const u=e.txHash?`${So}${e.txHash}`:"#";let p=e.amount||d.amount||d.wagerAmount||d.prizeWon||"0";const f=B(BigInt(p)),g=f>.001?f.toFixed(2):"";return`
        <a href="${u}" target="_blank" class="flex items-center justify-between p-2.5 hover:bg-zinc-800/60 border border-zinc-700/30 rounded-lg transition-all hover:border-zinc-600/50 group" style="background: rgba(39,39,42,0.3)" title="${n}">
            <div class="flex items-center gap-2.5">
                ...
                <div>
                    <p class="text-white text-xs font-medium">${o.label}${r}</p>
    ...
    `}function er(){if(!se.dashboard)return;se.dashboard.addEventListener("click",async t=>{const s=t.target;if(s.closest("#manual-refresh-btn")){const i=s.closest("#manual-refresh-btn");i.disabled=!0,i.innerHTML='<i class="fa-solid fa-rotate fa-spin"></i>',await Ft(!0),await Is(),I.activities=[],I.networkActivities=[],I.networkActivitiesTimestamp=0,I.faucet.lastCheck=0,await Rt(),setTimeout(()=>{i.innerHTML='<i class="fa-solid fa-rotate"></i> <span class="hidden sm:inline">Sync</span>',i.disabled=!1},1e3)}if(s.closest("#faucet-action-btn")){const i=s.closest("#faucet-action-btn");i.disabled||await Bn(i)}if(s.closest("#emergency-faucet-btn")&&await Bn(s.closest("#emergency-faucet-btn")),s.closest(".delegate-link")&&(t.preventDefault(),window.navigateTo("mine")),s.closest(".go-to-store")&&(t.preventDefault(),window.navigateTo("store")),s.closest(".go-to-rental")&&(t.preventDefault(),window.navigateTo("rental")),s.closest(".go-to-fortune")&&(t.preventDefault(),window.navigateTo("actions")),s.closest(".go-to-notary")&&(t.preventDefault(),window.navigateTo("notary")),s.closest("#open-booster-info")){const i=document.getElementById("booster-info-modal");i&&(i.classList.remove("hidden"),i.classList.add("flex"),setTimeout(()=>{i.classList.remove("opacity-0"),i.querySelector("div").classList.remove("scale-95")},10))}if(s.closest("#close-booster-modal")||s.id==="booster-info-modal"){const i=document.getElementById("booster-info-modal");i&&(i.classList.add("opacity-0"),i.querySelector("div").classList.add("scale-95"),setTimeout(()=>i.classList.add("hidden"),200))}if(s.closest("#close-gas-modal-dash")||s.id==="no-gas-modal-dash"){const i=document.getElementById("no-gas-modal-dash");i&&(i.classList.remove("flex"),i.classList.add("hidden"))}const n=s.closest(".nft-clickable-image");if(n){const i=n.dataset.address,o=n.dataset.tokenid;i&&o&&di(i,o)}const a=s.closest("#dashboardClaimBtn");if(a&&!a.disabled)try{if(a.innerHTML='<i class="fa-solid fa-circle-notch fa-spin"></i>',a.disabled=!0,!await Wo()){a.innerHTML='<i class="fa-solid fa-gift mr-2"></i> Claim',a.disabled=!1;return}const{stakingRewards:o,minerRewards:r}=await Jt();(o>0n||r>0n)&&await Xs(o,r,null)&&(m("Rewards claimed!","success"),await Ft(!0),I.activities=[],Rt())}catch{m("Claim failed","error")}finally{a.innerHTML='<i class="fa-solid fa-gift mr-2"></i> Claim',a.disabled=!1}if(s.closest("#page-prev")&&I.pagination.currentPage>1&&(I.pagination.currentPage--,$s()),s.closest("#page-next")){const i=Math.ceil(I.filteredActivities.length/I.pagination.itemsPerPage);I.pagination.currentPage<i&&(I.pagination.currentPage++,$s())}s.closest("#activity-sort-toggle")&&(I.filters.sort=I.filters.sort==="NEWEST"?"OLDEST":"NEWEST",Es())});const e=document.getElementById("activity-filter-type");e&&e.addEventListener("change",t=>{I.filters.type=t.target.value,Es()})}const ha={async render(e){qo(),Is(),Rt(),c.isConnected&&await Ft(!1)},update(e){const t=Date.now();t-I.lastUpdate>1e4&&(I.lastUpdate=t,Is(),e&&Ft(!1),Rt())}},nt=window.ethers,ut="./assets/stake.png",tr="https://sepolia.arbiscan.io/tx/";let We=!1,pt=0,Js=3650,es=0n,be=!1,As=[],_e=0n;function wa(e){if(e<=0)return"Ready";const t=Math.floor(e/86400),s=Math.floor(e%86400/3600),n=Math.floor(e%3600/60);if(t>365){const a=Math.floor(t/365),i=t%365;return`${a}y : ${i}d`}return t>0?`${t}d : ${s}h : ${n}m`:s>0?`${s}h : ${n}m`:`${n}m`}function sr(e){if(e>=365){const t=e/365;return t>=2?`${Math.floor(t)} Years`:`${t.toFixed(1)} Year`}return e>=30?`${Math.floor(e/30)} Month${e>=60?"s":""}`:`${e} Day${e>1?"s":""}`}function ya(e,t){try{const s=BigInt(e),n=BigInt(t);return s*(n/86400n)/10n**18n}catch{return 0n}}function nr(e){if(!e)return"";try{const t=e.seconds||e._seconds||new Date(e).getTime()/1e3;return new Date(t*1e3).toLocaleString("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return""}}function ar(){if(document.getElementById("staking-styles-v4"))return;const e=document.createElement("style");e.id="staking-styles-v4",e.textContent=`
    ...
    `,document.head.appendChild(e)}function ir(){const e=document.getElementById("mine");e&&(ar(),e.innerHTML=`
            ...
            <div class="staking-card rounded-2xl p-4 sm:p-5 mt-4 sm:mt-6">
                ...
                <div id="staking-history-list" class="space-y-2 max-h-[400px] overflow-y-auto">
                    <div class="text-center py-6">
                        <img src="${ut}" class="w-12 h-12 mx-auto opacity-30 animate-pulse mb-2" onerror="this.style.display='none'">
                        <p class="text-zinc-600 text-sm">Loading history...</p>
                    </div>
                </div>
            </div>
        </div>
    `,ur(),c.isConnected?Xe(!0):Qs())}async function Xe(e=!1){var s;if(!c.isConnected){Qs();return}const t=Date.now();if(!(!e&&We)&&!(!e&&t-pt<1e4)){We=!0,pt=t;try{const n=await Ge();es=n!=null&&n.tokenId?BigInt(n.tokenId):0n;try{const a=await fetch("https://getsystemdata-4wvdcuoouq-uc.a.run.app");if(a.ok){const i=await a.json();(s=i==null?void 0:i.economy)!=null&&s.totalPStake&&(_e=BigInt(i.economy.totalPStake),console.log("📊 Network pStake from Firebase:",Me(_e)))}}catch{console.log("Firebase system data not available, using blockchain fallback")}if(_e===0n&&(c.delegationManagerContractPublic||c.delegationManagerContract)){const a=c.delegationManagerContractPublic||c.delegationManagerContract;_e=await O(a,"totalNetworkPStake",[],0n),console.log("📊 Network pStake from blockchain (fallback)")}await Promise.all([V(!0),Ys(!0),qs()]),rr(),Ta(),at(),or()}catch(n){console.error("Staking load error:",n)}finally{We=!1}}}let st="ALL";async function or(){if(c.userAddress)try{const e=oe.getHistory||"https://gethistory-4wvdcuoouq-uc.a.run.app",t=await fetch(`${e}/${c.userAddress}`);t.ok&&(As=(await t.json()||[]).filter(n=>{const a=(n.type||"").toUpperCase();return a.includes("DELEGAT")||a.includes("STAKE")||a.includes("UNDELEGAT")||a.includes("CLAIM")||a.includes("REWARD")||a.includes("FORCE")}),ka())}catch(e){console.error("History load error:",e)}}function ka(){const e=document.getElementById("staking-history-list");if(!e)return;let t=As;if(st!=="ALL"&&(t=As.filter(s=>{const n=(s.type||"").toUpperCase();switch(st){case"STAKE":return(n.includes("DELEGAT")||n.includes("STAKE"))&&!n.includes("UNSTAKE")&&!n.includes("UNDELEGAT")&&!n.includes("FORCE");case"UNSTAKE":return n.includes("UNSTAKE")||n.includes("UNDELEGAT")||n.includes("FORCE");case"CLAIM":return n.includes("CLAIM")||n.includes("REWARD");default:return!0}})),t.length===0){e.innerHTML=`
            <div class="text-center py-8">
                <img src="${ut}" class="w-14 h-14 mx-auto opacity-20 mb-3" onerror="this.style.display='none'">
        ...
        `;return}e.innerHTML=t.slice(0,20).map(s=>{const n=(s.type||"").toUpperCase(),a=s.details||{},i=nr(s.timestamp||s.createdAt);let o,r,l,d,u="";if(n.includes("FORCE")||n.includes("UNDELEGAT")&&a.feePaid&&BigInt(a.feePaid||0)>0n){o="fa-bolt",r="#ef4444",l="rgba(239,68,68,0.15)",d="⚡ Force Unstaked";const v=a.feePaid;v&&BigInt(v)>0n&&(u=`<span class="ml-2 text-[9px] text-red-400">(penalty: ${B(BigInt(v)).toFixed(2)} BKC)</span>`)}else if(n.includes("DELEGAT")||n.includes("STAKE")&&!n.includes("UNSTAKE")){o="fa-lock",r="#4ade80",l="rgba(34,197,94,0.15)",d="🔒 Delegated";const v=a.pStakeGenerated;v&&(u=`<span class="ml-2 text-[10px] text-purple-400 font-bold">+${B(BigInt(v)).toFixed(0)} pStake</span>`);const y=a.lockDuration;if(y){const k=Number(y)/86400;u+=`<span class="ml-1 text-[9px] text-zinc-500">(${sr(k)})</span>`}}else if(n.includes("UNSTAKE")||n.includes("UNDELEGAT")){o="fa-unlock",r="#fb923c",l="rgba(249,115,22,0.15)",d="🔓 Unstaked";const v=a.amountReceived;v&&BigInt(v)>0n&&(u=`<span class="ml-2 text-[9px] text-green-400">+${B(BigInt(v)).toFixed(2)} BKC</span>`)}else if(n.includes("CLAIM")||n.includes("REWARD")){o="fa-coins",r="#fbbf24",l="rgba(245,158,11,0.15)",d="🪙 Rewards Claimed";const v=a.amountReceived,y=a.feePaid;if(v&&BigInt(v)>0n&&(u=`<span class="ml-2 text-[9px] text-green-400">+${B(BigInt(v)).toFixed(2)} BKC</span>`),y&&BigInt(y)>0n){const k=B(BigInt(y)).toFixed(2);u+=`<span class="ml-1 text-[9px] text-zinc-500">(fee: ${k})</span>`}}else o="fa-circle",r="#71717a",l="rgba(39,39,42,0.5)",d=s.type||"Activity";const p=s.txHash?`${tr}${s.txHash}`:"#";let f=s.amount||a.amount||a.amountReceived||"0";const g=B(BigInt(f)),b=g>.001?g.toFixed(2):"";return`
            <a href="${p}" target="_blank" class="history-item flex items-center justify-between p-3 hover:bg-zinc-800/60 border border-zinc-700/30 rounded-lg transition-all group bg-zinc-800/20" title="${i}">
                ...
                <div class="flex items-center gap-2">
                    ${b?`<span class="text-xs font-mono font-bold text-white">${b} <span class="text-zinc-500">BKC</span></span>`:""}
                    <i class="fa-solid fa-arrow-up-right-from-square text-zinc-600 group-hover:text-blue-400 text-[9px]"></i>
                </div>
            </a>
        `}).join("")}function rr(){var l;const e=(d,u)=>{const p=document.getElementById(d);p&&(p.textContent=u)};e("stat-network",Me(_e||c.totalNetworkPStake||0n)),e("stat-pstake",Me(c.userTotalPStake||0n)),e("balance-display",B(c.currentUserBalance||0n).toFixed(2));const t=c.userTotalPStake||0n,s=_e||c.totalNetworkPStake||0n;let n=0;s>0n&&t>0n&&(n=Number(t*10000n/s)/100);const a=document.getElementById("stat-pstake-percent");a&&(a.textContent=n>0?`${n.toFixed(2)}% of network`:"0% of network");const i=((l=c.systemFees)==null?void 0:l.DELEGATION_FEE_BIPS)||50n,o=Number(i)/100,r=document.getElementById("fee-info");r&&(r.textContent=`${o}% fee`),Jt().then(({stakingRewards:d,minerRewards:u})=>{const p=d+u;e("stat-rewards",B(p).toFixed(4));const f=document.getElementById("claim-btn");f&&(f.disabled=p<=0n,p>0n&&(f.onclick=()=>dr(d,u,f)))})}function Qs(){const e=(a,i)=>{const o=document.getElementById(a);o&&(o.textContent=i)};e("stat-network","--"),e("stat-pstake","--"),e("stat-rewards","--"),e("balance-display","0.00"),e("delegation-count","0");const t=document.getElementById("stat-pstake-percent");t&&(t.textContent="--% of network");const s=document.getElementById("delegations-list");s&&(s.innerHTML=`
        ...
        `)}let Pe=null;function Ta(){const e=document.getElementById("delegations-list");if(!e)return;Pe&&(clearInterval(Pe),Pe=null);const t=c.userDelegations||[],s=document.getElementById("delegation-count");if(s&&(s.textContent=`${t.length} active`),t.length===0){e.innerHTML=`
            <div class="text-center py-10">
                <img src="${ut}" class="w-14 h-14 mx-auto opacity-20 mb-3" onerror="this.outerHTML='<div class=\\'w-14 h-14 bg-zinc-800 rounded-full flex items-center justify-center mx-auto mb-3\\'><i class=\\'fa-solid fa-layer-group text-xl text-zinc-600\\'></i></div>'">
                <p class="text-zinc-500 text-sm mb-1">No active delegations</p>
                <p class="text-zinc-600 text-xs">Delegate BKC to start earning</p>
            </div>
        `;return}const n=[...t].sort((a,i)=>Number(a.unlockTime)-Number(i.unlockTime));e.innerHTML=n.map(a=>lr(a)).join(""),$n(),Pe=setInterval($n,6e4),e.querySelectorAll(".unstake-btn").forEach(a=>{a.addEventListener("click",()=>An(a.dataset.index,!1))}),e.querySelectorAll(".force-unstake-btn").forEach(a=>{a.addEventListener("click",()=>{confirm(`⚠️ Force Unstake will apply a 50% penalty!

Are you sure you want to continue?`)&&An(a.dataset.index,!0)})})}function $n(){const e=document.querySelectorAll(".countdown-timer"),t=Math.floor(Date.now()/1e3);e.forEach(s=>{const a=parseInt(s.dataset.unlockTime)-t;s.textContent=wa(a)})}function lr(e){const t=B(e.amount).toFixed(2),s=Me(ya(e.amount,e.lockDuration)),n=Number(e.unlockTime),a=Math.floor(Date.now()/1e3),i=n>a,o=i?n-a:0;return`
    ...
    `}function at(){var n;const e=document.getElementById("amount-input"),t=document.getElementById("stake-btn");if(!e)return;const s=e.value;if(!s||parseFloat(s)<=0){document.getElementById("preview-pstake").textContent="0",document.getElementById("preview-net").textContent="0.00 BKC",t&&(t.disabled=!0);return}try{const a=nt.parseUnits(s,18),i=((n=c.systemFees)==null?void 0:n.DELEGATION_FEE_BIPS)||50n,o=a*BigInt(i)/10000n,r=a-o,l=BigInt(Js)*86400n,d=ya(r,l);document.getElementById("preview-pstake").textContent=Me(d),document.getElementById("preview-net").textContent=`${B(r).toFixed(4)} BKC`;const u=c.currentUserBalance||0n;a>u?(e.classList.add("border-red-500"),t&&(t.disabled=!0)):(e.classList.remove("border-red-500"),t&&(t.disabled=be))}catch{t&&(t.disabled=!0)}}async function cr(){if(be)return;const e=document.getElementById("amount-input"),t=document.getElementById("stake-btn"),s=document.getElementById("stake-btn-text"),n=document.getElementById("stake-btn-icon"),a=document.getElementById("stake-mascot");if(!e||!t)return;const i=e.value;if(!i||parseFloat(i)<=0){m("Enter an amount","warning");return}const o=c.currentUserBalance||0n;let r;try{if(r=nt.parseUnits(i,18),r>o){m("Insufficient BKC balance","error");return}}catch{m("Invalid amount","error");return}try{const u=await new nt.BrowserProvider(window.ethereum).getBalance(c.userAddress),p=nt.parseEther("0.001");if(u<p){m("Insufficient ETH for gas. Need at least 0.001 ETH.","error");return}}catch(d){console.warn("ETH balance check failed:",d)}be=!0;const l=BigInt(Js)*86400n;t.disabled=!0,s.textContent="Processing...",n.className="fa-solid fa-spinner fa-spin",a&&(a.className="w-14 h-14 object-contain stake-rotate stake-glow");try{await vo(r,l,es,null)&&(e.value="",m("🔒 Delegation successful!","success"),a&&(a.className="w-14 h-14 object-contain stake-success",setTimeout(()=>{a.className="w-14 h-14 object-contain stake-pulse stake-float"},800)),We=!1,pt=0,await Xe(!0))}catch(d){console.error("Stake error:",d),m("Delegation failed: "+(d.reason||d.message||"Unknown error"),"error")}finally{be=!1,t.disabled=!1,s.textContent="Delegate BKC",n.className="fa-solid fa-lock",a&&(a.className="w-14 h-14 object-contain stake-pulse stake-float"),at()}}async function An(e,t){if(be)return;const s=document.querySelector(t?`.force-unstake-btn[data-index='${e}']`:`.unstake-btn[data-index='${e}']`);s&&(s.disabled=!0,s.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>'),be=!0;try{const n=BigInt(e),a=es||0n;console.log(`Attempting ${t?"force ":""}unstake:`,{delegationIndex:n.toString(),boosterId:a.toString()}),(t?await wo(n,a):await ho(n,a))&&(m(t?"⚡ Force unstaked (50% penalty applied)":"🔓 Unstaked successfully!",t?"warning":"success"),We=!1,pt=0,await Xe(!0))}catch(n){console.error("Unstake error:",n),m("Unstake failed: "+(n.reason||n.message||"Unknown error"),"error")}finally{be=!1,Ta()}}async function dr(e,t,s){if(be)return;be=!0;const n=s.textContent;s.disabled=!0,s.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>';try{await Xs(e,t,es,null)&&(m("🪙 Rewards claimed!","success"),We=!1,pt=0,await Xe(!0))}catch(a){console.error("Claim error:",a),m("Claim failed: "+(a.reason||a.message||"Unknown error"),"error")}finally{be=!1,s.disabled=!1,s.textContent=n}}function ur(){const e=document.getElementById("amount-input"),t=document.getElementById("max-btn"),s=document.getElementById("stake-btn"),n=document.getElementById("refresh-btn"),a=document.querySelectorAll(".duration-chip"),i=document.querySelectorAll(".history-tab");e==null||e.addEventListener("input",at),t==null||t.addEventListener("click",()=>{const o=c.currentUserBalance||0n;e&&(e.value=nt.formatUnits(o,18),at())}),a.forEach(o=>{o.addEventListener("click",()=>{a.forEach(r=>r.classList.remove("selected")),o.classList.add("selected"),Js=parseInt(o.dataset.days),at()})}),i.forEach(o=>{o.addEventListener("click",()=>{i.forEach(r=>{r.classList.remove("active"),r.classList.add("text-zinc-400")}),o.classList.add("active"),o.classList.remove("text-zinc-400"),st=o.dataset.filter,ka()})}),s==null||s.addEventListener("click",cr),n==null||n.addEventListener("click",()=>{const o=n.querySelector("i");o==null||o.classList.add("fa-spin"),Xe(!0).then(()=>{setTimeout(()=>o==null?void 0:o.classList.remove("fa-spin"),500)})})}function pr(){Pe&&(clearInterval(Pe),Pe=null)}function fr(e){e?Xe():Qs()}const za={render:ir,update:fr,cleanup:pr},Ee=window.ethers,mr="https://sepolia.arbiscan.io/tx/",Ln={Crystal:{color:"#a855f7",gradient:"from-purple-500/20 to-violet-600/20",border:"border-purple-500/40",text:"text-purple-400",glow:"shadow-purple-500/30",icon:"💎"},Diamond:{color:"#22d3ee",gradient:"from-cyan-500/20 to-blue-500/20",border:"border-cyan-500/40",text:"text-cyan-400",glow:"shadow-cyan-500/30",icon:"💠"},Platinum:{color:"#e2e8f0",gradient:"from-slate-300/20 to-gray-400/20",border:"border-slate-400/40",text:"text-slate-300",glow:"shadow-slate-400/30",icon:"⚪"},Gold:{color:"#fbbf24",gradient:"from-yellow-500/20 to-amber-500/20",border:"border-yellow-500/40",text:"text-yellow-400",glow:"shadow-yellow-500/30",icon:"🥇"},Silver:{color:"#9ca3af",gradient:"from-gray-400/20 to-slate-400/20",border:"border-gray-400/40",text:"text-gray-300",glow:"shadow-gray-400/30",icon:"🥈"},Bronze:{color:"#f97316",gradient:"from-orange-600/20 to-amber-700/20",border:"border-orange-600/40",text:"text-orange-400",glow:"shadow-orange-500/30",icon:"🥉"},Iron:{color:"#6b7280",gradient:"from-gray-500/20 to-zinc-600/20",border:"border-gray-500/40",text:"text-gray-400",glow:"shadow-gray-500/30",icon:"⚙️"}};function ts(e){return Ln[e]||Ln.Iron}const C={tradeDirection:"buy",selectedPoolBoostBips:null,buyPrice:0n,sellPrice:0n,netSellPrice:0n,poolNFTCount:0,userBalanceOfSelectedNFT:0,availableToSellCount:0,firstAvailableTokenId:null,firstAvailableTokenIdForBuy:null,bestBoosterTokenId:0n,bestBoosterBips:0,isDataLoading:!1,lastFetchTimestamp:0,tradeHistory:[]},Ca=new Map;let us=!1;const br=["function getPoolAddress(uint256 boostBips) view returns (address)","function isPool(address) view returns (bool)"];function Dt(e){return e?e.startsWith("https://")||e.startsWith("http://")?e:e.includes("ipfs.io/ipfs/")?`${Y}${e.split("ipfs.io/ipfs/")[1]}`:e.startsWith("ipfs://")?`${Y}${e.substring(7)}`:e:"./assets/bkc_logo_3d.png"}function gr(e){if(!e)return"";try{const t=e.seconds||e._seconds||new Date(e).getTime()/1e3;return new Date(t*1e3).toLocaleString("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return""}}function xr(){if(document.getElementById("swap-styles-v9"))return;const e=document.createElement("style");e.id="swap-styles-v9",e.textContent=`
    ...
    `,document.head.appendChild(e)}function vr(){return`
    ...
    `}const hr={async render(e){xr(),await Qn();const t=document.getElementById("store");t&&((t.innerHTML.trim()===""||e)&&(t.innerHTML=`
            ...
            `,zr()),C.selectedPoolBoostBips===null&&Z.length>0&&(C.selectedPoolBoostBips=Z[0].boostBips),await Se(),await Lt())},async update(){C.selectedPoolBoostBips!==null&&!C.isDataLoading&&document.getElementById("store")&&!document.hidden&&await Se()}};async function Lt(){const e=document.getElementById("history-list");if(!c.userAddress){e&&(e.innerHTML='<div class="text-center py-4 text-xs text-zinc-600">Connect wallet to view history</div>');return}try{const t=oe.getHistory||"https://gethistory-4wvdcuoouq-uc.a.run.app",s=await fetch(`${t}/${c.userAddress}`);if(!s.ok)throw new Error(`HTTP ${s.status}`);const n=await s.json();console.log("All history types:",[...new Set((n||[]).map(i=>i.type))]),C.tradeHistory=(n||[]).filter(i=>{const o=(i.type||"").toUpperCase();return o==="NFTBOUGHT"||o==="NFTSOLD"||o==="NFT_BOUGHT"||o==="NFT_SOLD"||o==="NFTPURCHASED"||o==="NFT_PURCHASED"||o.includes("NFTBOUGHT")||o.includes("NFTSOLD")||o.includes("NFTPURCHASED")}),console.log("NFT trade history:",C.tradeHistory.length,"items");const a=document.getElementById("history-count");a&&(a.textContent=C.tradeHistory.length),Sn()}catch(t){console.error("History load error:",t),C.tradeHistory=[],Sn()}}function Sn(){const e=document.getElementById("history-list");if(e){if(!c.isConnected){e.innerHTML='<div class="text-center py-4 text-xs text-zinc-600">Connect wallet to view history</div>';return}if(C.tradeHistory.length===0){e.innerHTML=`
            <div class="text-center py-6">
        ...
        `;return}e.innerHTML=C.tradeHistory.slice(0,20).map(t=>{const s=(t.type||"").toUpperCase(),n=t.details||{},a=gr(t.timestamp||t.createdAt),i=s.includes("BOUGHT")||s.includes("PURCHASED"),o=i?"fa-cart-plus":"fa-money-bill-transfer",r=i?"#22c55e":"#f59e0b",l=i?"rgba(34,197,94,0.15)":"rgba(245,158,11,0.15)",d=i?"🛒 Bought NFT":"💰 Sold NFT",u=i?"-":"+",p=t.txHash?`${mr}${t.txHash}`:"#";let f="";try{let v=t.amount||n.amount||n.price||n.payout||"0";if(typeof v=="string"&&v!=="0"){const y=B(BigInt(v));y>.001&&(f=y.toFixed(2))}}catch{}const g=n.tokenId||"",b=n.boostBips||n.boost||"";return`
        ...
        `}).join("")}}function wr(){return Z.map((e,t)=>{const s=ts(e.name);return`
            <button class="tier-chip flex flex-col items-center gap-1 p-2 rounded-xl border transition-all
                ...
                data-tier="${e.name}">
                <img src="${Dt(e.img)}" class="w-8 h-8 rounded-lg" onerror="this.src='./assets/bkc_logo_3d.png'">
        ...
        <div class="fade-in">
            ...
            <div class="swap-input-box rounded-2xl p-4 mb-1">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="token-selector flex items-center gap-2 px-3 py-2 rounded-xl cursor-default">
                        <img src="${s?"./assets/bkc_logo_3d.png":Dt(t==null?void 0:t.img)}" class="w-6 h-6 rounded" onerror="this.src='./assets/bkc_logo_3d.png'">
            ...
            <div class="swap-input-box rounded-2xl p-4 mt-1 mb-4">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="token-selector flex items-center gap-2 px-3 py-2 rounded-xl cursor-default">
                        <img src="${s?Dt(t==null?void 0:t.img):"./assets/bkc_logo_3d.png"}" class="w-6 h-6 rounded" onerror="this.src='./assets/bkc_logo_3d.png'">
            ...
            <div class="flex justify-between items-center text-[10px] text-zinc-600 mb-4 px-1">
                ...
                <span>Fee Discount: ${C.bestBoosterBips>0?`${((p=c.boosterDiscounts)==null?void 0:p[C.bestBoosterBips])||0}%`:"None"}</span>
            </div>
            
            <!-- Execute Button -->
            ${yr(s,o,r,d,l)}
        </div>
    `}function yr(e,t,s,n,a=!1){return c.isConnected?e?t?`
        ...
        `}function kr(e){const t=document.getElementById("trade-mascot");t&&(t.className=`w-14 h-14 object-contain ${e?"trade-buy":"trade-sell"}`)}function Tr(){const e=document.getElementById("inventory-grid"),t=document.getElementById("nft-count");if(!e)return;const s=c.myBoosters||[];if(t&&(t.textContent=s.length),!c.isConnected){e.innerHTML='<div class="col-span-4 text-center py-4 text-xs text-zinc-600">Connect wallet</div>';return}if(s.length===0){e.innerHTML=`
        ...
        `;return}const n=c.rentalListings||[],a=new Set(n.map(o=>{var r;return(r=o.tokenId)==null?void 0:r.toString()})),i=Math.floor(Date.now()/1e3);e.innerHTML=s.map(o=>{var k;const r=Z.find(E=>E.boostBips===Number(o.boostBips)),l=ts(r==null?void 0:r.name),d=Dt(o.image||(r==null?void 0:r.img)),u=C.firstAvailableTokenId&&BigInt(o.tokenId)===C.firstAvailableTokenId,p=(k=o.tokenId)==null?void 0:k.toString(),f=a.has(p),g=n.find(E=>{var L;return((L=E.tokenId)==null?void 0:L.toString())===p}),b=g&&g.rentalEndTime&&Number(g.rentalEndTime)>i,v=f||b;let y="";return b?y='<span class="absolute top-1 right-1 bg-blue-500 text-white text-[7px] px-1.5 py-0.5 rounded-full font-bold">🔑 RENTED</span>':f&&(y='<span class="absolute top-1 right-1 bg-green-500 text-white text-[7px] px-1.5 py-0.5 rounded-full font-bold">📋 LISTED</span>'),`
            <div class="inventory-item ${v?"opacity-50 cursor-not-allowed":"cursor-pointer"} rounded-xl p-2 border ${u&&!v?"border-amber-500 ring-2 ring-amber-500/50 bg-amber-500/10":"border-zinc-700/50 bg-zinc-800/30"} hover:bg-zinc-800/50 transition-all relative"
                 data-boost="${o.boostBips}" 
                 data-tokenid="${o.tokenId}"
                 data-unavailable="${v}">
                ${y}
                <img src="${d}" class="w-full aspect-square rounded-lg object-cover ${v?"grayscale":""}" onerror="this.src='./assets/bkc_logo_3d.png'">
                <p class="text-[9px] text-center mt-1 ${l.text} truncate">${(r==null?void 0:r.name)||"NFT"}</p>
                <p class="text-[8px] text-center ${u&&!v?"text-amber-400 font-bold":"text-zinc-600"}">#${o.tokenId}</p>
            </div>
        `}).join("")}async function Se(){var s,n;if(C.isDataLoading||C.selectedPoolBoostBips===null)return;const e=Date.now();if(e-C.lastFetchTimestamp<2e3)return;C.isDataLoading=!0,C.lastFetchTimestamp=e;const t=C.selectedPoolBoostBips;try{const a=await Ge();C.bestBoosterTokenId=a!=null&&a.tokenId?BigInt(a.tokenId):0n,C.bestBoosterBips=(a==null?void 0:a.boostBips)||0,await Promise.all([Qt(!0),gt()]);const i=c.rentalListings||[],o=new Set(i.map(S=>{var X;return(X=S.tokenId)==null?void 0:X.toString()})),r=Math.floor(Date.now()/1e3),d=(c.myBoosters||[]).filter(S=>Number(S.boostBips)===t),u=d.filter(S=>{var zt;const X=(zt=S.tokenId)==null?void 0:zt.toString(),le=i.find(as=>{var ln;return((ln=as.tokenId)==null?void 0:ln.toString())===X}),K=o.has(X),Tt=le&&le.rentalEndTime&&Number(le.rentalEndTime)>r;return!K&&!Tt});C.userBalanceOfSelectedNFT=d.length,C.availableToSellCount=u.length;const p=C.firstAvailableTokenId;p&&u.some(S=>BigInt(S.tokenId)===p)||(C.firstAvailableTokenId=u.length>0?BigInt(u[0].tokenId):null);const g=Z.find(S=>S.boostBips===t);if(!g){console.warn("Tier not found for boostBips:",t);return}const b=`pool_${g.name.toLowerCase()}`;let v=x[b];if(!v){const S=x.nftLiquidityPoolFactory;if(S&&c.publicProvider)try{v=await new Ee.Contract(S,br,c.publicProvider).getPoolAddress(t),v&&v!==Ee.ZeroAddress&&Ca.set(t,v)}catch(X){console.warn("Factory lookup failed:",X.message)}}if(!v||v===Ee.ZeroAddress){const S=document.getElementById("swap-interface");S&&(S.innerHTML=`
                ...
                `);return}const y=new Ee.Contract(v,Hs,c.publicProvider);let k=Ee.MaxUint256,E=0n,L=[],A=((s=c.systemFees)==null?void 0:s.NFT_POOL_SELL_TAX_BIPS)||1000n,$=BigInt(((n=c.boosterDiscounts)==null?void 0:n[C.bestBoosterBips])||0);try{k=await O(y,"getBuyPrice",[],Ee.MaxUint256)}catch(S){console.warn("getBuyPrice failed:",S.message)}try{E=await O(y,"getSellPrice",[],0n)}catch(S){console.warn("getSellPrice failed:",S.message)}try{const S=await y.getAvailableNFTs();L=Array.isArray(S)?[...S]:[]}catch(S){console.warn("getAvailableNFTs failed:",S.message),L=[]}C.poolNFTCount=L.length,C.firstAvailableTokenIdForBuy=L.length>0?BigInt(L[L.length-1]):null,C.buyPrice=k===Ee.MaxUint256?0n:k,C.sellPrice=E;const T=typeof A=="bigint"?A:BigInt(A),F=typeof $=="bigint"?$:BigInt($),q=T>F?T-F:0n,re=E*q/10000n;C.netSellPrice=E-re}catch(a){console.warn("Store Data Warning:",a.message);const i=document.getElementById("swap-interface");i&&(i.innerHTML=`
                <div class="text-center py-12">
                    <div class="w-12 h-12 bg-zinc-800 rounded-full flex items-center justify-center mx-auto mb-3">
            ...
            `);return}finally{C.isDataLoading=!1,Ba(),Tr()}}function zr(){const e=document.getElementById("store");e&&e.addEventListener("click",async t=>{if(t.target.closest("#refresh-btn")){const o=t.target.closest("#refresh-btn").querySelector("i");o.classList.add("fa-spin"),await Se(),Lt(),o.classList.remove("fa-spin");return}const s=t.target.closest(".tier-chip");if(s){const i=Number(s.dataset.boost);C.selectedPoolBoostBips!==i&&(C.selectedPoolBoostBips=i,Pn(i),await Se());return}if(t.target.closest("#swap-direction-btn")){C.tradeDirection=C.tradeDirection==="buy"?"sell":"buy",Ba();return}if(t.target.closest("#inventory-toggle")){const i=document.getElementById("inventory-panel"),o=document.getElementById("inventory-chevron");i&&o&&(i.classList.toggle("hidden"),o.style.transform=i.classList.contains("hidden")?"":"rotate(180deg)");return}if(t.target.closest("#history-toggle")){const i=document.getElementById("history-panel"),o=document.getElementById("history-chevron");i&&o&&(i.classList.toggle("hidden"),o.style.transform=i.classList.contains("hidden")?"":"rotate(180deg)");return}const n=t.target.closest(".inventory-item");if(n){if(n.dataset.unavailable==="true"){m("This NFT is listed for rental and cannot be sold","warning");return}const o=Number(n.dataset.boost),r=n.dataset.tokenid;C.selectedPoolBoostBips=o,C.tradeDirection="sell",r&&(C.firstAvailableTokenId=BigInt(r),console.log("User selected NFT #"+r+" for sale")),Pn(o),await Se();return}const a=t.target.closest("#execute-btn");if(a){if(t.preventDefault(),t.stopPropagation(),us||a.disabled)return;const i=a.dataset.action,o=document.getElementById("trade-mascot");if(i==="connect"){window.openConnectModal();return}const r=Z.find(p=>p.boostBips===C.selectedPoolBoostBips);if(!r)return;const l=`pool_${r.name.toLowerCase()}`,d=x[l]||Ca.get(r.boostBips);if(!d){m("Pool address not found","error");return}us=!0,a.disabled=!0;const u=a.innerHTML;a.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Processing...',o&&(o.className="w-14 h-14 object-contain trade-spin");try{C.tradeDirection==="buy"?await yo(d,C.buyPrice,a)&&(o&&(o.className="w-14 h-14 object-contain trade-success"),m("🟢 NFT Purchased!","success"),await Se(),Lt()):await ko(d,C.firstAvailableTokenId,a)&&(o&&(o.className="w-14 h-14 object-contain trade-success"),m("🔴 NFT Sold!","success"),await Se(),Lt())}finally{us=!1,a.disabled=!1,a.innerHTML=u,o&&setTimeout(()=>{const p=C.tradeDirection==="buy";o.className=`w-14 h-14 object-contain ${p?"trade-buy":"trade-sell"}`},800)}}})}const ps=window.ethers,Ia="./assets/reward.png",Cr="https://sepolia.arbiscan.io/tx/";let Ls=0,fs=!1,St=!1,Ye=[],Pt={stakingRewards:0n,minerRewards:0n,boosterTokenId:0n};window.handleRewardsClaim=async function(){St||await Sr(Pt.stakingRewards,Pt.minerRewards,Pt.boosterTokenId)};function Br(){if(document.getElementById("reward-styles-v10"))return;const e=document.createElement("style");e.id="reward-styles-v10",e.textContent=`
    ...
    `,document.head.appendChild(e)}const Zs={async render(e){Br();const t=document.getElementById("rewards");t&&((t.innerHTML.trim()===""||e)&&(t.innerHTML=Ar()),c.isConnected?(Lr(),this.update(e)):Mn())},async update(e=!1){if(!c.isConnected){Mn();return}const t=Date.now();if(!(!e&&fs)&&!(!e&&t-Ls<6e4)){fs=!0,it("loading");try{let s={highestBoost:0,boostName:"None",tokenId:null,source:"none"},n={netClaimAmount:0n,feeAmount:0n,totalRewards:0n},a={stakingRewards:0n,minerRewards:0n};try{await V()}catch{}try{s=await Ge()||s}catch{}try{n=await ea()||n}catch{}try{a=await Jt()||a}catch{}try{await Ir()}catch{}Ea(n,a,s),Ls=t,it("idle")}catch(s){console.error("Rewards Error:",s)}finally{fs=!1}}}};function it(e){const t=document.getElementById("reward-mascot");if(t)switch(t.className="w-12 h-12 object-contain",e){case"loading":t.classList.add("reward-spin");break;case"claiming":t.classList.add("reward-bounce");break;case"success":t.classList.add("reward-success");break;default:t.classList.add("reward-float","reward-pulse")}}async function Ir(){if(c.userAddress)try{const e=await fetch(`${oe.getHistory}/${c.userAddress}`);e.ok&&(Ye=(await e.json()).filter(s=>{const n=(s.type||"").toUpperCase();return n.includes("REWARD")||n.includes("CLAIM")}).slice(0,15))}catch{Ye=[]}}function Er(){return Ye.length===0?`
            <div class="text-center py-6">
                <div class="w-12 h-12 mx-auto rounded-full bg-zinc-800/50 flex items-center justify-center mb-2">
        ...
        `:Ye.map(e=>{const t=$r(e.timestamp||e.createdAt),s=e.details||{},n=e.txHash||"",a=n?`${Cr}${n}`:"#";let i="0",o="0";s.amountReceived?i=s.amountReceived:e.amount?i=e.amount:s.amount&&(i=s.amount),s.feePaid?o=s.feePaid:s.feeAmount&&(o=s.feeAmount);const r=Nn(i),l=Nn(o);let d,u,p,f;const g=(e.type||"").toUpperCase();return g.includes("STAKING")||g.includes("DELEGAT")?(d="fa-lock",u="#a855f7",p="rgba(168,85,247,0.15)",f="🔒 Staking Reward"):g.includes("MINING")||g.includes("MINER")?(d="fa-hammer",u="#f97316",p="rgba(249,115,22,0.15)",f="⛏️ Mining Reward"):g.includes("CLAIM")||g.includes("REWARD")?(d="fa-gift",u="#22c55e",p="rgba(34,197,94,0.15)",f="🎁 Claimed"):(d="fa-coins",u="#eab308",p="rgba(234,179,8,0.15)",f="💰 Reward"),`
            <a href="${a}" target="_blank" rel="noopener" 
               class="history-item flex items-center justify-between p-3 bg-zinc-800/30 hover:bg-zinc-800/50 rounded-xl transition-all group border border-zinc-700/30">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl flex items-center justify-center" style="background: ${p}">
                        <i class="fa-solid ${d} text-sm" style="color: ${u}"></i>
                    </div>
                    <div>
                        <p class="text-sm text-white font-medium">${f}</p>
        ...
        `}).join("")}function $r(e){if(!e)return"Unknown date";try{let t;if(e.seconds||e._seconds){const a=e.seconds||e._seconds;t=new Date(a*1e3)}else if(typeof e=="string")t=new Date(e);else if(typeof e=="number")t=new Date(e<1e10?e*1e3:e);else if(e instanceof Date)t=e;else return"Unknown date";if(isNaN(t.getTime()))return"Unknown date";const n=new Date-t;return n<6e4?"Just now":n<36e5?`${Math.floor(n/6e4)}m ago`:n<864e5?`${Math.floor(n/36e5)}h ago`:n<6048e5?`${Math.floor(n/864e5)}d ago`:t.toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}catch(t){return console.warn("Date parse error:",t),"Unknown date"}}function Nn(e){if(!e)return"0.00";try{if(typeof e=="string"){if(e.length>10&&!e.includes(".")){const t=ps.formatEther(BigInt(e));return parseFloat(t).toFixed(4)}return parseFloat(e).toFixed(4)}if(typeof e=="bigint"){const t=ps.formatEther(e);return parseFloat(t).toFixed(4)}if(typeof e=="number"&&e>1e10){const t=ps.formatEther(BigInt(Math.floor(e)));return parseFloat(t).toFixed(4)}return parseFloat(e).toFixed(4)}catch(t){return console.warn("Amount format error:",t),"0.00"}}function Ar(){return`
    ...
    `}function Mn(){const e=document.getElementById("rewards-content");e&&(e.innerHTML=`
        <div class="flex flex-col items-center justify-center py-12 bg-zinc-900/50 border border-zinc-800 rounded-2xl">
            <img src="${Ia}" class="w-16 h-16 opacity-30 mb-4" onerror="this.style.display='none'">
    ...
    `)}function Lr(){Ea({netClaimAmount:0n,feeAmount:0n,totalRewards:0n},{stakingRewards:0n,minerRewards:0n},{highestBoost:0,boostName:"None",tokenId:null,source:"none"})}function Ea(e,t,s){const n=document.getElementById("rewards-content");if(!n)return;const a=e||{},i=t||{},o=s||{},r=a.netClaimAmount||0n,l=a.totalRewards||0n,d=a.feeAmount||0n,u=i.stakingRewards||0n,p=i.minerRewards||0n,f=o.highestBoost||0,g=a.baseFeeBips||5e3,b=f/100,y=100-(a.finalFeeBips||g-g*f/1e4)/100,k=r>0n,E=f>0,L=BigInt(o.tokenId||0);Pt={stakingRewards:u,minerRewards:p,boosterTokenId:L};let A=0,$=0,T=0,F=0,q=0;try{A=B?B(r):Number(r)/1e18,$=B?B(l):Number(l)/1e18,T=B?B(d):Number(d)/1e18,F=B?B(u):Number(u)/1e18,q=B?B(p):Number(p)/1e18}catch{}const re=2*Math.PI*45,S=re-y/100*re;n.innerHTML=`
        <div class="space-y-4">
            <!-- MAIN CLAIM CARD -->
                ...
                <div class="flex flex-col items-center mb-5">
                    <div class="relative w-32 h-32 mb-3">
                        <svg class="w-full h-full" style="transform: rotate(-90deg)" viewBox="0 0 100 100">
                            ...
                            <circle cx="50" cy="50" r="45" fill="none" stroke="${E?"#4ade80":"#f59e0b"}" stroke-width="6" stroke-linecap="round" stroke-dasharray="${re}" stroke-dashoffset="${S}"/>
                        </svg>
                    ...
                    <p class="text-xs text-zinc-500">You keep <span class="${E?"text-green-400":"text-amber-400"} font-bold">${y.toFixed(1)}%</span> of earnings</p>
                </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="bg-zinc-900/50 border border-zinc-800 rounded-xl p-3">
                    ...
                    <p class="text-lg font-bold text-white font-mono">${$.toFixed(2)}</p>
                ...
                <div class="bg-zinc-900/50 border border-zinc-800 rounded-xl p-3">
                    ...
                    <p class="text-[10px] text-zinc-600">${(100-y).toFixed(1)}% fee</p>
                </div>
            </div>
            ...
            <div class="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4">
                ...
                <div class="space-y-2">
                    <div class="flex items-center justify-between p-2.5 bg-zinc-800/30 rounded-lg">
                        ...
                        <span class="text-sm font-mono font-bold text-white">${F.toFixed(2)} <span class="text-zinc-500 text-xs">BKC</span></span>
                    </div>
                    <div class="flex items-center justify-between p-2.5 bg-zinc-800/30 rounded-lg">
                        <div class="flex items-center gap-2.5">
                ...
                <div class="p-4">
                    ${E?`
                        <div class="flex items-center gap-3">
                            ...
                            <div class="text-right"><p class="text-xl font-bold text-green-400">+${b}%</p><p class="text-[10px] text-zinc-500">Discount</p></div>
                        </div>
                    `:`
                        <div class="text-center">
    ...
    `}async function Sr(e,t,s){if(St)return;const n=document.getElementById("claim-btn"),a=document.getElementById("claim-btn-text"),i=document.getElementById("claim-btn-icon");if(n){St=!0,n.disabled=!0,n.className="w-full py-4 rounded-xl font-bold text-base flex items-center justify-center gap-2 bg-zinc-700 text-zinc-400",a.textContent="Processing...",i.className="fa-solid fa-spinner fa-spin",it("claiming");try{await Xs(e,t,s,null)&&(it("success"),n.className="w-full py-4 rounded-xl font-bold text-base flex items-center justify-center gap-2 bg-green-500 text-white",a.textContent="🎉 Claimed!",i.className="fa-solid fa-check",m("🎁 Rewards claimed successfully!","success"),setTimeout(()=>{Ls=0,Ye=[],Zs.update(!0)},2500))}catch(o){console.error("Claim error:",o),m("Claim failed: "+(o.reason||o.message||"Error"),"error"),n.disabled=!1,n.className="w-full py-4 rounded-xl font-bold text-base flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500 to-orange-500 text-black shadow-lg shadow-amber-500/25",a.textContent="Claim Rewards",i.className="fa-solid fa-coins",it("idle")}finally{St=!1}}}window.RewardsPage=Zs;const Pr="https://sepolia.arbiscan.io/tx/",$a="https://sepolia.arbiscan.io/address/",Aa="0x16346f5a45f9615f1c894414989f0891c54ef07b",jt="./assets/fortune.png",Ss=1e3,Fn={pt:{title:"Compartilhe & Ganhe!",subtitle:"+1000 pontos para o Airdrop",later:"Talvez depois"},en:{title:"Share & Earn!",subtitle:"+1000 points for Airdrop",later:"Maybe later"},es:{title:"¡Comparte y Gana!",subtitle:"+1000 puntos para el Airdrop",later:"Quizás después"}},Nr={pt:{win:e=>`🎉 Ganhei ${e.toLocaleString()} BKC no Fortune Pool!

🐯 Loteria on-chain com resultados instantâneos!

👉 https://backcoin.org

@bacaborr #Backcoin #Web3 #Arbitrum`,lose:`🐯 Jogando Fortune Pool no @bacaborr!
...
@bacaborr #Backcoin #Web3 #Arbitrum`,lose:`🐯 Playing Fortune Pool on @bacaborr!
...
@bacaborr #Backcoin #Web3 #Arbitrum`,lose:`🐯 ¡Jugando Fortune Pool en @bacaborr!

Lotería on-chain verificable con Oracle seguro!

👉 https://backcoin.org

#Backcoin #Web3 #Arbitrum`}};let He="en";const ee=[{id:1,name:"Easy",emoji:"🍀",range:3,multiplier:200,chance:"33%",color:"emerald",hex:"#10b981",bgFrom:"from-emerald-500/20",bgTo:"to-green-600/10",borderColor:"border-emerald-500/50",textColor:"text-emerald-400"},{id:2,name:"Medium",emoji:"⚡",range:10,multiplier:500,chance:"10%",color:"violet",hex:"#8b5cf6",bgFrom:"from-violet-500/20",bgTo:"to-purple-600/10",borderColor:"border-violet-500/50",textColor:"text-violet-400"},{id:3,name:"Hard",emoji:"👑",range:100,multiplier:5e3,chance:"1%",color:"amber",hex:"#f59e0b",bgFrom:"from-amber-500/20",bgTo:"to-orange-600/10",borderColor:"border-amber-500/50",textColor:"text-amber-400"}],La=5700,w={mode:null,phase:"select",guess:50,guesses:[2,5,50],comboStep:0,wager:10,gameId:null,result:null,txHash:null,poolStatus:null,history:[],serviceFee:0n,serviceFee1x:0n,serviceFee5x:0n,tiersData:null};function Mr(){if(document.getElementById("fortune-styles-v2"))return;const e=document.createElement("style");e.id="fortune-styles-v2",e.textContent=`
    ...
    `,document.head.appendChild(e)}function Fr(){Mr();const e=document.getElementById("actions");if(!e){console.error("❌ FortunePool: Container #actions not found!");return}e.innerHTML=`
        <div class="max-w-md mx-auto px-4 py-6">
            <!-- Header -->
            <div class="text-center mb-6">
                <div class="relative inline-block">
                    <img id="tiger-mascot" src="${jt}" 
                ...
                <a href="${$a}${Aa}" target="_blank" rel="noopener" 
            ...
            <div class="bg-zinc-900/50 border border-zinc-800/50 rounded-2xl overflow-hidden">
                ...
                <div id="history-list" class="max-h-[300px] overflow-y-auto p-2">
                    <div class="p-6 text-center text-zinc-600 text-sm">
                        <img src="${jt}" class="w-12 h-12 mx-auto opacity-30 animate-pulse mb-2" onerror="this.style.display='none'">
                        Loading...
                    </div>
                </div>
            </div>
        </div>
    `,en(),G()}function Rr(){}function G(){const e=document.getElementById("game-area");if(e)switch(Dr(w.phase),w.phase){case"select":Rn(e);break;case"pick":jr(e);break;case"wager":Ur(e);break;case"processing":Wr(e);break;case"result":qr(e);break;default:Rn(e)}}function Dr(e){var s;const t=document.getElementById("tiger-mascot");if(t)switch(t.className="w-28 h-28 object-contain mx-auto",t.style.filter="",e){case"select":t.classList.add("tiger-float","tiger-pulse");break;case"pick":case"wager":t.classList.add("tiger-float");break;case"processing":t.classList.add("tiger-spin");break;case"result":((s=w.result)==null?void 0:s.prizeWon)>0?t.classList.add("tiger-celebrate"):(t.style.filter="grayscale(0.5)",t.classList.add("tiger-float"));break}}function Rn(e){var a,i;const t=w.serviceFee1x>0n?(Number(w.serviceFee1x)/1e18).toFixed(6):"0",s=w.serviceFee5x>0n?(Number(w.serviceFee5x)/1e18).toFixed(6):"0",n=w.serviceFee1x>0n||w.serviceFee5x>0n;e.innerHTML=`
        <div class="space-y-4">
            <!-- JACKPOT MODE -->
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        <div class="flex items-center gap-3 mb-1">
                            ...
                            <span class="px-2.5 py-1 rounded-full bg-violet-500/20 border border-violet-500/40 text-violet-400 text-sm font-black">${La}x</span>
                        </div>
    ...
    `,(a=document.getElementById("btn-jackpot"))==null||a.addEventListener("click",()=>{if(!c.isConnected)return m("Connect wallet first","warning");w.mode="jackpot",w.guess=50,w.phase="pick",G()}),(i=document.getElementById("btn-combo"))==null||i.addEventListener("click",()=>{if(!c.isConnected)return m("Connect wallet first","warning");w.mode="combo",w.guesses=[2,5,50],w.comboStep=0,w.phase="pick",G()})}function jr(e){w.mode==="jackpot"?Hr(e):_r(e)}function Hr(e){var r,l,d;const t=ee[2],s=w.guess,n=()=>{let u="";for(let p=1;p<=100;p++)u+=`
                <button class="number-btn w-full aspect-square rounded-lg text-sm font-bold transition-all
                    ${p===s?`bg-gradient-to-br ${t.bgFrom} ${t.bgTo} border-2 ${t.borderColor} ${t.textColor} scale-110 shadow-lg`:"bg-zinc-800/50 border border-zinc-700/50 text-zinc-400 hover:bg-zinc-700/50 hover:text-white"}" 
                    data-number="${p}">${p}</button>
            `;return u};e.innerHTML=`
        <div class="bg-gradient-to-br from-zinc-900 to-zinc-800/50 border border-zinc-700/50 rounded-2xl p-5">
            ...
            <div class="text-center mb-4">
                <div class="inline-flex items-center justify-center w-24 h-24 rounded-2xl bg-gradient-to-br ${t.bgFrom} ${t.bgTo} border-2 ${t.borderColor} glow-pulse" style="--glow-color: ${t.hex}40">
                    <span id="display-number" class="text-4xl font-black ${t.textColor}">${s}</span>
                </div>
    ...
    `;const a=document.getElementById("display-number"),i=document.getElementById("number-grid"),o=u=>{w.guess=u,a&&(a.textContent=u),i==null||i.querySelectorAll(".number-btn").forEach(f=>{parseInt(f.dataset.number)===u?f.className=`number-btn w-full aspect-square rounded-lg text-sm font-bold transition-all bg-gradient-to-br ${t.bgFrom} ${t.bgTo} border-2 ${t.borderColor} ${t.textColor} scale-110 shadow-lg`:f.className="number-btn w-full aspect-square rounded-lg text-sm font-bold transition-all bg-zinc-800/50 border border-zinc-700/50 text-zinc-400 hover:bg-zinc-700/50 hover:text-white"});const p=i==null?void 0:i.querySelector(`[data-number="${u}"]`);p==null||p.scrollIntoView({behavior:"smooth",block:"center"})};i==null||i.addEventListener("click",u=>{const p=u.target.closest(".number-btn");p&&o(parseInt(p.dataset.number))}),document.querySelectorAll(".quick-pick").forEach(u=>{u.addEventListener("click",()=>o(parseInt(u.dataset.number)))}),(r=document.getElementById("btn-random"))==null||r.addEventListener("click",()=>{o(Math.floor(Math.random()*100)+1)}),(l=document.getElementById("btn-back"))==null||l.addEventListener("click",()=>{w.phase="select",G()}),(d=document.getElementById("btn-next"))==null||d.addEventListener("click",()=>{w.phase="wager",G()})}function _r(e){var n,a;const t=ee[w.comboStep],s=w.guesses[w.comboStep];e.innerHTML=`
        <div class="bg-gradient-to-br from-zinc-900 to-zinc-800/50 border border-zinc-700/50 rounded-2xl p-5">
            <!-- Progress Pills -->
            <div class="flex justify-center gap-3 mb-5">
                ${ee.map((i,o)=>{const r=o===w.comboStep,l=o<w.comboStep;return`
                        <div class="flex items-center gap-2 px-3 py-2 rounded-xl border ${r?`bg-gradient-to-br ${i.bgFrom} ${i.bgTo} ${i.borderColor}`:l?"bg-emerald-500/10 border-emerald-500/50":"bg-zinc-800/50 border-zinc-700/50"}">
                            <span class="text-xl">${l?"✓":i.emoji}</span>
                            <div class="text-left">
                                <p class="text-xs font-bold ${r?i.textColor:l?"text-emerald-400":"text-zinc-500"}">${i.name}</p>
                                <p class="text-[10px] ${l?"text-emerald-400 font-bold":"text-zinc-600"}">${l?w.guesses[o]:i.multiplier+"x"}</p>
                            </div>
                        </div>
                    `}).join("")}
            </div>
            ...
            <div class="flex justify-center gap-2 mb-5">
                ${Array.from({length:t.range},(i,o)=>o+1).map(i=>`
                    <button class="num-btn w-12 h-12 rounded-xl font-bold text-lg transition-all ${i===s?`bg-gradient-to-br ${t.bgFrom} ${t.bgTo} border-2 ${t.borderColor} ${t.textColor}`:"bg-zinc-800/60 border border-zinc-700/50 text-zinc-400 hover:border-zinc-600"}" data-num="${i}">
                        ${i}
                    </button>
                `).join("")}
            </div>
            ...
            <div class="flex gap-3">
                ...
                <button id="btn-next" class="flex-1 py-3.5 bg-gradient-to-r from-${t.color}-500 to-${t.color}-600 text-black font-bold rounded-xl transition-all">
                    ${w.comboStep<2?"Next":"Continue"}<i class="fa-solid fa-arrow-right ml-2"></i>
                </button>
            </div>
        </div>
    `,document.querySelectorAll(".num-btn").forEach(i=>{i.addEventListener("click",()=>{const o=parseInt(i.dataset.num);w.guesses[w.comboStep]=o,document.querySelectorAll(".num-btn").forEach(r=>{parseInt(r.dataset.num)===o?r.className=`num-btn w-12 h-12 rounded-xl font-bold text-lg transition-all bg-gradient-to-br ${t.bgFrom} ${t.bgTo} border-2 ${t.borderColor} ${t.textColor}`:r.className="num-btn w-12 h-12 rounded-xl font-bold text-lg transition-all bg-zinc-800/60 border border-zinc-700/50 text-zinc-400 hover:border-zinc-600"})})}),(n=document.getElementById("btn-back"))==null||n.addEventListener("click",()=>{w.comboStep>0?(w.comboStep--,G()):(w.phase="select",G())}),(a=document.getElementById("btn-next"))==null||a.addEventListener("click",()=>{w.comboStep<2?(w.comboStep++,G()):(w.phase="wager",G())})}function Ur(e){const t=w.mode==="jackpot",s=t?[w.guess]:w.guesses,n=t?50:La,a=B(c.currentUserBalance||0n),i=a>=1,o=t?w.serviceFee1x:w.serviceFee5x,r=o>0n?Number(o)/1e18:0,l=o>0n;e.innerHTML=`
        <div class="bg-gradient-to-br from-zinc-900 to-zinc-800/50 border border-zinc-700/50 rounded-2xl p-5">
            <div class="text-center mb-5">
                <h2 class="text-xl font-bold text-white mb-2">🎰 Your Selection</h2>
                <div class="flex justify-center gap-3">
                    ${(t?[{tier:ee[2],pick:s[0]}]:s.map((d,u)=>({tier:ee[u],pick:d}))).map(({tier:d,pick:u})=>`
            ...
            <div class="mb-5">
                ...
                <div class="flex items-center gap-3 mb-3">
                    <input type="number" id="custom-wager" value="${w.wager}" min="1" max="${Math.floor(a)}"
                ...
                <div class="grid grid-cols-4 gap-2">
                    ${[10,50,100,Math.floor(a)].map(d=>`
                        <button class="percent-btn py-2 text-sm font-bold rounded-lg transition-all ${w.wager===d?"bg-amber-500/20 border border-amber-500/50 text-amber-400":"bg-zinc-800/60 border border-zinc-700/50 text-zinc-400 hover:border-zinc-600"}" data-value="${d}">
            ...
            <div class="p-4 bg-gradient-to-r from-emerald-900/20 to-green-900/10 border border-emerald-500/30 rounded-xl mb-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-black text-emerald-400" id="potential-win">${(w.wager*n).toLocaleString()}</p>
    ...
    `,Or(n,a)}function Or(e,t){var n,a,i,o;const s=r=>{w.wager=Math.max(1,Math.min(Math.floor(r),Math.floor(t)));const l=document.getElementById("custom-wager"),d=document.getElementById("potential-win");l&&(l.value=w.wager),d&&(d.textContent=(w.wager*e).toLocaleString()),document.querySelectorAll(".percent-btn").forEach(u=>{const p=parseInt(u.dataset.value),f=w.wager===p;u.classList.toggle("bg-amber-500/20",f),u.classList.toggle("border-amber-500/50",f),u.classList.toggle("text-amber-400",f),u.classList.toggle("border-zinc-700/50",!f),u.classList.toggle("bg-zinc-800/60",!f),u.classList.toggle("text-zinc-400",!f)})};document.querySelectorAll(".percent-btn").forEach(r=>{r.addEventListener("click",()=>{s(parseInt(r.dataset.value)||1)})}),(n=document.getElementById("custom-wager"))==null||n.addEventListener("input",r=>{s(parseInt(r.target.value)||1)}),(a=document.getElementById("btn-faucet"))==null||a.addEventListener("click",async()=>{m("Requesting tokens...","info");try{const l=await(await fetch(`https://faucet-4wvdcuoouq-uc.a.run.app?address=${c.userAddress}`)).json();l.success?(m("🎉 Tokens received!","success"),await V(),G()):m(l.error||"Error","error")}catch{m("Faucet error","error")}}),(i=document.getElementById("btn-back"))==null||i.addEventListener("click",()=>{w.phase="pick",w.mode==="combo"&&(w.comboStep=2),G()}),(o=document.getElementById("btn-play"))==null||o.addEventListener("click",async()=>{if(w.wager<1)return m("Min: 1 BKC","warning");w.phase="processing",G();try{const r=w.mode==="jackpot"?[w.guess]:w.guesses,l=w.mode==="combo",d=await Lo(w.wager,r,l);d!=null&&d.success?(w.gameId=d.gameId,w.txHash=d.txHash,w.result={rolls:d.rolls,prizeWon:d.prizeWon,matches:d.matches||[]},setTimeout(()=>{w.phase="result",G(),en()},1500)):(m((d==null?void 0:d.error)||"Transaction failed","error"),w.phase="wager",G())}catch(r){console.error("Play error:",r),m("Error: "+r.message,"error"),w.phase="wager",G()}})}function Wr(e){const t=w.mode==="jackpot",s=t?[w.guess]:w.guesses,n=t?[ee[2]]:ee;e.innerHTML=`
    ...
    `,n.forEach((a,i)=>{const o=document.getElementById(`spin-${i}`);if(!o)return;setInterval(()=>{o.textContent=Math.floor(Math.random()*a.range)+1},80)})}function qr(e){var u,p;const t=w.result;if(!t)return G();const s=w.mode==="jackpot",n=s?[w.guess]:w.guesses,a=t.rolls||[],i=s?[ee[2]]:ee,o=n.map((f,g)=>{const b=a[g]!==void 0?Number(a[g]):null;return b!==null&&b===f}),r=o.filter(f=>f).length,l=t.prizeWon>0||r>0;let d=t.prizeWon;!d&&r>0&&(d=0,o.forEach((f,g)=>{if(f){const b=s?ee[2]:ee[g];d+=w.wager*b.multiplier}})),e.innerHTML=`
        <div class="bg-gradient-to-br ${l?"from-emerald-900/30 to-green-900/10 border-emerald-500/30":"from-zinc-900 to-zinc-800/50 border-zinc-700/50"} border rounded-2xl p-6 relative overflow-hidden" id="result-container">
            ...
            <div class="text-center mb-6">
                ${l?`
                    ...
                    <p class="text-4xl font-black text-white">${d.toLocaleString()} BKC</p>
                `:`
                    <div class="text-6xl mb-3">😔</div>
            ...
            <!-- Results Grid -->
            <div class="grid ${s?"grid-cols-1":"grid-cols-3"} gap-4 mb-6">
                ${i.map((f,g)=>{const b=s?n[0]:n[g],v=a[g],y=o[g];return`
                        <div class="text-center p-4 rounded-xl ${y?"bg-emerald-500/20 border border-emerald-500/50":"bg-zinc-800/50 border border-zinc-700/50"}">
                            <p class="text-xs text-zinc-500 mb-2">${f.emoji} ${f.name}</p>
                            <div class="flex items-center justify-center gap-3">
                                <div class="text-center">
                                    <p class="text-[10px] text-zinc-600 mb-1">YOU</p>
                                    <div class="w-12 h-12 rounded-lg bg-gradient-to-br ${f.bgFrom} ${f.bgTo} border ${f.borderColor} flex items-center justify-center">
                                        <span class="text-xl font-black ${f.textColor}">${b}</span>
                                    </div>
                                </div>
                                <span class="text-2xl ${y?"text-emerald-400":"text-red-400"}">${y?"=":"≠"}</span>
                                <div class="text-center">
                                    <p class="text-[10px] text-zinc-600 mb-1">ROLL</p>
                                    <div class="w-12 h-12 rounded-lg ${y?"bg-emerald-500/30 border-emerald-500":"bg-zinc-700/50 border-zinc-600"} border flex items-center justify-center ${y?"match-pulse":"miss-shake"}">
                                        <span class="text-xl font-black ${y?"text-emerald-400":"text-zinc-300"}">${v}</span>
                                    </div>
                                </div>
                            </div>
                            ${y?`<p class="text-emerald-400 text-sm font-bold mt-2">+${f.multiplier}x WIN!</p>`:""}
                        </div>
                    `}).join("")}
            </div>
            ...
            ${w.txHash?`
                <div class="text-center mb-4">
                    <a href="${Pr}${w.txHash}" target="_blank" class="inline-flex items-center gap-2 text-xs text-zinc-500 hover:text-zinc-400">
                        <i class="fa-solid fa-external-link"></i>
                        View Transaction
                    </a>
                </div>
            ...
            <div class="bg-gradient-to-r ${l?"from-amber-500/10 to-orange-500/10 border-amber-500/30":"from-zinc-800/50 to-zinc-700/30 border-zinc-600/30"} border rounded-xl p-4 mb-4">
                <div class="flex items-center gap-3 mb-3">
                    <div class="w-10 h-10 rounded-full ${l?"bg-amber-500/20":"bg-zinc-700/50"} flex items-center justify-center">
                    ...
                    <div>
                        ...
                        <p class="text-amber-400 text-xs font-medium">+${Ss} Airdrop Points</p>
                    </div>
                </div>
                <button id="btn-share" class="w-full py-3 ${l?"bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-black":"bg-zinc-700 hover:bg-zinc-600 text-white border border-zinc-600"} font-bold rounded-xl transition-all">
    ...
    `,l&&(Yr(),d>w.wager*10&&Kr()),(u=document.getElementById("btn-new-game"))==null||u.addEventListener("click",()=>{w.phase="select",w.result=null,w.txHash=null,w.gameId=null,G(),en()}),(p=document.getElementById("btn-share"))==null||p.addEventListener("click",()=>{Gr(l,d)})}function Yr(){const e=document.createElement("div");e.className="confetti-container",document.body.appendChild(e);const t=["#f59e0b","#10b981","#8b5cf6","#ec4899","#06b6d4"],s=["●","■","★","🐯","🎉"];for(let n=0;n<60;n++){const a=document.createElement("div");a.className="confetti",a.style.cssText=`
            left: ${Math.random()*100}%;
            color: ${t[n%t.length]};
            font-size: ${8+Math.random()*12}px;
            animation-delay: ${Math.random()*2}s;
            animation-duration: ${2+Math.random()*2}s;
        `,a.textContent=s[n%s.length],e.appendChild(a)}setTimeout(()=>e.remove(),5e3)}function Kr(){const e=["🪙","💰","✨","⭐","🎉"];for(let t=0;t<30;t++)setTimeout(()=>{const s=document.createElement("div");s.className="coin",s.textContent=e[Math.floor(Math.random()*e.length)],s.style.left=`${Math.random()*100}%`,s.style.animationDelay=`${Math.random()*.5}s`,s.style.animationDuration=`${2+Math.random()*2}s`,document.body.appendChild(s),setTimeout(()=>s.remove(),4e3)},t*100)}function Gr(e,t){var l,d,u,p,f;const s=Fn[He],n=()=>{const g=Nr[He];return e?g.win(t):g.lose},a=`
        <div class="text-center">
            <img src="${jt}" class="w-16 h-16 mx-auto mb-2" alt="Fortune Pool" onerror="this.style.display='none'">
            ...
            <!-- Language Selector -->
            <div class="flex justify-center gap-2 mb-4">
                <button class="lang-btn px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${He==="pt"?"bg-amber-500/20 border-amber-500 text-amber-400":"bg-zinc-800 border-zinc-700 text-zinc-400 hover:text-white"} border" data-lang="pt">
                    🇧🇷 PT
                </button>
                <button class="lang-btn px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${He==="en"?"bg-amber-500/20 border-amber-500 text-amber-400":"bg-zinc-800 border-zinc-700 text-zinc-400 hover:text-white"} border" data-lang="en">
                    🇺🇸 EN
                </button>
                <button class="lang-btn px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${He==="es"?"bg-amber-500/20 border-amber-500 text-amber-400":"bg-zinc-800 border-zinc-700 text-zinc-400 hover:text-white"} border" data-lang="es">
                    🇪🇸 ES
                </button>
            </div>
    ...
    `;Kt(a,"max-w-xs");const i=g=>{He=g;const b=Fn[g],v=document.getElementById("share-modal-title"),y=document.getElementById("share-modal-subtitle"),k=document.getElementById("btn-close-share");v&&(v.textContent=b.title),y&&(y.textContent=b.subtitle),k&&(k.textContent=b.later),document.querySelectorAll(".lang-btn").forEach(E=>{E.dataset.lang===g?E.className="lang-btn px-3 py-1.5 rounded-lg text-xs font-medium transition-all bg-amber-500/20 border-amber-500 text-amber-400 border":E.className="lang-btn px-3 py-1.5 rounded-lg text-xs font-medium transition-all bg-zinc-800 border-zinc-700 text-zinc-400 hover:text-white border"})};document.querySelectorAll(".lang-btn").forEach(g=>{g.addEventListener("click",()=>i(g.dataset.lang))});const o=async g=>{if(!c.userAddress)return!1;try{const v=await(await fetch("https://us-central1-backchain-backand.cloudfunctions.net/trackShare",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({address:c.userAddress,gameId:w.gameId||Date.now(),type:"fortune",platform:g})})).json();return v.success?(m(`🎉 +${v.pointsAwarded||Ss} Airdrop Points!`,"success"),!0):(v.reason==="already_shared"&&console.log("Already shared this game"),!1)}catch(b){return console.error("Share tracking error:",b),m(`🎉 +${Ss} Airdrop Points!`,"success"),!0}},r=async(g,b)=>{await o(g),window.open(b,"_blank"),he()};(l=document.getElementById("share-twitter"))==null||l.addEventListener("click",()=>{const g=n();r("twitter",`https://twitter.com/intent/tweet?text=${encodeURIComponent(g)}`)}),(d=document.getElementById("share-telegram"))==null||d.addEventListener("click",()=>{const g=n();r("telegram",`https://t.me/share/url?url=https://backcoin.org&text=${encodeURIComponent(g)}`)}),(u=document.getElementById("share-whatsapp"))==null||u.addEventListener("click",()=>{const g=n();r("whatsapp",`https://wa.me/?text=${encodeURIComponent(g)}`)}),(p=document.getElementById("share-copy"))==null||p.addEventListener("click",async()=>{const g=n();try{await navigator.clipboard.writeText(g),m("📋 Copied!","success"),await o("copy")}catch{m("Copy failed","error")}he()}),(f=document.getElementById("btn-close-share"))==null||f.addEventListener("click",he)}async function Vr(){const e=c.fortunePoolContract||c.fortunePoolContractPublic;if(!e)return console.log("No fortune contract available"),null;try{const[t,s,n,a,i,o]=await Promise.all([e.prizePoolBalance().catch(()=>0n),e.gameCounter().catch(()=>0),e.serviceFee().catch(()=>0n),e.getRequiredServiceFee(!1).catch(()=>0n),e.getRequiredServiceFee(!0).catch(()=>0n),e.activeTierCount().catch(()=>3)]);w.serviceFee=n||0n,w.serviceFee1x=a||0n,w.serviceFee5x=i||0n;try{const[r,l]=await e.getAllTiers();w.tiersData=r.map((d,u)=>({range:Number(d),multiplier:Number(l[u])/1e4})),console.log("Tiers from contract:",w.tiersData)}catch{console.log("Using default tiers")}return{prizePool:t||0n,gameCounter:Number(s)||0,serviceFee:n||0n,serviceFee1x:a||0n,serviceFee5x:i||0n,tierCount:Number(o)||3}}catch(t){return console.error("getFortunePoolStatus error:",t),{prizePool:0n,gameCounter:0,serviceFee:0n}}}async function en(){try{const e=await Vr();if(e){const s=document.getElementById("prize-pool"),n=document.getElementById("total-games");s&&(s.textContent=B(e.prizePool||0n).toFixed(2)+" BKC"),n&&(n.textContent=(e.gameCounter||0).toLocaleString())}const t=document.getElementById("user-balance");t&&(t.textContent=B(c.currentUserBalance||0n).toFixed(2)+" BKC"),Xr()}catch(e){console.error("Pool error:",e)}}async function Xr(){var e;try{const t=oe.fortuneGames||"https://getfortunegames-4wvdcuoouq-uc.a.run.app",s=c.userAddress?`${t}?player=${c.userAddress}&limit=15`:`${t}?limit=15`,a=await(await fetch(s)).json();if(((e=a.games)==null?void 0:e.length)>0){Jr(a.games);const i=a.games.filter(r=>r.isWin||r.prizeWon&&BigInt(r.prizeWon)>0n).length,o=document.getElementById("win-rate");o&&(o.textContent=`🏆 ${i}/${a.games.length} wins`)}else{const i=document.getElementById("history-list");i&&(i.innerHTML=`
                <div class="p-8 text-center">
                    <img src="${jt}" class="w-16 h-16 mx-auto opacity-20 mb-3" onerror="this.style.display='none'">
                    <p class="text-zinc-500 text-sm">No games yet</p>
                    <p class="text-zinc-600 text-xs mt-1">Be the first to play!</p>
                </div>
            `)}}catch(t){console.error("loadHistory error:",t)}}function Jr(e){const t=document.getElementById("history-list");t&&(t.innerHTML=e.map(s=>{var f;const n=s.isWin||s.prizeWon&&BigInt(s.prizeWon)>0n,a=s.prizeWon?B(BigInt(s.prizeWon)):0,i=s.wagerAmount?B(BigInt(s.wagerAmount)):0,o=s.isCumulative,r=s.rolls||[],l=s.guesses||[],d=Qr(s.timestamp),u=s.player?`${s.player.slice(0,6)}...${s.player.slice(-4)}`:"???",p=c.userAddress&&((f=s.player)==null?void 0:f.toLowerCase())===c.userAddress.toLowerCase();return`
            <div class="p-3 rounded-xl mb-2 ${n?"bg-emerald-500/10 border border-emerald-500/30":"bg-zinc-800/30 border border-zinc-700/30"} transition-all hover:scale-[1.01]">
                <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center gap-2">
                        <span class="text-lg">${n?"🏆":"🎲"}</span>
                        <span class="text-xs ${p?"text-amber-400 font-bold":"text-zinc-500"}">${p?"You":u}</span>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex gap-1">
                        ${(o?ee:[ee[2]]).map((g,b)=>{const v=l[b],y=r[b];return`
                                <div class="w-6 h-6 rounded text-[10px] font-bold flex items-center justify-center ${v!==void 0&&y!==void 0&&Number(v)===Number(y)?"bg-emerald-500/30 text-emerald-400":"bg-zinc-700/50 text-zinc-500"}">
                                    ${y??"?"}
                                </div>
                            `}).join("")}
                    </div>
        ...
        `}).join(""))}function Qr(e){if(!e)return"";const t=Date.now(),s=typeof e=="number"?e*1e3:new Date(e).getTime(),n=t-s,a=Math.floor(n/6e4),i=Math.floor(n/36e5),o=Math.floor(n/864e5);return a<1?"Just now":a<60?`${a}m ago`:i<24?`${i}h ago`:`${o}d ago`}const Zr={render:Fr,cleanup:Rr},el=()=>{if(document.getElementById("about-styles-v4"))return;const e=document.createElement("style");e.id="about-styles-v4",e.innerHTML=`
        @keyframes pulse-hub {
    ...
    `,document.head.appendChild(e)};function tl(){return`
    ...
    `,document.head.appendChild(e)}async function Je(){var e;z.isConnected=c.isConnected,z.user=null,z.userSubmissions=[],z.platformUsage={},z.isBanned=!1;try{const t=await Ks();if(z.systemConfig=t.config,z.leaderboards=t.leaderboards,z.dailyTasks=t.dailyTasks||[],z.platformUsageConfig=t.platformUsageConfig||Pa,z.isConnected&&c.userAddress){const[s,n]=await Promise.all([tt(c.userAddress),Fi()]);if(z.user=s,z.userSubmissions=n,s&&s.isBanned){z.isBanned=!0;return}try{typeof wn=="function"&&(z.platformUsage=await wn()||{})}catch(a){console.warn("Could not load platform usage:",a),z.platformUsage={}}z.dailyTasks.length>0&&(z.dailyTasks=await Promise.all(z.dailyTasks.map(async a=>{try{if(!a.id)return{...a,eligible:!1,timeLeftMs:0};const i=await na(a.id,a.cooldownHours);return{...a,eligible:i.eligible,timeLeftMs:i.timeLeft}}catch{return{...a,eligible:!1,timeLeftMs:0}}})))}}catch(t){if(console.error("Airdrop Data Load Error:",t),t.code==="permission-denied"||(e=t.message)!=null&&e.includes("permission")){console.warn("Firebase permissions issue - user may need to connect wallet or sign in"),z.systemConfig=z.systemConfig||{},z.leaderboards=z.leaderboards||{top100ByPoints:[],top100ByPosts:[]},z.dailyTasks=z.dailyTasks||[];return}m("Error loading data. Please refresh.","error")}}function Na(){const{user:e}=z,t=(e==null?void 0:e.totalPoints)||0,s=(e==null?void 0:e.platformUsagePoints)||0,n=(e==null?void 0:e.approvedSubmissionsCount)||0,a=ml(n);return`
        <!-- Mobile Header -->
        <div class="md:hidden px-4 pt-4 pb-2">
            ...
            <div class="flex gap-1 bg-zinc-900/80 p-1 rounded-2xl border border-zinc-800">
                ${ms("earn","fa-coins","Earn")}
                ${ms("history","fa-clock-rotate-left","History")}
                ${ms("leaderboard","fa-trophy","Ranking")}
            </div>
        </div>
    ...
    `}function ms(e,t,s){const n=z.activeTab===e;return`
    ...
    `}function gs(){return z.isConnected?`
        <div class="px-4 airdrop-fade-up">
            ...
            <!-- Sub-tab Content -->
            <div id="earn-content">
                ${z.activeEarnTab==="post"?gl():""}
                ${z.activeEarnTab==="platform"?xl():""}
                ${z.activeEarnTab==="tasks"?vl():""}
            </div>
        </div>
    `:`
        ...
        `}function gl(){const{user:e}=z,s=`https://backcoin.org/?ref=${(e==null?void 0:e.referralCode)||"CODE"}`;return`
        <div class="space-y-4">
            <!-- Priority Banner -->
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex gap-3 items-start">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="bg-black/40 p-2.5 rounded-lg border border-zinc-700 mb-2">
                                ...
                                <p class="text-xs font-mono text-zinc-500 mt-1">${Ns}</p>
                            </div>
                    ...
                    <div class="flex gap-3 items-start">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="grid grid-cols-4 gap-2">
                                <a href="https://twitter.com/intent/tweet?text=${encodeURIComponent(s+" "+Ns)}" target="_blank" 
    ...
    `}function xl(){var o;const e=z.platformUsageConfig||Pa,t=z.platformUsage||{};let s=0,n=0;Object.keys(e).forEach(r=>{var l;e[r].enabled!==!1&&e[r].maxCount&&(s+=e[r].maxCount,n+=Math.min(((l=t[r])==null?void 0:l.count)||0,e[r].maxCount))});const a=s>0?n/s*100:0,i=((o=z.user)==null?void 0:o.platformUsagePoints)||0;return`
        <div class="space-y-4">
            <!-- Info Banner -->
            ...
            <div class="grid grid-cols-2 gap-2" id="platform-actions-grid">
                ${Object.entries(e).filter(([r,l])=>l.enabled!==!1).map(([r,l])=>{const d=t[r]||{count:0},u=d.count>=l.maxCount,p=Math.max(0,l.maxCount-d.count),f=d.count/l.maxCount*100,g=ul[r]||"";return`
    ...
    `}function vl(){const e=z.dailyTasks||[],t=e.filter(n=>n.eligible),s=e.filter(n=>!n.eligible&&n.timeLeftMs>0);return`
        <div class="space-y-4">
            <!-- Info Banner -->
                        ...
                        ${s.map(n=>`
                            <div class="bg-zinc-900/80 border border-zinc-800 rounded-xl p-3 opacity-50">
                                <div class="flex items-center justify-between">
                                    ...
                                    <span class="text-zinc-600 text-xs">${pl(n.timeLeftMs)}</span>
                                </div>
                            </div>
                        `).join("")}
                    </div>
    ...
    `}function hl(){const{user:e,userSubmissions:t}=z;if(!z.isConnected)return`
            <div class="text-center px-4 py-12 airdrop-fade-up">
        ...
        `;const s=Date.now(),n=Sa*60*60*1e3,a=t.filter(l=>["pending","auditing"].includes(l.status)&&l.submittedAt&&s-l.submittedAt.getTime()>=n),i=(e==null?void 0:e.approvedSubmissionsCount)||0,o=t.filter(l=>["pending","auditing"].includes(l.status)).length,r=t.filter(l=>l.status==="rejected").length;return`
            ...
            <div>
                ...
                <div class="bg-zinc-900/80 border border-zinc-800 rounded-xl overflow-hidden">
                    ${t.length===0?`<div class="p-8 text-center">
                        ...
                        </div>`:t.slice(0,10).map((l,d)=>{const u=d===Math.min(t.length,10)-1;["pending","auditing"].includes(l.status);const p=l.status==="approved",f=l.status==="rejected";let g,b,v;p?(g='<i class="fa-solid fa-check-circle text-green-400"></i>',b="",v=""):f?(g='<i class="fa-solid fa-times-circle text-red-400"></i>',b="",v=""):(g='<i class="fa-solid fa-shield-halved text-amber-400 animate-pulse"></i>',b="bg-amber-900/10",v=`
    ...
    `}function wl(){var u,p;const e=((u=z.leaderboards)==null?void 0:u.top100ByPosts)||[],t=((p=z.leaderboards)==null?void 0:p.top100ByPoints)||[],s=z.activeRanking||"posts";function n(f,g,b){var $,T;const v=z.user&&(($=f.walletAddress)==null?void 0:$.toLowerCase())===((T=z.user.walletAddress)==null?void 0:T.toLowerCase()),y=yl(g+1),k=b==="posts"?"bg-amber-500/10":"bg-green-500/10",E=b==="posts"?"text-amber-400":"text-green-400",L=b==="posts"?"text-white":"text-green-400",A=b==="posts"?"posts":"pts";return`
            <div class="flex items-center justify-between p-3 ${v?k:"hover:bg-zinc-800/50"} transition-colors">
                <div class="flex items-center gap-3">
                    <span class="w-8 h-8 rounded-full ${y.bg} flex items-center justify-center text-xs font-bold">${y.icon||g+1}</span>
                    <span class="font-mono text-xs ${v?E+" font-bold":"text-zinc-400"}">
                        ${Ve(f.walletAddress)}${v?" (You)":""}
                    </span>
                </div>
                <span class="font-bold ${L} text-sm">${(f.value||0).toLocaleString()} <span class="text-zinc-500 text-xs">${A}</span></span>
            </div>
        `}const a=s==="posts"?"bg-gradient-to-r from-amber-500 to-orange-500 text-black shadow-lg shadow-amber-500/20":"bg-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-700",i=s==="points"?"bg-gradient-to-r from-green-500 to-emerald-500 text-black shadow-lg shadow-green-500/20":"bg-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-700",o=s==="posts"?"":"hidden",r=s==="points"?"":"hidden",l=e.length===0?'<p class="p-6 text-center text-zinc-500 text-sm">No data yet - be the first!</p>':e.slice(0,50).map((f,g)=>n(f,g,"posts")).join(""),d=t.length===0?'<p class="p-6 text-center text-zinc-500 text-sm">No data yet - be the first!</p>':t.slice(0,50).map((f,g)=>n(f,g,"points")).join("");return`
        <div class="px-4 airdrop-fade-up">
    ...
    `}function yl(e){return e<=2?{icon:"💎",bg:"bg-cyan-500/20 text-cyan-300"}:e<=10?{icon:"🏆",bg:"bg-slate-400/20 text-slate-300"}:e<=20?{icon:"🥇",bg:"bg-yellow-500/20 text-yellow-400"}:e<=50?{icon:"🥈",bg:"bg-gray-400/20 text-gray-300"}:e<=150?{icon:"🥉",bg:"bg-amber-700/20 text-amber-600"}:e<=300?{icon:"⚔️",bg:"bg-zinc-600/20 text-zinc-400"}:{icon:null,bg:"bg-zinc-800 text-zinc-400"}}function fe(){const e=document.getElementById("main-content"),t=document.getElementById("airdrop-header");if(e){if(t&&(t.innerHTML=Na()),z.isBanned){e.innerHTML=`
        ...
        `;return}switch(document.querySelectorAll(".nav-pill-btn").forEach(s=>{const n=s.dataset.target;s.closest(".md\\:hidden")?n===z.activeTab?(s.classList.add("airdrop-tab-active","shadow-lg"),s.classList.remove("text-zinc-500")):(s.classList.remove("airdrop-tab-active","shadow-lg"),s.classList.add("text-zinc-500")):n===z.activeTab?(s.classList.remove("text-zinc-400","hover:text-white","hover:bg-zinc-800"),s.classList.add("airdrop-tab-active","shadow-lg","shadow-amber-500/20")):(s.classList.add("text-zinc-400","hover:text-white","hover:bg-zinc-800"),s.classList.remove("airdrop-tab-active","shadow-lg","shadow-amber-500/20"))}),z.activeTab){case"earn":e.innerHTML=gs();break;case"post":e.innerHTML=gs();break;case"history":e.innerHTML=hl();break;case"leaderboard":e.innerHTML=wl();break;default:e.innerHTML=gs()}}}function kl(){var s;const e=((s=z.user)==null?void 0:s.referralCode)||"CODE",t=`${e!=="CODE"?`https://backcoin.org/?ref=${e}`:"https://backcoin.org"} ${Ns}`;navigator.clipboard.writeText(t).then(()=>{m("Copied! Now paste it in your post.","success");const n=document.getElementById("copy-viral-btn");if(n){const a=n.innerHTML;n.innerHTML='<i class="fa-solid fa-check"></i> Copied!',n.classList.remove("cta-mega"),n.classList.add("bg-green-600"),setTimeout(()=>{n.innerHTML=a,n.classList.add("cta-mega"),n.classList.remove("bg-green-600")},2e3)}}).catch(()=>m("Failed to copy.","error"))}function jn(e){const t=e.target.closest(".nav-pill-btn");t&&(z.activeTab=t.dataset.target,fe())}function Tl(e){const t=e.target.closest(".earn-tab-btn");t&&t.dataset.earnTab&&(z.activeEarnTab=t.dataset.earnTab,fe())}function zl(e){const t=e.target.closest(".ranking-tab-btn");t&&t.dataset.ranking&&(z.activeRanking=t.dataset.ranking,fe())}function Cl(){z.isGuideOpen=!z.isGuideOpen,fe()}function Ma(e){var a;const t=`
        <div class="text-center">
            <!-- Imagem de CAUTION -->
    ...
    `;Kt(t,"max-w-md"),(a=document.getElementById("deletePostBtn"))==null||a.addEventListener("click",async i=>{const o=i.currentTarget,r=o.dataset.submissionId;o.disabled=!0,o.innerHTML='<i class="fa-solid fa-circle-notch fa-spin mr-1"></i> Deleting...';try{await aa(r),m("Post deleted. No penalty applied.","info"),he(),await Je(),fe()}catch(l){m(l.message,"error"),o.disabled=!1,o.innerHTML='<i class="fa-solid fa-trash mr-1"></i> Delete Post'}});const s=document.getElementById("confirmCheckbox"),n=document.getElementById("finalConfirmBtn");s==null||s.addEventListener("change",()=>{s.checked?(n.disabled=!1,n.className="flex-1 bg-green-600 hover:bg-green-500 text-white py-3 rounded-xl font-bold text-sm transition-colors cursor-pointer",n.innerHTML='<i class="fa-solid fa-check mr-1"></i> Confirm & Earn ✓'):(n.disabled=!0,n.className="flex-1 bg-green-600/50 text-green-200 py-3 rounded-xl font-bold text-sm cursor-not-allowed transition-colors",n.innerHTML='<i class="fa-solid fa-lock mr-1"></i> Confirm & Earn')}),n==null||n.addEventListener("click",Bl)}async function Bl(e){const t=e.currentTarget,s=t.dataset.submissionId;t.disabled=!0,t.innerHTML='<i class="fa-solid fa-circle-notch fa-spin mr-2"></i> Verifying...';try{await Ri(s),m("Success! Points added.","success"),he(),await Je(),fe()}catch{m("Verification failed.","error"),t.disabled=!1,t.innerHTML="Try Again"}}async function Il(e){const t=e.target.closest(".action-btn");if(!t)return;const s=t.dataset.action,n=t.dataset.id;if(s==="confirm"){const a=z.userSubmissions.find(i=>i.submissionId===n);a&&Ma(a)}else if(s==="delete"){if(!confirm("Remove this submission?"))return;try{await aa(n),m("Removed.","info"),await Je(),fe()}catch(a){m(a.message,"error")}}}async function El(e){const t=e.target.closest("#submit-content-btn");if(!t)return;const s=document.getElementById("content-url-input"),n=s==null?void 0:s.value.trim();if(!n||!n.startsWith("http"))return m("Enter a valid URL.","warning");const a=t.innerHTML;t.disabled=!0,t.innerHTML='<i class="fa-solid fa-circle-notch fa-spin"></i>';try{await Mi(n),m("📋 Submitted! Your post is now under security audit.","info"),s.value="",await Je(),z.activeTab="history",fe()}catch(i){m(i.message,"error")}finally{t.disabled=!1,t.innerHTML=a}}async function $l(e){const t=e.target.closest(".task-card");if(!t)return;const s=t.dataset.id,n=t.dataset.url;n&&window.open(n,"_blank");const a=z.dailyTasks.find(i=>i.id===s);if(!(!a||!a.eligible))try{await Pi(a,z.user.pointsMultiplier),m(`Task completed! +${a.points} pts`,"success"),await Je(),fe()}catch(i){i.message.includes("Cooldown")||m(i.message,"error")}}function Al(){const e=Date.now(),t=Sa*60*60*1e3,s=z.userSubmissions.filter(n=>["pending","auditing"].includes(n.status)&&n.submittedAt&&e-n.submittedAt.getTime()>=t);s.length>0&&(z.activeTab="history",fe(),setTimeout(()=>{Ma(s[0])},500))}const Ll={async render(e){const t=document.getElementById("airdrop");if(!t)return;bl();const s=fl();(t.innerHTML.trim()===""||e)&&(t.innerHTML=`
                ...
                <div id="airdrop-main" class="hidden">
                    <div id="airdrop-header">${Na()}</div>
                    <div id="airdrop-body" class="max-w-2xl mx-auto pb-24">
                        <div id="main-content"></div>
                    </div>
                </div>
            `,this.attachListeners());try{const n=new Promise(r=>setTimeout(r,4e3));await Promise.all([Je(),n]);const a=document.getElementById("loading-state"),i=document.getElementById("airdrop-main"),o=document.getElementById("main-content");a&&(a.style.transition="opacity 0.5s ease-out",a.style.opacity="0",await new Promise(r=>setTimeout(r,500)),a.classList.add("hidden")),i&&i.classList.remove("hidden"),o&&(o.classList.remove("hidden"),fe()),Al()}catch(n){console.error(n)}},attachListeners(){const e=document.getElementById("airdrop-body"),t=document.getElementById("airdrop-header");t==null||t.addEventListener("click",jn),e==null||e.addEventListener("click",s=>{s.target.closest("#guide-toggle-btn")&&Cl(),s.target.closest("#submit-content-btn")&&El(s),s.target.closest(".task-card")&&$l(s),s.target.closest(".action-btn")&&Il(s),s.target.closest("#copy-viral-btn")&&kl(),s.target.closest(".ranking-tab-btn")&&zl(s),s.target.closest(".earn-tab-btn")&&Tl(s),s.target.closest(".nav-pill-btn")&&jn(s);const n=s.target.closest(".platform-action-card");if(n&&!n.classList.contains("completed")){const a=n.dataset.targetPage;a&&(console.log("🎯 Navigating to:",a),Sl(a))}})},update(e){z.isConnected!==e&&this.render(!0)}};function Sl(e){console.log("🎯 Platform card clicked, navigating to:",e);const t=document.querySelector(`a[data-target="${e}"]`)||document.querySelector(`[data-target="${e}"]`);if(t){console.log("✅ Found menu link, clicking..."),t.click();const a=document.getElementById("sidebar");a&&window.innerWidth<768&&a.classList.add("hidden");return}const s=document.querySelectorAll("main > section"),n=document.getElementById(e);if(n){console.log("✅ Found section, showing directly..."),s.forEach(i=>i.classList.add("hidden")),n.classList.remove("hidden"),document.querySelectorAll(".sidebar-link").forEach(i=>{i.classList.remove("active","bg-zinc-700","text-white"),i.classList.add("text-zinc-400")});const a=document.querySelector(`[data-target="${e}"]`);a&&(a.classList.add("active","bg-zinc-700","text-white"),a.classList.remove("text-zinc-400"));return}console.warn("⚠️ Could not navigate to:",e)}const Fa=window.ethers,Ht="".toLowerCase(),Pl="",Ra="bkc_admin_auth_v3";window.__ADMIN_WALLET__=Ht;setTimeout(()=>{document.dispatchEvent(new CustomEvent("adminConfigReady")),console.log("✅ Admin config ready, wallet:",Ht?"configured":"not set")},100);function Hn(){return sessionStorage.getItem(Ra)==="true"}function Nl(){sessionStorage.setItem(Ra,"true")}function Ml(){return!c.isConnected||!c.userAddress||!Ht?!1:c.userAddress.toLowerCase()===Ht}const _n={pending:{text:"Pending Review",color:"text-amber-400",bgColor:"bg-amber-900/50",icon:"fa-clock"},auditing:{text:"Auditing",color:"text-blue-400",bgColor:"bg-blue-900/50",icon:"fa-magnifying-glass"},approved:{text:"Approved",color:"text-green-400",bgColor:"bg-green-900/50",icon:"fa-check-circle"},rejected:{text:"Rejected",color:"text-red-400",bgColor:"bg-red-900/50",icon:"fa-times-circle"},flagged_suspicious:{text:"Flagged",color:"text-red-300",bgColor:"bg-red-800/60",icon:"fa-flag"}},ss={faucet:{icon:"🚰",label:"Claim Faucet",points:1e3,maxCount:1,cooldownHours:0,enabled:!0},delegation:{icon:"📊",label:"Delegate BKC",points:2e3,maxCount:10,cooldownHours:24,enabled:!0},fortune:{icon:"🎰",label:"Play Fortune",points:1500,maxCount:10,cooldownHours:1,enabled:!0},buyNFT:{icon:"🛒",label:"Buy NFT",points:2500,maxCount:10,cooldownHours:0,enabled:!0},sellNFT:{icon:"💰",label:"Sell NFT",points:1500,maxCount:10,cooldownHours:0,enabled:!0},listRental:{icon:"🏷️",label:"List for Rent",points:1e3,maxCount:10,cooldownHours:0,enabled:!0},rentNFT:{icon:"⏰",label:"Rent NFT",points:2e3,maxCount:10,cooldownHours:0,enabled:!0},notarize:{icon:"📜",label:"Notarize Doc",points:2e3,maxCount:10,cooldownHours:0,enabled:!0},claimReward:{icon:"💸",label:"Claim Rewards",points:1e3,maxCount:10,cooldownHours:24,enabled:!0},unstake:{icon:"↩️",label:"Unstake",points:500,maxCount:10,cooldownHours:0,enabled:!0}};let h={allSubmissions:[],dailyTasks:[],ugcBasePoints:null,platformUsageConfig:null,editingTask:null,activeTab:"review-submissions",allUsers:[],selectedUserSubmissions:[],isSubmissionsModalOpen:!1,selectedWallet:null,usersFilter:"all",usersSearch:"",usersPage:1,usersPerPage:100,submissionsPage:1,submissionsPerPage:100,tasksPage:1,tasksPerPage:100};const ot=async()=>{var t;const e=document.getElementById("admin-content-wrapper");if(e){const s=document.createElement("div");e.innerHTML=s.innerHTML}try{c.userAddress&&(await sa(c.userAddress),console.log("✅ Firebase Auth: Admin authenticated"));const[s,n,a,i]=await Promise.all([Ui(),ji(),Ks(),Oi()]);h.allSubmissions=s,h.dailyTasks=n,h.allUsers=i,h.ugcBasePoints=((t=a.config)==null?void 0:t.ugcBasePoints)||{YouTube:5e3,"YouTube Shorts":2500,Instagram:3e3,"X/Twitter":1500,Facebook:2e3,Telegram:1e3,TikTok:3500,Reddit:1800,LinkedIn:2200,Other:1e3},h.platformUsageConfig=a.platformUsageConfig||ss,h.editingTask&&(h.editingTask=n.find(o=>o.id===h.editingTask.id)||null),nc()}catch(s){if(console.error("Error loading admin data:",s),e){const n=document.createElement("div");po(n,`Failed to load admin data: ${s.message}`),e.innerHTML=n.innerHTML}else m("Failed to load admin data.","error")}},tn=async()=>{const e=document.getElementById("presale-balance-amount");if(e){e.innerHTML='<span class="loader !w-5 !h-5 inline-block"></span>';try{if(!c.signer||!c.signer.provider)throw new Error("Admin provider not found.");if(!x.publicSale)throw new Error("PublicSale address not configured.");const t=await c.signer.provider.getBalance(x.publicSale),s=Fa.formatEther(t);e.textContent=`${parseFloat(s).toFixed(6)} ETH/BNB`}catch(t){console.error("Error loading presale balance:",t),e.textContent="Error"}}},Fl=async e=>{if(!c.signer){m("Por favor, conecte a carteira do Owner primeiro.","error");return}if(!window.confirm("Are you sure you want to withdraw ALL funds from the Presale contract to the Treasury wallet?"))return;const t=["function withdrawFunds() external"],s=x.publicSale,n=new Fa.Contract(s,t,c.signer),a=e.innerHTML;e.disabled=!0,e.innerHTML='<i class="fa-solid fa-spinner fa-spin mr-2"></i> Withdrawing...';try{console.log(`Calling withdrawFunds() on ${s}...`);const i=await n.withdrawFunds();m("Transaction sent. Awaiting confirmation...","info");const o=await i.wait();console.log("Funds withdrawn successfully!",o.hash),m("Funds withdrawn successfully!","success",o.hash),tn()}catch(i){console.error("Error withdrawing funds:",i);const o=i.reason||i.message||"Transaction failed.";m(`Error: ${o}`,"error")}finally{e.disabled=!1,e.innerHTML=a}},Rl=async e=>{const t=e.target.closest("button[data-action]");if(!t||t.disabled)return;const s=t.dataset.action,n=t.dataset.submissionId,a=t.dataset.userId;if(!s||!n||!a){console.warn("Missing data attributes for admin action:",t.dataset);return}const i=t.closest("tr"),o=t.closest("td").querySelectorAll("button");i?(i.style.opacity="0.5",i.style.pointerEvents="none"):o.forEach(r=>r.disabled=!0);try{(s==="approved"||s==="rejected")&&(await ia(a,n,s),m(`Submission ${s==="approved"?"APPROVED":"REJECTED"}!`,"success"),h.allSubmissions=h.allSubmissions.filter(r=>r.submissionId!==n),_t())}catch(r){m(`Failed to ${s} submission: ${r.message}`,"error"),console.error(r),i&&(i.style.opacity="1",i.style.pointerEvents="auto")}},Dl=async e=>{const t=e.target.closest(".ban-user-btn");if(!t||t.disabled)return;const s=t.dataset.userId,n=t.dataset.action==="ban";if(!s)return;const a=n?`Are you sure you want to PERMANENTLY BAN this user?
(This is reversible)`:`Are you sure you want to UNBAN this user?
(This will reset their rejection count to 0)`;if(!window.confirm(a))return;const i=t.innerHTML;t.disabled=!0,t.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>';try{await oa(s,n),m(`User ${n?"BANNED":"UNBANNED"}.`,"success");const o=h.allUsers.findIndex(r=>r.id===s);o>-1&&(h.allUsers[o].isBanned=n,h.allUsers[o].hasPendingAppeal=!1,n===!1&&(h.allUsers[o].rejectedCount=0)),Ce()}catch(o){m(`Failed: ${o.message}`,"error"),t.disabled=!1,t.innerHTML=i}},jl=async e=>{const t=e.target.closest(".resolve-appeal-btn");if(!t||t.disabled)return;const s=t.dataset.userId,a=t.dataset.action==="approve";if(!s)return;const i=a?"Are you sure you want to APPROVE this appeal and UNBAN the user?":"Are you sure you want to DENY this appeal? The user will remain banned.";if(!window.confirm(i))return;const o=t.closest("td").querySelectorAll("button"),r=new Map;o.forEach(l=>{r.set(l,l.innerHTML),l.disabled=!0,l.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>'});try{a&&await oa(s,!1),m(`Appeal ${a?"APPROVED":"DENIED"}.`,"success");const l=h.allUsers.findIndex(d=>d.id===s);l>-1&&(h.allUsers[l].hasPendingAppeal=!1,a&&(h.allUsers[l].isBanned=!1,h.allUsers[l].rejectedCount=0)),Ce()}catch(l){m(`Failed: ${l.message}`,"error"),o.forEach(d=>{d.disabled=!1,d.innerHTML=r.get(d)})}},Hl=async e=>{const t=e.target.closest(".re-approve-btn");if(!t||t.disabled)return;const s=t.dataset.submissionId,n=t.dataset.userId;if(!s||!n)return;const a=t.closest("tr");a&&(a.style.opacity="0.5"),t.disabled=!0,t.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>';try{await ia(n,s,"approved"),m("Submission re-approved!","success"),h.selectedUserSubmissions=h.selectedUserSubmissions.filter(o=>o.submissionId!==s),a&&a.remove();const i=h.allUsers.findIndex(o=>o.id===n);if(i>-1){const o=h.allUsers[i];o.rejectedCount=Math.max(0,(o.rejectedCount||0)-1),Ce()}if(h.selectedUserSubmissions.length===0){const o=document.querySelector("#admin-user-modal .p-6");o&&(o.innerHTML='<p class="text-zinc-400 text-center p-8">This user has no rejected submissions.</p>')}}catch(i){m(`Failed to re-approve: ${i.message}`,"error"),a&&(a.style.opacity="1"),t.disabled=!1,t.innerHTML='<i class="fa-solid fa-check"></i> Re-Approve'}},_l=async e=>{const t=e.target.closest(".view-rejected-btn");if(!t||t.disabled)return;const s=t.dataset.userId,n=t.dataset.wallet;if(s){h.selectedWallet=n,h.isSubmissionsModalOpen=!0,xs(!0,[]);try{const a=await Wi(s,"rejected");h.selectedUserSubmissions=a,xs(!1,a)}catch(a){m(`Error fetching user submissions: ${a.message}`,"error"),xs(!1,[],!0)}}},Ul=()=>{h.isSubmissionsModalOpen=!1,h.selectedUserSubmissions=[],h.selectedWallet=null;const e=document.getElementById("admin-user-modal");e&&e.remove(),document.body.style.overflow="auto"},Ol=e=>{const t=e.target.closest(".user-profile-link");if(!t)return;e.preventDefault();const s=t.dataset.userId;if(!s)return;const n=h.allUsers.find(a=>a.id===s);if(!n){m("Error: Could not find user data.","error");return}Vl(n)},Wl=()=>{const e=document.getElementById("admin-user-profile-modal");e&&e.remove(),document.body.style.overflow="auto"},ql=async e=>{e.preventDefault();const t=e.target;let s,n;try{if(s=new Date(t.startDate.value+"T00:00:00Z"),n=new Date(t.endDate.value+"T23:59:59Z"),isNaN(s.getTime())||isNaN(n.getTime()))throw new Error("Invalid date format.");if(s>=n)throw new Error("Start Date must be before End Date.")}catch(l){m(l.message,"error");return}const a={title:t.title.value.trim(),url:t.url.value.trim(),description:t.description.value.trim(),points:parseInt(t.points.value,10),cooldownHours:parseInt(t.cooldown.value,10),startDate:s,endDate:n};if(!a.title||!a.description){m("Please fill in Title and Description.","error");return}if(a.points<=0||a.cooldownHours<=0){m("Points and Cooldown must be positive numbers.","error");return}if(a.url&&!a.url.startsWith("http")){m("URL must start with http:// or https://","error");return}h.editingTask&&h.editingTask.id&&(a.id=h.editingTask.id);const i=t.querySelector('button[type="submit"]'),o=i.innerHTML;i.disabled=!0;const r=document.createElement("span");r.classList.add("inline-block"),i.innerHTML="",i.appendChild(r);try{await Hi(a),m(`Task ${a.id?"updated":"created"} successfully!`,"success"),t.reset(),h.editingTask=null,ot()}catch(l){m(`Failed to save task: ${l.message}`,"error"),console.error(l),i.disabled=!1,i.innerHTML=o}},Yl=async e=>{e.preventDefault();const t=e.target,s={YouTube:parseInt(t.youtubePoints.value,10),"YouTube Shorts":parseInt(t.youtubeShortsPoints.value,10),Instagram:parseInt(t.instagramPoints.value,10),"X/Twitter":parseInt(t.xTwitterPoints.value,10),Facebook:parseInt(t.facebookPoints.value,10),Telegram:parseInt(t.telegramPoints.value,10),TikTok:parseInt(t.tiktokPoints.value,10),Reddit:parseInt(t.redditPoints.value,10),LinkedIn:parseInt(t.linkedinPoints.value,10),Other:parseInt(t.otherPoints.value,10)};if(Object.values(s).some(o=>isNaN(o)||o<0)){m("All points must be positive numbers (or 0).","error");return}const n=t.querySelector('button[type="submit"]'),a=n.innerHTML;n.disabled=!0;const i=document.createElement("span");i.classList.add("inline-block"),n.innerHTML="",n.appendChild(i);try{await Di(s),m("UGC Base Points updated successfully!","success"),h.ugcBasePoints=s}catch(o){m(`Failed to update points: ${o.message}`,"error"),console.error(o)}finally{document.body.contains(n)&&(n.disabled=!1,n.innerHTML=a)}},Kl=e=>{const t=h.dailyTasks.find(s=>s.id===e);t&&(h.editingTask=t,ft())},Gl=async e=>{if(window.confirm("Are you sure you want to delete this task permanently?"))try{await _i(e),m("Task deleted.","success"),h.editingTask=null,ot()}catch(t){m(`Failed to delete task: ${t.message}`,"error"),console.error(t)}};function xs(e,t,s=!1){var o,r;const n=document.getElementById("admin-user-modal");n&&n.remove(),document.body.style.overflow="hidden";let a="";e?a='<div class="p-8"></div>':s?a='<p class="text-red-400 text-center p-8">Failed to load submissions.</p>':t.length===0?a='<p class="text-zinc-400 text-center p-8">This user has no rejected submissions.</p>':a=`
         ...
         <div id="admin-user-modal" class="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" style="backdrop-filter: blur(5px);">
             <div class="bg-sidebar border border-border-color rounded-xl shadow-lg w-full max-w-3xl max-h-[90vh] flex flex-col">
                 <div class="p-5 border-b border-border-color flex justify-between items-center">
                     <h2 class="text-xl font-bold text-white">Rejected Posts for ${Ve(h.selectedWallet)}</h2>
     ...
     `;document.body.insertAdjacentHTML("beforeend",i),e&&document.getElementById("admin-user-modal").querySelector(".p-8"),(o=document.getElementById("close-admin-modal-btn"))==null||o.addEventListener("click",Ul),(r=document.getElementById("modal-submissions-tbody"))==null||r.addEventListener("click",Hl)}function Vl(e){var a;const t=document.getElementById("admin-user-profile-modal");t&&t.remove(),document.body.style.overflow="hidden";const s=e.isBanned?'<span class="text-xs font-bold py-1 px-3 rounded-full bg-red-600 text-white">BANNED</span>':e.hasPendingAppeal?'<span class="text-xs font-bold py-1 px-3 rounded-full bg-yellow-600 text-white">APPEALING</span>':'<span class="text-xs font-bold py-1 px-3 rounded-full bg-green-600 text-white">ACTIVE</span>',n=`
     ...
     `;document.body.insertAdjacentHTML("beforeend",n),(a=document.getElementById("close-admin-profile-modal-btn"))==null||a.addEventListener("click",Wl)}const Xl=e=>{const t=e.target.closest(".user-filter-btn");!t||t.classList.contains("active")||(h.usersFilter=t.dataset.filter||"all",h.usersPage=1,Ce())},Jl=e=>{h.usersSearch=e.target.value,h.usersPage=1,Ce()},Ql=e=>{h.usersPage=e,Ce()},Zl=e=>{h.submissionsPage=e,_t()},ec=e=>{h.tasksPage=e,ft()},Ce=()=>{var A,$;const e=document.getElementById("manage-users-content");if(!e)return;const t=h.allUsers;if(!t)return;const n=(h.usersSearch||"").toLowerCase().trim().replace(/[^a-z0-9x]/g,""),a=h.usersFilter;let i=t;n&&(i=i.filter(T=>{var F,q;return((F=T.walletAddress)==null?void 0:F.toLowerCase().includes(n))||((q=T.id)==null?void 0:q.toLowerCase().includes(n))})),a==="banned"?i=i.filter(T=>T.isBanned):a==="appealing"&&(i=i.filter(T=>T.hasPendingAppeal===!0));const o=t.length,r=t.filter(T=>T.isBanned).length,l=t.filter(T=>T.hasPendingAppeal===!0).length,d=i.sort((T,F)=>T.hasPendingAppeal!==F.hasPendingAppeal?T.hasPendingAppeal?-1:1:T.isBanned!==F.isBanned?T.isBanned?-1:1:(F.totalPoints||0)-(T.totalPoints||0)),u=h.usersPage,p=h.usersPerPage,f=d.length,g=Math.ceil(f/p),b=(u-1)*p,v=u*p,y=d.slice(b,v),k=y.length>0?y.map(T=>{let F="border-b border-border-color hover:bg-zinc-800/50",q="";return T.hasPendingAppeal?(F+=" bg-yellow-900/40",q='<span class="ml-2 text-xs font-bold text-yellow-300">[APPEALING]</span>'):T.isBanned&&(F+=" bg-red-900/30 opacity-70",q='<span class="ml-2 text-xs font-bold text-red-400">[BANNED]</span>'),`
        <tr class="${F}">
    ...
    `;const E=document.getElementById("admin-users-pagination");E&&g>1&&Vs(E,h.usersPage,g,Ql),(A=document.getElementById("admin-users-tbody"))==null||A.addEventListener("click",T=>{T.target.closest(".user-profile-link")&&Ol(T),T.target.closest(".ban-user-btn")&&Dl(T),T.target.closest(".view-rejected-btn")&&_l(T),T.target.closest(".resolve-appeal-btn")&&jl(T)}),($=document.getElementById("user-filters-nav"))==null||$.addEventListener("click",Xl);const L=document.getElementById("user-search-input");if(L){let T;L.addEventListener("keyup",F=>{clearTimeout(T),T=setTimeout(()=>Jl(F),300)})}},Un=()=>{var n;const e=document.getElementById("manage-ugc-points-content");if(!e)return;const t=h.ugcBasePoints;if(!t)return;const s={YouTube:5e3,"YouTube Shorts":2500,Instagram:3e3,"X/Twitter":1500,Facebook:2e3,Telegram:1e3,TikTok:3500,Reddit:1800,LinkedIn:2200,Other:1e3};e.innerHTML=`
    ...
    `,(n=document.getElementById("ugcPointsForm"))==null||n.addEventListener("submit",Yl)},ft=()=>{var b,v,y;const e=document.getElementById("manage-tasks-content");if(!e)return;const t=h.editingTask,s=!!t,n=k=>{if(!k)return"";try{return(k.toDate?k.toDate():k instanceof Date?k:new Date(k)).toISOString().split("T")[0]}catch{return""}},a=h.tasksPage,i=h.tasksPerPage,o=[...h.dailyTasks].sort((k,E)=>{var $,T;const L=($=k.startDate)!=null&&$.toDate?k.startDate.toDate():new Date(k.startDate||0);return((T=E.startDate)!=null&&T.toDate?E.startDate.toDate():new Date(E.startDate||0)).getTime()-L.getTime()}),r=o.length,l=Math.ceil(r/i),d=(a-1)*i,u=a*i,p=o.slice(d,u),f=p.length>0?p.map(k=>{var T,F;const E=new Date,L=(T=k.startDate)!=null&&T.toDate?k.startDate.toDate():k.startDate?new Date(k.startDate):null,A=(F=k.endDate)!=null&&F.toDate?k.endDate.toDate():k.endDate?new Date(k.endDate):null;let $="text-zinc-500";return L&&A&&(E>=L&&E<=A?$="text-green-400":E<L&&($="text-blue-400")),`
    ...
    `;const g=document.getElementById("admin-tasks-pagination");g&&l>1&&Vs(g,h.tasksPage,l,ec),(b=document.getElementById("taskForm"))==null||b.addEventListener("submit",ql),(v=document.getElementById("cancelEditBtn"))==null||v.addEventListener("click",()=>{h.editingTask=null,ft()}),(y=document.getElementById("existing-tasks-list"))==null||y.addEventListener("click",k=>{const E=k.target.closest("button[data-id]");if(!E)return;const L=E.dataset.id;E.dataset.action==="edit"&&Kl(L),E.dataset.action==="delete"&&Gl(L)})},_t=()=>{var p;const e=document.getElementById("submissions-content");if(!e)return;if(!h.allSubmissions||h.allSubmissions.length===0){const f=document.createElement("div");e.innerHTML=f.innerHTML;return}const t=h.submissionsPage,s=h.submissionsPerPage,n=[...h.allSubmissions].sort((f,g)=>{var b,v;return(((b=g.submittedAt)==null?void 0:b.getTime())||0)-(((v=f.submittedAt)==null?void 0:v.getTime())||0)}),a=n.length,i=Math.ceil(a/s),o=(t-1)*s,r=t*s,d=n.slice(o,r).map(f=>{var g,b;return`
        <tr class="border-b border-border-color hover:bg-zinc-800/50">
            <td class="p-3 text-xs text-zinc-400 font-mono" title="${f.userId}">${Ve(f.walletAddress)}</td>
            ...
            <td class="p-3 text-xs font-semibold ${((g=_n[f.status])==null?void 0:g.color)||"text-gray-500"}">${((b=_n[f.status])==null?void 0:b.text)||f.status}</td>
            <td class="p-3 text-right">
                <div class="flex items-center justify-end gap-2">
    ...
    `;const u=document.getElementById("admin-submissions-pagination");u&&i>1&&Vs(u,h.submissionsPage,i,Zl),(p=document.getElementById("admin-submissions-tbody"))==null||p.addEventListener("click",Rl)},Ut=()=>{var i,o;const e=document.getElementById("platform-usage-content");if(!e)return;const t=h.platformUsageConfig||ss;let s=0;Object.values(t).forEach(r=>{r.enabled!==!1&&(s+=(r.points||0)*(r.maxCount||1))});const n=Object.entries(t).map(([r,l])=>`
    ...
    `;const a=document.getElementById("platform-usage-tbody");a==null||a.addEventListener("input",On),a==null||a.addEventListener("change",On),(i=document.getElementById("save-platform-config-btn"))==null||i.addEventListener("click",tc),(o=document.getElementById("reset-platform-config-btn"))==null||o.addEventListener("click",sc)},On=e=>{const t=e.target;if(!t.classList.contains("platform-input")&&!t.classList.contains("platform-toggle"))return;const s=t.closest("tr"),n=s==null?void 0:s.dataset.actionKey,a=t.dataset.field;if(!n||!a)return;h.platformUsageConfig[n]||(h.platformUsageConfig[n]={...ss[n]}),a==="enabled"?h.platformUsageConfig[n].enabled=t.checked:h.platformUsageConfig[n][a]=parseInt(t.value)||0;const i=h.platformUsageConfig[n],o=s.querySelector("td:last-child");o&&(o.textContent=((i.points||0)*(i.maxCount||1)).toLocaleString())},tc=async e=>{const t=e.target.closest("button");if(!t)return;const s=t.innerHTML;t.disabled=!0,t.innerHTML='<i class="fa-solid fa-spinner fa-spin mr-2"></i>Saving...';try{await ra(h.platformUsageConfig),m("✅ Platform Usage config saved!","success"),Ut()}catch(n){console.error("Error saving platform config:",n),m("Failed to save config: "+n.message,"error")}finally{t.disabled=!1,t.innerHTML=s}},sc=async()=>{if(confirm("Are you sure you want to reset to default values? This will save immediately."))try{h.platformUsageConfig={...ss},await ra(h.platformUsageConfig),m("✅ Config reset to defaults!","success"),Ut()}catch(e){console.error("Error resetting platform config:",e),m("Failed to reset config: "+e.message,"error")}},nc=()=>{var s;const e=document.getElementById("admin-content-wrapper");if(!e)return;e.innerHTML=`
        ...
        <div class="border-b border-border-color mb-6">
            <nav id="admin-tabs" class="-mb-px flex flex-wrap gap-x-6 gap-y-2">
                ...
                <button class="tab-btn ${h.activeTab==="platform-usage"?"active":""}" data-target="platform-usage">Platform Usage</button>
            </nav>
        </div>
        ...
        <div id="platform_usage_tab" class="tab-content ${h.activeTab==="platform-usage"?"active":""}">
            <div id="platform-usage-content" class="max-w-4xl mx-auto"></div>
        </div>
    `,(s=document.getElementById("withdraw-presale-funds-btn"))==null||s.addEventListener("click",n=>Fl(n.target)),tn(),h.activeTab==="manage-ugc-points"?Un():h.activeTab==="manage-tasks"?ft():h.activeTab==="review-submissions"?_t():h.activeTab==="manage-users"?Ce():h.activeTab==="platform-usage"&&Ut();const t=document.getElementById("admin-tabs");t&&!t._listenerAttached&&(t.addEventListener("click",n=>{const a=n.target.closest(".tab-btn");if(!a||a.classList.contains("active"))return;const i=a.dataset.target;h.activeTab=i,i!=="manage-users"&&(h.usersPage=1,h.usersFilter="all",h.usersSearch=""),i!=="review-submissions"&&(h.submissionsPage=1),i!=="manage-tasks"&&(h.tasksPage=1),document.querySelectorAll("#admin-tabs .tab-btn").forEach(r=>r.classList.remove("active")),a.classList.add("active"),e.querySelectorAll(".tab-content").forEach(r=>r.classList.remove("active"));const o=document.getElementById(i.replace(/-/g,"_")+"_tab");o?(o.classList.add("active"),i==="manage-ugc-points"&&Un(),i==="manage-tasks"&&ft(),i==="review-submissions"&&_t(),i==="manage-users"&&Ce(),i==="platform-usage"&&Ut()):console.warn(`Tab content container not found for target: ${i}`)}),t._listenerAttached=!0)},ac={render(){const e=document.getElementById("admin");if(e){if(!Ml()){e.innerHTML='<div class="text-center text-red-400 p-8 bg-sidebar border border-red-500/50 rounded-lg">Access Denied. This page is restricted to administrators.</div>';return}if(Hn()){e.innerHTML='<div id="admin-content-wrapper"></div>',ot();return}e.innerHTML=`
        ...
        <div class="bg-presale-bg-card border border-presale-border-color rounded-xl flex flex-col nft-card group overflow-hidden shadow-xl hover:shadow-amber-500/30 transition-shadow duration-300 snap-center flex-shrink-0 w-11/12 sm:w-full" data-tier-id="${e.id}">
            ...
            <div class="w-full h-48 overflow-hidden bg-presale-bg-darker relative">
                <img src="${cc(e.img)}" alt="${e.name}" class="w-full h-full object-cover nft-card-image transition-transform duration-500 group-hover:scale-105"/>
    ...
    `}function _a(){const e=document.getElementById("marketplace-grid");if(!e)return;const t=J.translations.en;if(J.nftTiers[0].priceInWei===0n){e.innerHTML=`<p class="text-lg text-amber-400 text-center col-span-full animate-pulse">${t.loadingText}</p>`,ja();return}e.innerHTML=J.nftTiers.map(Ha).join(""),ht(c.isConnected),Da("en")}const Ms={render:()=>{const e=`
        ...
        `;se.presale.innerHTML=e,Wn=!0,_a(),Ms.init(),Da("en"),c.isConnected&&Ms.update(!0)},init:()=>{const e=document.getElementById("marketplace-grid");e&&!e._listenersAttached&&(e.addEventListener("click",t=>{const s=t.target.closest(".buy-button");if(s&&!s.disabled){ic(s);return}const n=t.target.closest(".nft-card");if(!n)return;const a=n.querySelector(".quantity-input");if(!a||a.disabled)return;const i=t.target.closest(".quantity-minus"),o=t.target.closest(".quantity-plus");let r=parseInt(a.value);i&&r>1?a.value=r-1:o&&(a.value=r+1),a.dispatchEvent(new Event("input",{bubbles:!0}))}),e.addEventListener("input",t=>{const s=t.target.closest(".quantity-input");if(s){const n=s.closest(".nft-card");(parseInt(s.value)<1||isNaN(parseInt(s.value)))&&(s.value=1),sn(n)}}),e._listenersAttached=!0),oc()},update:e=>{Wn&&(e&&J.nftTiers[0].priceInWei===0n?ja():ht(e))}},vs=2e8,qn={airdrop:{amount:7e7},liquidity:{amount:13e7}},dc=()=>{if(document.getElementById("tokenomics-styles-v5"))return;const e=document.createElement("style");e.id="tokenomics-styles-v5",e.innerHTML=`
    ...
    `,document.head.appendChild(e)},rt=e=>e>=1e6?(e/1e6).toFixed(1)+"M":e>=1e3?(e/1e3).toFixed(0)+"K":e.toLocaleString();function uc(){return`
    ...
    `}function pc(){const e=c.totalSupply?B(c.totalSupply):4e7,t=(e/vs*100).toFixed(1);return`
        <div class="tk-section tk-fade-up" style="animation-delay: 0.1s">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div class="tk-card text-center">
                    ...
                    <p class="text-xl font-black text-white">${rt(vs)}</p>
            ...
            <p class="text-center text-zinc-600 text-[10px]">
                ...
                Remaining ${rt(vs-e)} BKC to be mined through ecosystem activity
            </p>
        </div>
    `}function fc(){return`
                ...
                <div class="space-y-3">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-zinc-500 text-[10px]">${rt(qn.airdrop.amount)} BKC</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-zinc-500 text-[10px]">${rt(qn.liquidity.amount)} BKC</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    `}function wc(){const e=document.getElementById("tokenomics");e&&(dc(),e.innerHTML=`
    ...
    `,document.head.appendChild(e)};function Bc(){const e=document.getElementById("notary");e&&(Cc(),e.innerHTML=`
    ...
    `,Ot(),ge(),ns(),Mc())}function Ot(){const e=document.getElementById("mobile-badge"),t=document.getElementById("desktop-badge"),s=document.getElementById("fee-amount"),n=document.getElementById("user-balance"),a=c.notaryFee||0n,i=c.currentUserBalance||0n;s&&(s.textContent=`${B(a)} BKC`),n&&(n.textContent=`${B(i)} BKC`,n.className=`font-mono font-bold ${i>=a?"text-green-400":"text-red-400"}`),c.isConnected?i>=a?(e&&(e.className="text-[10px] px-2 py-1 rounded-full bg-green-500/20 text-green-400",e.textContent="Ready"),t&&(t.innerHTML=`
            ...
            `,t.className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-zinc-800/50 text-sm"))}function Ic(){[1,2,3].forEach(s=>{const n=document.getElementById(`step-${s}`);n&&(s<P.step?(n.className="step-dot done",n.innerHTML='<i class="fa-solid fa-check text-xs"></i>'):s===P.step?(n.className="step-dot active",n.textContent=s):(n.className="step-dot pending",n.textContent=s))});const e=document.getElementById("line-1"),t=document.getElementById("line-2");e&&(e.className=`step-line mx-2 ${P.step>1?"done":""}`),t&&(t.className=`step-line mx-2 ${P.step>2?"done":""}`)}function ge(){const e=document.getElementById("action-panel");if(!e)return;if(Ic(),Ec(P.step),!c.isConnected){e.innerHTML=`
        ...
        `;return}switch(P.step){case 1:$c(e);break;case 2:Lc(e);break;case 3:Pc(e);break}}function Ec(e){const t=document.getElementById("notary-mascot"),s=document.getElementById("notary-mascot-mobile");[t,s].forEach(n=>{if(n)switch(n.className="w-full h-full object-contain",e){case 1:n.classList.add("notary-float","notary-pulse");break;case 2:n.classList.add("notary-float");break;case 3:n.classList.add("notary-pulse");break}})}function $c(e){e.innerHTML=`
    ...
    `,Ac()}function Ac(){const e=document.getElementById("dropzone"),t=document.getElementById("file-input");!e||!t||(e.onclick=()=>t.click(),["dragenter","dragover","dragleave","drop"].forEach(s=>{e.addEventListener(s,n=>{n.preventDefault(),n.stopPropagation()})}),e.addEventListener("dragenter",()=>e.classList.add("drag-over")),e.addEventListener("dragover",()=>e.classList.add("drag-over")),e.addEventListener("dragleave",()=>e.classList.remove("drag-over")),e.addEventListener("drop",s=>{var n,a;e.classList.remove("drag-over"),Yn((a=(n=s.dataTransfer)==null?void 0:n.files)==null?void 0:a[0])}),t.addEventListener("change",s=>{var n;return Yn((n=s.target.files)==null?void 0:n[0])}))}function Yn(e){if(e){if(e.size>kc){m("File too large (max 5MB)","error");return}P.file=e,P.step=2,ge()}}function Lc(e){var a,i,o;const t=P.file,s=t?(t.size/1024).toFixed(1):"0",n=Oa((t==null?void 0:t.type)||"");e.innerHTML=`
    ...
    `,(a=document.getElementById("btn-remove"))==null||a.addEventListener("click",()=>{P.file=null,P.description="",P.step=1,ge()}),(i=document.getElementById("btn-back"))==null||i.addEventListener("click",()=>{P.step=1,ge()}),(o=document.getElementById("btn-next"))==null||o.addEventListener("click",()=>{const r=document.getElementById("desc-input");P.description=(r==null?void 0:r.value)||"",P.step=3,ge()})}function Ua(e="",t=""){const s=e.toLowerCase(),n=t.toLowerCase();return s.includes("image")||/\.(jpg|jpeg|png|gif|webp|svg|bmp|ico)$/.test(n)?ve.image:s.includes("pdf")||n.endsWith(".pdf")?ve.pdf:s.includes("audio")||/\.(mp3|wav|ogg|flac|aac|m4a)$/.test(n)?ve.audio:s.includes("video")||/\.(mp4|avi|mov|mkv|webm|wmv)$/.test(n)?ve.video:s.includes("word")||s.includes("document")||/\.(doc|docx|odt|rtf)$/.test(n)?ve.document:s.includes("sheet")||s.includes("excel")||/\.(xls|xlsx|csv|ods)$/.test(n)?ve.spreadsheet:/\.(js|ts|py|java|cpp|c|h|html|css|json|xml|sol|rs|go|php|rb)$/.test(n)?ve.code:s.includes("zip")||s.includes("archive")||/\.(zip|rar|7z|tar|gz)$/.test(n)?ve.archive:ve.default}function Oa(e){return Ua(e).icon}function Sc(e=""){const t=e.toLowerCase();return t.includes("image")?"image":t.includes("pdf")?"pdf":t.includes("audio")?"audio":t.includes("video")?"video":t.includes("word")||t.includes("document")?"document":t.includes("sheet")||t.includes("excel")?"spreadsheet":t.includes("zip")||t.includes("archive")?"archive":"document"}function Pc(e){var a,i;const t=P.file,s=P.description||"No description",n=c.notaryFee||Ue.parseEther("1");e.innerHTML=`
            ...
            <div class="bg-zinc-800/50 border border-zinc-700 rounded-xl p-4 mb-4 text-left">
                <div class="flex items-center gap-3 pb-3 border-b border-zinc-700/50 mb-3">
                    <div class="w-10 h-10 rounded-lg bg-amber-500/10 flex items-center justify-center">
                        <i class="${Oa((t==null?void 0:t.type)||"")} text-amber-400"></i>
                    </div>
                    <div class="flex-1 min-w-0">
    ...
    `,(a=document.getElementById("btn-back"))==null||a.addEventListener("click",()=>{P.step=2,ge()}),(i=document.getElementById("btn-mint"))==null||i.addEventListener("click",Nc)}async function Nc(){var o,r;if(P.isProcessing)return;P.isProcessing=!0;const e=document.getElementById("btn-mint");e&&(e.disabled=!0,e.innerHTML='<i class="fa-solid fa-spinner fa-spin mr-2"></i> Signing...');const t=document.getElementById("processing-overlay"),s=document.getElementById("process-status"),n=document.getElementById("process-bar"),a=document.getElementById("notary-overlay-img"),i=(l,d)=>{n&&(n.style.width=`${l}%`),s&&(s.textContent=d)};try{const u=await(await c.provider.getSigner()).signMessage("I am signing to authenticate my file for notarization on Backchain.");t&&(t.classList.remove("hidden"),t.classList.add("flex")),i(10,"UPLOADING TO IPFS...");const p=new FormData;p.append("file",P.file),p.append("signature",u),p.append("address",c.userAddress),p.append("description",P.description||"No description");const f=oe.uploadFileToIPFS||"/api/upload",g=await fetch(f,{method:"POST",body:p,signal:AbortSignal.timeout(18e4)});if(!g.ok){if(g.status===413)throw new Error("File too large. Maximum size is 5MB.");if(g.status===401)throw new Error("Signature verification failed. Please try again.");if(g.status===500){const E=await g.json().catch(()=>({}));throw new Error(E.details||"Server error during upload.")}throw new Error(`Upload failed (${g.status})`)}const b=await g.json(),v=b.ipfsUri||b.metadataUri,y=b.contentHash;if(!v)throw new Error("No IPFS URI returned");if(!y)throw new Error("No content hash returned");if(i(50,"MINTING ON BLOCKCHAIN..."),a&&(a.className="w-full h-full object-contain notary-stamp"),await Io({ipfsUri:v,contentHash:y,title:((o=P.file)==null?void 0:o.name)||"Untitled Document",description:P.description||"No description",docType:Sc((r=P.file)==null?void 0:r.type)||"document",tags:[]},e))i(100,"SUCCESS!"),a&&(a.className="w-full h-full object-contain notary-success"),t&&(t.innerHTML=`
                ...
                `),setTimeout(()=>{t&&(t.classList.add("hidden"),t.classList.remove("flex")),P.file=null,P.description="",P.step=1,P.isProcessing=!1,ge(),ns(),V(!0),m("📜 Document notarized successfully!","success")},3e3);else throw new Error("Minting failed")}catch(l){console.error("Notary Error:",l),t&&(t.classList.add("hidden"),t.classList.remove("flex")),e&&(e.disabled=!1,e.innerHTML='<i class="fa-solid fa-stamp mr-2"></i> Sign & Mint'),P.isProcessing=!1,m(l.message||"Notarization failed","error")}}async function ns(){const e=document.getElementById("certificates-grid");if(e){if(!c.isConnected){e.innerHTML=`
    ...
    `;try{const s=`${oe.getNotarizedDocuments||"https://getnotarizeddocuments-4wvdcuoouq-uc.a.run.app"}/${c.userAddress}`;console.log("📜 Fetching certificates from Firebase:",s);const n=await fetch(s);if(!n.ok)throw new Error(`API returned ${n.status}`);const a=await n.json();if(!Array.isArray(a)||a.length===0){e.innerHTML=`
                <div class="col-span-full text-center py-8">
            ...
            `;return}const i=a.map(r=>({id:r.tokenId||"?",ipfs:r.ipfsCid||"",description:r.description||"",hash:r.contentHash||"",timestamp:r.createdAt||r.timestamp||"",txHash:r.txHash||"",blockNumber:r.blockNumber||0}));console.log(`📜 Found ${i.length} certificates in Firebase`);const o=i.sort((r,l)=>parseInt(l.id)-parseInt(r.id));e.innerHTML=o.map(r=>{var b;let l="";const d=r.ipfs||"";d.startsWith("ipfs://")?l=`https://gateway.pinata.cloud/ipfs/${d.replace("ipfs://","")}`:d.startsWith("https://")?l=d:d.length>0&&(l=`https://gateway.pinata.cloud/ipfs/${d}`);let u=r.description||"";u=u.split("---")[0].trim()||u,u=u.split(`
`)[0].trim()||"Notarized Document";const p=Ua("",u),f=l&&l.length>10,g=zc(r.timestamp);return`
                <div class="cert-card bg-zinc-900/50 border border-zinc-800/50 rounded-xl overflow-hidden hover:border-amber-500/30 transition-all">
                    ...
                    <div class="p-3">
                        ...
                        <p class="text-[9px] font-mono text-zinc-600 truncate mb-3" title="${r.hash}">
                            SHA-256: ${((b=r.hash)==null?void 0:b.slice(0,16))||"..."}...
                        </p>
                        ...
                        <div class="flex items-center justify-between pt-2 border-t border-zinc-800/50">
                            ...
                            ${r.txHash?`
                                <a href="${Tc}${r.txHash}" target="_blank" 
        ...
        `}}}function Mc(){var e;(e=document.getElementById("btn-refresh"))==null||e.addEventListener("click",async()=>{const t=document.getElementById("btn-refresh");t&&(t.innerHTML='<i class="fa-solid fa-rotate fa-spin"></i> Loading...'),await ns(),t&&(t.innerHTML='<i class="fa-solid fa-rotate"></i> Refresh')})}async function Fc(){const e=Date.now();if(!(e-P.lastFetch<3e4&&c.notaryFee>0n))try{!c.ecosystemManagerContractPublic&&!c.ecosystemManagerContract&&await qs();const t=c.ecosystemManagerContractPublic||c.ecosystemManagerContract;if(t){let s=0n;try{const n=Ue.id("NOTARY_SERVICE");s=await O(t,"getFee",[n],0n)}catch(n){console.warn("getFee with key failed:",n.message);try{s=await O(t,"notaryFee",[],0n)}catch(a){console.warn("notaryFee failed:",a.message);try{const i=Ue.id("NOTARY_SERVICE");s=await O(t,"fees",[i],0n)}catch(i){console.warn("fees mapping failed:",i.message)}}}s>0n?(c.notaryFee=s,console.log("📜 Notary fee loaded:",B(s),"BKC")):(c.notaryFee=Ue.parseEther("1"),console.log("📜 Using default notary fee: 1 BKC")),P.lastFetch=e}if(c.isConnected&&c.userAddress)try{if(c.bkcTokenContract||c.bkcTokenContractPublic){const s=c.bkcTokenContract||c.bkcTokenContractPublic,n=await O(s,"balanceOf",[c.userAddress],0n);n>0n&&(c.currentUserBalance=n,console.log("📜 User balance loaded:",B(n),"BKC"))}}catch(s){console.warn("Balance load error:",s.message)}Ot()}catch(t){console.warn("Notary data error:",t),(!c.notaryFee||c.notaryFee===0n)&&(c.notaryFee=Ue.parseEther("1"))}}const Wa={async render(e){e&&(Bc(),await Fc(),c.isConnected&&await V(),Ot(),ge())},reset(){P.file=null,P.description="",P.step=1,ge()},update(){Ot(),P.isProcessing||ge()},refreshHistory(){ns()},async addToWallet(e,t){var s,n,a,i;try{const o=["https://dweb.link/ipfs/","https://w3s.link/ipfs/","https://nftstorage.link/ipfs/","https://cloudflare-ipfs.com/ipfs/","https://ipfs.io/ipfs/"],r=p=>p?p.startsWith("ipfs://")?p.replace("ipfs://","").split("?")[0]:p.includes("/ipfs/")?p.split("/ipfs/")[1].split("?")[0]:p.match(/^Qm[a-zA-Z0-9]{44}/)||p.match(/^bafy[a-zA-Z0-9]+/)?p:"":"",l=(p,f=0)=>{if(!p)return"";if(p.startsWith("https://")&&!p.includes("/ipfs/")&&!p.includes("ipfs.io"))return p;const g=r(p);return g?`${o[f]}${g}`:p};let d=l(t||"");if(console.log("📜 Input imageUrl:",t),console.log("📜 Converted to:",d),c.decentralizedNotaryContract)try{const p=await c.decentralizedNotaryContract.tokenURI(e);if(console.log("📜 TokenURI response:",p==null?void 0:p.slice(0,200)),p&&p.startsWith("data:application/json;base64,")){const f=p.replace("data:application/json;base64,",""),g=JSON.parse(atob(f));console.log("📜 Parsed metadata:",JSON.stringify(g).slice(0,300)),g.image&&(d=l(g.image),console.log("📜 Image from metadata:",d))}}catch(p){console.warn("Could not fetch tokenURI:",p.message)}console.log("📜 Final image URL for wallet:",d);const u=((s=c.decentralizedNotaryContract)==null?void 0:s.target)||((n=c.decentralizedNotaryContract)!=null&&n.getAddress?await c.decentralizedNotaryContract.getAddress():null);if(!u){m("Contract address not found","error");return}console.log("📜 Contract address:",u),console.log("📜 Token ID:",e),m("Adding NFT #"+e+" to wallet...","info");try{await window.ethereum.request({method:"wallet_watchAsset",params:{type:"ERC721",options:{address:u,tokenId:String(e),image:d}}})?m("📜 NFT #"+e+" added to wallet!","success"):m("NFT not added (cancelled)","warning");return}catch(p){if(console.warn("First attempt with image failed:",(a=p.message)==null?void 0:a.slice(0,100)),p.code!==4001)try{await window.ethereum.request({method:"wallet_watchAsset",params:{type:"ERC721",options:{address:u,tokenId:String(e)}}})?m("📜 NFT #"+e+" added to wallet!","success"):m("NFT not added (cancelled)","warning");return}catch(f){throw f}throw p}}catch(o){if(console.error("Add to wallet error:",o),o.code===4001)return;o.code===4100||(i=o.message)!=null&&i.includes("spam")?m("MetaMask blocked request. Wait a few seconds and try again.","warning"):m("Could not add NFT: "+(o.message||"Unknown error"),"error")}}};window.NotaryPage=Wa;const mt=window.ethers,De="./assets/airbnft.png",Rc="https://sepolia.arbiscan.io/tx/",j={activeTab:"marketplace",filterTier:"ALL",selectedRentalId:null,isLoading:!1,isTransactionPending:!1,rentalHistory:[]};function wt(e){return e?e.startsWith("https://")||e.startsWith("http://")?e:e.includes("ipfs.io/ipfs/")?`${Y}${e.split("ipfs.io/ipfs/")[1]}`:e.startsWith("ipfs://")?`${Y}${e.substring(7)}`:e:"./assets/nft.png"}function qa(e){const t=Math.floor(Date.now()/1e3),s=e-t;if(s<=0)return{text:"Expired",expired:!0};const n=Math.floor(s/3600),a=Math.floor(s%3600/60);return n>0?{text:`${n}h ${a}m`,expired:!1}:{text:`${a}m`,expired:!1}}function je(e){return Z.find(s=>s.boostBips===Number(e))||{name:"Unknown",img:"./assets/bkc_logo_3d.png",boostBips:0}}function Ya(e){return{Diamond:"from-cyan-500 to-blue-600",Platinum:"from-slate-300 to-gray-500",Gold:"from-yellow-400 to-amber-600",Silver:"from-gray-300 to-zinc-500",Bronze:"from-orange-500 to-amber-700"}[e]||"from-zinc-500 to-zinc-700"}function Dc(e){if(!e)return"";try{const t=e.seconds||e._seconds||new Date(e).getTime()/1e3;return new Date(t*1e3).toLocaleString("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return""}}function jc(e="Loading..."){return`
        <div class="flex flex-col items-center justify-center py-16 gap-5">
            <div class="relative">
                ...
                <div class="relative w-24 h-24 rounded-full bg-gradient-to-br from-zinc-800 to-zinc-900 flex items-center justify-center shadow-xl shadow-green-500/20 overflow-hidden border-2 border-green-500/30">
                    <img src="${De}" alt="Loading" class="w-20 h-20 object-contain animate-pulse drop-shadow-lg" onerror="this.src='./assets/nft.png'">
                </div>
            </div>
            <div class="text-center">
    ...
    `}function Hc(){if(document.getElementById("rental-styles-v8"))return;const e=document.createElement("style");e.id="rental-styles-v8",e.innerHTML=`
    ...
    `,document.head.appendChild(e)}const _c={async render(e=!1){Hc();const t=document.getElementById("rental");t&&((t.innerHTML.trim()===""||e)&&(t.innerHTML=`
                ...
                ${ed()}
                ${td()}
            `,sd()),await yt())},update(){j.isLoading||Wt()}};function Ka(){const e=c.rentalListings||[],t=e.filter(l=>{var d,u;return c.isConnected&&((d=l.owner)==null?void 0:d.toLowerCase())===((u=c.userAddress)==null?void 0:u.toLowerCase())}),s=Math.floor(Date.now()/1e3),n=(c.myRentals||[]).filter(l=>{var d,u;return((d=l.tenant)==null?void 0:d.toLowerCase())===((u=c.userAddress)==null?void 0:u.toLowerCase())&&Number(l.endTime)>s}),a=t.reduce((l,d)=>l+Number(mt.formatEther(BigInt(d.totalEarnings||0))),0),i=e.filter(l=>{var d,u;return!(c.isConnected&&((d=l.owner)==null?void 0:d.toLowerCase())===((u=c.userAddress)==null?void 0:u.toLowerCase())||l.isRented||l.rentalEndTime&&Number(l.rentalEndTime)>s)}),o=i.length>0?Math.min(...i.map(l=>parseFloat(mt.formatEther(l.pricePerHour||"0")))):0,r=t.reduce((l,d)=>l+Number(d.rentalCount||0),0);return`
    ...
    `}function Wt(){const e=document.getElementById("tab-content");if(!e)return;switch(e.classList.remove("animate-fadeInUp"),e.offsetWidth,e.classList.add("animate-fadeInUp"),Uc(j.activeTab),j.activeTab){case"marketplace":e.innerHTML=Wc();break;case"my-listings":e.innerHTML=Yc();break;case"my-rentals":e.innerHTML=Gc();break;case"history":e.innerHTML=Jc();break}const t=document.getElementById("stats-dashboard");t&&(t.innerHTML=Ka()),Oc()}function Uc(e){const t=document.getElementById("airbnft-mascot");if(t)switch(t.className="w-16 h-16 object-contain",e){case"marketplace":t.classList.add("airbnft-float","airbnft-pulse");break;case"my-listings":t.classList.add("airbnft-orbit");break;case"my-rentals":t.classList.add("airbnft-float");break;case"history":t.classList.add("airbnft-pulse");break}}function Oc(){const t=(c.rentalListings||[]).filter(o=>{var r,l;return c.isConnected&&((r=o.owner)==null?void 0:r.toLowerCase())===((l=c.userAddress)==null?void 0:l.toLowerCase())}),s=Math.floor(Date.now()/1e3),n=(c.myRentals||[]).filter(o=>{var r,l;return((r=o.tenant)==null?void 0:r.toLowerCase())===((l=c.userAddress)==null?void 0:l.toLowerCase())&&Number(o.endTime)>s}),a=document.getElementById("my-listings-badge"),i=document.getElementById("my-rentals-badge");a&&(a.textContent=t.length),i&&(i.textContent=n.length)}function Wc(){const e=c.rentalListings||[],t=Math.floor(Date.now()/1e3),s=e.filter(n=>{var a,i;return!(c.isConnected&&((a=n.owner)==null?void 0:a.toLowerCase())===((i=c.userAddress)==null?void 0:i.toLowerCase())||n.isRented||n.rentalEndTime&&Number(n.rentalEndTime)>t||j.filterTier!=="ALL"&&je(n.boostBips).name!==j.filterTier)});return s.sort((n,a)=>{const i=BigInt(n.pricePerHour||0),o=BigInt(a.pricePerHour||0);return i<o?-1:i>o?1:0}),`
        <div>
            ...
            ${s.length===0?`
                <div class="text-center py-20 glass-card">
                    <img src="${De}" class="w-20 h-20 mx-auto opacity-30 mb-5" onerror="this.style.display='none'">
    ...
    `}function qc(e){const t=je(e.boostBips),s=B(BigInt(e.pricePerHour||0)).toFixed(2),n=`tier-${t.name.toLowerCase()}`,a=Ya(t.name),i=e.img||t.img||"./assets/nft.png";return`
        <div class="nft-card">
            <div class="nft-image">
                ...
                <img src="${wt(i)}" alt="${t.name}" onerror="this.src='./assets/nft.png'">
            </div>
            <div class="p-5">
        ...
        <div>
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    <div class="flex items-center gap-6">
                        ...
                        <div>
                            ...
                            <p class="text-3xl font-bold text-white">${t.reduce((i,o)=>i+Number(mt.formatEther(BigInt(o.totalEarnings||0))),0).toFixed(4)} <span class="text-lg text-zinc-500">BKC</span></p>
            ...
            ${t.length===0?`
                <div class="text-center py-16 glass-card">
                    <img src="${De}" class="w-16 h-16 mx-auto opacity-20 mb-4" onerror="this.style.display='none'">
    ...
    `}function Kc(e){const t=je(e.boostBips),s=B(BigInt(e.pricePerHour||0)).toFixed(2),n=Number(mt.formatEther(BigInt(e.totalEarnings||0))).toFixed(4),a=e.rentalCount||0,i=`tier-${t.name.toLowerCase()}`,o=Math.floor(Date.now()/1e3),r=e.isRented||e.rentalEndTime&&Number(e.rentalEndTime)>o,l=r&&e.rentalEndTime?qa(Number(e.rentalEndTime)):null;return`
        <div class="nft-card ${r?"border-amber-500/30":""}">
            <div class="nft-image">
                ...
                <img src="${wt(e.img||t.img)}" alt="${t.name}" onerror="this.src='./assets/nft.png'">
            </div>
            <div class="p-5">
    ...
    `}function Vc(e){const t=(c.rentalListings||[]).find(o=>o.tokenId===e.tokenId),s=je((t==null?void 0:t.boostBips)||0),n=qa(Number(e.endTime)),a=B(BigInt(e.paidAmount||0)).toFixed(2),i=`tier-${s.name.toLowerCase()}`;return Ya(s.name),`
        <div class="nft-card border-green-500/30">
            <div class="nft-image bg-gradient-to-br from-green-500/5 to-transparent">
                ...
                <img src="${wt((t==null?void 0:t.img)||s.img)}" alt="${s.name}" onerror="this.src='./assets/nft.png'">
            </div>
            <div class="p-5">
                <div class="flex justify-between items-start mb-3">
                    <div>
    ...
    `}function Xc(e){const t=(c.rentalListings||[]).find(i=>i.tokenId===e.tokenId),s=je((t==null?void 0:t.boostBips)||0),n=B(BigInt(e.paidAmount||0)).toFixed(2),a=`tier-${s.name.toLowerCase()}`;return`
        <div class="flex items-center gap-4 glass-card p-4">
            <img src="${wt((t==null?void 0:t.img)||s.img)}" class="w-12 h-12 rounded-xl object-contain bg-black/30" onerror="this.src='./assets/nft.png'">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1">
    ...
    `}function Jc(){return c.isConnected?(Qc(),j.rentalHistory.length===0?`
            <div class="text-center py-16 glass-card">
                <img src="${De}" class="w-20 h-20 mx-auto opacity-20 mb-4" onerror="this.style.display='none'">
        ...
        <div class="space-y-2 max-h-[500px] overflow-y-auto">
            ${j.rentalHistory.slice(0,20).map(e=>Zc(e)).join("")}
        </div>
    `):nn("View your rental history")}async function Qc(){if(c.userAddress)try{const e=oe.getHistory||"https://gethistory-4wvdcuoouq-uc.a.run.app",t=await fetch(`${e}/${c.userAddress}`);if(t.ok){const s=await t.json();j.rentalHistory=(s||[]).filter(n=>{const a=(n.type||"").toUpperCase();return a.includes("RENTAL")||a.includes("LIST")||a.includes("RENT")||a.includes("WITHDRAW")})}}catch(e){console.error("History load error:",e)}}function Zc(e){const t=(e.type||"").toUpperCase(),s=e.details||{},n=Dc(e.timestamp||e.createdAt);let a,i,o,r,l="";t.includes("LIST")?(a="fa-tag",i="#22c55e",o="rgba(34,197,94,0.15)",r="🏷️ Listed",s.pricePerHour&&(l=`<span class="ml-2 text-[10px] text-green-400">${B(BigInt(s.pricePerHour)).toFixed(2)} BKC/hr</span>`)):t.includes("RENT")&&!t.includes("WITHDRAW")?(a="fa-clock",i="#f59e0b",o="rgba(245,158,11,0.15)",r="⏰ Rented",s.duration&&(l=`<span class="ml-2 text-[10px] text-amber-400">${(Number(s.duration)/3600).toFixed(1)}h</span>`)):t.includes("WITHDRAW")?(a="fa-rotate-left",i="#ef4444",o="rgba(239,68,68,0.15)",r="↩️ Withdrawn"):(a="fa-circle",i="#71717a",o="rgba(39,39,42,0.5)",r=e.type||"Activity");const d=e.txHash?`${Rc}${e.txHash}`:"#";let u=e.amount||s.amount||"0";const p=B(BigInt(u)),f=p>.001?p.toFixed(2):"",g=s.tokenId||e.tokenId||"";return`
    ...
    `}function nn(e){return`
        <div class="text-center py-20 glass-card">
            <img src="${De}" class="w-20 h-20 mx-auto opacity-20 mb-5" onerror="this.style.display='none'">
    ...
    `}async function yt(){j.isLoading=!0;try{await Promise.all([gt(),c.isConnected?Ii():Promise.resolve(),c.isConnected?Qt():Promise.resolve()]),Wt()}catch(e){console.error("Refresh error:",e)}finally{j.isLoading=!1}}function sd(){var e,t,s,n,a,i,o;document.querySelectorAll(".rental-tab").forEach(r=>{r.addEventListener("click",()=>{document.querySelectorAll(".rental-tab").forEach(l=>l.classList.remove("active")),r.classList.add("active"),j.activeTab=r.dataset.tab,Wt()})}),document.addEventListener("click",r=>{const l=r.target.closest(".filter-chip");l&&(j.filterTier=l.dataset.filter,Wt())}),(e=document.getElementById("btn-refresh-rentals"))==null||e.addEventListener("click",async()=>{const r=document.getElementById("btn-refresh-rentals"),l=r==null?void 0:r.querySelector("i");l==null||l.classList.add("fa-spin"),await yt(),setTimeout(()=>l==null?void 0:l.classList.remove("fa-spin"),500)}),document.addEventListener("click",r=>{const l=r.target.closest(".rent-btn");if(l&&!l.disabled){nd(l.dataset.id);return}const d=r.target.closest(".withdraw-btn");if(d&&!d.disabled){ld(d);return}const u=r.target.closest("#btn-open-list-modal, #btn-open-list-modal-empty, #btn-open-list-modal-main");if(u&&!u.disabled){ad();return}}),(t=document.getElementById("close-rent-modal"))==null||t.addEventListener("click",Fs),(s=document.getElementById("close-list-modal"))==null||s.addEventListener("click",Rs),(n=document.getElementById("rent-modal"))==null||n.addEventListener("click",r=>{r.target.id==="rent-modal"&&Fs()}),(a=document.getElementById("list-modal"))==null||a.addEventListener("click",r=>{r.target.id==="list-modal"&&Rs()}),(i=document.getElementById("confirm-rent-btn"))==null||i.addEventListener("click",id),(o=document.getElementById("confirm-list-btn"))==null||o.addEventListener("click",rd)}function nd(e){if(!c.isConnected){m("Please connect your wallet first","warning");return}const t=(c.rentalListings||[]).find(d=>d.tokenId===e);if(!t){m("Listing not found","error");return}j.selectedRentalId=e;const s=je(t.boostBips),n=BigInt(t.pricePerHour||0),a=B(n).toFixed(2),i=`tier-${s.name.toLowerCase()}`,o=document.getElementById("rent-modal-content"),r=document.getElementById("modal-total-cost");o.innerHTML=`
        <div class="flex items-center gap-4 bg-zinc-800/50 p-4 rounded-xl border border-zinc-700/50">
            <img src="${wt(t.img||s.img)}" class="w-20 h-20 object-contain bg-black/30 rounded-xl" onerror="this.src='./assets/nft.png'">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-2">
                    <span class="tier-badge ${i}">${s.name}</span>
    ...
    `,r.innerHTML=`${a} <span class="text-sm text-zinc-500 font-normal">BKC</span>`;const l=document.getElementById("rent-modal");l.classList.remove("hidden"),l.classList.add("flex")}function Fs(){const e=document.getElementById("rent-modal");e.classList.remove("flex"),e.classList.add("hidden"),j.selectedRentalId=null}function ad(){const e=c.rentalListings||[],t=new Set(e.map(i=>{var o;return(o=i.tokenId)==null?void 0:o.toString()})),s=(c.myBoosters||[]).filter(i=>{var o;return!t.has((o=i.tokenId)==null?void 0:o.toString())}),n=document.getElementById("list-nft-select");n&&(s.length===0?n.innerHTML='<option value="">No NFTs available to list</option>':n.innerHTML=s.map(i=>{const o=je(i.boostBips);return`<option value="${i.tokenId}">#${i.tokenId} - ${o.name} (+${(i.boostBips||0)/100}%)</option>`}).join("")),document.getElementById("list-price-input").value="";const a=document.getElementById("list-modal");a.classList.remove("hidden"),a.classList.add("flex")}function Rs(){const e=document.getElementById("list-modal");e.classList.remove("flex"),e.classList.add("hidden")}async function id(){if(j.isTransactionPending)return;const e=j.selectedRentalId,t=(c.rentalListings||[]).find(l=>l.tokenId===e);if(!t)return;const s=document.getElementById("confirm-rent-btn"),n=document.getElementById("airbnft-mascot"),a=s.innerHTML,i=1,r=BigInt(t.pricePerHour||0)*BigInt(i);j.isTransactionPending=!0,s.innerHTML='<i class="fa-solid fa-spinner fa-spin mr-2"></i>Processing...',s.disabled=!0,n&&(n.className="w-16 h-16 object-contain airbnft-spin");try{await zo({tokenId:e,hours:i,totalCost:r.toString()},s)&&(n&&(n.className="w-16 h-16 object-contain airbnft-success"),Fs(),m("⏰ NFT rented successfully! Boost is now active for 1 hour.","success"),od(5),await yt())}finally{j.isTransactionPending=!1,s.innerHTML=a,s.disabled=!1,n&&setTimeout(()=>{n.className="w-16 h-16 object-contain airbnft-float airbnft-pulse"},800)}}function od(e){document.querySelectorAll(".rent-btn").forEach(s=>{s.disabled=!0,s.classList.add("opacity-50","cursor-not-allowed");let n=e;const a=s.innerHTML,i=setInterval(()=>{s.innerHTML=`<i class="fa-solid fa-clock mr-1"></i>${n}s`,n--,n<0&&(clearInterval(i),s.innerHTML=a,s.disabled=!1,s.classList.remove("opacity-50","cursor-not-allowed"))},1e3)})}async function rd(){if(j.isTransactionPending)return;const e=document.getElementById("list-nft-select").value,t=document.getElementById("list-price-input").value;if(!e)return m("Please select an NFT","error");if(!t||parseFloat(t)<=0)return m("Please enter a valid price","error");const s=document.getElementById("confirm-list-btn"),n=document.getElementById("airbnft-mascot"),a=s.innerHTML;j.isTransactionPending=!0,s.innerHTML='<i class="fa-solid fa-spinner fa-spin mr-2"></i>Processing...',s.disabled=!0,n&&(n.className="w-16 h-16 object-contain airbnft-spin");try{const i=mt.parseUnits(t,18),o=document.getElementById("list-min-hours"),r=document.getElementById("list-max-hours"),l=o&&parseInt(o.value)||1,d=r&&parseInt(r.value)||168;await To({tokenId:e,pricePerHour:i,minHours:l,maxHours:d},s)&&(n&&(n.className="w-16 h-16 object-contain airbnft-success"),Rs(),m("🏷️ NFT listed successfully!","success"),await yt())}finally{j.isTransactionPending=!1,s.innerHTML=a,s.disabled=!1,n&&setTimeout(()=>{n.className="w-16 h-16 object-contain airbnft-float airbnft-pulse"},800)}}async function ld(e){if(j.isTransactionPending)return;const t=e.dataset.id;if(!confirm("Are you sure you want to withdraw this NFT from the rental market?"))return;const s=e.innerHTML,n=document.getElementById("airbnft-mascot");j.isTransactionPending=!0,e.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>',e.disabled=!0,n&&(n.className="w-16 h-16 object-contain airbnft-spin");try{await Co(t,e)&&(m("↩️ NFT withdrawn successfully!","success"),await yt())}finally{j.isTransactionPending=!1,e.innerHTML=s,e.disabled=!1,n&&setTimeout(()=>{n.className="w-16 h-16 object-contain airbnft-float airbnft-pulse"},800)}}const cd={render:async e=>{const t=document.getElementById("socials");if(!t||!e&&t.innerHTML.trim()!=="")return;const s=`
            <style>
        ...
        `},cleanup:()=>{}},Ga=document.createElement("style");Ga.innerHTML=`
...
`;document.head.appendChild(Ga);const dd={render:async e=>{if(!e)return;const t=document.getElementById("creditcard");t&&(t.innerHTML=`
    ...
    `}const M={state:{tokens:{ETH:{name:"Ethereum",symbol:"ETH",balance:5.45,price:3050,logo:"https://cryptologos.cc/logos/ethereum-eth-logo.png?v=026"},USDT:{name:"Tether USD",symbol:"USDT",balance:12500,price:1,logo:"https://cryptologos.cc/logos/tether-usdt-logo.png?v=026"},ARB:{name:"Arbitrum",symbol:"ARB",balance:2450,price:1.1,logo:"https://cryptologos.cc/logos/arbitrum-arb-logo.png?v=026"},WBTC:{name:"Wrapped BTC",symbol:"WBTC",balance:.15,price:62e3,logo:"https://cryptologos.cc/logos/wrapped-bitcoin-wbtc-logo.png?v=026"},MATIC:{name:"Polygon",symbol:"MATIC",balance:5e3,price:.85,logo:"https://cryptologos.cc/logos/polygon-matic-logo.png?v=026"},BNB:{name:"BNB Chain",symbol:"BNB",balance:12.5,price:580,logo:"https://cryptologos.cc/logos/bnb-bnb-logo.png?v=026"},BKC:{name:"Backcoin",symbol:"BKC",balance:1500,price:.12,logo:"assets/bkc_logo_3d.png",isNative:!0}},settings:{slippage:.5,deadline:20},swap:{tokenIn:"ETH",tokenOut:"BKC",amountIn:"",loading:!1},mining:{rate:.05}},calculate:()=>{const{tokens:e,swap:t,mining:s}=M.state;if(!t.amountIn||parseFloat(t.amountIn)===0)return null;const n=e[t.tokenIn],a=e[t.tokenOut],o=parseFloat(t.amountIn)*n.price,r=o*.003,l=o-r,d=e.BKC.price,u=r*s.rate/d,p=l/a.price,f=Math.min(o/1e5*100,5).toFixed(2);return{amountOut:p,usdValue:o,feeUsd:r,miningReward:u,priceImpact:f,rate:n.price/a.price}},render:async e=>{if(!e)return;const t=document.getElementById("dex");t&&(t.innerHTML=`
        ...
        `,M.updateUI(),M.bindEvents())},updateUI:()=>{const{tokens:e,swap:t}=M.state,s=e[t.tokenIn],n=e[t.tokenOut],a=(l,d)=>{document.getElementById(`symbol-${l}`).innerText=d.symbol;const u=document.getElementById(`img-${l}-container`);d.logo?u.innerHTML=`<img src="${d.logo}" class="w-6 h-6 rounded-full" onerror="this.style.display='none'">`:u.innerHTML=`<div class="w-6 h-6 rounded-full bg-zinc-700 flex items-center justify-center text-[10px]">${d.symbol[0]}</div>`};a("in",s),a("out",n),document.getElementById("bal-in").innerText=s.balance.toFixed(4),document.getElementById("bal-out").innerText=n.balance.toFixed(4);const i=M.calculate(),o=document.getElementById("btn-swap"),r=document.getElementById("swap-details");if(!t.amountIn)document.getElementById("input-out").value="",document.getElementById("usd-in").innerText="$0.00",document.getElementById("usd-out").innerText="$0.00",r.classList.add("hidden"),o.innerText="Enter an amount",o.className="w-full mt-4 bg-zinc-800 text-zinc-500 font-bold text-lg py-4 rounded-2xl cursor-not-allowed",o.disabled=!0;else if(parseFloat(t.amountIn)>s.balance)o.innerText=`Insufficient ${s.symbol} balance`,o.className="w-full mt-4 bg-[#3d1818] text-red-500 font-bold text-lg py-4 rounded-2xl cursor-not-allowed border border-red-900/30",o.disabled=!0,r.classList.add("hidden");else if(i){document.getElementById("input-out").value=i.amountOut.toFixed(6),document.getElementById("usd-in").innerText=`~$${i.usdValue.toFixed(2)}`,document.getElementById("usd-out").innerText=`~$${(i.usdValue-i.feeUsd).toFixed(2)}`,r.classList.remove("hidden"),document.getElementById("fee-display").innerText=`$${i.feeUsd.toFixed(2)}`,document.getElementById("mining-reward-display").innerText=`+${i.miningReward.toFixed(4)} BKC`;const l=document.getElementById("price-impact");parseFloat(i.priceImpact)>2?(l.classList.remove("hidden"),l.innerText=`Price Impact: -${i.priceImpact}%`):l.classList.add("hidden"),o.innerHTML=t.loading?'<i class="fa-solid fa-circle-notch fa-spin"></i> Swapping...':"Swap",o.className="w-full mt-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold text-lg py-4 rounded-2xl shadow-lg shadow-blue-900/20 cursor-pointer border border-blue-500/20",o.disabled=t.loading}},bindEvents:()=>{document.getElementById("input-in").addEventListener("input",t=>{M.state.swap.amountIn=t.target.value,M.updateUI()}),document.getElementById("btn-max-in").addEventListener("click",()=>{const t=M.state.tokens[M.state.swap.tokenIn].balance;M.state.swap.amountIn=t.toString(),document.getElementById("input-in").value=t,M.updateUI()}),document.getElementById("btn-switch").addEventListener("click",()=>{const t=M.state.swap.tokenIn;M.state.swap.tokenIn=M.state.swap.tokenOut,M.state.swap.tokenOut=t,M.state.swap.amountIn="",document.getElementById("input-in").value="",M.updateUI()}),document.getElementById("btn-swap").addEventListener("click",async()=>{const t=document.getElementById("btn-swap");if(t.disabled)return;M.state.swap.loading=!0,M.updateUI(),await new Promise(i=>setTimeout(i,1500));const s=M.calculate(),{tokens:n,swap:a}=M.state;n[a.tokenIn].balance-=parseFloat(a.amountIn),n[a.tokenOut].balance+=s.amountOut,n.BKC.balance+=s.miningReward,M.state.swap.loading=!1,M.state.swap.amountIn="",document.getElementById("input-in").value="",t.className="w-full mt-4 bg-green-600 text-white font-bold text-lg py-4 rounded-2xl shadow-[0_0_20px_rgba(22,163,74,0.4)]",t.innerHTML=`<i class="fa-solid fa-check"></i> Swap Success! +${s.miningReward.toFixed(2)} BKC Mined!`,setTimeout(()=>{M.updateUI()},3e3)});const e=t=>{const s=document.getElementById("token-modal"),n=document.getElementById("token-list");s.classList.remove("hidden"),(()=>{n.innerHTML=Object.values(M.state.tokens).map(i=>{const o=M.state.swap[`token${t==="in"?"In":"Out"}`]===i.symbol;return M.state.swap[`token${t==="in"?"Out":"In"}`]===i.symbol?"":`
                    ...
                    `}).join(""),document.querySelectorAll(".token-item").forEach(i=>{i.addEventListener("click",()=>{M.state.swap[`token${t==="in"?"In":"Out"}`]=i.dataset.symbol,s.classList.add("hidden"),M.updateUI()})})})(),document.querySelectorAll(".quick-token").forEach(i=>{i.onclick=()=>{M.state.swap[`token${t==="in"?"In":"Out"}`]=i.dataset.symbol,s.classList.add("hidden"),M.updateUI()}})};document.getElementById("btn-token-in").addEventListener("click",()=>e("in")),document.getElementById("btn-token-out").addEventListener("click",()=>e("out")),document.getElementById("close-modal").addEventListener("click",()=>document.getElementById("token-modal").classList.add("hidden")),document.getElementById("token-modal").addEventListener("click",t=>{t.target.id==="token-modal"&&t.target.classList.add("hidden")})}},ud={render:async e=>{if(!e)return;const t=document.getElementById("dao");t&&(t.innerHTML=`
    ...
    `}function ws(e,t,s,n,a,i,o){const r=an[Fe];let l=`
        <div class="mb-10">
    ...
    `,d=o;return n.forEach(u=>{l+=pd(u,d++)}),l+="</div></div>",{html:l,nextIndex:d}}function fd(e){var t,s,n,a,i,o,r,l;Fe=e,localStorage.setItem("backcoin-tutorials-lang",e),(t=document.getElementById("tutorials-btn-en"))==null||t.classList.toggle("bg-amber-500",e==="en"),(s=document.getElementById("tutorials-btn-en"))==null||s.classList.toggle("text-zinc-900",e==="en"),(n=document.getElementById("tutorials-btn-en"))==null||n.classList.toggle("bg-zinc-700",e!=="en"),(a=document.getElementById("tutorials-btn-en"))==null||a.classList.toggle("text-zinc-300",e!=="en"),(i=document.getElementById("tutorials-btn-pt"))==null||i.classList.toggle("bg-amber-500",e==="pt"),(o=document.getElementById("tutorials-btn-pt"))==null||o.classList.toggle("text-zinc-900",e==="pt"),(r=document.getElementById("tutorials-btn-pt"))==null||r.classList.toggle("bg-zinc-700",e!=="pt"),(l=document.getElementById("tutorials-btn-pt"))==null||l.classList.toggle("text-zinc-300",e!=="pt"),Va()}function Va(){const e=document.getElementById("tutorials-content");if(!e)return;const t=an[Fe];let s=`
        <!-- Hero -->
        <div class="text-center mb-10">
    ...
    `,n=ws("getting-started","rocket","emerald",hs.gettingStarted,"catGettingStarted","catGettingStartedDesc",0);s+=n.html,n=ws("ecosystem","cubes","amber",hs.ecosystem,"catEcosystem","catEcosystemDesc",n.nextIndex),s+=n.html,n=ws("advanced","graduation-cap","cyan",hs.advanced,"catAdvanced","catAdvancedDesc",n.nextIndex),s+=n.html,e.innerHTML=s}const Xa={render:function(e=!1){const t=document.getElementById("tutorials");t&&(e||t.innerHTML.trim()==="")&&(t.innerHTML=`
                    ...
                    <div class="flex items-center justify-between mb-8">
                        ...
                        <div class="flex items-center gap-1 bg-zinc-800 p-1 rounded-lg border border-zinc-700">
                            <button id="tutorials-btn-en" onclick="TutorialsPage.setLang('en')" 
                                    class="flex items-center gap-2 px-3 py-2 rounded-md text-xs font-bold transition-all ${Fe==="en"?"bg-amber-500 text-zinc-900":"bg-zinc-700 text-zinc-300 hover:bg-zinc-600"}">
                            ...
                            <button id="tutorials-btn-pt" onclick="TutorialsPage.setLang('pt')" 
                                    class="flex items-center gap-2 px-3 py-2 rounded-md text-xs font-bold transition-all ${Fe==="pt"?"bg-amber-500 text-zinc-900":"bg-zinc-700 text-zinc-300 hover:bg-zinc-600"}">
            ...
            `,Va())},update:function(e){},cleanup:function(){},setLang:fd};window.TutorialsPage=Xa;const md=window.inject||(()=>{console.warn("Dev Mode: Analytics disabled.")});if(window.location.hostname!=="localhost"&&window.location.hostname!=="127.0.0.1")try{md()}catch(e){console.error("Analytics Error:",e)}const on="".toLowerCase();window.__ADMIN_WALLET__=on;on&&console.log("✅ Admin access granted");let lt=null,et=null,ys=!1;const ae={dashboard:ha,mine:za,store:hr,rewards:Zs,actions:Zr,notary:Wa,airdrop:Ll,tokenomics:yc,about:dl,admin:ac,presale:Ms,rental:_c,socials:cd,creditcard:dd,dex:M,dao:ud,tutorials:Xa};function Ja(e){return!e||e.length<42?"...":`${e.slice(0,6)}...${e.slice(-4)}`}function bd(e){if(!e)return"0.00";const t=B(e);return t>=1e9?(t/1e9).toFixed(2)+"B":t>=1e6?(t/1e6).toFixed(2)+"M":t>=1e4?t.toLocaleString("en-US",{minimumFractionDigits:0,maximumFractionDigits:0}):t.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}function kt(e,t=!1){const s=document.querySelector("main > div.container"),n=document.querySelectorAll(".sidebar-link");if(!s)return;if(lt===e&&!t){ae[e]&&typeof ae[e].update=="function"&&ae[e].update(c.isConnected);return}et&&typeof et=="function"&&(et(),et=null),Array.from(s.children).forEach(i=>{i.tagName==="SECTION"&&(i.classList.add("hidden"),i.classList.remove("active"))}),n.forEach(i=>{i.classList.remove("active"),i.classList.add("text-zinc-400","hover:text-white","hover:bg-zinc-700")});const a=document.getElementById(e);if(a&&ae[e]){a.classList.remove("hidden"),a.classList.add("active");const i=lt!==e;lt=e;const o=document.querySelector(`.sidebar-link[data-target="${e}"]`);o&&(o.classList.remove("text-zinc-400","hover:text-white","hover:bg-zinc-700"),o.classList.add("active")),ae[e]&&typeof ae[e].render=="function"&&ae[e].render(i||t),typeof ae[e].cleanup=="function"&&(et=ae[e].cleanup),i&&window.scrollTo(0,0)}else e!=="dashboard"&&e!=="faucet"&&(console.warn(`Route '${e}' not found, redirecting to dashboard.`),kt("dashboard",!0))}window.navigateTo=kt;const Kn="wallet-btn text-xs font-mono text-center max-w-fit whitespace-nowrap relative font-bold py-2 px-4 rounded-md transition-colors";function rn(e=!1){ys||(ys=!0,requestAnimationFrame(()=>{gd(e),ys=!1}))}function gd(e){const t=document.getElementById("admin-link-container"),s=document.getElementById("statUserBalance"),n=document.getElementById("connectButtonDesktop"),a=document.getElementById("connectButtonMobile"),i=document.getElementById("mobileAppDisplay");let o=c.userAddress;const r=[n,a];if(c.isConnected&&o){const d=bd(c.currentUserBalance),p=`
            <div class="status-dot"></div>
            <span>${Ja(o)}</span>
            <div class="balance-pill">
                ${d} BKC
            </div>
        `;r.forEach(f=>{f&&(f.innerHTML=p,f.className=Kn+" wallet-btn-connected")}),i&&(i.textContent="Backcoin.org",i.classList.add("text-amber-400"),i.classList.remove("text-white")),s&&(s.textContent=B(c.currentUserBalance).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})),t&&(t.style.display=o.toLowerCase()===on.toLowerCase()?"block":"none")}else{const d='<i class="fa-solid fa-plug"></i> Connect Wallet';r.forEach(u=>{u&&(u.innerHTML=d,u.className=Kn+" wallet-btn-disconnected")}),i&&(i.textContent="Backcoin.org",i.classList.add("text-amber-400"),i.classList.remove("text-white")),t&&(t.style.display="none"),s&&(s.textContent="--")}const l=lt||"dashboard";e||!lt?kt(l,!0):ae[l]&&typeof ae[l].update=="function"&&ae[l].update(c.isConnected)}function xd(e){const{isConnected:t,address:s,isNewConnection:n,wasConnected:a}=e,i=n||t!==a;c.isConnected=t,s&&(c.userAddress=s),rn(i),t&&n?m(`Connected: ${Ja(s)}`,"success"):!t&&a&&m("Wallet disconnected.","info")}function vd(){const e=document.getElementById("testnet-banner"),t=document.getElementById("close-testnet-banner");if(!(!e||!t)){if(localStorage.getItem("hideTestnetBanner")==="true"){e.remove();return}e.style.transform="translateY(0)",t.addEventListener("click",()=>{e.style.transform="translateY(100%)",setTimeout(()=>e.remove(),500),localStorage.setItem("hideTestnetBanner","true")})}}function hd(){const e=document.querySelectorAll(".sidebar-link"),t=document.getElementById("menu-btn"),s=document.getElementById("sidebar"),n=document.getElementById("sidebar-backdrop"),a=document.getElementById("connectButtonDesktop"),i=document.getElementById("connectButtonMobile"),o=document.getElementById("shareProjectBtn");vd(),e.length>0&&e.forEach(l=>{l.addEventListener("click",async d=>{d.preventDefault();const u=l.dataset.target;if(u==="faucet"){m("Accessing Testnet Faucet...","info"),await $o("BKC")&&rn(!0);return}u&&(kt(u,!1),s&&s.classList.contains("translate-x-0")&&(s.classList.remove("translate-x-0"),s.classList.add("-translate-x-full"),n&&n.classList.add("hidden")))})});const r=()=>{fa()};a&&a.addEventListener("click",r),i&&i.addEventListener("click",r),o&&o.addEventListener("click",()=>ui(c.userAddress)),t&&s&&n&&(t.addEventListener("click",()=>{s.classList.contains("translate-x-0")?(s.classList.add("-translate-x-full"),s.classList.remove("translate-x-0"),n.classList.add("hidden")):(s.classList.remove("-translate-x-full"),s.classList.add("translate-x-0"),n.classList.remove("hidden"))}),n.addEventListener("click",()=>{s.classList.add("-translate-x-full"),s.classList.remove("translate-x-0"),n.classList.add("hidden")}))}window.addEventListener("load",async()=>{console.log("🚀 App Initializing..."),se.earn||(se.earn=document.getElementById("mine"));try{if(!await vi())throw new Error("Failed to load contract addresses")}catch(t){console.error("❌ Critical Initialization Error:",t),m("Initialization failed. Please refresh.","error");return}hd(),await oo(),ro(xd),pi();const e=document.getElementById("preloader");e&&(e.style.display="none"),kt("dashboard",!0),console.log("✅ App Ready.")});window.EarnPage=za;window.openConnectModal=fa;window.disconnectWallet=lo;window.updateUIState=rn;
