    ...
    DOMElements.dashboard.innerHTML = `
        <div class="flex flex-col gap-6 pb-10 max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                ...
                ${renderMetricCard('Total Burned', 'fa-fire', 'dash-metric-burned', 'BKC permanently removed from supply', 'red')}
...
function renderMetricCard(label, icon, id, tooltip, color = 'zinc') {
    const colorClasses = {
        ...
        red: 'text-red-400' // V11.0: Added for burn metric
    };
...
async function updateGlobalMetrics() {
    try {
        ...
        let totalPStake = 0n;
        let totalTVL = 0n;
        let totalBurned = 0n; // V11.0: Replace economicOutput with totalBurned
        ...
        const burnedNum = formatBigNumber(totalBurned); // V11.0
        ...
        // V11.0: Show Total Burned with fire styling
        const burnedEl = document.getElementById('dash-metric-burned');
        if (burnedEl) {
            if (burnedNum > 0) {
                burnedEl.innerHTML = `<span class="text-red-400">${formatCompact(burnedNum)}</span> <span class="text-xs text-zinc-500">BKC 🔥</span>`;
            } else {
                burnedEl.innerHTML = `<span class="text-zinc-500">0</span> <span class="text-xs text-zinc-500">BKC</span>`;
            }
        }
