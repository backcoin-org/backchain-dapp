...
function renderDashboardLayout() {
    ...
    DOMElements.dashboard.innerHTML = `
        <div class="flex flex-col gap-6 pb-10 max-w-7xl mx-auto">
            ...
            <!-- MOBILE: BKC Balance Card (visible on mobile only) -->
            <div class="lg:hidden glass-panel p-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full flex items-center justify-center" style="background: rgba(245,158,11,0.2)">
                        <img src="./assets/bkc_logo_3d.png" class="w-6 h-6" alt="BKC">
                    </div>
                    <div>
                        <p class="text-zinc-500 text-[10px] uppercase">Your Balance</p>
                        <p id="dash-mobile-balance" class="text-xl font-bold text-amber-400 font-mono">--</p>
                    </div>
                </div>
                <a href="#" class="go-to-store text-xs text-amber-400 hover:text-amber-300">
                    <i class="fa-solid fa-plus mr-1"></i>Get BKC
                </a>
            </div>
...
async function updateUserHub(forceRefresh = false) {
    ...
    try {
        ...
        // V7.7: Update mobile balance card
        const mobileBalanceEl = document.getElementById('dash-mobile-balance');
        if (mobileBalanceEl && State.bkcBalance !== undefined) {
            const balanceNum = formatBigNumber(State.bkcBalance || 0n);
            mobileBalanceEl.innerText = `${balanceNum.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} BKC`;
        }
