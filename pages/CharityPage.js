...
// ✅ PRODUCTION V3.1 - Charity Pool with Standalone Fallback
// Fixed: Added defensive loading and error handling

const ethers = window.ethers;

// ============================================================================
// FALLBACK IMPORTS - Works even if modules fail
// ============================================================================
let State, showToast, addresses, loadCharityStats, loadCampaigns, loadCampaignDetails;
let clearCharityCache, CampaignStatus, isCampaignActive;
let executeCreateCampaign, executeDonate, executeCancelCampaign, executeWithdraw, uploadCampaignImage;

// Try to import modules, use fallbacks if they fail
try {
    const stateModule = await import('../state.js');
    State = stateModule.State;
} catch (e) {
    console.warn('State module not found, using window.State');
    State = window.State || { isConnected: false, userAddress: null, publicProvider: null };
}

try {
    const uiModule = await import('../ui-feedback.js');
    showToast = uiModule.showToast;
} catch (e) {
    console.warn('UI module not found, using fallback');
    showToast = (msg, type) => { console.log(`[${type}] ${msg}`); alert(msg); };
}

try {
    const configModule = await import('../config.js');
    addresses = configModule.addresses;
} catch (e) {
    console.warn('Config not found, using fallback addresses');
    addresses = {
        charityPool: '0xA4d87df3c6e4c11044ef17a58A27dB607F59D50f',
        bkcToken: window.addresses?.bkcToken || '0x...'
    };
}

// Campaign Status Enum
CampaignStatus = { ACTIVE: 0, COMPLETED: 1, CANCELLED: 2, WITHDRAWN: 3 };
isCampaignActive = (c) => {
...
function injectStyles() {
    ...
    s.textContent = `
.charity-page { 
    --cp-primary:#f59e0b; --cp-success:#10b981; --cp-danger:#ef4444; 
    --cp-bg:#18181b; --cp-bg2:#27272a; --cp-bg3:#3f3f46; 
    --cp-border:rgba(63,63,70,0.6); --cp-text:#fafafa; --cp-muted:#a1a1aa; 
    --cp-radius:16px; 
...
/* Hero */
.cp-hero { 
    background:linear-gradient(135deg,rgba(16,185,129,0.08),rgba(236,72,153,0.08),rgba(245,158,11,0.05)); 
...
.cp-stat-val.g { color:var(--cp-success); } 
.cp-stat-val.b { color:#3b82f6; } 
.cp-stat-val.o { color:var(--cp-primary); } 
.cp-stat-val.p { color:#8b5cf6; }
...
.cp-form-input, .cp-form-textarea, .cp-form-select { width:100%; padding:0.75rem; background:var(--cp-bg2); border:1px solid var(--cp-border); border-radius:10px; color:var(--cp-text); font-size:0.875rem; box-sizing:border-box; }
...
const fmtTime = (d) => { 
    const r = Number(d) - Math.floor(Date.now()/1000); 
    if (r <= 0) return {text:'Ended',ended:true,urgent:false}; 
    const dy = Math.floor(r/86400), hr = Math.floor((r%86400)/3600); 
    if (dy > 0) return {text:`${dy}d ${hr}h left`,ended:false,urgent:dy<=3}; 
    if (hr > 0) return {text:`${hr}h left`,ended:false,urgent:true}; 
    return {text:`${Math.floor((r%3600)/60)}m left`,ended:false,urgent:true}; 
};

const calcProg = (r,g) => { 
    if (!g) return 0; 
    try { 
        return Math.min((parseFloat(ethers.formatEther(r||0)) / parseFloat(ethers.formatEther(g))) * 100, 100); 
    } catch { return 0; } 
};
...
const canWithdraw = (c) => { 
    if (!c || !State?.userAddress) return false; 
    const now = Math.floor(Date.now()/1000); 
    const isCreator = c.creator?.toLowerCase() === State.userAddress?.toLowerCase(); 
    const ended = Number(c.deadline) <= now; 
    const hasFunds = BigInt(c.raisedAmount?.toString()||'0') > 0n; 
    return isCreator && hasFunds && ((ended && c.status === 0) || c.status === 2); 
};
...
async function loadStats() {
    const defaultStats = { totalRaised: 0n, totalBurned: 0n, totalCampaigns: 0, totalSuccessful: 0 };
    
    try {
        const provider = State?.publicProvider || new ethers.JsonRpcProvider('https://arb-sepolia.g.alchemy.com/v2/demo');
...
async function loadCampaignsData() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const campaigns = (data.campaigns || []).map(c => ({
                ...
                goalAmount: BigInt(c.goalAmount?.toString() || '0'),
                raisedAmount: BigInt(c.raisedAmount?.toString() || '0'),
                donationCount: Number(c.donationCount || 0),
                deadline: Number(c.deadline || 0),
                createdAt: Number(c.createdAt || 0),
                status: Number(c.status || 0),
                category: c.category || 'humanitarian',
            ...
            console.log('✅ Loaded', campaigns.length, 'campaigns from Firebase');
            return campaigns;
        }
    } catch (e) {
        console.warn('Firebase API failed:', e.message);
    }
    ...
    try {
        ...
        const count = await contract.campaignCounter().catch(() => 0n);
        const total = Number(count);
        console.log('📊 Campaign count:', total);
        ...
        for (let i = 1; i <= total; i++) {
            try {
                ...
                campaigns.push({
                    ...
                    title: c[1],
                    description: c[2],
                    goalAmount: BigInt(c[3].toString()),
                    raisedAmount: BigInt(c[4].toString()),
                    donationCount: Number(c[5]),
                    deadline: Number(c[6]),
                    createdAt: Number(c[7]),
                    status: Number(c[8]),
                    category: 'humanitarian',
                    imageUrl: null
                });
            } catch (e) {
                console.warn(`Failed to load campaign ${i}:`, e.message);
            }
        }
        console.log('✅ Loaded', campaigns.length, 'campaigns from blockchain');
        return campaigns;
    } catch (e) {
        console.error('Blockchain loading failed:', e);
        return [];
    }
}
...
async function loadData() {
    ...
    try {
        ...
        console.log('✅ Data loaded:', { stats, campaignCount: campaigns.length });
    } catch (e) {
...
const renderBadge = (status) => { 
    ...
    return `<span class="cp-badge" style="background:${cfg.color}20;color:${cfg.color};border:1px solid ${cfg.color}"><i class="fa-solid ${cfg.icon}"></i> ${cfg.label}</span>`; 
};

const renderCatBadge = (cat) => { 
    const c = CATEGORIES[cat]; 
    return c ? `<span class="cp-badge" style="background:${c.color}20;color:${c.color}">${c.emoji} ${c.name}</span>` : ''; 
};
...
const renderCard = (c) => {
    ...
    return `<div class="cp-card" onclick="CharityPage.viewCampaign('${c.id}')">
        ...
        <div class="cp-card-body">
            ...
            <h3 class="cp-card-title">${c.title || 'Untitled Campaign'}</h3>
...
const renderLoading = () => `<div class="cp-loading"><div class="cp-spinner"></div><span>Loading campaigns...</span></div>`;
    ...
    <div class="cp-modal-content">
        ...
        <div class="cp-modal-body">
            <div class="cp-form-group"><label class="cp-form-label">Category *</label><div class="cp-cat-selector">
                <label class="cp-cat-option" id="opt-animal" onclick="CharityPage.selCatOpt('animal')"><input type="radio" name="category" value="animal"><div class="cp-cat-option-icon">🐾</div><div class="cp-cat-option-name">Animal</div></label>
                <label class="cp-cat-option selected" id="opt-humanitarian" onclick="CharityPage.selCatOpt('humanitarian')"><input type="radio" name="category" value="humanitarian" checked><div class="cp-cat-option-icon">💗</div><div class="cp-cat-option-name">Humanitarian</div></label>
            </div></div>
...
function openDonate(id) { 
    ...
    document.getElementById('donate-campaign-id').value = id; 
    document.getElementById('donate-amount').value = ''; 
    document.getElementById('donate-info').innerHTML = `<div style="padding:0.75rem;background:var(--cp-bg3);border-radius:8px"><strong style="color:var(--cp-text)">${c.title}</strong><div style="font-size:0.8rem;color:var(--cp-muted);margin-top:0.25rem">${fmt(c.raisedAmount)} / ${fmt(c.goalAmount)} BKC</div></div>`; 
    openModal('donate'); 
}
...
async function openMyCampaigns() { 
    if (!State?.isConnected) { showToast('Connect wallet first','warning'); return; } 
    openModal('my'); 
    const el = document.getElementById('my-campaigns-content'); 
    el.innerHTML = renderLoading(); 
    const mine = CS.campaigns.filter(c=>c.creator?.toLowerCase()===State.userAddress?.toLowerCase()); 
    if (!mine.length) { 
        el.innerHTML = `<div class="cp-empty" style="padding:1.5rem"><i class="fa-solid fa-folder-open"></i><h3>No Campaigns</h3><button class="cp-btn cp-btn-primary" onclick="CharityPage.closeModal('my');CharityPage.openCreate()"><i class="fa-solid fa-plus"></i> Create</button></div>`; 
        return; 
    } 
...
function onImage(e) { 
    ...
    if (!f) return; 
    if (f.size > 5*1024*1024) return showToast('Max 5MB','error'); 
    if (!f.type.startsWith('image/')) return showToast('Select image','error'); 
    CS.pendingImage = f; 
    const reader = new FileReader(); 
    reader.onload = (ev) => { 
        const el = document.getElementById('img-upload'); 
        if (el) el.innerHTML = `<img src="${ev.target.result}" class="cp-img-preview"><button type="button" class="cp-img-remove" onclick="event.stopPropagation();CharityPage.removeImg()"><i class="fa-solid fa-xmark"></i></button><input type="file" id="campaign-image" accept="image/*" onchange="CharityPage.onImage(event)">`; 
    }; 
...
function resetImgUpload() { 
    const el = document.getElementById('img-upload'); 
    if (el) el.innerHTML = `<input type="file" id="campaign-image" accept="image/*" onchange="CharityPage.onImage(event)"><div class="cp-img-upload-placeholder" id="img-placeholder"><i class="fa-solid fa-cloud-arrow-up"></i><p>Click to upload (max 5MB)</p></div>`; 
}

function setAmt(v) { const el = document.getElementById('detail-amount'); if (el) el.value = v; }

// ============================================================================
// TRANSACTIONS (Simplified - uses existing modules or inline)
// ============================================================================
...
async function create() { 
    ...
    try { 
        ...
        if (btn) btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Confirm in wallet...';
        ...
        if (campaignId) {
            try {
                await fetch(CHARITY_API.saveCampaign, {
                    ...
                    body: JSON.stringify({
                        campaignId, creator: State.userAddress, title, description: desc || '',
                        category: cat, imageUrl: imgUrl || null,
                        websiteUrl: document.getElementById('campaign-website')?.value?.trim() || null,
                        youtubeUrl: document.getElementById('campaign-youtube')?.value?.trim() || null,
                        twitterUrl: document.getElementById('campaign-twitter')?.value?.trim() || null,
                        telegramUrl: document.getElementById('campaign-telegram')?.value?.trim() || null,
                        txHash: receipt.hash
                    })
                });
        ...
        showToast(msg.includes('user rejected') ? 'Transaction cancelled' : msg.slice(0, 80), 'error'); 
    } finally { 
...
async function donateDetail(id) { 
    ...
    // Set values and use donate modal logic
    document.getElementById('donate-campaign-id').value = id;
...
async function withdraw(id) { 
    ...
    if (prog < 100) msg += `\n10% will be burned (goal not met)`; 
...
function copyLink() { 
    const c = CS.currentCampaign; 
    if (!c) return; 
    navigator.clipboard.writeText(getShareUrl(c.id)).then(()=>showToast('Link copied!','success')).catch(()=>showToast('Copy failed','error')); 
}
...
function selectCat(cat) { 
    CS.selectedCategory = CS.selectedCategory === cat ? null : cat; 
    updateGrid(); 
}
...
// Main Render
function render() { 
    console.log('🎨 CharityPage render called');
    injectStyles(); 
    
    let container = document.getElementById('charity-container');
    if (!container) {
        container = document.querySelector('.page-content');
        if (container) container.id = 'charity-container';
    }
    
    if (!container) {
        console.error('❌ No container found for CharityPage');
        return;
    }
    ...
    if (id) {
        ...
        loadData().then(()=>{ 
            if (CS.currentView==='main') { 
                const c = document.getElementById('charity-container'); 
...
const CharityPage = {
    async render(isActive) { 
        console.log('🚀 CharityPage.render called, isActive:', isActive);
        if (isActive) render(); 
    },
