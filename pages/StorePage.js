...
function renderTierCards() {
    ...
    grid.innerHTML = `
        <div class="nft-tier-card ${isUserBronze ? 'nft-user-tier' : ''}" style="border-color:${isUserBronze ? bronzeStyle.color : 'var(--nft-border)'}" data-boost="${bronzeTier.boostBips}">
            ${isUserBronze ? '<div class="nft-user-badge">YOUR TIER</div>' : ''}
            <div class="nft-tier-top">
                <div class="nft-tier-img" style="background:${bronzeStyle.bg};border:1px solid ${bronzeStyle.border}">
                    <img src="${bronzeStyle.image}" alt="Bronze" onerror="this.outerHTML='<span style=\\'font-size:28px\\'>${bronzeStyle.icon}</span>'">
                </div>
                <div>
            ...
            <div class="nft-tier-stats">
                ...
                <div class="nft-tier-stat">
                    ...
                    <div class="nft-tier-stat-val" style="color:${userOwned > 0 ? bronzeStyle.color : 'var(--nft-text-3)'}">${userOwned}</div>
                </div>
            </div>
            ...
            <div class="nft-tier-actions">
                ...
                ` : `
                    <button class="nft-action-btn nft-buy-btn" data-action="buy" data-boost="${bronzeTier.boostBips}" ${buyDisabled ? 'disabled' : ''}>
                        <i class="fa-solid fa-cart-plus" style="font-size:10px"></i> ${buyLabel}
                    </button>
                `}
                ${userOwned > 0 ? `
                    <button class="nft-action-btn nft-sell-btn" data-action="sell" data-boost="${bronzeTier.boostBips}" ${sellDisabled ? 'disabled' : ''}>
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:6px;padding:10px 0 2px;flex-wrap:wrap">
            ...
            <span style="padding:3px 8px;border-radius:6px;background:${TIER_CONFIG.Bronze.bg};border:1px solid ${TIER_CONFIG.Bronze.border};font-size:9px;font-weight:700;color:${TIER_CONFIG.Bronze.color}">${TIER_CONFIG.Bronze.icon} Bronze</span>
            <i class="fa-solid fa-arrow-right" style="font-size:7px;color:var(--nft-text-3)"></i>
            <span style="padding:3px 8px;border-radius:6px;background:${TIER_CONFIG.Silver.bg};border:1px solid ${TIER_CONFIG.Silver.border};font-size:9px;font-weight:700;color:${TIER_CONFIG.Silver.color}">${TIER_CONFIG.Silver.icon} Silver 70%</span>
            <i class="fa-solid fa-arrow-right" style="font-size:7px;color:var(--nft-text-3)"></i>
            <span style="padding:3px 8px;border-radius:6px;background:${TIER_CONFIG.Gold.bg};border:1px solid ${TIER_CONFIG.Gold.border};font-size:9px;font-weight:700;color:${TIER_CONFIG.Gold.color}">${TIER_CONFIG.Gold.icon} Gold 80%</span>
            <i class="fa-solid fa-arrow-right" style="font-size:7px;color:var(--nft-text-3)"></i>
            <span style="padding:3px 8px;border-radius:6px;background:${TIER_CONFIG.Diamond.bg};border:1px solid ${TIER_CONFIG.Diamond.border};font-size:9px;font-weight:700;color:${TIER_CONFIG.Diamond.color}">${TIER_CONFIG.Diamond.icon} Diamond 100%</span>
        </div>
    `;
}
...
function renderFusionExplainer() {
    ...
    const steps = tiers.map((name, i) => {
        const s = TIER_CONFIG[name];
        const isLast = i === tiers.length - 1;
        return `
            <div style="display:flex;flex-direction:column;align-items:center;gap:2px;flex:1;min-width:50px">
                <div style="font-size:${isLast ? '28' : '20'}px;${isLast ? 'filter:drop-shadow(0 0 8px rgba(34,211,238,0.3))' : ''}">${s.icon}</div>
                <div style="font-size:9px;font-weight:800;color:${s.color}">${name}</div>
                <div style="font-size:8px;color:var(--nft-text-3)">${s.keepRate}% keep</div>
            </div>
            ${!isLast ? '<div style="display:flex;align-items:center;color:var(--nft-text-3);font-size:10px;opacity:0.4;padding-top:0"><i class="fa-solid fa-chevron-right"></i></div>' : ''}
        `;
    });
...
function renderFuseTab(nftsByTier) {
        ...
        previewHtml = `
            <div class="nft-forge-preview">
                <div class="nft-forge-source">
                    <span class="nft-forge-source-icon">${tierStyle.icon}</span>
                    <span class="nft-forge-source-id" style="color:${tierStyle.color}">#${fuseSelected[0]}</span>
                </div>
                <div class="nft-forge-op">+</div>
                <div class="nft-forge-source">
                    <span class="nft-forge-source-icon">${tierStyle.icon}</span>
                    <span class="nft-forge-source-id" style="color:${tierStyle.color}">#${fuseSelected[1]}</span>
                </div>
                <div class="nft-forge-arrow"><i class="fa-solid fa-angles-right"></i></div>
                <div class="nft-forge-result">
                    <span class="nft-forge-result-icon">${resultStyle.icon}</span>
                    <span class="nft-forge-result-name" style="color:${resultStyle.color}">${resultName}</span>
                </div>
            </div>
    ...
    const hint = fuseSelected.length === 0
        ? `<i class="fa-solid fa-hand-pointer"></i> Select 2 ${tierName} NFTs to forge:`
        : fuseSelected.length === 1
            ? `<i class="fa-solid fa-hand-pointer"></i> Select 1 more ${tierName}:`
            : '';
    ...
    return `
        ...
        <div class="nft-fusion-grid" style="${splitSelectedTokenId !== null ? 'margin-bottom:0' : ''}">${splittableNfts.map(nft => {
            ...
            return `<div class="nft-fusion-nft ${isSelected ? 'selected' : ''}" data-split-nft="${nft.tokenId}" data-split-nft-tier="${nft.tier}" style="${isSelected ? `border-color:${style.color}` : ''}">
                <div class="nft-fusion-nft-check" style="background:${style.color}">&check;</div>
            ...
            for (let target = nft.tier - 1; target >= 0; target--) {
                ...
                options.push(`
                    <div class="nft-split-option ${isActive ? 'active' : ''}" data-split-target="${target}" data-split-source="${nft.tier}" data-split-tokenid="${nft.tokenId}">
                        ...
                        <span class="nft-split-option-fee" id="split-fee-${nft.tokenId}-${target}"><i class="fa-solid fa-spinner fa-spin" style="font-size:8px;opacity:0.3"></i></span>
                    </div>
                `);
            }
                ...
                confirmHtml = `
                    <div class="nft-split-confirm">
                        <div class="nft-split-confirm-text">
                            ...
                            Burn <strong style="color:${style.color}">${name} #${nft.tokenId}</strong> &rarr; ${count}x <strong style="color:${targetStyle.color}">${targetName}</strong>?<br>
            ...
            return `
                <div class="nft-split-options">
                    <div class="nft-split-header">
                        <span class="nft-split-header-icon">${style.icon}</span>
                        <div class="nft-split-header-info">
                            <div class="nft-split-header-name" style="color:${style.color}">${name} #${nft.tokenId}</div>
