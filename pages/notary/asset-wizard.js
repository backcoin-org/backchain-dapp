...
// Notary V5 — Asset registration wizard (3 steps)
// ============================================================================

import { State } from '../../state.js';
import { NT, ASSET_TYPES } from './state.js';
...
export function renderAssetWizard(el) {
    ...
    el.innerHTML = `
        <div class="nt-card" style="margin-top:16px;padding:24px">
            ...
            <!-- Step indicators -->
            <div class="nt-steps">
                <div class="nt-step-dot ${NT.assetWizStep >= 1 ? (NT.assetWizStep > 1 ? 'done' : 'active') : 'pending'}">
                    ${NT.assetWizStep > 1 ? '<i class="fa-solid fa-check" style="font-size:12px"></i>' : '1'}
                </div>
                <div class="nt-step-line ${NT.assetWizStep > 1 ? 'done' : ''}"></div>
                <div class="nt-step-dot ${NT.assetWizStep >= 2 ? (NT.assetWizStep > 2 ? 'done' : 'active') : 'pending'}">
                    ${NT.assetWizStep > 2 ? '<i class="fa-solid fa-check" style="font-size:12px"></i>' : '2'}
                </div>
                <div class="nt-step-line ${NT.assetWizStep > 2 ? 'done' : (NT.assetWizStep === 2 ? 'active' : '')}"></div>
                <div class="nt-step-dot ${NT.assetWizStep >= 3 ? 'active' : 'pending'}">3</div>
            </div>

            <!-- Step content -->
            <div id="nt-asset-wiz-panel">
                ${NT.assetWizStep === 1 ? renderStep1() : ''}
                ${NT.assetWizStep === 2 ? renderStep2() : ''}
                ${NT.assetWizStep === 3 ? renderStep3() : ''}
            </div>
        </div>
    `;

    // Restore form values after render
    if (NT.assetWizStep === 2) {
...
// Step 1: Choose asset type + optional document
function renderStep1() {
    return `
        <div style="animation:nt-fadeIn 0.3s ease">
            <div style="font-size:13px;font-weight:600;color:var(--nt-text);margin-bottom:12px">Select Asset Type</div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:20px">
                ${ASSET_TYPES.map(t => `
                    <div onclick="NotaryPage.onAssetTypeChange(${t.id})" style="padding:16px;border-radius:var(--nt-radius-sm);background:${NT.assetWizType === t.id ? t.bg : 'var(--nt-bg3)'};border:2px solid ${NT.assetWizType === t.id ? t.color : 'var(--nt-border)'};cursor:pointer;transition:all var(--nt-transition);text-align:center">
                        <i class="${t.icon}" style="font-size:24px;color:${t.color};margin-bottom:6px;display:block"></i>
                        <div style="font-size:12px;font-weight:600;color:${NT.assetWizType === t.id ? t.color : 'var(--nt-text-2)'}">${t.name}</div>
                    </div>
                `).join('')}
            </div>

            <div style="font-size:13px;font-weight:600;color:var(--nt-text);margin-bottom:8px">Supporting Document <span style="color:var(--nt-text-3);font-weight:400">(optional)</span></div>
            <div id="nt-asset-dropzone" class="nt-dropzone" style="padding:24px 16px;margin-bottom:16px"
                ...
                ` : `
                    <i class="fa-solid fa-cloud-arrow-up" style="font-size:24px;color:var(--nt-text-3);margin-bottom:8px"></i>
                    <div style="font-size:12px;color:var(--nt-text-3)">Upload deed, title, or photo</div>
                `}
    ...
    return `
        <div style="animation:nt-fadeIn 0.3s ease">
            ...
            <div style="margin-bottom:16px">
                ...
                <input id="nt-asset-desc" type="text" placeholder="e.g. Apartment at Rua Augusta, 1500"
            ...
            <div style="margin-bottom:20px">
                ...
                <textarea id="nt-asset-meta" placeholder="Registration number, plate, patent ID..."
