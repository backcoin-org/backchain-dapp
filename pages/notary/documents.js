...
export function renderDocuments(el) {
    if (!State.isConnected) {
        el.innerHTML = `
            <div class="nt-card" style="margin-top:16px;text-align:center;padding:48px 20px">
                ...
                <div style="font-size:13px;color:var(--nt-text-3);margin-bottom:20px">${t('notary.documentsTab.connectToView')}</div>
    ...
    if (!NT.certificates.length) {
        el.innerHTML = `
            <div class="nt-card" style="margin-top:16px;text-align:center;padding:48px 20px">
                ...
                <div style="font-size:16px;font-weight:700;color:var(--nt-text);margin-bottom:8px">${t('notary.documentsTab.noDocuments')}</div>
                <div style="font-size:13px;color:var(--nt-text-3);margin-bottom:20px">${t('notary.documentsTab.certifyFirst')}</div>
                <button class="nt-btn-primary" onclick="NotaryPage.setTab('notarize')">
                    <i class="fa-solid fa-plus" style="margin-right:8px"></i>${t('notary.notarize')}
                </button>
            </div>
        `;
        return;
    ...
    el.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:space-between;margin-top:16px;margin-bottom:4px">
            <div style="font-size:13px;color:var(--nt-text-2)">${t('notary.documentsTab.certCount', { count: String(NT.certificates.length) })}</div>
            <div style="display:flex;gap:8px;align-items:center">
                <button class="nt-btn-primary" style="padding:6px 14px;font-size:12px" onclick="NotaryPage.setTab('notarize')">
                    <i class="fa-solid fa-plus" style="margin-right:6px"></i>${t('notary.notarize')}
                </button>
                <button class="nt-btn-icon" onclick="NotaryPage.refreshHistory()" title="${t('common.refresh')}">
...
function renderCertCard(cert) {
    ...
    const desc = cert.description?.split('---')[0].trim().split('\n')[0].trim() || t('notary.documentsTab.notarizedDocument');
    ...
    return `
        <div class="nt-cert-card" onclick="NotaryPage.viewCert(${cert.id})">
            <div class="nt-cert-thumb">
                ...
                ${cert.received ? `<span style="position:absolute;top:8px;left:8px;font-size:10px;font-weight:600;color:#fff;background:rgba(99,102,241,0.9);padding:2px 8px;border-radius:20px;display:flex;align-items:center;gap:3px"><i class="fa-solid fa-inbox" style="font-size:8px"></i>${t('notary.documentsTab.received')}</span>` :
            ...
            <div class="nt-cert-info">
                ...
                <div style="display:flex;gap:6px" onclick="event.stopPropagation()">
                    <a href="${EXPLORER_ADDR}${addresses?.notary}?a=${cert.id}" target="_blank" class="nt-card-action" title="${t('notary.verify')}">
                        <i class="fa-solid fa-cube"></i> ${t('notary.verify')}
                    </a>
                    <button class="nt-card-action" onclick="NotaryPage.addToWallet('${cert.id}')" title="${t('common.share')}">
