 ...
 * @notice Minimal interface for ecosystem configuration
 */
 ...
 * @notice Minimal interface for mining operations
 */
...
/**
 * @title IBKCToken
 * @notice Minimal interface for BKC token with burn capability
 */
interface IBKCToken is IERC20Upgradeable {
 ...
 * @notice Decentralized crowdfunding platform with deflationary mechanics
 * 
 * @dev CharityPool enables transparent, blockchain-based fundraising campaigns
 *      with built-in burn mechanisms that create deflationary pressure on $BKC.
 *      This module is 100% modular and plug-and-play - it reads all configuration
 *      from EcosystemManager and requires no changes to existing contracts.
 *
 *      ┌─────────────────────────────────────────────────────────────────┐
 *      │                      CHARITY POOL                               │
 *      │            "Crowdfunding with Deflationary Impact"              │
 *      ├─────────────────────────────────────────────────────────────────┤
 *      │                                                                 │
 *      │   CREATE              DONATE              WITHDRAW              │
 *      │   ──────              ──────              ────────              │
 *      │   Free                % fee               ETH fee               │
 *      │                       ├─ % → Mining       + % Burn if           │
 *      │                       └─ % → Burn 🔥      goal not met          │
 *      │                                                                 │
 *      │   All fees are configurable by owner via setFees()              │
 *      │                                                                 │
 *      └─────────────────────────────────────────────────────────────────┘
 *
 *      Fee Structure (default values, configurable):
 *      ┌─────────────────────────────────────────────────────────────────┐
 *      │  Action     │ Default Fee      │ Destination                    │
 *      ├─────────────┼──────────────────┼────────────────────────────────┤
 *      │  Donate     │ 4% BKC (default) │ MiningManager (PoP trigger)    │
 *      │  Donate     │ 1% BKC (default) │ Burn 🔥                        │
 *      │  Withdraw   │ 0.001 ETH (def.) │ Treasury                       │
 *      │  Withdraw   │ 10% BKC (default)│ Burn 🔥 (only if goal not met)│
 *      └─────────────────────────────────────────────────────────────────┘
 *
 *      Campaign Lifecycle:
 *      1. Creator creates campaign (free) with goal and deadline
 *      2. Donors contribute BKC (fees configurable via setFees)
 *      3. After deadline OR cancellation, creator withdraws
 *      4. If goal not met: configurable % of funds burned as penalty
 *
...
{
    using SafeERC20Upgradeable for IBKCToken;
     ...
     *      - If goal not met: 10% of funds burned
     *
