...
export function generateRoutes(config = {}) {
    ...
    const routes = [];
    const include = (mod) => !enabled || enabled.has(mod);
    // ── Staking ─────────────────────────────────────────────────────────
    if (include('staking')) {
        routes.push({
    ...
    if (include('nft')) {
        routes.push({
            ...
            handler: async (ctx, p) => new NftModule(ctx).getPoolInfo(Number(p.tier)),
        });
        routes.push({
            method: 'GET', path: `${base}/nft/pool/:tier/stats`,
            handler: async (ctx, p) => new NftModule(ctx).getPoolStats(Number(p.tier)),
        });
        routes.push({
