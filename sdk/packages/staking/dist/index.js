...
import { STAKING_POOL_ABI, calculateFee, ACTION_IDS, } from '@backchain/core';
export class StakingModule {
    ...
    async delegate(amount, lockDays) {
        ...
        const fee = await calculateFee(this.ctx.provider, addr.backchainEcosystem, ACTION_IDS.STAKING_DELEGATE);
    ...
    async claimRewards() {
        const addr = this.ctx.addresses;
        const fee = await calculateFee(this.ctx.provider, addr.backchainEcosystem, ACTION_IDS.STAKING_CLAIM);
        const contract = this.ctx.provider.getWriteContract(addr.stakingPool, STAKING_POOL_ABI);
