...
import {
    FORTUNE_POOL_ABI, BKC_TOKEN_ABI,
...
export class FortuneModule {
    ...
    async play(wagerAmount: bigint, guesses: number[], tierMask: number = 7): Promise<TxResult & { gameId: bigint; secret: string }> {
        ...
        let fee = 0n;
        if (tierMask & 1) fee += await calculateFee(this.ctx.provider, addr.backchainEcosystem, ACTION_IDS.FORTUNE_TIER0);
        if (tierMask & 2) fee += await calculateFee(this.ctx.provider, addr.backchainEcosystem, ACTION_IDS.FORTUNE_TIER1);
        if (tierMask & 4) fee += await calculateFee(this.ctx.provider, addr.backchainEcosystem, ACTION_IDS.FORTUNE_TIER2);
