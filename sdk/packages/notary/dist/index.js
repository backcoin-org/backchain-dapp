...
import { NOTARY_ABI, calculateFee, notaryActionId } from '@backchain/core';
export class NotaryModule {
    ...
    async certify(documentHash, meta, docType = 0) {
        ...
        const fee = await calculateFee(this.ctx.provider, this.ctx.addresses.backchainEcosystem, notaryActionId(docType));
    ...
    async batchCertify(documents) {
        ...
        const perDocFee = await calculateFee(this.ctx.provider, this.ctx.addresses.backchainEcosystem, notaryActionId(types[0]));
