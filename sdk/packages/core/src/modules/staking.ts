...
import { STAKING_POOL_ABI, BKC_TOKEN_ABI } from '../contracts/abis.js';
...
export class StakingModule {
    ...
    async delegate(amount: bigint, lockDays: number): Promise<TxResult> {
        ...
        const fee = await calculateFee(
            this.sdk.provider, addr.backchainEcosystem, ACTION_IDS.STAKING_DELEGATE
        );
    ...
    async claimRewards(): Promise<TxResult> {
        const addr = this.sdk.addresses;
        const fee = await calculateFee(
            this.sdk.provider, addr.backchainEcosystem, ACTION_IDS.STAKING_CLAIM
        );
