...
import type { TxResult, PostView, UserProfile, ContentType, BadgeTier } from '../types/index.js';
...
export class AgoraModule {
    ...
    async createPost(content: string, tag: number = 0, contentType: number = 0): Promise<TxResult & { postId: bigint }> {
        let fee = 0n;
        if (contentType === 1) fee = await calculateFee(this.sdk.provider, this.sdk.addresses.backchainEcosystem, ACTION_IDS.AGORA_POST_IMAGE);
        else if (contentType === 2) fee = await calculateFee(this.sdk.provider, this.sdk.addresses.backchainEcosystem, ACTION_IDS.AGORA_POST_VIDEO);
        else if (contentType === 4) fee = await calculateFee(this.sdk.provider, this.sdk.addresses.backchainEcosystem, ACTION_IDS.AGORA_LIVE);
