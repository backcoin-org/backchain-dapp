...
export function nftActionId(prefix: string, tier: number): string {
    return ethers.keccak256(
        ethers.AbiCoder.defaultAbiCoder().encode(['string', 'uint8'], [prefix, tier])
    );
}

/** Notary certify action ID: keccak256(abi.encode("NOTARY_CERTIFY_T", docType)) */
export function notaryActionId(docType: number): string {
    return ethers.keccak256(
        ethers.AbiCoder.defaultAbiCoder().encode(['string', 'uint8'], ['NOTARY_CERTIFY_T', docType])
    );
}
