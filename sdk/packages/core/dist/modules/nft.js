...
import { NFT_POOL_ABI, REWARD_BOOSTER_ABI } from '../contracts/abis.js';
...
export class NftModule {
    ...
    async buy(tier, slippageBps = 500) {
        ...
        // Calculate ETH fee
        const fee = await calculateFee(this.sdk.provider, this.sdk.addresses.backchainEcosystem, nftActionId('NFT_BUY_T', tier));
    ...
    async sell(tier, tokenId, minPayout = 0n) {
        ...
        // Calculate ETH fee
        const fee = await calculateFee(this.sdk.provider, this.sdk.addresses.backchainEcosystem, nftActionId('NFT_SELL_T', tier));
        const writePool = this.sdk.provider.getWriteContract(poolAddr, NFT_POOL_ABI);
